...
if ($action2=="1")
 ...
 // $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $resultats=$idconnect->query("SELECT id_champ FROM phpau_parametres WHERE id_champ='$champ'");
 ...
 $nb_resultats=mysqli_num_rows($resultats);
   ...
   if ($nb_resultats>0)
     {
       //Màj des paramètres
       mysqli_query ($idconnect,("UPDATE phpau_parametres SET pts_victoire='$pts_victoire', pts_nul='$pts_nul', pts_defaite='$pts_defaite', accession='$accession', barrage='$barrage', relegation='$relegation', id_equipe_fetiche='$id_equipe_fetiche', fiches_clubs='$fiches_clubs', estimation='$estimation' WHERE id_champ='$champ'")) or die (mysqli_error($idconnect));
     }
...
if ($malus)
 {
  ...
  while($x<$y)
   {
	  //   $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');
    mysqli_query($idconnect, ("UPDATE phpau_equipes SET penalite = '$malus[$x]'  WHERE id = '$id_equipe[$x]'")) or die(mysql_error($idconnect));
    $x++;
   }
 }
?>
