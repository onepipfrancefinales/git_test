...
function affiche_points ($user_id, $gr_champ, $idconnect)
{
 // $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $result=$idconnect->query("SELECT points 
						    FROM phpau_clmnt_pronos, phpau_membres 
						    WHERE phpau_membres.id=phpau_clmnt_pronos.id_membre 
						    and phpau_membres.id_prono='$user_id' and type='general' 
...
$result=$idconnect->query("SELECT phpau_clubs.nom, CLEXT.nom, phpau_matchs.buts_dom, phpau_matchs.buts_ext, phpau_matchs.id, phpau_matchs.date_reelle, phpau_journees.numero
...
AND phpau_matchs.buts_dom is null
AND phpau_matchs.buts_ext is null
AND phpau_clubs.nom!='exempte'
AND CLEXT.nom!='exempte'
ORDER by phpau_matchs.date_reelle, phpau_clubs.nom
LIMIT 0, 10");
...
$result2=$idconnect->query( "SELECT pronostic 
							FROM phpau_pronostics, phpau_gr_championnats 
...
if ($nb_pronos == "0") {$prono="0";}
{
  ...
  $resultats2=$idconnect->query("SELECT tps_avant_prono FROM phpau_gr_championnats WHERE id='$gr_champ'");
...
{
   mysqli_query($idconnect, ("DELETE FROM phpau_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_en_cours'"));

   $result=$idconnect->query("SELECT id_membre, pseudo, sum(points) as total, sum(participation) as participations
   FROM phpau_membres, phpau_pronostics, phpau_matchs, phpau_gr_championnats
   WHERE phpau_pronostics.id_champ=phpau_gr_championnats.id
   AND phpau_gr_championnats.id='$gr_champ'
 ...
 //  $result=mysql_query ($query) or die ("probleme " .mysql_error());
   while ($row=mysqli_fetch_array($result))
         ...
         mysqli_query($idconnect, ("INSERT INTO phpau_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_en_cours')"));
         }
}          

if ($type=="mensuel_30_jours")
{
   mysqli_query($idconnect, ("DELETE FROM phpau_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_30_jours'"));
   ...
   //$result=mysql_query ($query) or die ("probleme " .mysql_error());
       ...
       {
       $row[1]=addslashes($row[1]);
       mysqli_query($idconnect, ("INSERT INTO phpau_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_30_jours')"));
       }
}

if ($type=="hebdo")
{
   mysqli_query($idconnect, ("DELETE FROM phpau_clmnt_pronos WHERE id_champ='$gr_champ' AND type='hebdo'"));
...
$result=$idconnect->query("SELECT pseudo, points, participation FROM phpau_clmnt_pronos
...
function pseudo_admin ($gr_champ, $idconnect)
{
 $resultat=$idconnect->query("SELECT pseudo 
							  FROM phpau_membres, phpau_gr_championnats 
							  WHERE phpau_gr_championnats.id_master=phpau_membres.id 
							  AND phpau_gr_championnats.id='$gr_champ'");
...
function champ_prono ($gr_champ, $idconnect)
{
 ...
 $resultat=$idconnect->query("SELECT DISTINCT id, nom FROM phpau_gr_championnats WHERE  phpau_gr_championnats.activ_prono='1' ORDER by id");
