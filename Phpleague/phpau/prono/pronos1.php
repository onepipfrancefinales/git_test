...
if ($action == "reset") {
  ...
  while ($row = mysqli_fetch_array($resultat)) {
    $resultat1 = $idconnect->query("SELECT phpau_matchs.date_reelle 
                                    FROM phpau_matchs 
                                    WHERE phpau_matchs.id='$row[0]'");
    ...
    if ($date_actuelle < ($date_match_timestamp + $temps_avant_prono * 60)) {
      mysqli_query($idconnect, ("UPDATE  phpau_pronostics 
                                 SET pronostic='0' 
...
if ($action == "valid_pronos") {
  ...
  for ($i = 1; $i <= $_REQUEST['nb_fiche']; $i++) {
    ...
    $resultat = $idconnect->query("SELECT phpau_matchs.date_reelle 
                                   FROM phpau_matchs 
                                   WHERE phpau_matchs.id='$id_match[$i]'");
    ...
    if ($f_prono[$i] !== "undefined") {
      ...
      mysqli_query($idconnect, ("DELETE FROM phpau_pronostics 
                                 WHERE pronostic=' '"));

      $resultat = $idconnect->query("SELECT * 
                                     FROM phpau_matchs, phpau_pronostics, phpau_membres 
                                     WHERE phpau_membres.id_prono='$user_id'
      ...
      if ($nb_prono == "0") {
        if ($date_actuelle < ($date_match_timestamp + $temps_avant_prono * 60)) {
          mysqli_query($idconnect, ("INSERT INTO phpau_pronostics (id_membre, pronostic, id_match, id_champ) 
...
//echo "<br>";
  ...
  $resultat = $idconnect->query("
			 SELECT phpau_clubs.nom, CLEXT.nom, phpau_matchs.id, phpau_matchs.date_reelle, phpau_journees.numero, phpau_matchs.id_equipe_dom, phpau_matchs.id_equipe_ext
			 FROM phpau_clubs, phpau_clubs as CLEXT, phpau_matchs, phpau_journees, phpau_equipes, phpau_equipes as EXT, phpau_gr_championnats
			 WHERE phpau_clubs.id=phpau_equipes.id_club
			 AND CLEXT.id=EXT.id_club
			 AND phpau_equipes.id=phpau_matchs.id_equipe_dom
       ...
       AND phpau_clubs.nom!='Exempt'
       AND CLEXT.nom!='Exempt'
			 AND CLEXT.nom!='exempte'

       ORDER by phpau_matchs.date_reelle, phpau_matchs.id
			 LIMIT $debut, $fin ");
...
/*
       ...
       AND CLEXT.nom!='Exempt'
			 AND CLEXT.nom!='exempte'
       AND phpau_matchs.date_reelle < '$dateAffichageMaxi'
       ORDER by phpau_matchs.date_reelle, phpau_matchs.id
			 LIMIT $debut, $fin ");
*/
  $i = 0;
