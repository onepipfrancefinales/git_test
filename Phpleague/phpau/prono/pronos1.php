<?php
//***********************************************************************/
// phpau : gestionnaire de championnat                              */
// ============================================                         */
//                                                                      */
// Version : 0.82                                                       */
// Copyright (c) 2004    Alexis MANGIN                                  */
// http://phpau.univert.org                                         */
//                                                                      */
...
//                                                                      */
//***********************************************************************/
// Support technique : http://phpau.univert.org/forum               */
//                                                                      */
//***********************************************************************/
?>
...
$resultat=$idconnect->query("SELECT phpau_matchs.id
  ...
  //$resultat=mysql_query($requete) or die ("probleme " .mysql_error());
      while ($row=mysqli_fetch_array($resultat))
      {
        $resultat1=$idconnect->query("SELECT phpau_matchs.date_reelle FROM phpau_matchs WHERE phpau_matchs.id='$row[0]'");
       // $resultat1=mysql_query($requete1);
        ...
        {
           mysqli_query($idconnect, ("UPDATE  phpau_pronostics SET pronostic='0' WHERE id_match='$row[0]' AND id_membre='$id'"));}
           mysqli_query($idconnect, ("DELETE FROM phpau_pronostics WHERE pronostic='0'"));
        }
      }
  ...
  if ($action == "valid_pronos")
  {
    ...
    {   
     ...
     $resultat=$idconnect->query("SELECT phpau_matchs.date_reelle FROM phpau_matchs WHERE phpau_matchs.id='$id_match[$i]'");
    // $resultat=mysql_query($requete);
     ...
     $resultat=$idconnect->query("SELECT tps_avant_prono FROM phpau_gr_championnats WHERE id='$gr_champ'");
     //$resultat=mysql_query($requete);
     ...
     {
       mysqli_query($idconnect, ("DELETE FROM phpau_pronostics WHERE pronostic=' '"));
       $resultat =$idconnect->query( "SELECT * FROM phpau_matchs, phpau_pronostics, phpau_membres WHERE phpau_membres.id_prono='$user_id'
                   AND phpau_membres.id=phpau_pronostics.id_membre
                   AND phpau_pronostics.id_match=phpau_matchs.id
                   AND phpau_pronostics.id_match='$id_match[$i]'");
      // $resultat=mysql_query($requete);
       $nb_prono=mysqli_num_rows($resultat);

       $resultat = $idconnect->query("SELECT id FROM phpau_membres WHERE id_prono='$user_id'");
       //$resultat = mysql_query($requete);
       ...
       if ($nb_prono == "1")
       {
         if ($date_actuelle<($date_match_timestamp+$temps_avant_prono*60))
         {
           mysqli_query($idconnect, ("UPDATE phpau_pronostics SET pronostic='$f_prono[$i]'
                        WHERE phpau_pronostics.id_membre='$id'
                        AND phpau_pronostics.id_match='$id_match[$i]'"));
         }
       }
       if ($nb_prono == "0")
       {
         if ($date_actuelle<($date_match_timestamp+$temps_avant_prono*60))
         {
           mysqli_query($idconnect, ("INSERT INTO phpau_pronostics (id_membre, pronostic, id_match, id_champ) VALUES ('$id','$f_prono[$i]','$id_match[$i]', '$gr_champ')"));
         }
       }
  ...
  {    if ($debut=="0") 
    ...
    $resultat=$idconnect->query("
			 SELECT phpau_clubs.nom, CLEXT.nom, phpau_matchs.id, phpau_matchs.date_reelle, phpau_journees.numero
			 FROM phpau_clubs, phpau_clubs as CLEXT, phpau_matchs, phpau_journees, phpau_equipes, phpau_equipes as EXT, phpau_gr_championnats
			 WHERE phpau_clubs.id=phpau_equipes.id_club
			 AND CLEXT.id=EXT.id_club
			 AND phpau_equipes.id=phpau_matchs.id_equipe_dom
    ...
    if (mysqli_num_rows($resultat)=="0") 
      ...
      while ($row=mysqli_fetch_array($resultat) and $i<$nb_matchs)
      {
       ...
       $resultat2=$idconnect->query( " SELECT pronostic 
									   ...
									   WHERE phpau_pronostics.id_match='$row[2]' 
									   AND phpau_membres.id=phpau_pronostics.id_membre 
       ...
       $resultat2=$idconnect->query("SELECT tps_avant_prono FROM phpau_gr_championnats WHERE id='$gr_champ'");
      //$resultat2=mysql_query($requete2) or die ("probleme " .mysql_error());
