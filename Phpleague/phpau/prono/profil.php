...
elseif ($action == "supp" and $confirm=="oui")
 {
   ...
   $result=$idconnect->query("SELECT id FROM phpau_membres WHERE pseudo='$user_pseudo'");
  // $result=mysql_query($requete) or die ("probleme " .mysql_error());
   $row=mysqli_fetch_array($result);
   
   mysqli_query($idconnect, ("DELETE FROM phpau_pronostics WHERE id_membre='$row[0]'"));
   mysqli_query($idconnect, ("DELETE FROM phpau_membres WHERE id='$row[0]'"));
   mysqli_query($idconnect, ("DELETE FROM phpau_clmnt_pronos WHERE id_membre='$row[0]'"));
  ?>
  ...
  if ($action == "1")
      ...
      $result = $idconnect->query("
          ...
          if (empty ($ancien_mdp) and empty ($nouveau_mdp) and empty ($nouveau_mdp2))
           {
             mysqli_query ($idconnect, ("UPDATE phpau_membres 
          ...
          elseif (!empty ($ancien_mdp) and !empty ($nouveau_mdp) and !empty ($nouveau_mdp2))
           {
             ...
             if ($ancien_mdp_crypt=$mot_de_passe_correct and $nouveau_mdp=$nouveau_mdp2)
             {
              ...
              mysqli_query ($idconnect, ("update phpau_membres SET nom_site='$site', mail='$mail', mot_de_passe='$nouveau_mdp_crypt', nom='$nom', prenom='$prenom', adresse='$adresse', code_postal='$code_postal', ville='$ville', pays='$pays', date_naissance='$date_naissance', profession='$profession', mobile='$mobile'  WHERE id_prono='$user_id' and pseudo='$user_pseudo'"));
              $message.="profil mis à jour";
             }
