...
$resultat=$idconnect->query("SELECT phpal_matchs.id
  ...
  //$resultat=mysql_query($requete) or die ("probleme " .mysql_error());
      while ($row=mysqli_fetch_array($resultat))
      {
        $resultat1=$idconnect->query("SELECT phpal_matchs.date_reelle FROM phpal_matchs WHERE phpal_matchs.id='$row[0]'");
       // $resultat1=mysql_query($requete1);
        ...
        {
           mysqli_query($idconnect, ("UPDATE  phpal_pronostics SET pronostic='0' WHERE id_match='$row[0]' AND id_membre='$id'"));}
           mysqli_query($idconnect, ("DELETE FROM phpal_pronostics WHERE pronostic='0'"));
        }
      }
  ...
  if ($action == "valid_pronos")
  {
    ...
    {   
     ...
     $resultat=$idconnect->query("SELECT phpal_matchs.date_reelle FROM phpal_matchs WHERE phpal_matchs.id='$id_match[$i]'");
    // $resultat=mysql_query($requete);
     ...
     $resultat=$idconnect->query("SELECT tps_avant_prono FROM phpal_gr_championnats WHERE id='$gr_champ'");
     //$resultat=mysql_query($requete);
     ...
     {
       mysqli_query($idconnect, ("DELETE FROM phpal_pronostics WHERE pronostic=' '"));
       $resultat =$idconnect->query( "SELECT * FROM phpal_matchs, phpal_pronostics, phpal_membres WHERE phpal_membres.id_prono='$user_id'
      ...
      // $resultat=mysql_query($requete);
       ...
       $resultat = $idconnect->query("SELECT id FROM phpal_membres WHERE id_prono='$user_id'");
       //$resultat = mysql_query($requete);
       ...
       if ($nb_prono == "1")
       {
         if ($date_actuelle<($date_match_timestamp+$temps_avant_prono*60))
         {
           mysqli_query($idconnect, ("UPDATE phpal_pronostics SET pronostic='$f_prono[$i]'
       ...
       if ($nb_prono == "0")
       {
         if ($date_actuelle<($date_match_timestamp+$temps_avant_prono*60))
         {
           mysqli_query($idconnect, ("INSERT INTO phpal_pronostics (id_membre, pronostic, id_match, id_champ) VALUES ('$id','$f_prono[$i]','$id_match[$i]', '$gr_champ')"));
         }
       }
  ...
  {    if ($debut=="0") 
    ...
    $resultat=$idconnect->query("
			 SELECT phpal_clubs.nom, CLEXT.nom, phpal_matchs.id, phpal_matchs.date_reelle, phpal_journees.numero
			 FROM phpal_clubs, phpal_clubs as CLEXT, phpal_matchs, phpal_journees, phpal_equipes, phpal_equipes as EXT, phpal_gr_championnats
			 WHERE phpal_clubs.id=phpal_equipes.id_club
			 AND CLEXT.id=EXT.id_club
			 AND phpal_equipes.id=phpal_matchs.id_equipe_dom
    ...
    if (mysqli_num_rows($resultat)=="0") 
      ...
      while ($row=mysqli_fetch_array($resultat) and $i<$nb_matchs)
      {
       ...
       $resultat2=$idconnect->query( " SELECT pronostic 
									   ...
									   WHERE phpal_pronostics.id_match='$row[2]' 
									   AND phpal_membres.id=phpal_pronostics.id_membre 
       ...
       $resultat2=$idconnect->query("SELECT tps_avant_prono FROM phpal_gr_championnats WHERE id='$gr_champ'");
      //$resultat2=mysql_query($requete2) or die ("probleme " .mysql_error());
