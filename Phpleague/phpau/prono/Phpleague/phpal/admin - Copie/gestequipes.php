...
elseif ($go<>"1")
{
       ...
       if ($nb2!=$nb_rens)
       {
         mysqli_query($idconnect, ("DELETE FROM phpal_donnee WHERE id_clubs=$id"));
				
			$result=$idconnect->query("SELECT id FROM phpal_rens");
			while($row=mysqli_fetch_array($result))
			{
			 mysqli_query($idconnect, ("INSERT INTO phpal_donnee (id_clubs, id_rens) VALUES ('$id', '$row[0]')")) or die ("probleme " .mysql_error());
			}
        }
       
       echo "<h3>";
       $query=$idconnect->query("SELECT id, nom FROM phpal_clubs WHERE id='$id'");
       ...
       $result = mysql_query ($query) or die(mysql_error());;
        ...
        $result2 = mysql_query ($query2);
         ...
         $result=$idconnect->query("SELECT phpal_classe.nom, phpal_classe.id FROM phpal_classe order by rang");
        // $result = mysql_query ($query);
		 ...
		 $result=$idconnect->query("SELECT annee, phpal_divisions.nom, phpal_championnats.id, phpal_equipes.id
