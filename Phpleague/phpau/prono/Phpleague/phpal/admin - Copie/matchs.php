...
if (empty ($numero))
  {
    $result=$idconnect->query("
			SELECT max(phpal_journees.numero) 
...
for ($counter=$nb_rencontres; $counter>0 ; $counter=$counter-1 ) // Nb de rencontres dans la journée
	   ...
	   $result= $idconnect->query("
				SELECT DISTINCT phpal_clubs.nom, phpal_equipes.id 
				FROM phpal_clubs, phpal_equipes 
				WHERE phpal_equipes.id_champ='$champ' 
				AND phpal_clubs.id=phpal_equipes.id_club 
				ORDER BY phpal_clubs.nom");
...
echo "<small>MATCH_SUM = ".$sum_day."  </small>";
    $result=$idconnect->query("
			SELECT count(DISTINCT phpal_matchs.id_equipe_dom), 
			count(DISTINCT phpal_matchs.id_equipe_ext) 
			FROM phpal_matchs, phpal_journees   
			WHERE phpal_matchs.id_journee=phpal_journees.id    
			AND phpal_journees.id_champ='$champ'    
			AND phpal_journees.numero='$x' ");
			//$result=mysql_query($query);
...
$result=$idconnect->query(" 
		SELECT sum(phpal_matchs.id_equipe_dom), 
			   sum(phpal_matchs.id_equipe_ext) 
		FROM phpal_matchs, phpal_journees 
		WHERE phpal_matchs.id_journee=phpal_journees.id 
		AND phpal_journees.id_champ='$champ'  ");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT   phpal_matchs.id_equipe_dom as DOM,  count(*) as ct 
		 FROM phpal_matchs, phpal_journees 
		 WHERE  phpal_journees.id_champ='$champ' 
		 AND phpal_matchs.id_journee=phpal_journees.id 
		 GROUP BY DOM");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT phpal_matchs.id_equipe_ext as DOM,  
		 count(*) as ct 
		 FROM phpal_matchs, phpal_journees 
		 WHERE  phpal_journees.id_champ='$champ' 
		 AND phpal_matchs.id_journee=phpal_journees.id 
