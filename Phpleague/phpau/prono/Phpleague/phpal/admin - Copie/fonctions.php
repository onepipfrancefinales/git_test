...
function champ_menu($idconnect)
{
 $result=$idconnect->query(" 
		 SELECT phpal_championnats.id, phpal_divisions.nom, phpal_saisons.annee, (phpal_saisons.annee)+1 
		 FROM phpal_championnats, phpal_divisions, phpal_saisons 
...
function db_clmnt($champ, $debut, $fin, $cache, $idconnect)
{
  ...
  if($cache=="1")
	...
	$dom=$idconnect->query("
		 SELECT dom.id, count(dom.id), phpal_clubs.nom, sum(buts_dom), sum(buts_ext) 
	     FROM phpal_equipes as dom, phpal_clubs, phpal_matchs, phpal_journees, phpal_championnats
		 WHERE dom.id_champ='$champ'
         AND dom.id_club=phpal_clubs.id
         AND dom.id=phpal_matchs.id_equipe_dom
  ...
  $result=$idconnect->query("SELECT phpal_clubs.nom, phpal_equipes.penalite, phpal_equipes.id
	...
	*/
    while($row = mysqli_fetch_array($result))
    {
		...
		if($cache=="1")
		{
		 mysqli_query($idconnect, ("INSERT INTO phpal_clmnt_cache(NOM, POINTS, JOUES, G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN, DOMPOINTS, DOMJOUES, DOMG, DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, EXTPOINTS, EXTJOUES, EXTG, EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, ID_EQUIPE, ID_CHAMP) 
		...
		{
		 mysqli_query($idconnect, ("INSERT INTO phpal_clmnt(NOM, POINTS, JOUES, G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN, DOMPOINTS, DOMJOUES, DOMG, DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, EXTPOINTS, EXTJOUES, EXTG, EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, ID_EQUIPE, ID_CHAMP) 
									  VALUES('$NOM','$POINTS','$JOUES','$G','$N','$P','$BUTSPOUR','$BUTSCONTRE','$DIFF','$PEN','$DOMPOINTS','$DOMJOUES','$DOMG','$DOMN','$DOMP','$DOMBUTSPOUR','$DOMBUTSCONTRE','$DOMDIFF','$EXTPOINTS','$EXTJOUES','$EXTG','$EXTN','$EXTP','$EXTBUTSPOUR','$EXTBUTSCONTRE','$EXTDIFF','$ID_EQUIPE','$ID_CHAMP')"));
...
// Nombres de renseignement dans cette classe à partir de l'id_classe  (utilisé dans phpal_classe.php)
...
// Nombres de classes enregistrées (ulilisé dans phpal_classe.php)
