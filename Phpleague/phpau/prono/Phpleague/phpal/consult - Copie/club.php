...
$result= $idconnect->query("SELECT phpal_clubs.id, phpal_clubs.nom, id_champ, id_club
...
$result= $idconnect->query("SELECT phpal_classe.nom, phpal_classe.id 
							FROM phpal_classe 
							order by rang");
//$result=mysql_query ($query);
...
$result= $idconnect->query("SELECT phpal_equipes.id 
							FROM phpal_equipes, phpal_clubs 
							WHERE phpal_clubs.id='$id_clubs' 
							AND id_champ='$champ' 
...
{ $row[0]=addslashes($row[0]);
 $resultats1 = $idconnect->query("SELECT nom, prenom, phpal_joueurs.id
