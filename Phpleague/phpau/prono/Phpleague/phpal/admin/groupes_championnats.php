...
{   
  mysqli_query ($idconnect,("INSERT INTO phpal_gr_championnats (nom) VALUES ('$nom_group')")) or die ("probleme " .mysql_error($idconnect));
}


if ($confirm=="ok" and $gr_champ and $action=="supp")
{
      mysqli_query($idconnect,("DELETE FROM phpal_gr_championnats WHERE id ='$gr_champ' "))or die ("probleme " .mysql_error($idconnect));
}
?>

<br /><font class=phpal><?php echo GR_LEAGUE; ?></font><br /><br />



<table class=phpal width="90%">
            <tr>
              <td class=phpal2><?php echo MENU_ID; ?></td>
              <td class=phpal2><?php echo MENU_NOM; ?></td>
              <td class=phpal2></td>
            </tr>
            <?php affich_gr_championnats ($gr_champ, $action, $idconnect); ?>
            <tr>
              <td class=phpal5 align="right" colspan="4"><a href="?page=groupes_championnats&action=creer">> <?php echo ADMIN_GR_CHAMP_CREER;?></a></td>
            </tr>
