...
{
  $query="SELECT points FROM phpal_clmnt_pronos, phpal_membres WHERE phpal_membres.id=phpal_clmnt_pronos.id_membre and phpal_membres.id_prono='$user_id' and type='general' AND id_champ='$gr_champ'";
...
{
  $query="SELECT id_prono FROM phpal_clmnt_pronos, phpal_membres WHERE phpal_membres.id=phpal_clmnt_pronos.id_membre AND id_champ='$gr_champ' AND type='general' AND id_champ='$gr_champ' ORDER by points desc, participation desc, phpal_membres.pseudo";
  $result=mysql_query($query) or die (mysql_error());
  $i = "1";
  while ($row=mysql_fetch_array($result))
  {
...
function affiche_clmnt_mensuel_en_cours ($user_id, $gr_champ)
{
  $query="SELECT id_prono
          FROM phpal_clmnt_pronos, phpal_membres
          WHERE phpal_membres.id=phpal_clmnt_pronos.id_membre AND id_champ='$gr_champ' AND type='mensuel_en_cours' AND id_champ='$gr_champ' ORDER by points desc, participation desc, phpal_membres.pseudo";
  $result=mysql_query($query) or die (mysql_error());
  $i = "1";
  while ($row=mysql_fetch_array($result))
  { 
...
{
  $query="SELECT id_prono FROM phpal_clmnt_pronos, phpal_membres WHERE phpal_membres.id=phpal_clmnt_pronos.id_membre AND id_champ='$gr_champ' AND id_champ='$gr_champ' AND type='mensuel_30_jours' ORDER by points desc, participation desc, phpal_membres.pseudo";
  $result=mysql_query($query) or die (mysql_error());
  $i = "1";
  while ($row=mysql_fetch_array($result))
  { if ($row[0]==$user_id){$class=$i;}
...
{
  $query="SELECT id_prono FROM phpal_clmnt_pronos, phpal_membres WHERE phpal_membres.id=phpal_clmnt_pronos.id_membre AND id_champ='$gr_champ' AND type='hebdo' AND id_champ='$gr_champ' ORDER by points desc, participation desc, phpal_membres.pseudo";
  $result=mysql_query($query) or die (mysql_error());
  $i = "1";
  while ($row=mysql_fetch_array($result))
  { if ($row[0]==$user_id){$class=$i;}
...
{
$query="SELECT pseudo, points, participation FROM phpal_clmnt_pronos
...
{
$query="SELECT phpal_clubs.nom, CLEXT.nom, phpal_matchs.buts_dom, phpal_matchs.buts_ext, phpal_matchs.id, phpal_matchs.date_reelle, phpal_journees.numero
...
$query2= "SELECT pronostic FROM phpal_pronostics, phpal_gr_championnats WHERE phpal_pronostics.id_match='$row[4]' AND phpal_gr_championnats.id='$gr_champ' AND id_membre=id_master";
...
{
   mysql_query("DELETE FROM phpal_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_en_cours'") or die (mysql_error());

   $query="SELECT id_membre, pseudo, sum(points) as total, sum(participation) as participations
   FROM phpal_membres, phpal_pronostics, phpal_matchs, phpal_gr_championnats
   WHERE phpal_pronostics.id_champ=phpal_gr_championnats.id
   AND phpal_gr_championnats.id='$gr_champ'
   AND id_membre=phpal_membres.id
   AND phpal_matchs.id=id_match
   ...
   while ($row=mysql_fetch_array($result))
         ...
         mysql_query("INSERT INTO phpal_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_en_cours')") or die (mysql_error());
         }
}          

if ($type=="mensuel_30_jours")
{
   mysql_query("DELETE FROM phpal_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_30_jours'") or die (mysql_error());
   ...
   $result=mysql_query ($query) or die ("probleme " .mysql_error());
       ...
       {
       $row[1]=addslashes($row[1]);
       mysql_query("INSERT INTO phpal_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_30_jours')") or die (mysql_error());
       }
}

if ($type=="hebdo")
{
   mysql_query("DELETE FROM phpal_clmnt_pronos WHERE id_champ='$gr_champ' AND type='hebdo'") or die (mysql_error());
   ...
   $result=mysql_query ($query) or die ("probleme " .mysql_error());
       ...
       {
       $row[1]=addslashes($row[1]);
       mysql_query("INSERT INTO phpal_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'hebdo')") or die (mysql_error());
       }
}
...
{
  $requete="SELECT pseudo FROM phpal_membres, phpal_gr_championnats WHERE phpal_gr_championnats.id_master=phpal_membres.id AND phpal_gr_championnats.id='$gr_champ'";
...
{
  $requete="SELECT DISTINCT id, nom FROM phpal_gr_championnats WHERE  phpal_gr_championnats.activ_prono='1' ORDER by id";
  $resultat=mysql_query ($requete) or die ("probleme " .mysql_error());;
...
if ($user_pseudo and $user_mdp)
	{
        $requete= "SELECT mot_de_passe, id_prono FROM phpal_membres WHERE pseudo='$user_pseudo'";
