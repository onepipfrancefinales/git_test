...
$autoidentification = isset($_POST['autoidentification']) ? $_POST['autoidentification'] : NULL;
	...
	else
	{
        ...
        $pass = $_REQUEST['pass'];
        $query = "SELECT mot_de_passe FROM phpal_membres WHERE pseudo='$user'";
	...
	$time = time();
	$query = "SELECT * FROM phpal_membres WHERE pseudo='$user'";
	$result = mysql_query($query) or die('Erreur SQL !<br />'.$sql.'<br />'.mysql_error());
	$row = mysql_fetch_array($result);
	$mot_de_passe = $row['mot_de_passe'];

	mysql_query("UPDATE phpal_membres SET ip='$ip', last_connect='$time' WHERE pseudo='$user'");
