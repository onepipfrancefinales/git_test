...
if ($email_verif=="ok" and $pseudo_verif=="ok" and $mdp_verif=="ok" and $mail_verif=="ok")
{
  ...
  mysql_query("INSERT INTO phpal_membres (pseudo, id_prono, mot_de_passe, mail, nom_site, nom, prenom, adresse, code_postal, ville, pays, date_naissance, profession, mobile, ip, last_connect, admin )
               VALUES ('$pseudo', '$id_prono', '$mdpcrypt', '$mail', '$site', '$nom', '$prenom', '$adresse', '$code_postal', '$ville', '$mdp', '$date_naissance', '$profession', '$mobile','$ip','$last_connect','1' )") or die ("probleme " .mysql_error());

  $requete="SELECT id FROM phpal_membres WHERE id_prono='$id_prono'";
  $result=mysql_query($requete) or die ("probleme " .mysql_error());
  $row=mysql_fetch_array($result);
  $id_membre=$row[0];

  mysql_query("INSERT INTO phpal_pronostics (id_membre, id_champ) VALUES ('$id_membre', '$gr_champ')") or die ("probleme " .mysql_error());

  $requete = "SELECT pseudo, mail, nom_site, url_site FROM phpal_membres WHERE admin='1'";
  $result=mysql_query($requete) or die ("probleme " .mysql_error());
  $row=mysql_fetch_array($result);
  $pseudo_admin=$row[0];
  $mail_admin=$row[1];
...
<p><font face=\"Verdana\" size=\"2\"><a href=\"mailto:mailto:francefinalesrugby@free.fr\">Administrateur France Finales Rugby App</a></font></p>
  ...
  if ($email)
  {
    ...
    echo "<tr><td colspan=\"2\" align=\"center\">".PRONO_INSCRIPTION_SUCCES."<br /><a href=\"http://francefinalesrugby.franceserv.com/phpaleague/phpal/admin2/index.php?champ=$champ\">".PRONO_INSCRIPTION_CONNEXION."</a></td></tr>";
    echo "</table>";
  }
  else 
  {
    echo "<table align=\"center\">";
    echo "<tr><td colspan=\"2\" align=\"center\">".PRONO_INSCRIPTION_ECHOUE."<br /><a href=\"http://francefinalesrugby.franceserv.com/phpaleague/phpal/prono/index.php?page=inscription&champ=$champ\">Connexion !</a></td></tr>";
    echo "</table>";
  }
...
{
 echo "<table align=\"center\">";
  echo "<tr><td colspan=\"2\" align=\"center\">$message<br /><a href=\"http://francefinalesrugby.franceserv.com/phpaleague/phpal/prono/index.php?page=inscription&champ=$champ\">".R�essayer."</a></td></tr>";
  echo "</table>";
}
