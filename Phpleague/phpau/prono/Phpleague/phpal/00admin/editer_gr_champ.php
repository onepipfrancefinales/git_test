...
if ($action3=="2")
 {
 $requete="SELECT id FROM phpal_gr_championnats WHERE id='$gr_champ'";
 ...
 $nb_resultats=mysql_num_rows($resultats);
   ...
   if ($nb_resultats>0)
     {
       //Màj des paramètres
       mysql_query ("UPDATE phpal_gr_championnats SET pts_prono_exact='$pts_prono_exact', pts_prono_participation='$pts_prono_participation', id_master='$id_master', tps_avant_prono='$tps_avant_prono', activ_prono='$activ_prono' WHERE id='$gr_champ'") or die (mysql_error());
     }

   else
     {
       // Insertion des paramètres dans la bdd
       $requete="INSERT INTO phpal_gr_championnats (id_champ, pts_prono_exact, pts_prono_participation, id_master, tps_avant_prono) VALUES ('$champ', '$pts_prono_exact', '$pts_prono_participation', '$id_master', '$tps_avant_prono')";
...
{
  $requete = "SELECT * FROM phpal_gr_championnats WHERE id='$gr_champ'";
  ...
  if (isset($row["tps_avant_prono"])){$tps_avant_prono = $row["tps_avant_prono"];} else {$tps_avant_prono=0;}
        ...
        mysql_query ("DELETE FROM phpal_gr_championnats WHERE id = '$gr_champ'") or die ("probleme " .mysql_error());

         reset ($champ);
	 while ( list($cle, $val)= each($champ))
         {
mysql_query ("INSERT INTO phpal_gr_championnats (id, nom, id_champ, activ_prono, pts_prono_exact, pts_prono_participation, id_master,tps_avant_prono)
...
<table class=phpal width="80%">
            <tr>
              <td class=phpal2 align="center" colspan="4"><?php echo ADMIN_GR_CHAMP_EDIT." " ; affich_gr_champ ($gr_champ, $idconnect); ?></td><td class=phpal2 align="right"><a href="#" onclick="window.open('Assistant_fr/equipes_2.htm','Assistant','toolbar=0,location=0,directories=0,status=0,scrollbars=1,resizable=0,copyhistory=0,menuBar=0,width=512,height=512');return false;"><img border="0" alt="Assistant" src="aide.gif"></a></td>
            </tr>

            <tr>
              <td align="center" class=phpal6 colspan="4"><b><?php echo ADMIN_GR_CHAMP_EDIT_2; ?></b></td>
            </tr>
            
            <tr>
              <td class=phpal3 colspan="3">
              ...
              <td class=phpal3 align=right colspan="2">
            ...
            <tr>
              <td align="center" class=phpal6 colspan="4"><b><?php echo ADMIN_GR_CHAMP_EDIT_5; ?></b></td>
            </tr>
              <td class=phpal3 colspan="5" align="center"><?php  champ_gr_menu ($gr_champ, $idconnect); ?></td>
...
<table class=phpal width="80%">
            <tr>
              <td class=phpal2 align="center" colspan="3"><?php echo ADMIN_PARAM_MSG13." "; affich_gr_champ ($gr_champ, $idconnect); ?></td>
            </tr>
            <tr>
...
$query2="SELECT pseudo, id FROM phpal_membres order by pseudo";
