...
reset ($date);
	...
	$query="UPDATE phpab_journees SET date_prevue='$dateUS' WHERE numero='$y' AND id_champ='$champ'";
	$result= mysql_query($query);

   $query2="SELECT phpab_journees.id FROM phpab_journees, phpab_matchs WHERE phpab_journees.id=phpab_matchs.id_journee and phpab_journees.numero='$y' AND id_champ='$champ'";
   $result2=mysql_query($query2) or die ("probleme " .mysql_error());
      while ($row2= mysql_fetch_array($result2))
      {
        $date_us_matchs="$dateUS $heure:$minute:00";
        $query="UPDATE phpab_matchs SET date_reelle='$date_us_matchs' WHERE phpab_matchs.id_journee=$row2[0]";
        $result= mysql_query($query);
      }
...
<table class=phpab width="80%">
  ...
  $requete="SELECT * FROM phpab_journees WHERE id_champ='$champ'";
  ...
  {
    mysql_query("DELETE FROM phpab_journees WHERE id_champ='$champ'");
    $x=1;
    while ($x <= ($nb_equipes*2)-2)
		{
		$query="INSERT INTO phpab_journees (numero, id_champ) VALUES ('$x','$champ')";
		$result= mysql_query($query);
		$x++;
		}
  }
  ...
  $result=mysql_query("SELECT numero, date_prevue FROM phpab_journees WHERE id_champ='$champ' ORDER BY numero");
    $i=0;
