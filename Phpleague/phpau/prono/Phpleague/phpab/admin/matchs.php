...
if (empty ($numero))
  {
    $requete="SELECT max(phpab_journees.numero) FROM phpab_journees, phpab_matchs where phpab_journees.id=phpab_matchs.id_journee and buts_dom is not NULL and phpab_journees.id_champ='$champ'";
...
{ 
  ...
  // recherche de id_journee de la journee miroir si existe
    if ($miroir<>"none")
     {     
       $query = "SELECT id FROM phpab_journees WHERE  id_champ='$champ' AND numero='$miroir'";
  ...
  $x=$numero-1;
  $query = "SELECT id FROM phpab_journees WHERE id_champ='$champ' AND numero='$x'";
  $result=mysql_query ($query);
       while ($row=mysql_fetch_array($result))
       {
         $id_journee=$row[0] ;
...
<table class=phpab width="80%">
            ...
            <?php
  $requete="SELECT id FROM phpab_journees WHERE id_champ='$champ'";
  $resultats=mysql_query($requete);
...
$query = "SELECT id FROM phpab_journees WHERE id_champ='$champ' AND numero='$numero'";
$result=mysql_query ($query);
          while ($row=mysql_fetch_array($result))
          {
          $id_journee=$row[0] ;
...
for ($counter=$nb_rencontres; $counter>0 ; $counter=$counter-1 ) // Nb de rencontres dans la journée
		...
		$counter0=$counter-1;
		$query=" SELECT * FROM phpab_matchs WHERE id_journee='$id_journee' LIMIT $counter0,1";
        ...
        $query = "SELECT DISTINCT phpab_clubs.nom, phpab_equipes.id 
        FROM phpab_clubs, phpab_equipes WHERE phpab_equipes.id_champ='$champ' AND phpab_clubs.id=phpab_equipes.id_club ORDER BY phpab_clubs.nom";
...
//JOURNEE MIROIR ?
		...
		$miroir="none";
                ...
                $query = "SELECT numero, id FROM phpab_journees WHERE id_champ='$champ'  ORDER BY phpab_journees.numero";
                $result=mysql_query($query);
                while ($row = mysql_fetch_array($result)) {
...
// CHECK SUM DU CHAMPIONNAT
  $query=" SELECT sum(id) FROM phpab_equipes WHERE id_champ='$champ' ";
...
while($x<=$nb_journees)
    {
$query=" SELECT sum(phpab_matchs.id_equipe_dom), sum(phpab_matchs.id_equipe_ext) FROM phpab_matchs, phpab_journees WHERE phpab_matchs.id_journee=phpab_journees.id AND phpab_journees.id_champ='$champ'   AND phpab_journees.numero='$x' ";
$result=mysql_query($query);
$sum=mysql_fetch_array($result);
$sum_day=$sum[0]+$sum[1];
echo "<small>MATCH_SUM = ".$sum_day."  </small>";
    $query="SELECT count(DISTINCT phpab_matchs.id_equipe_dom), count(DISTINCT phpab_matchs.id_equipe_ext) FROM phpab_matchs, phpab_journees   WHERE phpab_matchs.id_journee=phpab_journees.id    AND phpab_journees.id_champ='$champ'    AND phpab_journees.numero='$x' ";
    $result=mysql_query($query);
...
$query=" SELECT sum(phpab_matchs.id_equipe_dom), sum(phpab_matchs.id_equipe_ext) FROM phpab_matchs, phpab_journees WHERE phpab_matchs.id_journee=phpab_journees.id AND phpab_journees.id_champ='$champ'  ";
$result=mysql_query($query);
$sum=mysql_fetch_array($result);
$sumsum=$sum[0];
 $query="SELECT sum(phpab_matchs.id_equipe_dom), sum(phpab_matchs.id_equipe_ext) FROM phpab_matchs, phpab_journees WHERE  phpab_journees.id_champ='$champ' AND phpab_matchs.id_journee=phpab_journees.id  ";
$result=mysql_query($query);
while($row=mysql_fetch_array($result))
    {
 ...
 $query="SELECT   phpab_matchs.id_equipe_dom as DOM,  count(*) as ct FROM phpab_matchs, phpab_journees WHERE  phpab_journees.id_champ='$champ' AND phpab_matchs.id_journee=phpab_journees.id GROUP BY DOM";
$result=mysql_query($query); 
while($row=mysql_fetch_array($result))
		{
 ...
 $query="SELECT phpab_matchs.id_equipe_ext as DOM,  count(*) as ct FROM phpab_matchs, phpab_journees WHERE  phpab_journees.id_champ='$champ' AND phpab_matchs.id_journee=phpab_journees.id GROUP BY DOM";
$result=mysql_query($query); 
while($row=mysql_fetch_array($result))
		{
		if ($row[1]<>$nb_journees/2)
