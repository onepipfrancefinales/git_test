...
if ($confirm=="ok" and $champ and $action=="supp")
{
  ...
  //$resultats=mysql_query($requete);
     while($row = mysqli_fetch_array($resultats))
      {
        mysqli_query ("DELETE FROM phpab_matchs where id_equipe_dom='$row[0]' ") or die ("probleme" .mysqli_error());
        mysqli_query ("DELETE FROM phpab_matchs where id_equipe_ext='$row[0]' ") or die ("probleme" .mysqli_error());
      }
    mysqli_query("DELETE FROM phpab_championnats where id='$champ' ") or die ("probleme " .mysqli_error());
    mysqli_query("DELETE FROM phpab_journees WHERE  id_champ = '$champ'")or die ("probleme " .mysqli_error());
    mysqli_query("DELETE FROM phpab_equipes WHERE id_champ ='$champ' ")or die ("probleme " .mysqli_error());
    mysqli_query("DELETE FROM phpab_parametres WHERE id_champ ='$champ' ")or die ("probleme " .mysqli_error());
