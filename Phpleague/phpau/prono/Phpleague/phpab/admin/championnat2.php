...
{
  mysql_query ("INSERT INTO phpab_saisons (annee) values ('$saison')") or die ("probleme " .mysql_error());
}

if ($action2=="creer" and $division and $action=="creer")
{
  mysql_query ("INSERT INTO phpab_divisions (nom) values ('$division')") or die ("probleme " .mysql_error());
}

if ($action3=="creer" and $action=="creer")
{
  mysql_query ("INSERT INTO phpab_championnats (id_division, id_saison) values ('$division','$saison')") or die ("probleme " .mysql_error());
}


if ($action2=="supp" and $saison and $action=="creer")
{
  mysql_query ("DELETE FROM phpab_saisons WHERE id ='$saison'") or die ("probleme " .mysql_error());
}

if ($action2=="supp" and $division and $action=="creer")
{
  mysql_query ("DELETE FROM phpab_divisions WHERE id ='$division'") or die ("probleme " .mysql_error());
}

if ($confirm=="ok" and $champ and $action=="supp")
{
  $requete="SELECT phpab_equipes.id FROM phpab_equipes, phpab_championnats WHERE id_champ='$champ'";
  $resultats=mysql_query($requete);
     while($row = mysql_fetch_array($resultats))
      {
        mysql_query ("DELETE FROM phpab_matchs where id_equipe_dom='$row[0]' ") or die ("probleme" .mysql_error());
        mysql_query ("DELETE FROM phpab_matchs where id_equipe_ext='$row[0]' ") or die ("probleme" .mysql_error());
      }
    mysql_query("DELETE FROM phpab_championnats where id='$champ' ") or die ("probleme " .mysql_error());
    mysql_query("DELETE FROM phpab_journees WHERE  id_champ = '$champ'")or die ("probleme " .mysql_error());
    mysql_query("DELETE FROM phpab_equipes WHERE id_champ ='$champ' ")or die ("probleme " .mysql_error());
    mysql_query("DELETE FROM phpab_parametres WHERE id_champ ='$champ' ")or die ("probleme " .mysql_error());
