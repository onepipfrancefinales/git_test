...
function affiche_clmnt_general ($user_id, $gr_champ, $idconnect)
{
  ...
  FROM phpab_clmnt_pronos, phpab_membres 
  WHERE phpab_membres.id=phpab_clmnt_pronos.id_membre 
  AND id_champ='$gr_champ' AND type='general' 
  AND id_champ='$gr_champ' 
...
$result2=$idconnect->query("SELECT pronostic 
							...
							AND phpab_gr_championnats.id='$gr_champ' AND id_membre=id_master");
...
if ($nb_pronos == "0") {$prono="0";}
{
  ...
  $resultats2=$idconnect->query("SELECT tps_avant_prono 
  FROM phpab_gr_championnats WHERE id='$gr_champ'");
...
if ($type=="mensuel_30_jours")
{
   ...
   //$result=mysql_query ($query) or die ("probleme " .mysql_error());
       ...
       {
       $row[1]=addslashes($row[1]);
       $bdd->exec("INSERT INTO phpab_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_30_jours')") ;
       }
}
...
if ($type=="hebdo")
{
  ...
  // $result=mysql_query ($query) or die ("probleme " .mysql_error());
       ...
       {
       $row[1]=addslashes($row[1]);
       mysqli_query ($idconnect,("INSERT INTO phpab_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'hebdo')"));
       }
}
if (isset($_REQUEST['complet'])) {$complet=$_REQUEST['complet'];} else {$complet='';}

$result=$idconnect->query(" SELECT pseudo, points, participation FROM phpab_clmnt_pronos
...
// $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db2');	
 ...
 $result=$idconnect->query(" SELECT pseudo 
...
// $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db2');	
  ...
  $resultat=$idconnect->query(" SELECT DISTINCT id, nom 
							  FROM phpab_gr_championnats WHERE  phpab_gr_championnats.activ_prono='1' 
							  ORDER by id");
  //$resultat=mysql_query ($requete) or die ("probleme " .mysql_error());;
...
function nb_equipes($id_champ)
		...
		//$idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db2');	
		 ...
		 FROM phpab_equipes WHERE id_champ='$id_champ'");
...
if ($user_pseudo and $user_mdp)
	{
	   ...
	   FROM phpab_membres WHERE pseudo='$user_pseudo'");
