...
if ($email_verif=="ok" and $pseudo_verif=="ok" and $mdp_verif=="ok" and $mail_verif=="ok")
{
 ...
 // $result=mysql_query($requete) or die ("probleme " .mysql_error());
  ...
  $result =$idconnect->query( "SELECT pseudo, mail, nom_site, url_site FROM phpab_membres WHERE admin='1'");
//  $result=mysql_query($requete) or die ("probleme " .mysql_error());
  $row=mysqli_fetch_array($result);
  $pseudo_admin=$row[0];
  $mail_admin=$row[1];
...
<p><font face=\"Verdana\" size=\"2\"><a href=\"mailto:mailto:francefinalesrugby@free.fr\">Administrateur France Finales Rugby App</a></font></p>
  ...
  if ($email)
  {
    ...
    echo "<tr><td colspan=\"2\" align=\"center\">".PRONO_INSCRIPTION_SUCCES."<br /><a href=\"/Phpfedleague/phpab/admin2/index.php?champ=$champ\">".PRONO_INSCRIPTION_CONNEXION."</a></td></tr>";
    echo "</table>";
  }
  else 
  {
    echo "<table align=\"center\">";
    echo "<tr><td colspan=\"2\" align=\"center\">".PRONO_INSCRIPTION_ECHOUE."<br /><a href=\"/Phpleague/phpab/prono/index.php?page=inscription&champ=$champ\">Connexion !</a></td></tr>";
    echo "</table>";
  }
