...
if (isset($type))
{
   ...
   $res=$idconnect->query("SELECT accession, barrage, estimation, relegation, id_equipe_fetiche, fiches_clubs
    ...
    $res2=$idconnect->query("SELECT phpau_equipes.id FROM phpau_equipes, phpau_clubs
...
if ($debut=="1" and $fin==$nb_journees)
{
   ...
   $res3=$idconnect->query("SELECT max(phpau_journees.numero) FROM phpau_journees, phpau_matchs WHERE phpau_journees.id=phpau_matchs.id_journee and buts_dom is not NULL and     phpau_journees.id_champ='$champ'");
