...
function aff_journee($champ, $numero, $legende, $proba, $fiches_clubs, $id_equipe_fetiche, $idconnect)
  ...
  $color=0;
        $result=$idconnect->query("SELECT cldom.nom as cldom, clext.nom as clext, phpau_matchs.buts_dom, phpau_matchs.buts_ext,
...
//mise en forme tableau oppositions
        ...
        $annee = 0;
             ...
             $extproba= $row[3];
                ...
                {
                $result2=$idconnect->query("SELECT DOMBUTSPOUR, DOMG, DOMN, DOMP, DOMBUTSCONTRE FROM phpau_clmnt_cache WHERE ID_EQUIPE='$row[eqdom]'");
                ...
                $result2=$idconnect->query("SELECT EXTG, EXTN, EXTP, EXTBUTSCONTRE, EXTBUTSPOUR FROM phpau_clmnt_cache WHERE ID_EQUIPE='$row[eqext]'");
               // $result2=mysql_query($query2);
...
/************* mise en commentaire suite passage php7 ******************
  $requete=$idconnect->query("SELECT phpau_clubs.nom, CLEXT.nom, phpau_matchs.buts_dom, phpau_matchs.buts_ext,
...
{
  $nb_requete=0;
  if($cache=="1"){ mysqli_query("DELETE FROM phpau_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysqli_error());}
  else{ mysqli_query("DELETE FROM phpau_clmnt") or die (mysqli_error());}
          $nb_requete++;
...
$result=$idconnect->query("SELECT pts_victoire, pts_nul, pts_defaite FROM phpau_parametres WHERE id_champ='$champ'");
$nb_requete++;
...
$query=$idconnect->query("SELECT phpau_clubs.nom FROM phpau_clubs, phpau_equipes, phpau_championnats
