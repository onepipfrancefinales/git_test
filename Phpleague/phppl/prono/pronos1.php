...
if ($action == "reset") {
  ...
  while ($row = mysqli_fetch_array($resultat)) {
    $resultat1 = $idconnect->query("SELECT phppl_matchs.date_reelle 
                                    FROM phppl_matchs 
                                    WHERE phppl_matchs.id='$row[0]'");
...
if ($action == "valid_pronos") {
  ...
  for ($i = 1; $i <= $_REQUEST['nb_fiche']; $i++) {
    ...
    $resultat = $idconnect->query("SELECT phppl_matchs.date_reelle 
                                   FROM phppl_matchs 
                                   WHERE phppl_matchs.id='$id_match[$i]'");
    ...
    if ($f_prono[$i] !== "undefined") {
      ...
      mysqli_query($idconnect, ("DELETE FROM phppl_pronostics 
                                 WHERE pronostic=' '"));

      $resultat = $idconnect->query("SELECT * 
                                     FROM phppl_matchs, phppl_pronostics, phppl_membres 
                                     WHERE phppl_membres.id_prono='$user_id'
      ...
      if ($nb_prono == "0") {
        if ($date_actuelle < ($date_match_timestamp + $temps_avant_prono * 60)) {
          mysqli_query($idconnect, ("INSERT INTO phppl_pronostics (id_membre, pronostic, id_match, id_champ) 
