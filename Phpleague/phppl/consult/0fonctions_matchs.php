...
function nb_journees($id_champ, $idconnect)
         {
        global $nb_journees;
$result=$idconnect->query("SELECT id FROM phpfed2_equipes WHERE id_champ='$id_champ'");
...
function nb_equipes($id_champ, $idconnect)
 ...
 $res=$idconnect->query("SELECT id 
        ...
        // $query="SELECT id FROM phpfed2_equipes WHERE id_champ='$id_champ'";
        // $result=mysql_query($query);
...
function aff_journee($champ, $bdd)
{
 ...
 $res3=$bdd->query("SELECT phpfed2_matchs.id_journee
					FROM phpfed2_journees, phpfed2_matchs 
					WHERE phpfed2_journees.id = phpfed2_matchs.id_journee 
					AND phpfed2_matchs.buts_dom is not NULL 
					AND phpfed2_matchs.buts_ext is not NULL
					AND phpfed2_journees.id_champ = '$champ'
					ORDER BY phpfed2_matchs.id_journee asc");
 ...
 //echo $numero;
 // echo "</br>";
 $result=$bdd->query("SELECT cldom.nom as cldom, clext.nom as clext, phpfed2_matchs.buts_dom, phpfed2_matchs.buts_ext,
                        phpfed2_journees.date_prevue, cldom.id as cliddom, clext.id as clidext, date_reelle,
                        dom.id as eqdom, ext.id as eqext, phpfed2_matchs.id as id_match
                FROM phpfed2_equipes as dom, phpfed2_equipes as ext, phpfed2_matchs, phpfed2_journees,
                     phpfed2_clubs as cldom, phpfed2_clubs as clext
                WHERE phpfed2_matchs.id_equipe_dom=dom.id
                        AND phpfed2_matchs.id_equipe_ext=ext.id
                        AND phpfed2_journees.id_champ='$champ'
                        AND phpfed2_journees.numero='$numero'
  ...
  echo "<br />";
  echo "<table align=\"center\" class=\"tablephpfed22\"  width=\"90%\" cellspacing=\"1\" cellpadding=\"1\"><tr><td><table cellspacing=\"0\" align=\"center\" width=\"100%\" cellpadding=\"1\">\n";
  $x=1;
  ...
  while ($row=$result->fetch())
	{
     ...
     if (($color%2)==0) {$classe="ligne1";} else {$classe="ligne2";}
                ...
                //echo "<tr bgcolor=\"$bgcolor\" class=\"trphpfed2\">";
...
function aff_journeeSupp($champ, $bdd)
{
 ...
 $result=$bdd->query("SELECT cldom.nom as cldom, clext.nom as clext, phpfed2_matchs.buts_dom, phpfed2_matchs.buts_ext,
                ...
                WHERE phpfed2_matchs.id_equipe_dom=dom.id
                        AND phpfed2_matchs.id_equipe_ext=ext.id
                        AND phpfed2_journees.id_champ='$champ'
                        AND phpfed2_journees.numero='$journeeSupp'
                        AND dom.id_club=cldom.id
                        AND ext.id_club=clext.id
                        AND phpfed2_matchs.id_journee=phpfed2_journees.id
                        AND cldom.nom!='exempte'
...
function classementDetaille($champ, $bdd)
{
 ...
 while ($row = $res->fetch())
  ...
  echo "<table class=\"tablephpfed22\" align=\"center\" cellspacing=\"0\" width=\"80%\"><tr class=\"trphpfed23\"><th colspan=\"11\">".$legende."</th></tr>\n";
  echo "<tr class=\"trphpfed23\">
...
function oppositionsDetaillees($champ, $id_equipe, $bdd)
{
 $resultat=$bdd->query("
		  SELECT phpfed2_clubs.nom FROM phpfed2_clubs, phpfed2_equipes 
		  WHERE phpfed2_equipes.id='$id_equipe' 
		  AND phpfed2_equipes.id_club=phpfed2_clubs.id");
...
echo "<table class=\"tablephpfed22\" align=\"center\" cellspacing=\"0\"  width=\"90%\">";

$resultat=$bdd->query("
		SELECT phpfed2_journees.numero, cldom.nom, clext.nom, phpfed2_matchs.buts_dom, phpfed2_matchs.buts_ext, phpfed2_matchs.date_reelle, phpfed2_matchs.id
        FROM phpfed2_equipes as dom, phpfed2_equipes as ext, phpfed2_matchs, phpfed2_journees, phpfed2_clubs as cldom  , phpfed2_clubs as clext
        WHERE phpfed2_matchs.id_equipe_dom=dom.id
        AND phpfed2_matchs.id_equipe_ext=ext.id
        AND (phpfed2_matchs.id_equipe_ext='$id_equipe'
        OR phpfed2_matchs.id_equipe_dom='$id_equipe')
        AND phpfed2_journees.id_champ='$champ'
        AND dom.id_club=cldom.id
        AND ext.id_club=clext.id
        AND phpfed2_matchs.id_journee=phpfed2_journees.id
...
echo "<br />";echo "<br />";
	echo "<tr class=\"trphpfed23\"><td align=\"center\"><font color=\"#FFFF00\"><b>".JOURNEE."</b></font></td><td width=\"5% align=\"center\"></td><td width=\"30%\" ><font color=\"#FFFF00\"><b>".DATE."</b></font></td><td colspan=\"5\ align=\"center\"><font color=\"#FFFF00\"><b>Oppositions</b></font></td></tr>";
    while  ($row=$resultat->fetch())
    {
...
{	
 ...
 $resultats2=$bdd->query("SELECT phpfed2_clubs.nom, CLEXT.nom, phpfed2_matchs.buts_dom, phpfed2_matchs.buts_ext, phpfed2_matchs.id, phpfed2_matchs.date_reelle
 ...
 $result=$bdd->query("SELECT phpfed2_journees.numero, phpfed2_journees.date_prevue, cldom.nom, clext.nom, phpfed2_matchs.buts_dom, phpfed2_matchs.buts_ext, dom.id, ext.id
 ...
 while ($row=$result->fetch())
 {
  ...
  { 
   ...
   {   
    ...
    ?>    
	<table class="tablephpfed22" cellspacing="0" width="290" align="center">
	  <tr class="trphpfed23"><td colspan="3" align="center"><b><font color="#FFFF00">
...
function db_clmnt($champ, $debut, $fin, $cache, $idconnect)
{
  ...
  if($cache=="1"){ 
  mysqli_query("DELETE FROM phpfed2_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysqli_error());
  }
  else{ 
  mysqli_query("DELETE FROM phpfed2_clmnt") or die (mysqli_error());}
          $nb_requete++;
...
$resultats=$idconnect->query=("SELECT pts_victoire, pts_nul, pts_defaite FROM phpfed2_parametres WHERE id_champ='$champ'");
$nb_requete++;
...
// victoires domicile
$resultats=$idconnect->query("SELECT dom.id, count(dom.id), phpfed2_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phpfed2_equipes as dom, phpfed2_clubs, phpfed2_matchs, phpfed2_journees, phpfed2_championnats
WHERE dom.id_champ='$champ'
      AND dom.id_club=phpfed2_clubs.id
      AND dom.id=phpfed2_matchs.id_equipe_dom
      AND buts_dom > buts_ext
      AND phpfed2_championnats.id=phpfed2_journees.id_champ
      AND phpfed2_journees.id=phpfed2_matchs.id_journee
      AND phpfed2_journees.numero>='$debut'
      AND phpfed2_journees.numero<='$fin'
      GROUP by phpfed2_clubs.nom ");
      $nb_requete++;
...
// Defaites domicile
$dom=$idconnect->query("SELECT dom.id, count(dom.id), phpfed2_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phpfed2_equipes as dom, phpfed2_clubs, phpfed2_matchs, phpfed2_journees, phpfed2_championnats
WHERE dom.id_champ='$champ'
      AND dom.id_club=phpfed2_clubs.id
      AND dom.id=phpfed2_matchs.id_equipe_dom
      AND buts_dom < buts_ext
...
$result=$idconnect->query("SELECT phpfed2_clubs.nom FROM phpfed2_clubs, phpfed2_equipes, phpfed2_championnats
WHERE phpfed2_equipes.id_champ=phpfed2_championnats.id
...
// victoires exterieur
$dom=$idconnect->query("SELECT ext.id, count(ext.id), phpfed2_clubs.nom, sum(buts_ext), sum(buts_dom) FROM phpfed2_equipes as ext, phpfed2_clubs, phpfed2_matchs, phpfed2_journees, phpfed2_championnats
WHERE ext.id_champ='$champ'
      AND ext.id_club=phpfed2_clubs.id
      AND ext.id=phpfed2_matchs.id_equipe_ext
...
$result=$idconnect->query("SELECT phpfed2_clubs.nom, phpfed2_equipes.penalite, phpfed2_equipes.id
        FROM phpfed2_clubs, phpfed2_equipes, phpfed2_championnats
        WHERE phpfed2_equipes.id_champ=phpfed2_championnats.id
        AND phpfed2_championnats.id='$champ'
        AND phpfed2_equipes.id_club=phpfed2_clubs.id");
...
//mysql_query("LOCK TABLE phpfed2_clmnt WRITE, phpfed2_equipes WRITE, phpfed2_clubs WRITE") or die (mysql_error());

    if (mysqli_num_rows($result)==0)
    {
    $result="SELECT phpfed2_clubs.nom, phpfed2_equipes.id 
            FROM phpfed2_clubs, phpfed2_equipes, phpfed2_championnats
            WHERE phpfed2_equipes.id_champ=phpfed2_championnats.id
            AND phpfed2_championnats.id='$champ'
            AND phpfed2_equipes.id_club=phpfed2_clubs.id";
   ...
   //$result=mysqli_query($query) or die (mysqli_error());
    ...
    if($cache=="1"){$question.="phpfed2_clmnt_cache ";}
    else{$question.="phpfed2_clmnt ";}
...
if($cache=="1")
	{
	mysqli_query($idconnect, ("DELETE FROM phpfed2_clmnt_cache WHERE nom='exempte'"));
	}
else
	{
	mysqli_query($idconnect, ("DELETE FROM phpfed2_clmnt WHERE nom='exempte'"));
	}
...
{
echo "<table class=\"tablephpfed22\" align=\"center\" cellspacing=\"0\" width=\"80%\" bgcolor=\"#FFFFFF\"><tr class=\"trphpfed23\"><th colspan=\"11\">".$legende."<br /></th></tr>\n";
echo "<tr class=\"trphpfed23\"><th>".CLMNT_POSITION."</th><th align=\"left\">".MENU_NOM."</th><th align=\"left\">".TEAM."</th><th align=\"left\">".ADMIN_JOUEURS_2."</th></tr>\n";
...
//               statistiques, amélioration de la gestion des buteurs pour phpfed2eague.          //
// Auteur      : Alexis MANGIN                                                                 //
// Email       : Alexis@univert.org                                                            //
// Url         : http://www.univert.org                                                        //
// Démo        : http://phpfed2eague.univert.org/demo.php                                         //
// Description : Edition, gestion, fiches phpfed2_clubs, statistiques, mini-classement...         //
...
//{
//$query="SELECT phpfed2_donnee.id, phpfed2_donnee.nom, id_rens, id_clubs, phpfed2_rens.id, phpfed2_rens.nom, phpfed2_rens.id_classe, phpfed2_clubs.id, etat, phpfed2_donnee.url, phpfed2_rens.url
//FROM phpfed2_donnee, phpfed2_rens, phpfed2_clubs
//WHERE id_clubs='$id_clubs'
//      AND id_clubs=phpfed2_clubs.id
//      AND id_rens=phpfed2_rens.id
...
function clmntmini($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini,  $champmini, $requetemini, $nb_dessusmini, $nb_dessousmini, $lienmini, $phpfed2EAGUE_RACINE, $id_equipe_fetiche)
{

echo "<table class=tablephpfed22 align=\"center\" cellspacing=\"0\" width=\"200\"><tr class=trphpfed23><th colspan=10>".$legendemini;
echo "<tr class=trphpfed23>";
        ...
        $x=0;
               ...
               echo "<td align=\"left\">";
                    ...
                    if ($x==0)
                    {
                     ...
                     else {echo "<a href=\"".$phpfed2EAGUE_RACINE."consult/detaileq.php?champ=$champmini&amp;id_equipe=".$row['ID_EQUIPE']."\" target=\"_parent\">$row[$x]</a>";}
                     if ($row['ID_EQUIPE']==$id_equipe_fetiche){echo "</b>";}
                     }
...
{
echo "<table class=\"tablephpfed22\" align=\"center\" cellspacing=\"0\" width=\"80%\"><tr class=\"trphpfed23\"><th colspan=\"11\">".$legende;
echo "</th></tr>\n<tr class=\"trphpfed23\">
<th align=\"center\">".CLMNT_POSITION."</th>
<th align=\"left\">".CLMNT_EQUIPE."</th>
<th align=\"left\">".CLMNT_POINTS."</th>\n";
...
echo "<table class=tablephpfed22 align=\"center\"  cellspacing=\"0\" width=\"95%\"><tr class=trphpfed23><th colspan=10>".$legendemini;
echo "<tr class=trphpfed23>
<th align=\"center\">".CLMNT_POSITION."
<th align=\"center\">".CLMNT_EQUIPE."
<th align=\"left\">".CLMNT_POINTS."
...
<form method="get" action=""><div align="center">
  ...
  <?php       
       $query = "SELECT phpfed2_divisions.nom, phpfed2_saisons.annee, phpfed2_championnats.id
                 FROM phpfed2_championnats, phpfed2_divisions, phpfed2_saisons
                 WHERE phpfed2_championnats.id_division=phpfed2_divisions.id
                 AND phpfed2_championnats.id_saison=phpfed2_saisons.id
                 ORDER BY phpfed2_saisons.annee DESC, phpfed2_championnats.id";
...
function demande_equipe($champ)
{
  ...
  $requete = "SELECT phpfed2_clubs.nom, id_champ, phpfed2_equipes.id
              FROM phpfed2_clubs, phpfed2_equipes
              WHERE phpfed2_equipes.id_champ='$champ' and phpfed2_clubs.id=phpfed2_equipes.id_club
              ORDER BY nom";
$resultat = mysqli_query($requete) or die (mysqli_error());
