...
for ($counter=$nb_rencontres; $counter>0 ; $counter=$counter-1 ) // Nb de rencontres dans la journ�e
        ...
        $query = "SELECT DISTINCT phppl_clubs.nom, phppl_equipes.id FROM phppl_clubs, phppl_equipes
     ...
     //   $result=mysql_query($query) or die(mysql_error());
	   ...
	   $result= $idconnect->query("
				SELECT DISTINCT phppl_clubs.nom, phppl_equipes.id 
...
echo "<small>MATCH_SUM = ".$sum_day."  </small>";
    $result=$idconnect->query("
			SELECT count(DISTINCT phppl_matchs.id_equipe_dom), 
			count(DISTINCT phppl_matchs.id_equipe_ext) 
			FROM phppl_matchs, phppl_journees   
			WHERE phppl_matchs.id_journee=phppl_journees.id    
			AND phppl_journees.id_champ='$champ'    
			AND phppl_journees.numero='$x' ");
			//$result=mysql_query($query);
/*
    while( $row = (mysql_fetch_array($result)) )
        {
...
$result=$idconnect->query(" 
		SELECT sum(phppl_matchs.id_equipe_dom), 
			   sum(phppl_matchs.id_equipe_ext) 
		FROM phppl_matchs, phppl_journees 
		WHERE phppl_matchs.id_journee=phppl_journees.id 
		AND phppl_journees.id_champ='$champ'  ");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT   phppl_matchs.id_equipe_dom as DOM,  count(*) as ct 
		 FROM phppl_matchs, phppl_journees 
		 WHERE  phppl_journees.id_champ='$champ' 
		 AND phppl_matchs.id_journee=phppl_journees.id 
		 GROUP BY DOM");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT phppl_matchs.id_equipe_ext as DOM,  
		 count(*) as ct 
		 FROM phppl_matchs, phppl_journees 
		 WHERE  phppl_journees.id_champ='$champ' 
		 AND phppl_matchs.id_journee=phppl_journees.id 
