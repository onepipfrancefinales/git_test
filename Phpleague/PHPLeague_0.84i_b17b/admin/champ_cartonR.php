...
if ($action3 == 2)
{
		...
		if (isset($nbdom[$x]) OR isset($joueursDom[$a]))
		{
			while($nbdom[$x] > $y)
			{
				...
				else
				{
					...
					{
						$REQ1 = @mysql_query('INSERT INTO phpl_compoCR (id_match, CR, id_effectif) VALUES ('.$matchs_id[$x].', "1", '.$joueursDom[$a].')');
					}
				}
		...
		if (isset($nbext[$x]) OR isset($joueursExt[$b]))
		{
			while($nbext[$x] > $r)
			{
				...
				else
				{
					...
					{
						$REQ2 = @mysql_query('INSERT INTO phpl_compoCR (id_match, CR, id_effectif) VALUES ('.$matchs_id[$x].', "1", '.$joueursExt[$b].')');
					}
				}
	...
	if ($REQ1)
	{
		echo InfoReqSql('InfJoueurDom', 1, 'highlight', 'check', ADMIN_CART_R_DOM_ADD1);
	}
	else
	{
		echo InfoReqSql('InfJoueurDom', 0, 'error', 'alert', GestErreur(ADMIN_CART_R_DOM_ADD3, $REQ1));
	}

	if ($REQ2)
	{
		echo InfoReqSql('InfJoueurExt', 1, 'highlight', 'check', ADMIN_CART_R_EXT_ADD2);
	}
	else
	{
		echo InfoReqSql('InfJoueurExt', 0, 'error', 'alert', GestErreur(ADMIN_CART_R_EXT_ADD4, $REQ2));
	}
}

if ($action4 == 'supp')
{
	$REQ = @mysql_query('DELETE FROM phpl_compoCR WHERE id='.$id_compoCR_supp);
	...
	<table class="Large100P ui-widget ui-widget-content">
		<thead>
			<tr class="ui-widget-header">
				...
				<th class="Large25P TxtRight"><?php echo CR; ?></th>
				<th class="Large10P">&nbsp;</th>
				<th class="Large25P TxtLeft"><?php echo CR; ?></th>
...
AND phpl_compoCR.id_match='$row_menu[4]'";
	 ...
	 $row_menu[0] = stripslashes($row_menu[0]);
		...
		echo "<td class=\"TxtRight\">$row_menu[0]</td>
			<td class=\"TxtRight\">
				...
				<input type=\"hidden\" name=\"nbdom[]\" value=\"4\">
				<input type=\"hidden\" name=\"butd[]\" value=\"1\">
				<input type=\"hidden\" name=\"butv[]\" value=\"1\">
				<input type=\"hidden\" name=\"nbext[]\" value=\"4\">";
...
if (!$result3) die (mysql_error());
 while ($row3=mysql_fetch_array($result3))
		{
			$JOUEUR_DOM_DEL = stripslashes($row3[0].' '.$row3[1]);

			echo '<div class="DeleteRight"><a href="?page=championnat&champ='.$champ.'&annee='.$annee.'&action=compoCR&action4=supp&id_journee='.$id_journee.'&numero='.$numero.'&id_compoCR_supp='.$row3[2].'" class="Large200M"><span>'.$JOUEUR_DOM_DEL.'</span></a></div>'."\n";
		}
...
$y=0;
  while ($y<4)
        { $query3="SELECT phpl_joueurs.nom, phpl_joueurs.prenom, phpl_compoCR.id
          FROM phpl_compoCR, phpl_joueurs, phpl_effectif
  ...
  $nb_buteurs=mysql_num_rows( $result3 );
         ...
         $queryJ="SELECT phpl_effectif.id, phpl_effectif.id_equipe, phpl_joueurs.nom, phpl_joueurs.prenom
                  ...
                  AND phpl_effectif.id_equipe=$row_menu[6] ORDER BY phpl_joueurs.nom, phpl_joueurs.prenom";
	 ...
	 echo "<option value=\"0\"></option>";
		...
		while ($row3 = mysql_fetch_array($result3))
		{
			$JOUEUR_EXT_DEL = stripslashes($row3[0].' '.$row3[1]);

			echo '<div class="DeleteLeft"><a href="?page=championnat&champ='.$champ.'&annee='.$annee.'&action=compoCR&action4=supp&id_journee='.$id_journee.'&numero='.$numero.'&id_compoCR_supp='.$row3[2].'" class="Large200M"><span>'.$JOUEUR_EXT_DEL.'</span></a></div>'."\n";
		}
		...
		<tfoot>
			<tr class="ui-widget-footer">
				<td colspan="5" class="TxtCenter">
					...
					<input type="hidden" name="action" value="compoCR" />
