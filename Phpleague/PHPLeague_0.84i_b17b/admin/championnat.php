...
if ($confirm == 'ok' AND $champ AND $action == 'supp')
{
	...
	if ($REQ1)
	{
		...
		if ($REQ2) { $RESULT.= InfoReqSql('InfSupp2', 0, 'highlight', 'check', 'Equipes domiciles du championnat supprim�es avec succ�s'); }
		else       { $RESULT.= InfoReqSql('InfSupp2', 0, 'error', 'alert', GestErreur(ADMIN_GRP_CHAMP_SUPP2, $REQ4)); }

		if ($REQ3) { $RESULT.= InfoReqSql('InfSupp3', 0, 'highlight', 'check', 'Equipes externes du championnat supprim�es avec succ�s'); }
		...
		if ($REQ4) { $RESULT.= InfoReqSql('InfSupp4', 0, 'highlight', 'check', 'Championnat supprim� avec succ�s'); }
		else       { $RESULT.= InfoReqSql('InfSupp4', 0, 'error', 'alert', GestErreur(ADMIN_GRP_CHAMP_SUPP2, $REQ4)); }

		if ($REQ5) { $RESULT.= InfoReqSql('InfSupp5', 0, 'highlight', 'check', 'Journ�es du championnat supprim� avec succ�s'); }
		else       { $RESULT.= InfoReqSql('InfSupp5', 0, 'error', 'alert', GestErreur(ADMIN_GRP_CHAMP_SUPP2, $REQ5)); }

		if ($REQ6) { $RESULT.= InfoReqSql('InfSupp6', 0, 'highlight', 'check', 'Equipes du championnat supprim� avec succ�s'); }
		else       { $RESULT.= InfoReqSql('InfSupp6', 0, 'error', 'alert', GestErreur(ADMIN_GRP_CHAMP_SUPP2, $REQ6)); }

		if ($REQ7) { $RESULT.= InfoReqSql('InfSupp7', 0, 'highlight', 'check', 'Param�tres du championnat supprim� avec succ�s'); }
...
elseif ($action == 'creer')							   { $STITRE = ' &#062; Création d\'un championnat'; $INC_PAGE = 'creer_champ.php'; }
