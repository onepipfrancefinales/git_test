...
{
	if (empty($REQ)) { $CODE = 'inconnu'; $ERR = 'inconnu'; } else { $CODE = mysql_errno(); $ERR = mysql_error(); }
...
function affich_championnats($champ, $action, $annee)
{
	...
	if (NbrRecord('id', 'phpl_championnats') > 0)
	{
		...
		<select name="action" id="action">
			...
			<optgroup label="Gestion des joueurs">
			...
			<optgroup label="Gestion du championnat">
	...
	echo '		<input type="button" value="'.ADMIN_CHAMP_CREER.'" onclick="window.location.href=\'?page=championnat&amp;action=creer\'" />
	</form>'."\n";
}
...
function NbrRecord($CHAMPS, $TABLE)
{
	$REQ = mysql_query('SELECT '.$CHAMPS.' FROM '.$TABLE);
	$NBR = mysql_num_rows($REQ);
...
function nb_rens()
{
	$REQ = mysql_query('SELECT id FROM phpl_rens WHERE id_classe > 0');
	$NBR = mysql_num_rows($REQ);
...
function aff_rens($id_classe, $id_clubs)
{
	...
	else
	{
		while($ROW = mysql_fetch_array($REQ))
		{
			...
			elseif (empty($ROW[9]))
			{
				$TMP.= '				<td class="Large50P"><b><a href="'.$ROW[10].'" rel="external">'.$rens_nom.'</a></b></td>
				<td class="Large50P">'.$donnee_nom.'</td>'."\n";
			}
			elseif (empty($ROW[10]))
			{
				$TMP.= '				<td class="Large50P"><b>'.$rens_nom.'</b></td>
				<td class="Large50P"><a href="'.$ROW[9].'" rel="external">'.$donnee_nom.'</a></td>'."\n";
			}
			else
			{
				$TMP.= '				<td class="Large50P"><b><a href="'.$ROW[10].'" rel="external">'.$rens_nom.'</a></b></td>
...
// Upload de fichiers
// Information PHP :: maint_infophp.php
function UploadFiles($DIV_BOUTTON, $DIV_GALLERIE, $URL_UPLOAD, $REP_RACINE, $REP_IMAGE)
{

?>
<script type="text/javascript">
	$(document).ready(function() {
		$('#<?php echo $DIV_GALLERIE; ?>').load('gest_photos_gallerie.php?rep=<?php echo $REP_RACINE; ?>', function(data) { 
			$('.loader').fadeOut('fast', function() { 
				$('#<?php echo $DIV_GALLERIE; ?>').fadeIn(1000); 
		...
		$(function(){
			var btnUpload = $('#<?php echo $DIV_BOUTTON; ?>');
			var status    = $('.Etat');

			new AjaxUpload(btnUpload, {
				action: '<?php echo $URL_UPLOAD.'?rep='.$REP_IMAGE; ?>',
				name: 'file',
				onSubmit: function(file, ext){
					...
					status.append('&nbsp;<img src="images/loader_12.gif" />');
				},
				onComplete: function(file, response) {
					...
					if(response === 'success')
					{
						$('<li></li>').appendTo('.files_list').html('<img src="<?php echo $REP_RACINE; ?>/' + file + '" alt="" /><br />'+file).addClass('success');

						$('#<?php echo $DIV_GALLERIE; ?>').fadeOut('slow').load('gest_photos_gallerie.php?rep=<?php echo $REP_RACINE; ?>').fadeIn("slow");
...
function ListCountry($NAME, $ID, $SELVAL)
{
	global $lang;

	echo '<select name="'.$NAME.'" id="'.$ID.'" style="float: left;">
	<option value="">&nbsp;</option>'."\n";

	$FICHIER = fopen(dirname(__FILE__).'/'.REP_BASES.'country_'.$lang.'.csv', 'r');

	while (($VAL = fgetcsv($FICHIER, 1024, ';')) !== FALSE)
	{
		if ($SELVAL == strtolower($VAL[0]))
		{
			$SELECT = ' selected="selected"';
		}
		else
		...
		echo '<option value="'.strtolower($VAL[0]).'"'.$SELECT.'>'.$VAL[1].'</option>'."\n";
	}
...
function ListCountry_old($NAME, $ID)
{
	echo '<select name="'.$NAME.'" id="'.$ID.'" size="Large200M">
	<option value="" title="../'.REP_IMG_DRAPEAU.'vide.png">&nbsp;</option>'."\n";

	$FICHIER = fopen(dirname(__FILE__).'/'.REP_BASES.'country.csv', 'r');

	while (($VAL = fgetcsv($FICHIER, 1024, ';')) !== FALSE)
	{
		if (file_exists('../'.REP_IMG_DRAPEAU.strtolower($VAL[0]).'.png'))
		{
			$IMG = '../'.REP_IMG_DRAPEAU.strtolower($VAL[0]).'.png';
		}
		else
		{
			$IMG = '../'.REP_IMG_DRAPEAU.'vide.png';
		}

		echo '<option value="'.strtolower($VAL[0]).'" title="'.$IMG.'">'.$VAL[1].'</option>'."\n";
	}
