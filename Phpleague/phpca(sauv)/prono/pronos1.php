...
if ($action == "reset") {
  ...
  while ($row = mysqli_fetch_array($resultat)) {
    $resultat1 = $idconnect->query("SELECT phpca_matchs.date_reelle 
                                    FROM phpca_matchs 
                                    WHERE phpca_matchs.id='$row[0]'");
...
if ($action == "valid_pronos") {
  ...
  for ($i = 1; $i <= $_REQUEST['nb_fiche']; $i++) {
    ...
    $resultat = $idconnect->query("SELECT phpca_matchs.date_reelle 
                                   FROM phpca_matchs 
                                   WHERE phpca_matchs.id='$id_match[$i]'");
    ...
    if ($f_prono[$i] !== "undefined") {
      ...
      mysqli_query($idconnect, ("DELETE FROM phpca_pronostics 
                                 WHERE pronostic=' '"));

      $resultat = $idconnect->query("SELECT * 
                                     FROM phpca_matchs, phpca_pronostics, phpca_membres 
                                     WHERE phpca_membres.id_prono='$user_id'
      ...
      if ($nb_prono == "0") {
        if ($date_actuelle < ($date_match_timestamp + $temps_avant_prono * 60)) {
          mysqli_query($idconnect, ("INSERT INTO phpca_pronostics (id_membre, pronostic, id_match, id_champ) 
...
//echo "<br>";
  ...
  $resultat = $idconnect->query("
			 SELECT phpca_clubs.nom, CLEXT.nom, phpca_matchs.id, phpca_matchs.date_reelle, phpca_journees.numero, phpca_matchs.id_equipe_dom, phpca_matchs.id_equipe_ext
			 FROM phpca_clubs, phpca_clubs as CLEXT, phpca_matchs, phpca_journees, phpca_equipes, phpca_equipes as EXT, phpca_gr_championnats
			 WHERE phpca_clubs.id=phpca_equipes.id_club
			 AND CLEXT.id=EXT.id_club
			 AND phpca_equipes.id=phpca_matchs.id_equipe_dom
       ...
       AND phpca_clubs.nom!='Exempt'
       AND CLEXT.nom!='Exempt'
			 AND CLEXT.nom!='exempte'

       ORDER by phpca_matchs.date_reelle, phpca_matchs.id
			 LIMIT $debut, $fin ");
...
/*
  $resultat = $idconnect->query("
       ...
       AND CLEXT.nom!='Exempt'
			 AND CLEXT.nom!='exempte'
       AND phpca_matchs.date_reelle < '$dateAffichageMaxi'
       ORDER by phpca_matchs.date_reelle, phpca_matchs.id
			 LIMIT $debut, $fin ");
