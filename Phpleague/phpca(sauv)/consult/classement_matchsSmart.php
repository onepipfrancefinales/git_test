...
if (isset($type))
{
   ...
   $res=$idconnect->query("SELECT accession, barrage, estimation, relegation, id_equipe_fetiche, fiches_clubs
    ...
    $res2=$idconnect->query("SELECT phpca_equipes.id FROM phpca_equipes, phpca_clubs
...
if ($debut=="1" and $fin==$nb_journees)
{
   ...
   $res3=$idconnect->query("SELECT max(phpca_journees.numero) FROM phpca_journees, phpca_matchs WHERE phpca_journees.id=phpca_matchs.id_journee and buts_dom is not NULL and     phpca_journees.id_champ='$champ'");
