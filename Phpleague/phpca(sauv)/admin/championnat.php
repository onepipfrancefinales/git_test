...
{
  mysqli_query($idconnect, ("INSERT INTO phpca_saisons (annee) values ('$saison')"));
}

if ($action2=="creer" and $division and $action=="creer")
{
  mysqli_query($idconnect, ("INSERT INTO phpca_divisions (nom) values ('$division')"));
}
...
{
  mysqli_query($idconnect, ("DELETE FROM phpca_saisons WHERE id ='$saison'")) ;
}

if ($action2=="supp" and $division and $action=="creer")
{
  mysqli_query($idconnect, ("DELETE FROM phpca_divisions WHERE id ='$division'")) ;
}
...
if ($confirm=="ok" and $champ and $action=="supp")
{
 // $idconnect=@mysqli_connect('127.0.0.0','root','','onepip-france-db3');	
  ...
  //$resultats=mysql_query($requete);
   ...
   mysqli_query($idconnect, ("DELETE FROM phpca_equipes WHERE id_champ ='$champ' "));
   mysqli_query($idconnect, ("DELETE FROM phpca_parametres WHERE id_champ ='$champ' "));
