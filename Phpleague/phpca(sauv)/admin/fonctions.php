...
function affich_championnats($champ, $action, $idconnect)
{
  ...
  $resultats = $idconnect->query("SELECT phpca_championnats.id, phpca_divisions.nom, phpca_saisons.annee 
			...
			AND	phpca_divisions.id between $valLigueMin  and $valLigueMax 
			ORDER BY id, nom");
...
function champ_menu_par_ligue($ligue, $idconnect)
{
  ...
  $result = $idconnect->query(" 
		 ...
		 WHERE phpca_divisions.id = phpca_championnats.id_division 
		 AND phpca_saisons.id= phpca_championnats.id_saison 
     AND  phpca_divisions.id between '$ligue' AND $ligueMax 
		 ORDER by annee desc, phpca_divisions.nom");
...
WHERE ext.id_champ='$champ'
  ...
  $result = $idconnect->query("SELECT phpca_clubs.nom, phpca_equipes.penalite, phpca_equipes.id, phpca_equipes.pts_admin, phpca_equipes.jour_pere
							 FROM phpca_clubs, phpca_equipes, phpca_championnats
							 WHERE phpca_equipes.id_champ=phpca_championnats.id
							 AND phpca_championnats.id='$champ'
							 AND phpca_equipes.id_club=phpca_clubs.id");
  ...
  while ($row = mysqli_fetch_array($result)) {
    ...
    if ($cache == "1") {
      mysqli_query($idconnect, ("INSERT INTO phpca_clmnt_cache(NOM, POINTS, JOUES, JOURPERE,  G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN,PTS_ADMIN,  DOMPOINTS, DOMJOUES, DOMG, DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, EXTPOINTS, EXTJOUES, EXTG, EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, ID_EQUIPE, ID_CHAMP) 
      ...
      mysqli_query($idconnect, ("INSERT INTO phpca_clmnt(NOM, POINTS, JOUES, JOURPERE, G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN, PTS_ADMIN, DOMPOINTS, DOMJOUES, DOMG, DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, EXTPOINTS, EXTJOUES, EXTG, EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, ID_EQUIPE, ID_CHAMP) 
									  VALUES('$NOM','$POINTS','$JOUES',$JOURPERE, '$G','$N','$P','$BUTSPOUR','$BUTSCONTRE','$DIFF','$PEN', '$PTS_ADMIN', '$DOMPOINTS','$DOMJOUES','$DOMG','$DOMN','$DOMP','$DOMBUTSPOUR','$DOMBUTSCONTRE','$DOMDIFF','$EXTPOINTS','$EXTJOUES','$EXTG','$EXTN','$EXTP','$EXTBUTSPOUR','$EXTBUTSCONTRE','$EXTDIFF','$ID_EQUIPE','$ID_CHAMP')"));
    }
  }
...
// Nombres de classes enregistr�es (ulilis� dans phpca_classe.php)
