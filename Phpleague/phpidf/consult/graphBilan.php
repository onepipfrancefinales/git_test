...
$result=$idconnect->query("SELECT phpidf_clubs.nom, phpidf_equipes.id_club, phpidf_equipes.id_champ FROM phpidf_equipes, phpidf_clubs where phpidf_clubs.id=phpidf_equipes.id_club AND phpidf_equipes.id='$equipe'");
...
$result=$idconnect->query("SELECT phpidf_equipes.id FROM phpidf_equipes, phpidf_clubs WHERE id_champ='$champ' AND phpidf_equipes.id_club=phpidf_clubs.id AND phpidf_clubs.nom!='exempte'");
//$result=mysql_query($query);
$nb_equipes=mysqli_num_rows($result)+1;

// on détermine la dernière journee jouée
$result=$idconnect->query("SELECT max(phpidf_journees.numero) FROM phpidf_journees, phpidf_matchs where phpidf_journees.id=phpidf_matchs.id_journee and buts_dom is not NULL AND phpidf_journees.id_champ='$champ'");
//$result=mysql_query($query);
while ($row=mysqli_fetch_array($result)){$fin=$row[0];}

// On détermine le nombre de journée total
$result=$idconnect->query("SELECT id FROM phpidf_journees WHERE id_champ='$champ'");
//$result=mysql_query($query);
$nb_journees=mysqli_num_rows($result);

// On déterrmine l'année et le nom de la division
$result=$idconnect->query("SELECT phpidf_saisons.annee, phpidf_divisions.nom FROM phpidf_saisons, phpidf_divisions, phpidf_championnats where phpidf_championnats.id=$champ AND phpidf_divisions.id=phpidf_championnats.id_division and phpidf_championnats.id_saison=phpidf_saisons.id");
//$result=mysql_query($query);
...
$requete_sql = "SELECT classement FROM phpidf_clmnt_graph WHERE id_equipe='$equipe' ORDER BY fin";
