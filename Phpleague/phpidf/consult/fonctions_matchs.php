...
function nb_equipes($id_champ, $idconnect)
 ...
 $res=$idconnect->query("SELECT id 
        ...
        // $query="SELECT id FROM phpidf_equipes WHERE id_champ='$id_champ'";
        // $result=mysql_query($query);
...
//  echo "journee : ".$journee;echo "</br>";
 ...
 //echo $numero;
 // echo "</br>";
 $result=$bdd->query("SELECT cldom.nom as cldom, clext.nom as clext, phpidf_matchs.buts_dom, phpidf_matchs.buts_ext,
  ...
  echo "<br />";
  echo "<table align=\"center\" class=\"tablephpidf2\"  width=\"90%\" cellspacing=\"1\" cellpadding=\"1\"><tr><td><table cellspacing=\"0\" align=\"center\" width=\"100%\" cellpadding=\"1\">\n";
  $x=1;
...
function aff_journeeSupp($champ, $bdd)
{
 ...
 $result=$bdd->query("SELECT cldom.nom as cldom, clext.nom as clext, phpidf_matchs.buts_dom, phpidf_matchs.buts_ext,
                ...
                WHERE phpidf_matchs.id_equipe_dom=dom.id
                        AND phpidf_matchs.id_equipe_ext=ext.id
                        AND phpidf_journees.id_champ='$champ'
                        AND phpidf_journees.numero='$journeeSupp'
                        AND dom.id_club=cldom.id
                        AND ext.id_club=clext.id
                        AND phpidf_matchs.id_journee=phpidf_journees.id
                        AND cldom.nom!='exempte'
...
function classementDetaille($champ, $bdd)
{
 ...
 while ($row = $res->fetch())
  ...
  echo "<table class=\"tablephpidf2\" cellspacing=\"0\" width=\"80%\"><tr class=\"trphpidf3\"><th colspan=\"14\">".$legende."</th></tr>\n";
  echo "<tr class=\"trphpidf3\">
 ...
 // {
	...
	//echo "<table class=\"tablephpidf2\" align=\"center\" cellspacing=\"0\" width=\"80%\"><tr class=\"trphpidf3\"><th colspan=\"11\">".$legende."</th></tr>\n";
	//echo "<tr class=\"trphpidf3\>";
...
{
 $resultat=$bdd->query("
		  SELECT phpidf_clubs.nom FROM phpidf_clubs, phpidf_equipes 
		  WHERE phpidf_equipes.id='$id_equipe' 
...
echo "<table class=\"tablephpidf2\" align=\"center\" cellspacing=\"0\"  width=\"90%\">";

$resultat=$bdd->query("
		SELECT phpidf_journees.numero, cldom.nom, clext.nom, phpidf_matchs.buts_dom, phpidf_matchs.buts_ext, phpidf_matchs.date_reelle, phpidf_matchs.id
        ...
        WHERE phpidf_matchs.id_equipe_dom=dom.id
        AND phpidf_matchs.id_equipe_ext=ext.id
        AND (phpidf_matchs.id_equipe_ext='$id_equipe'
        OR phpidf_matchs.id_equipe_dom='$id_equipe')
        AND phpidf_journees.id_champ='$champ'
        AND dom.id_club=cldom.id
        AND ext.id_club=clext.id
        AND phpidf_matchs.id_journee=phpidf_journees.id
...
echo "<br />";echo "<br />";
	if ($smart == false)
	echo "<tr class=\"trphpidf3\"><td align=\"center\"><font color=\"#FFFF00\"><b>".JOURNEE."</b></font></td><td width=\"5% align=\"center\"></td><td width=\"30%\" ><font color=\"#FFFF00\"><b>".DATE."</b></font></td><td colspan=\"5\ align=\"center\"><font color=\"#FFFF00\"><b>Oppositions</b></font></td></tr>";
    else
	echo"<tr class=\"trphpidf3\"><td align=\"center\"><font color=\"#FFFF00\"><b>".JOURNEE."</b></font></td><td width=\"5% align=\"center\"></td><td colspan=\"5\ align=\"center\"><font color=\"#FFFF00\"><b>Oppositions</b></font></td></tr>";
...
{	
 ...
 $resultats2=$bdd->query("SELECT phpidf_clubs.nom, CLEXT.nom, phpidf_matchs.buts_dom, phpidf_matchs.buts_ext, phpidf_matchs.id, phpidf_matchs.date_reelle
 ...
 $result=$bdd->query("SELECT phpidf_journees.numero, phpidf_journees.date_prevue, cldom.nom, clext.nom, phpidf_matchs.buts_dom, phpidf_matchs.buts_ext, dom.id, ext.id
 ...
 while ($row=$result->fetch())
 {
  ...
  { 
   ...
   //changement de la date US en FR
	...
	?>
   <br>
   <table class="tablephpidf2" cellspacing="0" width="290" align="center">
	 <tr class="trphpidf3">
   ...
   {   
	...
	?>    
	<table class="tablephpidf2" cellspacing="0" width="290" align="center">
	  <tr class="trphpidf3"><td colspan="3" align="center"><b><font color="#FFFF00">
...
function db_clmnt($champ, $debut, $fin, $cache, $idconnect)
{
  ...
  if($cache=="1"){ 
  mysqli_query("DELETE FROM phpidf_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysqli_error());
  }
  else{ 
  mysqli_query("DELETE FROM phpidf_clmnt") or die (mysqli_error());}
          $nb_requete++;
...
$resultats=$idconnect->query=("SELECT pts_victoire, pts_nul, pts_defaite FROM phpidf_parametres WHERE id_champ='$champ'");
$nb_requete++;
...
// victoires domicile
$resultats=$idconnect->query("SELECT dom.id, count(dom.id), phpidf_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phpidf_equipes as dom, phpidf_clubs, phpidf_matchs, phpidf_journees, phpidf_championnats
...
// Defaites domicile
$dom=$idconnect->query("SELECT dom.id, count(dom.id), phpidf_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phpidf_equipes as dom, phpidf_clubs, phpidf_matchs, phpidf_journees, phpidf_championnats
WHERE dom.id_champ='$champ'
      AND dom.id_club=phpidf_clubs.id
      AND dom.id=phpidf_matchs.id_equipe_dom
      AND buts_dom < buts_ext
...
$result=$idconnect->query("SELECT phpidf_clubs.nom FROM phpidf_clubs, phpidf_equipes, phpidf_championnats
...
$result=$idconnect->query("SELECT phpidf_clubs.nom, phpidf_equipes.penalite, phpidf_equipes.id
...
//mysql_query("LOCK TABLE phpidf_clmnt WRITE, phpidf_equipes WRITE, phpidf_clubs WRITE") or die (mysql_error());
    ...
    {
    $result="SELECT phpidf_clubs.nom, phpidf_equipes.id 
            FROM phpidf_clubs, phpidf_equipes, phpidf_championnats
            WHERE phpidf_equipes.id_champ=phpidf_championnats.id
            AND phpidf_championnats.id='$champ'
            AND phpidf_equipes.id_club=phpidf_clubs.id";
...
if($cache=="1")
	{
	mysqli_query($idconnect, ("DELETE FROM phpidf_clmnt_cache WHERE nom='exempte'"));
	}
else
	{
	mysqli_query($idconnect, ("DELETE FROM phpidf_clmnt WHERE nom='exempte'"));
	}
...
//               statistiques, amélioration de la gestion des buteurs pour phpidfeague.          //
// Auteur      : Alexis MANGIN                                                                 //
// Email       : Alexis@univert.org                                                            //
// Url         : http://www.univert.org                                                        //
// Démo        : http://phpidfeague.univert.org/demo.php                                         //
