...
{
  mysqli_query($idconnect, ("INSERT INTO phpidf_saisons (annee) values ('$saison')"));
}

if ($action2=="creer" and $division and $action=="creer")
{
  mysqli_query($idconnect, ("INSERT INTO phpidf_divisions (nom) values ('$division')"));
}

if ($action3=="creer" and $action=="creer")
{
  mysqli_query($idconnect, ("INSERT INTO phpidf_championnats (id_division, id_saison) values ('$division','$saison')"));
}


if ($action2=="supp" and $saison and $action=="creer")
{
  mysqli_query($idconnect, ("DELETE FROM phpidf_saisons WHERE id ='$saison'")) ;
}

if ($action2=="supp" and $division and $action=="creer")
{
  mysqli_query($idconnect, ("DELETE FROM phpidf_divisions WHERE id ='$division'")) ;
}
 ...
 // $idconnect=@mysqli_connect('127.0.0.0','root','','onepip-france-db3');	
  ...
  //$resultats=mysql_query($requete);
     while($row = mysqli_fetch_array($resultats))
      {
        mysqli_query($idconnect, ("DELETE FROM phpidf_matchs where id_equipe_dom='$row[0]' "));
        mysqli_query($idconnect, ("DELETE FROM phpidf_matchs where id_equipe_ext='$row[0]' ")) ;
      }
   mysqli_query($idconnect, ("DELETE FROM phpidf_championnats where id='$champ' "));
   mysqli_query($idconnect, ("DELETE FROM phpidf_journees WHERE  id_champ = '$champ'"));
   mysqli_query($idconnect, ("DELETE FROM phpidf_equipes WHERE id_champ ='$champ' "));
   mysqli_query($idconnect, ("DELETE FROM phpidf_parametres WHERE id_champ ='$champ' "));
