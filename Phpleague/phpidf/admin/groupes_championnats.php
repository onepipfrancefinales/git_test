...
{   
  mysqli_query ($idconnect,("INSERT INTO phpidf_gr_championnats (nom) VALUES ('$nom_group')")) or die ("probleme " .mysql_error($idconnect));
}

// Suppression d'un groupe de championnat
if ($confirm=="ok" and $gr_champ and $action=="supp")
{
  mysqli_query($idconnect,("DELETE FROM phpidf_gr_championnats WHERE id ='$gr_champ' "))or die ("probleme " .mysql_error($idconnect));
}
?>

<br /><font class=phpidf><?php echo GR_LEAGUE; ?></font><br /><br />

<table class=phpidf width="90%">
            <tr>
              <td class=phpidf2><?php echo MENU_ID; ?></td>
              <td class=phpidf2><?php echo MENU_NOM; ?></td>
              <td class=phpidf2></td>
            </tr>
