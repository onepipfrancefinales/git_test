...
echo "<font class=phppr>".ADMIN_RENS_TITRE."</font><br /><br /><br />";
...
switch($go) // Ajout/ suppression
{
     ...
     mysql_query (" DELETE FROM phppr_rens WHERE id='$rens' ") or die ("probleme " .mysql_error());
     mysql_query (" DELETE FROM phppr_donnee WHERE id_rens='$rens' ") or die ("probleme " .mysql_error());
    ...
    <table class=phppr width="80%">
	 ...
	 mysql_query ("INSERT INTO phppr_rens (nom) VALUES ('$rens')") or die ("probleme " .mysql_error());
         $rens2=rens2($rens, $idconnect);

         $query="SELECT id FROM phppr_clubs";
         $result=mysql_query($query);
         while($row=mysql_fetch_array($result))
         {
         mysql_query ("INSERT INTO phppr_donnee (id_clubs, id_rens) VALUES ('$row[0]', '$rens2')") or die ("probleme " .mysql_error());
         }
	 ...
	 while ( list ($cle, $val)= each ($url) and list ($cle, $val2)= each ($nom) and list ($cle, $val3)= each ($id))
         {
          ...
          mysql_query ("UPDATE phppr_rens SET url='$val', nom='$rens_nom' WHERE id='$val3'") or die ("probleme " .mysql_error());
         }
               ?>
    <table class=phppr width="80%">
            <tr>
...
switch($ga)
{
	 ...
	 mysql_query (" UPDATE phppr_rens SET id_classe='0' WHERE phppr_rens.id='$val'") or die ("probleme " .mysql_error());
	 ?>
    <table class=phppr width="80%">
            <tr>
              <td class=phppr2 align="center" colspan="2"><?php echo ADMIN_CLASSE_5; ?></td>
	...
	while ( list($val, $value)= each($classe))
	 mysql_query ("update phppr_rens SET id_classe='$value' WHERE id='$rens'") or die ("probleme " .mysql_error());
...
if ($actionc=="1")
   {
    ...
    reset ($id);
	 while ( list ($cle, $val)= each ($rang) and list ($cle, $val2)= each ($id))
         {
                 mysql_query ("UPDATE phppr_rens SET rang='$val' WHERE id='$val2'") or die ("probleme " .mysql_error());
...
<table class=phppr width="80%">
            <tr>
              <td class=phppr2 align="center" colspan="2"><?php echo ADMIN_RENS_TITRE; ?></td>
            </tr>
            <tr>
...
$result = mysql_query("SELECT id, nom FROM phppr_rens ORDER BY nom");
...
echo ADMIN_RENS_10;
echo "<form method=\"post\" action=\"\">";
echo "<table class=phppr cellspacing=\"0\" align=center border=\"0\" width=\"80%\"><center>";
echo "<tr class=phppr3><td>".ADMIN_RENS_12."</td><td>".ADMIN_RENS_13."</td></tr>";
$query="SELECT id, nom, url FROM phppr_rens";
$result = mysql_query($query);
...
echo "<select name=\"rens\">";
echo "<option value=\"0\"> </option>";
$result = mysql_query("SELECT id, nom, id_classe FROM phppr_rens ORDER BY nom");
        while($row = mysql_fetch_array($result))
        {
           $a=$row[1]+1;
           $rens_nom = stripslashes($row[1]);
...
$result = mysql_query("select id, nom FROM phppr_classe ");
        while($row = mysql_fetch_array($result))
        {
...
echo ADMIN_RENS_5;
echo "<br />";
$query="SELECT phppr_rens.id, phppr_rens.id_classe, phppr_rens.nom, phppr_classe.id, phppr_classe.nom
       FROM phppr_rens, phppr_classe
       WHERE phppr_rens.id_classe=phppr_classe.id";
...
echo ADMIN_RENS_14;
$query="SELECT phppr_rens.nom FROM phppr_rens where id_classe='0'";
$result = mysql_query($query);
...
echo "<br />";
echo ADMIN_RENS_9;
$query = "SELECT id, nom, rang from phppr_classe ORDER by rang";
echo "<form method=\"post\" action=\"\">";
$result = mysql_query($query);
     
     while($row = mysql_fetch_array($result))
     {
        echo "<table class=phppr border=\"0\" cellpadding=\"2\" cellspacing=\"0\" valign=\"bottom\" align=\"center\" width=\"90%\"><tr class=phppr3><td><b> $row[1] </b></td></tr>";
        $query2="SELECT id, nom, rang, id_classe FROM phppr_rens WHERE id_classe='$row[0]' ORDER by rang";
