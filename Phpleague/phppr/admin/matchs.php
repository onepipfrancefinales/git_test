...
if (empty ($numero))
  {
    $result=$idconnect->query("
			SELECT max(phppr_journees.numero) 
...
for ($counter=$nb_rencontres; $counter>0 ; $counter=$counter-1 ) // Nb de rencontres dans la journée
        ...
        $query = "SELECT DISTINCT phppr_clubs.nom, phppr_equipes.id FROM phppr_clubs, phppr_equipes
	   ...
	   $result= $idconnect->query("
				SELECT DISTINCT phppr_clubs.nom, phppr_equipes.id 
				FROM phppr_clubs, phppr_equipes 
				WHERE phppr_equipes.id_champ='$champ' 
				AND phppr_clubs.id=phppr_equipes.id_club 
				ORDER BY phppr_clubs.nom");
...
echo "<small>MATCH_SUM = ".$sum_day."  </small>";
    $result=$idconnect->query("
			SELECT count(DISTINCT phppr_matchs.id_equipe_dom), 
			count(DISTINCT phppr_matchs.id_equipe_ext) 
			FROM phppr_matchs, phppr_journees   
			WHERE phppr_matchs.id_journee=phppr_journees.id    
			AND phppr_journees.id_champ='$champ'    
			AND phppr_journees.numero='$x' ");
			//$result=mysql_query($query);
...
$result=$idconnect->query(" 
		SELECT sum(phppr_matchs.id_equipe_dom), 
			   sum(phppr_matchs.id_equipe_ext) 
		FROM phppr_matchs, phppr_journees 
		WHERE phppr_matchs.id_journee=phppr_journees.id 
		AND phppr_journees.id_champ='$champ'  ");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT   phppr_matchs.id_equipe_dom as DOM,  count(*) as ct 
		 FROM phppr_matchs, phppr_journees 
		 WHERE  phppr_journees.id_champ='$champ' 
		 AND phppr_matchs.id_journee=phppr_journees.id 
		 GROUP BY DOM");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT phppr_matchs.id_equipe_ext as DOM,  
		 count(*) as ct 
		 FROM phppr_matchs, phppr_journees 
		 WHERE  phppr_journees.id_champ='$champ' 
		 AND phppr_matchs.id_journee=phppr_journees.id 
