...
{
  $resultats=$idconnect->query("SELECT phppr_championnats.id, phppr_divisions.nom, phppr_saisons.annee 
    ...
    while ($row = mysqli_fetch_array($resultats))
      {
        ...
        if ($champ=="$row[0]") {$class="phppr7";}
        elseif (($i%2)==0) {$class="phppr3";}
        else {$class="phppr4";}
    ...
    while ($row = mysqli_fetch_array($resultats))
      {
        ...
        if ($gr_champ=="$row[0]") {$class="phppr7";}
        elseif (($i%2)==0) {$class="phppr3";}
        else {$class="phppr4";}
...
{
  $resultats=$idconnect->query("SELECT phppr_divisions.nom, phppr_saisons.annee 
              FROM phppr_championnats, phppr_divisions, phppr_saisons
              WHERE phppr_championnats.id_division=phppr_divisions.id 
              AND phppr_championnats.id_saison=phppr_saisons.id 
              AND phppr_championnats.id='$champ' ORDER by annee desc");
...
{
  $result=$idconnect->query(" SELECT phppr_divisions.id, phppr_divisions.nom 
							  FROM phppr_divisions 
							  ORDER by nom");
...
{
  $result=$idconnect->query(" SELECT phppr_saisons.id, phppr_saisons.annee 
							  FROM phppr_saisons 
							  ORDER by annee");
...
{    
  $result=$idconnect->query(" SELECT phppr_clubs.id, phppr_clubs.nom 
							  FROM phppr_clubs 
							  ORDER by nom");
...
function champ_menu($idconnect)
{
 $result=$idconnect->query(" 
		 SELECT phppr_championnats.id, phppr_divisions.nom, phppr_saisons.annee, (phppr_saisons.annee)+1 
		 FROM phppr_championnats, phppr_divisions, phppr_saisons 
		 WHERE phppr_divisions.id=phppr_championnats.id_division 
		 AND phppr_saisons.id=phppr_championnats.id_saison 
		 ORDER by annee desc, phppr_divisions.nom");
...
{
  $result=$idconnect->query(" SELECT phppr_equipes.id, phppr_clubs.nom 
    ...
    while ($row=mysqli_fetch_array($result))
    {
         $result2=$idconnect->query(" 
					SELECT phppr_championnats.id, phppr_divisions.nom, phppr_saisons.annee, (phppr_saisons.annee)+1 
					 ...
					 WHERE phppr_divisions.id=phppr_championnats.id_division 
					 AND phppr_saisons.id=phppr_championnats.id_saison 
					 AND phppr_championnats.id='$row[0]'  
					 ORDER by annee desc, phppr_divisions.nom");
...
{ 
 $resultats= $idconnect->query("SELECT phppr_clubs.nom, CLEXT.nom, phppr_matchs.buts_dom, phppr_matchs.buts_ext, phppr_matchs.id, phppr_matchs.date_reelle
  ...
  $resultats3=$idconnect->query(" SELECT phppr_clubs.nom, CLEXT.nom, phppr_matchs.buts_dom, phppr_matchs.buts_ext, phppr_matchs.id, phppr_matchs.date_reelle 
								  FROM phppr_clubs, phppr_clubs as CLEXT, phppr_matchs, phppr_journees, phppr_equipes, phppr_equipes as EXT 
								  WHERE phppr_clubs.id=phppr_equipes.id_club 
								  AND CLEXT.id=EXT.id_club 
								  AND phppr_equipes.id=phppr_matchs.id_equipe_dom 
...
function nom_club($id_equipe, $idconnect)
{
  $result=$idconnect->query(" SELECT nom 
							  FROM phppr_clubs, phppr_equipes 
							  WHERE phppr_clubs.id=phppr_equipes.id_club 
							  and phppr_equipes.id='$id_equipe'");
...
function nb_matchs ($numero, $champ, $idconnect)
{
  $result=$idconnect->query("select * 
							 from phppr_matchs, phppr_journees 
							 where phppr_matchs.id_journee=phppr_journees.id 
							 and phppr_journees.numero=$numero 
							 and phppr_journees.id_champ=$champ");
...
function db_clmnt($champ, $debut, $fin, $cache, $idconnect)
{
  ...
  if($cache=="1")
	{ 
	 mysqli_query($idconnect,("DELETE FROM phppr_clmnt_cache WHERE ID_CHAMP='$champ'")) or die (mysqli_error($idconnect));
	}
	else
	{ 
	 mysqli_query($idconnect,("DELETE FROM phppr_clmnt")) or die (mysqli_error($idconnect));
	}
     $nb_requete++;
	...
	$dom=$idconnect->query("SELECT dom.id, count(dom.id), phppr_clubs.nom, sum(buts_dom), sum(buts_ext) 
						  FROM phppr_equipes as dom, phppr_clubs, phppr_matchs, phppr_journees, phppr_championnats
	...
	$dom=$idconnect->query("
		 SELECT dom.id, count(dom.id), phppr_clubs.nom, sum(buts_dom), sum(buts_ext) 
	     FROM phppr_equipes as dom, phppr_clubs, phppr_matchs, phppr_journees, phppr_championnats
		 WHERE dom.id_champ='$champ'
         AND dom.id_club=phppr_clubs.id
         AND dom.id=phppr_matchs.id_equipe_dom
   ...
   $result=$idconnect->query("SELECT phppr_clubs.nom 
...
// victoires exterieur	
  ...
  $dom=$idconnect->query("SELECT ext.id, count(ext.id), phppr_clubs.nom, sum(buts_ext), sum(buts_dom) FROM phppr_equipes as ext, phppr_clubs, phppr_matchs, phppr_journees, phppr_championnats
...
// TABLEAU DE CLASSEMENT
  ...
  $result=$idconnect->query("SELECT phppr_clubs.nom, phppr_equipes.penalite, phppr_equipes.id
							 FROM phppr_clubs, phppr_equipes, phppr_championnats
							 WHERE phppr_equipes.id_champ=phppr_championnats.id
							 AND phppr_championnats.id='$champ'
							 AND phppr_equipes.id_club=phppr_clubs.id");
	...
	*/
	//mysqlY_query("LOCK TABLE phppr_clmnt WRITE, phppr_equipes WRITE, phppr_clubs WRITE") or die (mysql_error());
	/*
    if (mysqli_num_rows($result)==0)
    {
	   // $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  
		$result=$idconnect->query("SELECT phppr_clubs.nom, phppr_equipes.id 
            FROM phppr_clubs, phppr_equipes, phppr_championnats
            WHERE phppr_equipes.id_champ=phppr_championnats.id
            AND phppr_championnats.id='$champ'
            AND phppr_equipes.id_club=phppr_clubs.id");
	...
	*/
    while($row = mysqli_fetch_array($result))
    {
		...
		if($cache=="1")
		{
		 mysqli_query($idconnect, ("INSERT INTO phppr_clmnt_cache(NOM, POINTS, JOUES, G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN, DOMPOINTS, DOMJOUES, DOMG, DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, EXTPOINTS, EXTJOUES, EXTG, EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, ID_EQUIPE, ID_CHAMP) 
		...
		{
		 mysqli_query($idconnect, ("INSERT INTO phppr_clmnt(NOM, POINTS, JOUES, G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN, DOMPOINTS, DOMJOUES, DOMG, DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, EXTPOINTS, EXTJOUES, EXTG, EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, ID_EQUIPE, ID_CHAMP) 
									  VALUES('$NOM','$POINTS','$JOUES','$G','$N','$P','$BUTSPOUR','$BUTSCONTRE','$DIFF','$PEN','$DOMPOINTS','$DOMJOUES','$DOMG','$DOMN','$DOMP','$DOMBUTSPOUR','$DOMBUTSCONTRE','$DOMDIFF','$EXTPOINTS','$EXTJOUES','$EXTG','$EXTN','$EXTP','$EXTBUTSPOUR','$EXTBUTSCONTRE','$EXTDIFF','$ID_EQUIPE','$ID_CHAMP')"));
	...
	if($cache=="1")
	{$requete="DELETE FROM phppr_clmnt_cache WHERE nom='exempte'" or die (mysql_error($idconnect));}
	else
	{$requete="DELETE FROM phppr_clmnt WHERE nom='exempte'" or die (mysql_error($idconnect));}
...
// Nombres de renseignement dans cette classe à partir de l'id_classe  (utilisé dans phppr_classe.php)
function nb_classe($data, $idconnect)
{
  $result=$idconnect->query("SELECT id FROM phppr_rens WHERE id_classe='$data'");
//if (!$result) die mysql_error();
  $nb_classe=mysqli_num_rows( $result );
  return("$nb_classe");
}

// Nombres de classes enregistrées (ulilisé dans phppr_classe.php)
function nb_classe2($idconnect)
{	
  $result=$idconnect->query("SELECT * FROM phppr_classe");
//if (!$result) die mysql_error();
...
{
  $result=$idconnect->query("SELECT id FROM phppr_rens where id_classe>'0'");
...
{	
  $result=$idconnect->query("SELECT * FROM phppr_rens");
...
{
  $result=$idconnect->query("select id, nom from phppr_rens where nom='$rens'");
//if (!$result) die mysql_error();
  $row=mysqli_fetch_array($result);
  $rens2=$row[0];
  return("$rens2");
...
function aff_rens ($id_classe, $id_clubs, $idconnect)
  {
  $result=$idconnect->query("SELECT phppr_donnee.id, phppr_donnee.nom, id_rens, id_clubs, phppr_rens.id, phppr_rens.nom, phppr_rens.id_classe, phppr_clubs.id, etat, phppr_donnee.url, phppr_rens.url
