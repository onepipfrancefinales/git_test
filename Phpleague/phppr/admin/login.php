...
if (isset($_POST['autoidentification'])) {$autoidentification=$_POST['autoidentification'];} else {$autoidentification='';}
	...
	{
    $query2=$idconnect->query("SELECT mot_de_passe FROM phppr_membres WHERE pseudo='$pseudo' and admin='1'");
	...
	$time = time();
	 $result=$idconnect->query("SELECT * FROM phppr_membres WHERE pseudo='$pseudo' and admin='1'");
	//$result = mysql_query($query) or die ('Erreur SQL !<br />'.$sql.'<br />'.mysql_error());
	$row = mysqli_fetch_array($result);
	$mot_de_passe = $row['mot_de_passe'];

	mysqli_query("UPDATE phppr_membres SET ip='$ip', last_connect='$time' WHERE pseudo='$pseudo'");
