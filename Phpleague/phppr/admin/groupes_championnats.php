...
{   
  mysqli_query ($idconnect,("INSERT INTO phppr_gr_championnats (nom) VALUES ('$nom_group')")) or die ("probleme " .mysql_error($idconnect));
}


if ($confirm=="ok" and $gr_champ and $action=="supp")
{
      mysqli_query($idconnect,("DELETE FROM phppr_gr_championnats WHERE id ='$gr_champ' "))or die ("probleme " .mysql_error($idconnect));
}
?>

<br /><font class=phppr><?php echo GR_LEAGUE; ?></font><br /><br />

<table class=phppr width="90%">
            <tr>
              <td class=phppr2><?php echo MENU_ID; ?></td>
              <td class=phppr2><?php echo MENU_NOM; ?></td>
              <td class=phppr2></td>
            </tr>
