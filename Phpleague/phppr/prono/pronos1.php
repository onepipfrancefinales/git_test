...
$resultat=$idconnect->query("SELECT phppr_matchs.id
  ...
  //$resultat=mysql_query($requete) or die ("probleme " .mysql_error());
      while ($row=mysqli_fetch_array($resultat))
      {
        $resultat1=$idconnect->query("SELECT phppr_matchs.date_reelle FROM phppr_matchs WHERE phppr_matchs.id='$row[0]'");
       // $resultat1=mysql_query($requete1);
        ...
        {
           mysqli_query($idconnect, ("UPDATE  phppr_pronostics SET pronostic='0' WHERE id_match='$row[0]' AND id_membre='$id'"));}
           mysqli_query($idconnect, ("DELETE FROM phppr_pronostics WHERE pronostic='0'"));
        }
      }
  ...
  if ($action == "valid_pronos")
  {
    ...
    {   
     ...
     $resultat=$idconnect->query("SELECT phppr_matchs.date_reelle FROM phppr_matchs WHERE phppr_matchs.id='$id_match[$i]'");
    // $resultat=mysql_query($requete);
     ...
     $resultat=$idconnect->query("SELECT tps_avant_prono FROM phppr_gr_championnats WHERE id='$gr_champ'");
     //$resultat=mysql_query($requete);
     ...
     {
       mysqli_query($idconnect, ("DELETE FROM phppr_pronostics WHERE pronostic=' '"));
       $resultat =$idconnect->query( "SELECT * FROM phppr_matchs, phppr_pronostics, phppr_membres WHERE phppr_membres.id_prono='$user_id'
                   AND phppr_membres.id=phppr_pronostics.id_membre
                   AND phppr_pronostics.id_match=phppr_matchs.id
                   AND phppr_pronostics.id_match='$id_match[$i]'");
      // $resultat=mysql_query($requete);
       $nb_prono=mysqli_num_rows($resultat);

       $resultat = $idconnect->query("SELECT id FROM phppr_membres WHERE id_prono='$user_id'");
       //$resultat = mysql_query($requete);
       ...
       if ($nb_prono == "1")
       {
         if ($date_actuelle<($date_match_timestamp+$temps_avant_prono*60))
         {
           mysqli_query($idconnect, ("UPDATE phppr_pronostics SET pronostic='$f_prono[$i]'
                        WHERE phppr_pronostics.id_membre='$id'
                        AND phppr_pronostics.id_match='$id_match[$i]'"));
         }
       }
       if ($nb_prono == "0")
       {
         if ($date_actuelle<($date_match_timestamp+$temps_avant_prono*60))
         {
           mysqli_query($idconnect, ("INSERT INTO phppr_pronostics (id_membre, pronostic, id_match, id_champ) VALUES ('$id','$f_prono[$i]','$id_match[$i]', '$gr_champ')"));
         }
       }
  ...
  {    if ($debut=="0") 
    ...
    $resultat=$idconnect->query("
			 SELECT phppr_clubs.nom, CLEXT.nom, phppr_matchs.id, phppr_matchs.date_reelle, phppr_journees.numero
			 FROM phppr_clubs, phppr_clubs as CLEXT, phppr_matchs, phppr_journees, phppr_equipes, phppr_equipes as EXT, phppr_gr_championnats
			 WHERE phppr_clubs.id=phppr_equipes.id_club
			 AND CLEXT.id=EXT.id_club
			 AND phppr_equipes.id=phppr_matchs.id_equipe_dom
    ...
    if (mysqli_num_rows($resultat)=="0") 
      ...
      while ($row=mysqli_fetch_array($resultat) and $i<$nb_matchs)
      {
       ...
       $resultat2=$idconnect->query( " SELECT pronostic 
									   ...
									   WHERE phppr_pronostics.id_match='$row[2]' 
									   AND phppr_membres.id=phppr_pronostics.id_membre 
       ...
       $resultat2=$idconnect->query("SELECT tps_avant_prono FROM phppr_gr_championnats WHERE id='$gr_champ'");
      //$resultat2=mysql_query($requete2) or die ("probleme " .mysql_error());
