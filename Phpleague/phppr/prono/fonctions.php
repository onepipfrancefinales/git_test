...
function affiche_points ($user_id, $gr_champ, $idconnect)
{
 // $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $result=$idconnect->query("SELECT points 
						    FROM phppr_clmnt_pronos, phppr_membres 
						    WHERE phppr_membres.id=phppr_clmnt_pronos.id_membre 
						    and phppr_membres.id_prono='$user_id' and type='general' 
...
function affiche_clmnt_general ($user_id, $gr_champ, $idconnect)
{
  ...
  $result=$idconnect->query(" SELECT id_prono 
							  ...
							  AND type='general' 
							  AND id_champ='$gr_champ' 
							  ORDER by points desc, participation desc, phppr_membres.pseudo");
  //$result=mysql_query($query) or die (mysql_error());
  $i = "1";
  while ($row=mysqli_fetch_array($result))
  {
...
$result=$idconnect->query("SELECT phppr_clubs.nom, CLEXT.nom, phppr_matchs.buts_dom, phppr_matchs.buts_ext, phppr_matchs.id, phppr_matchs.date_reelle, phppr_journees.numero
...
AND phppr_matchs.buts_dom is null
AND phppr_matchs.buts_ext is null
AND phppr_clubs.nom!='exempte'
AND CLEXT.nom!='exempte'
ORDER by phppr_matchs.date_reelle, phppr_clubs.nom
LIMIT 0, 10");
...
$result2=$idconnect->query( "SELECT pronostic 
							FROM phppr_pronostics, phppr_gr_championnats 
...
if ($nb_pronos == "0") {$prono="0";}
{
  ...
  $resultats2=$idconnect->query("SELECT tps_avant_prono FROM phppr_gr_championnats WHERE id='$gr_champ'");
...
{
   mysqli_query($idconnect, ("DELETE FROM phppr_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_en_cours'"));

   $result=$idconnect->query("SELECT id_membre, pseudo, sum(points) as total, sum(participation) as participations
   FROM phppr_membres, phppr_pronostics, phppr_matchs, phppr_gr_championnats
   WHERE phppr_pronostics.id_champ=phppr_gr_championnats.id
   AND phppr_gr_championnats.id='$gr_champ'
 ...
 //  $result=mysql_query ($query) or die ("probleme " .mysql_error());
   while ($row=mysqli_fetch_array($result))
         ...
         mysqli_query($idconnect, ("INSERT INTO phppr_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_en_cours')"));
         }
}          

if ($type=="mensuel_30_jours")
{
   mysqli_query($idconnect, ("DELETE FROM phppr_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_30_jours'"));
   ...
   //$result=mysql_query ($query) or die ("probleme " .mysql_error());
       ...
       {
       $row[1]=addslashes($row[1]);
       mysqli_query($idconnect, ("INSERT INTO phppr_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_30_jours')"));
       }
}

if ($type=="hebdo")
{
   mysqli_query($idconnect, ("DELETE FROM phppr_clmnt_pronos WHERE id_champ='$gr_champ' AND type='hebdo'"));
...
$result=$idconnect->query("SELECT pseudo, points, participation FROM phppr_clmnt_pronos
...
function pseudo_admin ($gr_champ, $idconnect)
{
 $resultat=$idconnect->query("SELECT pseudo 
							  FROM phppr_membres, phppr_gr_championnats 
							  WHERE phppr_gr_championnats.id_master=phppr_membres.id 
							  AND phppr_gr_championnats.id='$gr_champ'");
...
function champ_prono ($gr_champ, $idconnect)
{
 ...
 $resultat=$idconnect->query("SELECT DISTINCT id, nom FROM phppr_gr_championnats WHERE  phppr_gr_championnats.activ_prono='1' ORDER by id");
