...
function aff_journee($champ, $numero, $legende, $proba, $fiches_clubs, $id_equipe_fetiche, $idconnect)
  ...
  $color=0;
        $result=$idconnect->query("SELECT cldom.nom as cldom, clext.nom as clext, phppr_matchs.buts_dom, phppr_matchs.buts_ext,
...
//mise en forme tableau oppositions
        echo "<table align=\"center\" class=\"tablephppr2\"  width=\"100%\" cellspacing=\"1\" cellpadding=\"1\"><tr><td><table cellspacing=\"0\" align=\"center\" width=\"100%\" cellpadding=\"1\">\n";
        $x=1;
        $minute = 0;
        $heure = 0;
        $jour = 0;
        ...
        $annee = 0;
             ...
             $extproba= $row[3];
                ...
                {
                $result2=$idconnect->query("SELECT DOMBUTSPOUR, DOMG, DOMN, DOMP, DOMBUTSCONTRE FROM phppr_clmnt_cache WHERE ID_EQUIPE='$row[eqdom]'");
                ...
                $result2=$idconnect->query("SELECT EXTG, EXTN, EXTP, EXTBUTSCONTRE, EXTBUTSPOUR FROM phppr_clmnt_cache WHERE ID_EQUIPE='$row[eqext]'");
               // $result2=mysql_query($query2);
...
/************* mise en commentaire suite passage php7 ******************
  $requete=$idconnect->query("SELECT phppr_clubs.nom, CLEXT.nom, phppr_matchs.buts_dom, phppr_matchs.buts_ext,
...
{
  $nb_requete=0;
  if($cache=="1"){ mysqli_query("DELETE FROM phppr_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysqli_error());}
  else{ mysqli_query("DELETE FROM phppr_clmnt") or die (mysqli_error());}
          $nb_requete++;
...
$result=$idconnect->query("SELECT pts_victoire, pts_nul, pts_defaite FROM phppr_parametres WHERE id_champ='$champ'");
$nb_requete++;
...
$query=$idconnect->query("SELECT phppr_clubs.nom FROM phppr_clubs, phppr_equipes, phppr_championnats
