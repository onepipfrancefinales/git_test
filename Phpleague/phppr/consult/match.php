...
if (!isset($_GET['id_match']))
...
else
 {
  ...
  $requete="SELECT cldom.nom AS cldom, clext.nom AS clext, phppr_journees.numero, phppr_matchs.date_reelle, phppr_matchs.buts_dom, phppr_matchs.buts_ext, phppr_matchs.date_reelle,
                   cldom.url_logo AS logo_dom, clext.url_logo AS logo_ext
            FROM phppr_clubs AS cldom, phppr_clubs AS clext, phppr_equipes AS dom, phppr_equipes AS ext, phppr_matchs, phppr_journees
            WHERE phppr_matchs.id = '$id_match'
  ...
  $requete1="SELECT nom, prenom, phppr_joueurs.id as id_joueur
             FROM phppr_joueurs, phppr_effectif, phppr_buteurs, phppr_equipes, phppr_matchs
             WHERE phppr_joueurs.id=phppr_effectif.id_joueur
             AND phppr_effectif.id=phppr_buteurs.id_effectif
             AND phppr_buteurs.id_match='$id_match'
             AND phppr_matchs.id_equipe_dom = phppr_equipes.id
             AND phppr_equipes.id=phppr_effectif.id_equipe
             AND phppr_matchs.id=phppr_buteurs.id_match";
  $resultats1=mysql_query($requete1) or die (mysql_error());
  

  $requete2="SELECT nom, prenom, phppr_joueurs.id as id_joueur
             FROM phppr_joueurs, phppr_effectif, phppr_buteurs, phppr_equipes, phppr_matchs
             WHERE phppr_joueurs.id=phppr_effectif.id_joueur
             AND phppr_effectif.id=phppr_buteurs.id_effectif
             AND phppr_buteurs.id_match='$id_match'
             AND phppr_matchs.id_equipe_ext = phppr_equipes.id
             AND phppr_equipes.id=phppr_effectif.id_equipe
             AND phppr_matchs.id=phppr_buteurs.id_match";
