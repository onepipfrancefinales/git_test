...
function nb_journees($id_champ, $idconnect)
         {
        global $nb_journees;
$result=$idconnect->query("SELECT id FROM phppr_equipes WHERE id_champ='$id_champ'");
...
function aff_journee($champ, $bdd)
{
 ...
 $res3=$bdd->query("SELECT phppr_matchs.id_journee
					...
					ORDER BY phppr_matchs.id_journee asc");
...
//  echo "journee : ".$journee;echo "</br>";
 ...
 //echo $numero;
 // echo "</br>";
 $result=$bdd->query("SELECT cldom.nom as cldom, clext.nom as clext, phppr_matchs.buts_dom, phppr_matchs.buts_ext,
  ...
  echo "<br />";
  echo "<table align=\"center\" class=\"tablephppr2\"  width=\"90%\" cellspacing=\"1\" cellpadding=\"1\"><tr><td><table cellspacing=\"0\" align=\"center\" width=\"100%\" cellpadding=\"1\">\n";
  $x=1;
...
function aff_journeeSupp($champ, $bdd)
{
 ...
 $result=$bdd->query("SELECT cldom.nom as cldom, clext.nom as clext, phppr_matchs.buts_dom, phppr_matchs.buts_ext,
                ...
                WHERE phppr_matchs.id_equipe_dom=dom.id
                        AND phppr_matchs.id_equipe_ext=ext.id
                        AND phppr_journees.id_champ='$champ'
                        AND phppr_journees.numero='$journeeSupp'
                        AND dom.id_club=cldom.id
                        AND ext.id_club=clext.id
                        AND phppr_matchs.id_journee=phppr_journees.id
                        AND cldom.nom!='exempte'
...
function classementDetaille($champ, $bdd)
{
 ...
 while ($row = $res->fetch())
  ...
  echo "<table class=\"tablephppr2\" align=\"center\" cellspacing=\"0\" width=\"80%\"><tr class=\"trphppr3\"><th colspan=\"11\">".$legende."</th></tr>\n";
  echo "<tr class=\"trphppr3\">
...
echo "<table class=\"tablephppr2\" align=\"center\" cellspacing=\"0\"  width=\"90%\">";
...
echo "<br />";echo "<br />";
	if ($smart == false)
	echo "<tr class=\"trphppr3\"><td align=\"center\"><font color=\"#FFFF00\"><b>".JOURNEE."</b></font></td><td width=\"5% align=\"center\"></td><td width=\"30%\" ><font color=\"#FFFF00\"><b>".DATE."</b></font></td><td colspan=\"5\ align=\"center\"><font color=\"#FFFF00\"><b>Oppositions</b></font></td></tr>";
    else
	echo"<tr class=\"trphppr3\"><td align=\"center\"><font color=\"#FFFF00\"><b>".JOURNEE."</b></font></td><td width=\"5% align=\"center\"></td><td colspan=\"5\ align=\"center\"><font color=\"#FFFF00\"><b>Oppositions</b></font></td></tr>";
...
{	
 ...
 $resultats2=$bdd->query("SELECT phppr_clubs.nom, CLEXT.nom, phppr_matchs.buts_dom, phppr_matchs.buts_ext, phppr_matchs.id, phppr_matchs.date_reelle
 ...
 $result=$bdd->query("SELECT phppr_journees.numero, phppr_journees.date_prevue, cldom.nom, clext.nom, phppr_matchs.buts_dom, phppr_matchs.buts_ext, dom.id, ext.id
 ...
 while ($row=$result->fetch())
 {
  ...
  { 
   ...
   {   
    ...
    ?>    
	<table class="tablephppr2" cellspacing="0" width="290" align="center">
	  <tr class="trphppr3"><td colspan="3" align="center"><b><font color="#FFFF00">
...
function db_clmnt($champ, $debut, $fin, $cache, $idconnect)
{
  ...
  if($cache=="1"){ 
  mysqli_query("DELETE FROM phppr_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysqli_error());
  }
  else{ 
  mysqli_query("DELETE FROM phppr_clmnt") or die (mysqli_error());}
          $nb_requete++;
...
$resultats=$idconnect->query=("SELECT pts_victoire, pts_nul, pts_defaite FROM phppr_parametres WHERE id_champ='$champ'");
$nb_requete++;
...
// victoires domicile
$resultats=$idconnect->query("SELECT dom.id, count(dom.id), phppr_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phppr_equipes as dom, phppr_clubs, phppr_matchs, phppr_journees, phppr_championnats
...
// Defaites domicile
$dom=$idconnect->query("SELECT dom.id, count(dom.id), phppr_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phppr_equipes as dom, phppr_clubs, phppr_matchs, phppr_journees, phppr_championnats
WHERE dom.id_champ='$champ'
      AND dom.id_club=phppr_clubs.id
      AND dom.id=phppr_matchs.id_equipe_dom
      AND buts_dom < buts_ext
...
$result=$idconnect->query("SELECT phppr_clubs.nom FROM phppr_clubs, phppr_equipes, phppr_championnats
...
//mysql_query("LOCK TABLE phppr_clmnt WRITE, phppr_equipes WRITE, phppr_clubs WRITE") or die (mysql_error());
    ...
    {
    $result="SELECT phppr_clubs.nom, phppr_equipes.id 
            FROM phppr_clubs, phppr_equipes, phppr_championnats
            WHERE phppr_equipes.id_champ=phppr_championnats.id
            AND phppr_championnats.id='$champ'
            AND phppr_equipes.id_club=phppr_clubs.id";
...
if($cache=="1")
	{
	mysqli_query($idconnect, ("DELETE FROM phppr_clmnt_cache WHERE nom='exempte'"));
	}
else
	{
	mysqli_query($idconnect, ("DELETE FROM phppr_clmnt WHERE nom='exempte'"));
	}
...
{
echo "<table class=\"tablephppr2\" align=\"center\" cellspacing=\"0\" width=\"80%\" bgcolor=\"#FFFFFF\"><tr class=\"trphppr3\"><th colspan=\"11\">".$legende."<br /></th></tr>\n";
...
//               statistiques, amélioration de la gestion des buteurs pour phppreague.          //
// Auteur      : Alexis MANGIN                                                                 //
// Email       : Alexis@univert.org                                                            //
// Url         : http://www.univert.org                                                        //
// Démo        : http://phppreague.univert.org/demo.php                                         //
...
{
echo "<table class=\"tablephppr2\" align=\"center\" cellspacing=\"0\" width=\"80%\"><tr class=\"trphppr3\"><th colspan=\"11\">".$legende;
echo "</th></tr>\n<tr class=\"trphppr3\">
<th align=\"center\">".CLMNT_POSITION."</th>
<th align=\"left\">".CLMNT_EQUIPE."</th>
<th align=\"left\">".CLMNT_POINTS."</th>\n";
...
echo "<table class=tablephppr2 align=\"center\"  cellspacing=\"0\" width=\"95%\"><tr class=trphppr3><th colspan=10>".$legendemini;
echo "<tr class=trphppr3>
<th align=\"center\">".CLMNT_POSITION."
<th align=\"center\">".CLMNT_EQUIPE."
<th align=\"left\">".CLMNT_POINTS."
