...
$result=$idconnect->query("SELECT phppr_clubs.nom, phppr_equipes.id_club, phppr_equipes.id_champ FROM phppr_equipes, phppr_clubs where phppr_clubs.id=phppr_equipes.id_club AND phppr_equipes.id='$equipe'");
...
$result=$idconnect->query("SELECT phppr_equipes.id FROM phppr_equipes, phppr_clubs WHERE id_champ='$champ' AND phppr_equipes.id_club=phppr_clubs.id AND phppr_clubs.nom!='exempte'");
//$result=mysql_query($query);
$nb_equipes=mysqli_num_rows($result)+1;

// on détermine la dernière journee jouée
$result=$idconnect->query("SELECT max(phppr_journees.numero) FROM phppr_journees, phppr_matchs where phppr_journees.id=phppr_matchs.id_journee and buts_dom is not NULL AND phppr_journees.id_champ='$champ'");
//$result=mysql_query($query);
while ($row=mysqli_fetch_array($result)){$fin=$row[0];}

// On détermine le nombre de journée total
$result=$idconnect->query("SELECT id FROM phppr_journees WHERE id_champ='$champ'");
//$result=mysql_query($query);
$nb_journees=mysqli_num_rows($result);

// On déterrmine l'année et le nom de la division
$result=$idconnect->query("SELECT phppr_saisons.annee, phppr_divisions.nom FROM phppr_saisons, phppr_divisions, phppr_championnats where phppr_championnats.id=$champ AND phppr_divisions.id=phppr_championnats.id_division and phppr_championnats.id_saison=phppr_saisons.id");
//$result=mysql_query($query);
...
$requete_sql = "SELECT classement FROM phppr_clmnt_graph WHERE id_equipe='$equipe' ORDER BY fin";
