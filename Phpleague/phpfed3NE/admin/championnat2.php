<?php
//***********************************************************************/
// phpfed3NEceeague : gestionnaire de championnat                              */
// ============================================                         */
//                                                                      */
// Version : 0.82                                                       */
// Copyright (c) 2004    Alexis MANGIN                                  */
// http://phpfed3NEceeague.univert.org                                         */
//                                                                      */
...
//                                                                      */
//***********************************************************************/
// Support technique : http://phpfed3NEceeague.univert.org/forum               */
//                                                                      */
//***********************************************************************/
...
{
  mysqli_query($idconnect, ("INSERT INTO phpfed3NE_saisons (annee) values ('$saison')")) or die ("probleme " .mysqli_error());
}

if ($action2=="creer" and $division and $action=="creer")
{
  mysqli_query($idconnect, ("INSERT INTO phpfed3NE_divisions (nom) values ('$division')")) or die ("probleme " .mysqli_error());
}

if ($action3=="creer" and $action=="creer")
{
  mysqli_query($idconnect, ("INSERT INTO phpfed3NE_championnats (id_division, id_saison) values ('$division','$saison')")) or die ("probleme " .mysqli_error());
}


if ($action2=="supp" and $saison and $action=="creer")
{
  mysqli_query($idconnect, ("DELETE FROM phpfed3NE_saisons WHERE id ='$saison'")) or die ("probleme " .mysqli_error());
}

if ($action2=="supp" and $division and $action=="creer")
{
  mysqli_query($idconnect, ("DELETE FROM phpfed3NE_divisions WHERE id ='$division'")) or die ("probleme " .mysqli_error());
}

if ($confirm=="ok" and $champ and $action=="supp")
{
   $resultats=$idconnect->query("SELECT phpfed3NE_equipes.id FROM phpfed3NE_equipes, phpfed3NE_championnats WHERE id_champ='$champ'");
 // $resultats=mysql_query($requete);
     while($row = mysqli_fetch_array($resultats))
      {
        mysqli_query($idconnect, ("DELETE FROM phpfed3NE_matchs where id_equipe_dom='$row[0]' ")) or die ("probleme" .mysqli_error());
        mysqli_query($idconnect, ("DELETE FROM phpfed3NE_matchs where id_equipe_ext='$row[0]' ")) or die ("probleme" .mysqli_error());
      }
    mysqli_query($idconnect, ("DELETE FROM phpfed3NE_championnats where id='$champ' ")) or die ("probleme " .mysqli_error());
    mysqli_query($idconnect, ("DELETE FROM phpfed3NE_journees WHERE  id_champ = '$champ'")) or die ("probleme " .mysqli_error());
    mysqli_query($idconnect, ("DELETE FROM phpfed3NE_equipes WHERE id_champ ='$champ' ")) or die ("probleme " .mysqli_error());
    mysqli_query($idconnect, ("DELETE FROM phpfed3NE_parametres WHERE id_champ ='$champ' ")) or die ("probleme " .mysqli_error());

}
?>

<br /><font class=phpfed3NE><?php echo LEAGUE; ?></font><br /><br />
...
<table class=phpfed3NE width="90%">
    ...
    <td class=phpfed3NE2 width="6%"> 
      <?php //echo MENU_ID; ?>
    </td>
              
    <td class=phpfed3NE2 width="36%"> 
      <?php //echo MENU_NOM; ?>
    </td>
              
    <td class=phpfed3NE2 width="12%"> 
      <?php //echo ADMIN_JOURNEES_MSG3; ?>
    </td>
              
    <td class=phpfed3NE2 width="46%"></td>
            </tr>
