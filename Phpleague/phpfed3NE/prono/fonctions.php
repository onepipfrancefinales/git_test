...
function affiche_points ($user_id, $gr_champ, $idconnect)
{
 // $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $result=$idconnect->query("SELECT points 
						    FROM phpfed3NE_clmnt_pronos, phpfed3NE_membres 
						    WHERE phpfed3NE_membres.id=phpfed3NE_clmnt_pronos.id_membre 
						    and phpfed3NE_membres.id_prono='$user_id' and type='general' 
...
$result=$idconnect->query("SELECT phpfed3NE_clubs.nom, CLEXT.nom, phpfed3NE_matchs.buts_dom, phpfed3NE_matchs.buts_ext, phpfed3NE_matchs.id, phpfed3NE_matchs.date_reelle, phpfed3NE_journees.numero
...
$result2=$idconnect->query( "SELECT pronostic 
							FROM phpfed3NE_pronostics, phpfed3NE_gr_championnats 
...
if ($nb_pronos == "0") {$prono="0";}
{
  ...
  $resultats2=$idconnect->query("SELECT tps_avant_prono FROM phpfed3NE_gr_championnats WHERE id='$gr_champ'");
...
{
   mysqli_query($idconnect, ("DELETE FROM phpfed3NE_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_en_cours'"));

   $result=$idconnect->query("SELECT id_membre, pseudo, sum(points) as total, sum(participation) as participations
   FROM phpfed3NE_membres, phpfed3NE_pronostics, phpfed3NE_matchs, phpfed3NE_gr_championnats
   WHERE phpfed3NE_pronostics.id_champ=phpfed3NE_gr_championnats.id
   AND phpfed3NE_gr_championnats.id='$gr_champ'
 ...
 //  $result=mysql_query ($query) or die ("probleme " .mysql_error());
   while ($row=mysqli_fetch_array($result))
         ...
         mysqli_query($idconnect, ("INSERT INTO phpfed3NE_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_en_cours')"));
         }
}          

if ($type=="mensuel_30_jours")
{
   mysqli_query($idconnect, ("DELETE FROM phpfed3NE_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_30_jours'"));
   ...
   //$result=mysql_query ($query) or die ("probleme " .mysql_error());
       ...
       {
       $row[1]=addslashes($row[1]);
       mysqli_query($idconnect, ("INSERT INTO phpfed3NE_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_30_jours')"));
       }
}

if ($type=="hebdo")
{
   mysqli_query($idconnect, ("DELETE FROM phpfed3NE_clmnt_pronos WHERE id_champ='$gr_champ' AND type='hebdo'"));
...
$result=$idconnect->query("SELECT pseudo, points, participation FROM phpfed3NE_clmnt_pronos
...
function pseudo_admin ($gr_champ, $idconnect)
{
 $resultat=$idconnect->query("SELECT pseudo 
							  FROM phpfed3NE_membres, phpfed3NE_gr_championnats 
							  WHERE phpfed3NE_gr_championnats.id_master=phpfed3NE_membres.id 
							  AND phpfed3NE_gr_championnats.id='$gr_champ'");
...
function champ_prono ($gr_champ, $idconnect)
{
 ...
 $resultat=$idconnect->query("SELECT DISTINCT id, nom FROM phpfed3NE_gr_championnats WHERE  phpfed3NE_gr_championnats.activ_prono='1' ORDER by id");
