...
$resultat=$idconnect->query("SELECT phpfed3NE_matchs.id
  ...
  //$resultat=mysql_query($requete) or die ("probleme " .mysql_error());
      while ($row=mysqli_fetch_array($resultat))
      {
        $resultat1=$idconnect->query("SELECT phpfed3NE_matchs.date_reelle FROM phpfed3NE_matchs WHERE phpfed3NE_matchs.id='$row[0]'");
       // $resultat1=mysql_query($requete1);
        ...
        {
           mysqli_query($idconnect, ("UPDATE  phpfed3NE_pronostics SET pronostic='0' WHERE id_match='$row[0]' AND id_membre='$id'"));}
  ...
  if ($action == "valid_pronos")
  {
    ...
    {   
     ...
     $resultat=$idconnect->query("SELECT phpfed3NE_matchs.date_reelle FROM phpfed3NE_matchs WHERE phpfed3NE_matchs.id='$id_match[$i]'");
    // $resultat=mysql_query($requete);
     ...
     $resultat=$idconnect->query("SELECT tps_avant_prono FROM phpfed3NE_gr_championnats WHERE id='$gr_champ'");
     //$resultat=mysql_query($requete);
     ...
     {
       mysqli_query($idconnect, ("DELETE FROM phpfed3NE_pronostics WHERE pronostic=' '"));
       $resultat =$idconnect->query( "SELECT * FROM phpfed3NE_matchs, phpfed3NE_pronostics, phpfed3NE_membres WHERE phpfed3NE_membres.id_prono='$user_id'
      ...
      // $resultat=mysql_query($requete);
       ...
       $resultat = $idconnect->query("SELECT id FROM phpfed3NE_membres WHERE id_prono='$user_id'");
       //$resultat = mysql_query($requete);
       ...
       if ($nb_prono == "1")
       {
         if ($date_actuelle<($date_match_timestamp+$temps_avant_prono*60))
         {
           mysqli_query($idconnect, ("UPDATE phpfed3NE_pronostics SET pronostic='$f_prono[$i]'
       ...
       if ($nb_prono == "0")
       {
         if ($date_actuelle<($date_match_timestamp+$temps_avant_prono*60))
         {
           mysqli_query($idconnect, ("INSERT INTO phpfed3NE_pronostics (id_membre, pronostic, id_match, id_champ) VALUES ('$id','$f_prono[$i]','$id_match[$i]', '$gr_champ')"));
         }
       }
  ...
  {    if ($debut=="0") 
    ...
    $resultat=$idconnect->query("
			 SELECT phpfed3NE_clubs.nom, CLEXT.nom, phpfed3NE_matchs.id, phpfed3NE_matchs.date_reelle, phpfed3NE_journees.numero
			 FROM phpfed3NE_clubs, phpfed3NE_clubs as CLEXT, phpfed3NE_matchs, phpfed3NE_journees, phpfed3NE_equipes, phpfed3NE_equipes as EXT, phpfed3NE_gr_championnats
			 WHERE phpfed3NE_clubs.id=phpfed3NE_equipes.id_club
			 AND CLEXT.id=EXT.id_club
			 AND phpfed3NE_equipes.id=phpfed3NE_matchs.id_equipe_dom
    ...
    if (mysqli_num_rows($resultat)=="0") 
      ...
      while ($row=mysqli_fetch_array($resultat) and $i<$nb_matchs)
      {
       ...
       $resultat2=$idconnect->query("SELECT tps_avant_prono FROM phpfed3NE_gr_championnats WHERE id='$gr_champ'");
      //$resultat2=mysql_query($requete2) or die ("probleme " .mysql_error());
