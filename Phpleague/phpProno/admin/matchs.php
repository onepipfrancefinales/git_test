...
if (empty ($numero))
  {
    $result=$idconnect->query("
			SELECT max(phpProno_journees.numero) 
...
for ($counter=$nb_rencontres; $counter>0 ; $counter=$counter-1 ) // Nb de rencontres dans la journée
        ...
        $query = "SELECT DISTINCT phpProno_clubs.nom, phpProno_equipes.id FROM phpProno_clubs, phpProno_equipes
	   ...
	   $result= $idconnect->query("
				SELECT DISTINCT phpProno_clubs.nom, phpProno_equipes.id 
				FROM phpProno_clubs, phpProno_equipes 
				WHERE phpProno_equipes.id_champ='$champ' 
				AND phpProno_clubs.id=phpProno_equipes.id_club 
				ORDER BY phpProno_clubs.nom");
...
echo "<small>MATCH_SUM = ".$sum_day."  </small>";
    $result=$idconnect->query("
			SELECT count(DISTINCT phpProno_matchs.id_equipe_dom), 
			count(DISTINCT phpProno_matchs.id_equipe_ext) 
			FROM phpProno_matchs, phpProno_journees   
			WHERE phpProno_matchs.id_journee=phpProno_journees.id    
			AND phpProno_journees.id_champ='$champ'    
			AND phpProno_journees.numero='$x' ");
			//$result=mysql_query($query);
...
$result=$idconnect->query(" 
		SELECT sum(phpProno_matchs.id_equipe_dom), 
			   sum(phpProno_matchs.id_equipe_ext) 
		FROM phpProno_matchs, phpProno_journees 
		WHERE phpProno_matchs.id_journee=phpProno_journees.id 
		AND phpProno_journees.id_champ='$champ'  ");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT   phpProno_matchs.id_equipe_dom as DOM,  count(*) as ct 
		 FROM phpProno_matchs, phpProno_journees 
		 WHERE  phpProno_journees.id_champ='$champ' 
		 AND phpProno_matchs.id_journee=phpProno_journees.id 
		 GROUP BY DOM");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT phpProno_matchs.id_equipe_ext as DOM,  
		 count(*) as ct 
		 FROM phpProno_matchs, phpProno_journees 
		 WHERE  phpProno_journees.id_champ='$champ' 
		 AND phpProno_matchs.id_journee=phpProno_journees.id 
