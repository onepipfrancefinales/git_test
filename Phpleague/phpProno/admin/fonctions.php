...
{
  $resultats=$idconnect->query("SELECT phpProno_championnats.id, phpProno_divisions.nom, phpProno_saisons.annee 
    ...
    while ($row = mysqli_fetch_array($resultats))
      {
        ...
        if ($champ=="$row[0]") {$class="phpProno7";}
        elseif (($i%2)==0) {$class="phpProno3";}
        else {$class="phpProno4";}
    ...
    while ($row = mysqli_fetch_array($resultats))
      {
        ...
        if ($gr_champ=="$row[0]") {$class="phpProno7";}
        elseif (($i%2)==0) {$class="phpProno3";}
        else {$class="phpProno4";}
...
{
  $resultats=$idconnect->query("SELECT phpProno_divisions.nom, phpProno_saisons.annee 
              FROM phpProno_championnats, phpProno_divisions, phpProno_saisons
              WHERE phpProno_championnats.id_division=phpProno_divisions.id 
              AND phpProno_championnats.id_saison=phpProno_saisons.id 
              AND phpProno_championnats.id='$champ' ORDER by annee desc");
...
{
  $result=$idconnect->query(" SELECT phpProno_divisions.id, phpProno_divisions.nom 
							  FROM phpProno_divisions 
							  ORDER by nom");
...
{
  $result=$idconnect->query(" SELECT phpProno_saisons.id, phpProno_saisons.annee 
							  FROM phpProno_saisons 
							  ORDER by annee");
...
{    
  $result=$idconnect->query(" SELECT phpProno_clubs.id, phpProno_clubs.nom 
							  FROM phpProno_clubs 
							  ORDER by nom");
...
function champ_menu($idconnect)
{
 $result=$idconnect->query(" 
		 SELECT phpProno_championnats.id, phpProno_divisions.nom, phpProno_saisons.annee, (phpProno_saisons.annee)+1 
		 FROM phpProno_championnats, phpProno_divisions, phpProno_saisons 
		 WHERE phpProno_divisions.id=phpProno_championnats.id_division 
		 AND phpProno_saisons.id=phpProno_championnats.id_saison 
		 ORDER by annee desc, phpProno_divisions.nom");
...
{
  $result=$idconnect->query(" SELECT phpProno_equipes.id, phpProno_clubs.nom 
							  FROM phpProno_clubs, phpProno_equipes 
							  WHERE id_champ='$champ' 
							  AND phpProno_clubs.id=phpProno_equipes.id_club 
							  ORDER by nom");
    ...
    while ($row=mysqli_fetch_array($result))
    {
         $result2=$idconnect->query(" 
					SELECT phpProno_championnats.id, phpProno_divisions.nom, phpProno_saisons.annee, (phpProno_saisons.annee)+1 
					 ...
					 WHERE phpProno_divisions.id=phpProno_championnats.id_division 
					 AND phpProno_saisons.id=phpProno_championnats.id_saison 
					 AND phpProno_championnats.id='$row[0]'  
					 ORDER by annee desc, phpProno_divisions.nom");
...
{ 
 $resultats= $idconnect->query("SELECT phpProno_clubs.nom, CLEXT.nom, phpProno_matchs.buts_dom, phpProno_matchs.buts_ext, phpProno_matchs.id, phpProno_matchs.date_reelle
  ...
  $resultats3=$idconnect->query(" SELECT phpProno_clubs.nom, CLEXT.nom, phpProno_matchs.buts_dom, phpProno_matchs.buts_ext, phpProno_matchs.id, phpProno_matchs.date_reelle 
								  FROM phpProno_clubs, phpProno_clubs as CLEXT, phpProno_matchs, phpProno_journees, phpProno_equipes, phpProno_equipes as EXT 
								  WHERE phpProno_clubs.id=phpProno_equipes.id_club 
								  AND CLEXT.id=EXT.id_club 
								  AND phpProno_equipes.id=phpProno_matchs.id_equipe_dom 
...
function nom_club($id_equipe, $idconnect)
{
  $result=$idconnect->query(" SELECT nom 
							  FROM phpProno_clubs, phpProno_equipes 
							  WHERE phpProno_clubs.id=phpProno_equipes.id_club 
							  and phpProno_equipes.id='$id_equipe'");
...
function nb_matchs ($numero, $champ, $idconnect)
{
  $result=$idconnect->query("select * 
							 from phpProno_matchs, phpProno_journees 
							 where phpProno_matchs.id_journee=phpProno_journees.id 
							 and phpProno_journees.numero=$numero 
							 and phpProno_journees.id_champ=$champ");
...
function db_clmnt($champ, $debut, $fin, $cache, $idconnect)
{
  ...
  if($cache=="1")
	{ 
	 mysqli_query($idconnect,("DELETE FROM phpProno_clmnt_cache WHERE ID_CHAMP='$champ'")) or die (mysqli_error($idconnect));
	}
	else
	{ 
	 mysqli_query($idconnect,("DELETE FROM phpProno_clmnt")) or die (mysqli_error($idconnect));
	}
     $nb_requete++;
	...
	$dom=$idconnect->query("SELECT dom.id, count(dom.id), phpProno_clubs.nom, sum(buts_dom), sum(buts_ext) 
						  FROM phpProno_equipes as dom, phpProno_clubs, phpProno_matchs, phpProno_journees, phpProno_championnats
	...
	$dom=$idconnect->query("
		 SELECT dom.id, count(dom.id), phpProno_clubs.nom, sum(buts_dom), sum(buts_ext) 
	     FROM phpProno_equipes as dom, phpProno_clubs, phpProno_matchs, phpProno_journees, phpProno_championnats
		 WHERE dom.id_champ='$champ'
         AND dom.id_club=phpProno_clubs.id
         AND dom.id=phpProno_matchs.id_equipe_dom
   ...
   $result=$idconnect->query("SELECT phpProno_clubs.nom 
...
// TABLEAU DE CLASSEMENT
  ...
  $result=$idconnect->query("SELECT phpProno_clubs.nom, phpProno_equipes.penalite, phpProno_equipes.id
	...
	*/
	//mysqlY_query("LOCK TABLE phpProno_clmnt WRITE, phpProno_equipes WRITE, phpProno_clubs WRITE") or die (mysql_error());
	/*
    if (mysqli_num_rows($result)==0)
    {
	   // $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  
		$result=$idconnect->query("SELECT phpProno_clubs.nom, phpProno_equipes.id 
            FROM phpProno_clubs, phpProno_equipes, phpProno_championnats
            WHERE phpProno_equipes.id_champ=phpProno_championnats.id
            AND phpProno_championnats.id='$champ'
            AND phpProno_equipes.id_club=phpProno_clubs.id");
	...
	*/
    while($row = mysqli_fetch_array($result))
    {
		...
		if($cache=="1")
		{
		 mysqli_query($idconnect,  ("INSERT INTO phpProno_clmnt_cache(NOM, POINTS, JOUES, G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN, DOMPOINTS, DOMJOUES, DOMG, DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, EXTPOINTS, EXTJOUES, EXTG, EXTN, EXTP,EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, ID_EQUIPE, ID_CHAMP) 
		...
		{
		 mysqli_query($idconnect,  ("INSERT INTO phpProno_clmnt(NOM, POINTS, JOUES, G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN, DOMPOINTS, DOMJOUES, DOMG, DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, EXTPOINTS, EXTJOUES, EXTG, EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, ID_EQUIPE, ID_CHAMP) 
									  VALUES('$NOM','$POINTS','$JOUES','$G','$N','$P','$BUTSPOUR','$BUTSCONTRE','$DIFF','$PEN','$DOMPOINTS','$DOMJOUES','$DOMG','$DOMN','$DOMP','$DOMBUTSPOUR','$DOMBUTSCONTRE','$DOMDIFF','$EXTPOINTS','$EXTJOUES','$EXTG','$EXTN','$EXTP','$EXTBUTSPOUR','$EXTBUTSCONTRE','$EXTDIFF','$ID_EQUIPE','$ID_CHAMP')"));
	...
	if($cache=="1")
	{$requete="DELETE FROM phpProno_clmnt_cache WHERE nom='exempte'" or die (mysql_error($idconnect));}
	else
	{$requete="DELETE FROM phpProno_clmnt WHERE nom='exempte'" or die (mysql_error($idconnect));}
...
// Nombres de renseignement dans cette classe à partir de l'id_classe  (utilisé dans phpProno_classe.php)
function nb_classe($data, $idconnect)
{
  $result=$idconnect->query("SELECT id FROM phpProno_rens WHERE id_classe='$data'");
//if (!$result) die mysql_error();
  $nb_classe=mysqli_num_rows( $result );
  return("$nb_classe");
}

// Nombres de classes enregistrées (ulilisé dans phpProno_classe.php)
function nb_classe2($idconnect)
{	
  $result=$idconnect->query("SELECT * FROM phpProno_classe");
//if (!$result) die mysql_error();
...
{
  $result=$idconnect->query("SELECT id FROM phpProno_rens where id_classe>'0'");
...
{	
  $result=$idconnect->query("SELECT * FROM phpProno_rens");
...
{
  $result=$idconnect->query("select id, nom from phpProno_rens where nom='$rens'");
//if (!$result) die mysql_error();
  $row=mysqli_fetch_array($result);
  $rens2=$row[0];
  return("$rens2");
...
function aff_rens ($id_classe, $id_clubs, $idconnect)
  {
  $result=$idconnect->query("SELECT phpProno_donnee.id, phpProno_donnee.nom, id_rens, id_clubs, phpProno_rens.id, phpProno_rens.nom, phpProno_rens.id_classe, phpProno_clubs.id, etat, phpProno_donnee.url, phpProno_rens.url
