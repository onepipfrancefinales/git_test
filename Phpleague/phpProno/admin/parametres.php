...
if ($action2=="1")
 ...
 if ($nb_resultats>0)
   {
   //Màj des paramètres
    mysqli_query ($idconnect,(" UPDATE phpProno_parametres 
   ...
   else
   {
    ...
    $requete="INSERT INTO phpProno_parametres (id_champ, pts_victoire, pts_nul, pts_defaite, accession, barrage, relegation,id_equipe_fetiche, fiches_clubs, estimation) 
...
if ($malus)
 {
  ...
  while($x<$y)
   {
	mysqli_query($idconnect, (" UPDATE phpProno_equipes 
...
<table class=phpProno width="80%">
            <tr>
              <td class=phpProno2 align="center" colspan="3"><?php echo ADMIN_PARAM_TITRE." "; affich_champ ($champ, $idconnect); ?></td>
            </tr>
            <tr>
              <td>
...
// $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $resultats=$idconnect->query("SELECT * FROM phpProno_parametres WHERE id_champ='$champ'");
//$resultats = mysql_query ($requete);
$existant=mysqli_fetch_array($resultats);

echo "<tr>";
echo "<td class=phpProno3><form method=\"post\" action=\"\">";
...
// $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $resultats=$idconnect->query("SELECT phpProno_clubs.nom, phpProno_equipes.id FROM phpProno_equipes, phpProno_clubs WHERE phpProno_equipes.id_champ='$champ' AND phpProno_clubs.id=phpProno_equipes.id_club ORDER by nom");
//$resultats = mysqli_query ($requete);
echo "<tr><td class=phpProno3>";
...
<table class=phpProno width="80%">
            <tr>
              <td class=phpProno2 align="center" colspan="3"><?php echo ADMIN_TAPVERT_TITRE." "; affich_champ ($champ, $idconnect); ?></td>
            </tr>
            <tr>
 ...
 // $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $result=$idconnect->query("SELECT id FROM phpProno_equipes WHERE phpProno_equipes.id_champ='$champ'");
...
// $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $result=$idconnect->query("SELECT DISTINCT phpProno_clubs.nom, phpProno_equipes.id, penalite FROM phpProno_clubs, phpProno_equipes WHERE phpProno_equipes.id_champ='$champ' AND phpProno_clubs.id=phpProno_equipes.id_club ORDER BY phpProno_clubs.nom");
