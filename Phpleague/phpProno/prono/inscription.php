...
elseif ($go=="1")
{
      $pseudo=addslashes($pseudo);
       ...
       $resultat = $idconnect->query("SELECT * FROM phpProno_membres where pseudo='$pseudo'");
...
if ($email_verif=="ok" and $pseudo_verif=="ok" and $mdp_verif=="ok" and $mail_verif=="ok")
{
  ...
  //mysqli_query($idconnect, ("INSERT INTO phpProno_membres (pseudo, id_prono, mot_de_passe, mail, nom_site, nom, prenom, adresse, code_postal, ville, pays, date_naissance, profession, mobile, ip, last_connect, admin )
  //             VALUES ('$pseudo', '$id_prono', '$mdpcrypt', '$mail', '$site', '$nom', '$prenom', '$adresse', '$code_postal', '$ville', '$mdp', '$date_naissance', '$profession', '$mobile','$ip','$last_connect','1' )"));
   ...
   mysqli_query($idconnect, ("INSERT INTO phpProno_membres (pseudo, id_prono, mot_de_passe, mail, nom_site, nom, prenom, adresse, ville, pays, admin )
               VALUES ('$pseudo', '$id_prono', '$mdpcrypt', '$mail', '$pseudo', '$pseudo', '$pseudo', '$pseudo',  '$pseudo', '$mdp','1' )"));

  
  mysqli_query($idconnect, ("INSERT INTO phpca_membres (pseudo, id_prono, mot_de_passe, mail, nom_site, nom, prenom, adresse, ville, pays, admin )
               VALUES ('$pseudo', '$id_prono', '$mdpcrypt', '$mail', '$pseudo', '$pseudo', '$pseudo', '$pseudo',  '$pseudo', '$mdp','1' )"));

 
  $result=$idconnect->query("SELECT id FROM phpProno_membres WHERE id_prono='$id_prono'");
 // $result=mysql_query($requete) or die ("probleme " .mysql_error());
  $row=mysqli_fetch_array($result);
  $id_membre=$row[0];

  mysqli_query($idconnect,("INSERT INTO phpProno_pronostics (id_membre, id_champ) 
							VALUES ('$id_membre', '$gr_champ')"));

  $result =$idconnect->query( "SELECT pseudo, mail, nom_site, url_site FROM phpProno_membres WHERE pseudo='$pseudo' and admin='1'");
//$result=mysql_query($requete) or die ("probleme " .mysql_error());
  $row=mysqli_fetch_array($result);
  $pseudo_admin=$row[0];
  $mail_admin=$row[1];
...
$message="<html><head><title>phpPronoeague</title></head><body>
<p><font size=\"2\" face=\"Verdana\">Bonjour et bienvenue sur France Finales Rugby, </font></p>
<p><font size=\"2\" face=\"Verdana\">Vous venez de vous inscrire sur France Finales Rugby App. Vous pouvez d&eacute;sormais proc&eacute;der &agrave; la mise &agrave; jours des r&eacute;sultats des rencontres de votre comit&eacute;.</font></p>
<p><font size=\"2\" face=\"Verdana\">Voici les informations qui vous
permettront d'acc&eacute;der &agrave; la mise &agrave; jours des r&eacute;sultats :</font></p>
<p><font face=\"Verdana\" size=\"2\">Login :&nbsp;  $pseudo 
<br />
Mot de passe :  $mdp </font></p>

<p><font face=\"Verdana\" size=\"2\">Vous pouvez &eacute;galement consulter stats, classements complets, calendriers, les diff&eacute;rents palmar&egrave;s, etc...en consultant
...
echo "-----------------------"; echo "<br/>";   
  ...
  if ($email)
  {
    ...
    echo "<tr><td colspan=\"2\" align=\"center\">".PRONO_INSCRIPTION_SUCCES."<br /><a href=\"/Phpleague/phpProno/prono/index.php\">".PRONO_INSCRIPTION_CONNEXION."</a></td></tr>";
    echo "</table>";
  }
  else 
  {
    echo "<table align=\"center\">";
    echo "<tr><td colspan=\"2\" align=\"center\">".PRONO_INSCRIPTION_ECHOUE."<br /><a href=\"/Phpleague/phpProno/prono/index.php?page=inscription\"><font color=\"#FFFFFF\">Connexion !</font></a></td></tr>";
    echo "</table>";
  }
...
else
{
  echo "<table align=\"center\">";
  ?>
  <tr><td colspan=\"2\" align=\"center\"><font color="#FFFFFF"><?php echo $message;?></font><br /><a href="/Phpleague/phpProno/prono/index.php?page=inscription&champ=$champ"><font color="#FFFFFF" align="center">R&eacute;essayer</font></a></td></tr>
<?php 
 echo "</table>";
}
