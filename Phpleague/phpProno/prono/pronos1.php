...
$resultat=$idconnect->query("SELECT phpProno_matchs.id
  ...
  //$resultat=mysql_query($requete) or die ("probleme " .mysql_error());
      while ($row=mysqli_fetch_array($resultat))
      {
        $resultat1=$idconnect->query("SELECT phpProno_matchs.date_reelle FROM phpProno_matchs WHERE phpProno_matchs.id='$row[0]'");
       // $resultat1=mysql_query($requete1);
        ...
        {
           mysqli_query($idconnect, ("UPDATE  phpProno_pronostics SET pronostic='0' WHERE id_match='$row[0]' AND id_membre='$id'"));}
           mysqli_query($idconnect, ("DELETE FROM phpProno_pronostics WHERE pronostic='0'"));
        }
      }
  ...
  if ($action == "valid_pronos")
  {
    ...
    {   
     ...
     $resultat=$idconnect->query("SELECT phpProno_matchs.date_reelle FROM phpProno_matchs WHERE phpProno_matchs.id='$id_match[$i]'");
    // $resultat=mysql_query($requete);
     ...
     $resultat=$idconnect->query("SELECT tps_avant_prono FROM phpProno_gr_championnats WHERE id='$gr_champ'");
     //$resultat=mysql_query($requete);
     ...
     {
       mysqli_query($idconnect, ("DELETE FROM phpProno_pronostics WHERE pronostic=' '"));
       $resultat =$idconnect->query( "SELECT * FROM phpProno_matchs, phpProno_pronostics, phpProno_membres WHERE phpProno_membres.id_prono='$user_id'
                   AND phpProno_membres.id=phpProno_pronostics.id_membre
                   AND phpProno_pronostics.id_match=phpProno_matchs.id
                   AND phpProno_pronostics.id_match='$id_match[$i]'");
      // $resultat=mysql_query($requete);
       $nb_prono=mysqli_num_rows($resultat);

       $resultat = $idconnect->query("SELECT id FROM phpProno_membres WHERE id_prono='$user_id'");
       //$resultat = mysql_query($requete);
       ...
       if ($nb_prono == "1")
       {
         if ($date_actuelle<($date_match_timestamp+$temps_avant_prono*60))
         {
           mysqli_query($idconnect, ("UPDATE phpProno_pronostics SET pronostic='$f_prono[$i]'
                        WHERE phpProno_pronostics.id_membre='$id'
                        AND phpProno_pronostics.id_match='$id_match[$i]'"));
         }
       }
       if ($nb_prono == "0")
       {
         if ($date_actuelle<($date_match_timestamp+$temps_avant_prono*60))
         {
           mysqli_query($idconnect, ("INSERT INTO phpProno_pronostics (id_membre, pronostic, id_match, id_champ) VALUES ('$id','$f_prono[$i]','$id_match[$i]', '$gr_champ')"));
         }
       }
  ...
  {    if ($debut=="0") 
    ...
    $resultat=$idconnect->query("
			 SELECT phpProno_clubs.nom, CLEXT.nom, phpProno_matchs.id, phpProno_matchs.date_reelle, phpProno_journees.numero
			 FROM phpProno_clubs, phpProno_clubs as CLEXT, phpProno_matchs, phpProno_journees, phpProno_equipes, phpProno_equipes as EXT, phpProno_gr_championnats
			 WHERE phpProno_clubs.id=phpProno_equipes.id_club
			 AND CLEXT.id=EXT.id_club
			 AND phpProno_equipes.id=phpProno_matchs.id_equipe_dom
    ...
    if (mysqli_num_rows($resultat)=="0") 
      ...
      while ($row=mysqli_fetch_array($resultat) and $i<$nb_matchs)
      {
       ...
       $resultat2=$idconnect->query( " SELECT pronostic 
									   ...
									   WHERE phpProno_pronostics.id_match='$row[2]' 
									   AND phpProno_membres.id=phpProno_pronostics.id_membre 
       ...
       $resultat2=$idconnect->query("SELECT tps_avant_prono FROM phpProno_gr_championnats WHERE id='$gr_champ'");
      //$resultat2=mysql_query($requete2) or die ("probleme " .mysql_error());
