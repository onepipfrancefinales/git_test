...
$result=$idconnect->query("
		...
		FROM phpProno_clmnt_pronos, phpProno_membres, phpProno_gr_championnats
		WHERE type='general' 
		AND phpProno_membres.id=phpProno_clmnt_pronos.id_membre 
		AND phpProno_gr_championnats.id='$gr_champ' 
		AND phpProno_clmnt_pronos.id_champ='$gr_champ' 
		AND phpProno_membres.id_prono='$user_id'
		GROUP BY participation
		ORDER by points 
		DESC, participation 
		DESC, phpProno_membres.pseudo");
 ...
 if ($user_id==$row["id_prono"] and $row["participation"]!==0)
   {
     ...
     echo "<tr><td><div class=\"blanc\"><b>G&eacute;n&eacute;ral</b></div></td>";
...
FROM phpProno_clmnt_pronos, phpProno_membres, phpProno_gr_championnats
WHERE type='mensuel_en_cours' AND phpProno_membres.id=phpProno_clmnt_pronos.id_membre and phpProno_gr_championnats.id='$gr_champ' and phpProno_clmnt_pronos.id_champ='$gr_champ' and phpProno_membres.id_prono='$user_id'
...
$result=$idconnect->query("SELECT points, participation, id_prono, pts_prono_participation,pts_prono_exact
FROM phpProno_clmnt_pronos, phpProno_membres, phpProno_gr_championnats
WHERE type='mensuel_30_jours' AND phpProno_membres.id=phpProno_clmnt_pronos.id_membre and phpProno_gr_championnats.id='$gr_champ' and phpProno_membres.id_prono='$user_id'
ORDER by points desc, participation desc, phpProno_membres.pseudo");
//$result=mysql_query($query) or die ("probleme " .mysql_error());
...
$result=$idconnect->query("SELECT points, participation, id_prono, pts_prono_participation,pts_prono_exact
FROM phpProno_clmnt_pronos, phpProno_membres, phpProno_gr_championnats
WHERE type='hebdo' AND phpProno_membres.id=phpProno_clmnt_pronos.id_membre and phpProno_gr_championnats.id='$gr_champ' and phpProno_membres.id_prono='$user_id'
ORDER by points desc, participation desc, phpProno_membres.pseudo");
//$result=mysql_query($query) or die ("probleme " .mysql_error());
