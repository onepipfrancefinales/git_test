...
function affiche_points ($user_id, $gr_champ, $idconnect)
{
  $result=$idconnect->query("SELECT points 
						    FROM phpProno_clmnt_pronos, phpProno_membres 
						    WHERE phpProno_membres.id=phpProno_clmnt_pronos.id_membre 
						    and phpProno_membres.id_prono='$user_id' and type='general' 
...
function affiche_clmnt_general ($user_id, $gr_champ, $idconnect)
{
	$result=$idconnect->query(" SELECT id_prono 
							  ...
							  AND type='general' 
							  AND id_champ='$gr_champ' 
							  ORDER by points desc, participation desc, phpProno_membres.pseudo");
  //$result=mysql_query($query) or die (mysql_error());
  $i = "1";
  while ($row=mysqli_fetch_array($result))
  {
...
{
$result=$idconnect->query("SELECT phpProno_clubs.nom, CLEXT.nom, phpProno_matchs.buts_dom, phpProno_matchs.buts_ext, phpProno_matchs.id, phpProno_matchs.date_reelle, phpProno_journees.numero
...
AND phpProno_matchs.buts_dom is null
AND phpProno_matchs.buts_ext is null
AND phpProno_clubs.nom!='exempte'
AND CLEXT.nom!='exempte'
ORDER by phpProno_matchs.date_reelle, phpProno_clubs.nom
LIMIT 0, 10");
...
$result2=$idconnect->query( "SELECT pronostic 
							FROM phpProno_pronostics, phpProno_gr_championnats 
...
if ($nb_pronos == "0") {$prono="0";}
{
  ...
  $resultats2=$idconnect->query("SELECT tps_avant_prono FROM phpProno_gr_championnats WHERE id='$gr_champ'");
...
{
   mysqli_query($idconnect, ("DELETE FROM phpProno_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_en_cours'"));

   $result=$idconnect->query("SELECT id_membre, pseudo, sum(points) as total, sum(participation) as participations
   FROM phpProno_membres, phpProno_pronostics, phpProno_matchs, phpProno_gr_championnats
   WHERE phpProno_pronostics.id_champ=phpProno_gr_championnats.id
   AND phpProno_gr_championnats.id='$gr_champ'
   AND id_membre=phpProno_membres.id
   AND phpProno_matchs.id=id_match
 ...
 //  $result=mysql_query ($query) or die ("probleme " .mysql_error());
   while ($row=mysqli_fetch_array($result))
         ...
         mysqli_query($idconnect, ("INSERT INTO phpProno_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_en_cours')"));
         }
}          

if ($type=="mensuel_30_jours")
{
   mysqli_query($idconnect, ("DELETE FROM phpProno_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_30_jours'"));
   ...
   //$result=mysql_query ($query) or die ("probleme " .mysql_error());
       ...
       {
       $row[1]=addslashes($row[1]);
       mysqli_query($idconnect, ("INSERT INTO phpProno_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_30_jours')"));
       }
}

if ($type=="hebdo")
{
   mysqli_query($idconnect, ("DELETE FROM phpProno_clmnt_pronos WHERE id_champ='$gr_champ' AND type='hebdo'"));
  ...
  // $result=mysql_query ($query) or die ("probleme " .mysql_error());
       ...
       {
       $row[1]=addslashes($row[1]);
       mysqli_query($idconnect, ("INSERT INTO phpProno_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) 
								  VALUES ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'hebdo')"));
       }
}
if (isset($_REQUEST['complet'])) {$complet=$_REQUEST['complet'];} else {$complet='';}

$result=$idconnect->query("SELECT pseudo, points, participation FROM phpProno_clmnt_pronos
...
function pseudo_admin ($gr_champ, $idconnect)
{
 $resultat=$idconnect->query("SELECT pseudo 
							  FROM phpProno_membres, phpProno_gr_championnats 
							  WHERE phpProno_gr_championnats.id_master=phpProno_membres.id 
							  AND phpProno_gr_championnats.id='$gr_champ'");
...
function champ_prono ($gr_champ, $idconnect)
{
 ...
 $resultat=$idconnect->query("SELECT DISTINCT id, nom FROM phpProno_gr_championnats WHERE  phpProno_gr_championnats.activ_prono='1' ORDER by id");
