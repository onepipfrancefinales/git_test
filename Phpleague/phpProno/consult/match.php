...
if (!isset($_GET['id_match']))
...
else
 {
  ...
  $requete="SELECT cldom.nom AS cldom, clext.nom AS clext, phpProno_journees.numero, phpProno_matchs.date_reelle, phpProno_matchs.buts_dom, phpProno_matchs.buts_ext, phpProno_matchs.date_reelle,
                   cldom.url_logo AS logo_dom, clext.url_logo AS logo_ext
            FROM phpProno_clubs AS cldom, phpProno_clubs AS clext, phpProno_equipes AS dom, phpProno_equipes AS ext, phpProno_matchs, phpProno_journees
            WHERE phpProno_matchs.id = '$id_match'
  ...
  $requete1="SELECT nom, prenom, phpProno_joueurs.id as id_joueur
             FROM phpProno_joueurs, phpProno_effectif, phpProno_buteurs, phpProno_equipes, phpProno_matchs
             WHERE phpProno_joueurs.id=phpProno_effectif.id_joueur
             AND phpProno_effectif.id=phpProno_buteurs.id_effectif
             AND phpProno_buteurs.id_match='$id_match'
             AND phpProno_matchs.id_equipe_dom = phpProno_equipes.id
             AND phpProno_equipes.id=phpProno_effectif.id_equipe
             AND phpProno_matchs.id=phpProno_buteurs.id_match";
  $resultats1=mysql_query($requete1) or die (mysql_error());
  

  $requete2="SELECT nom, prenom, phpProno_joueurs.id as id_joueur
             FROM phpProno_joueurs, phpProno_effectif, phpProno_buteurs, phpProno_equipes, phpProno_matchs
             WHERE phpProno_joueurs.id=phpProno_effectif.id_joueur
             AND phpProno_effectif.id=phpProno_buteurs.id_effectif
             AND phpProno_buteurs.id_match='$id_match'
             AND phpProno_matchs.id_equipe_ext = phpProno_equipes.id
             AND phpProno_equipes.id=phpProno_effectif.id_equipe
             AND phpProno_matchs.id=phpProno_buteurs.id_match";
