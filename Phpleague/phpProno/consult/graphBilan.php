...
$result=$idconnect->query("SELECT phpProno_clubs.nom, phpProno_equipes.id_club, phpProno_equipes.id_champ FROM phpProno_equipes, phpProno_clubs where phpProno_clubs.id=phpProno_equipes.id_club AND phpProno_equipes.id='$equipe'");
...
$result=$idconnect->query("SELECT phpProno_equipes.id FROM phpProno_equipes, phpProno_clubs WHERE id_champ='$champ' AND phpProno_equipes.id_club=phpProno_clubs.id AND phpProno_clubs.nom!='exempte'");
//$result=mysql_query($query);
$nb_equipes=mysqli_num_rows($result)+1;

// on détermine la dernière journee jouée
$result=$idconnect->query("SELECT max(phpProno_journees.numero) FROM phpProno_journees, phpProno_matchs where phpProno_journees.id=phpProno_matchs.id_journee and buts_dom is not NULL AND phpProno_journees.id_champ='$champ'");
//$result=mysql_query($query);
while ($row=mysqli_fetch_array($result)){$fin=$row[0];}

// On détermine le nombre de journée total
$result=$idconnect->query("SELECT id FROM phpProno_journees WHERE id_champ='$champ'");
//$result=mysql_query($query);
$nb_journees=mysqli_num_rows($result);

// On déterrmine l'année et le nom de la division
$result=$idconnect->query("SELECT phpProno_saisons.annee, phpProno_divisions.nom FROM phpProno_saisons, phpProno_divisions, phpProno_championnats where phpProno_championnats.id=$champ AND phpProno_divisions.id=phpProno_championnats.id_division and phpProno_championnats.id_saison=phpProno_saisons.id");
//$result=mysql_query($query);
...
$requete_sql = "SELECT classement FROM phpProno_clmnt_graph WHERE id_equipe='$equipe' ORDER BY fin";
