...
function nb_journees($id_champ, $idconnect)
         ...
         $reponse=$idconnect->query("SELECT id FROM phpProno_equipes WHERE id_champ='$id_champ'");
...
$result=$idconnect->query("SELECT cldom.nom as cldom, clext.nom as clext, phpProno_matchs.buts_dom, phpProno_matchs.buts_ext,
...
//mise en forme tableau oppositions
        echo "<table align=\"center\" class=\"tablephpProno2\"  width=\"100%\" cellspacing=\"1\" cellpadding=\"1\"><tr><td><table cellspacing=\"0\" align=\"center\" width=\"100%\" cellpadding=\"1\">\n";
        $x=1;
        $minute = 0;
        $heure = 0;
        $jour = 0;
        ...
        $annee = 0;
             ...
             $extproba= $row[3];
                ...
                {
                  $result2=$idconnect->query("SELECT DOMBUTSPOUR, DOMG, DOMN, DOMP, DOMBUTSCONTRE FROM phpProno_clmnt_cache WHERE ID_EQUIPE='$row[eqdom]'");
                //$result2=mysqli_query($query2);
                 ...
                 $result2=$idconnect->query("SELECT EXTG, EXTN, EXTP, EXTBUTSCONTRE, EXTBUTSPOUR FROM phpProno_clmnt_cache WHERE ID_EQUIPE='$row[eqext]'");
...
//suppression des liens sur les scores
  ...
  $requete="SELECT phpProno_clubs.nom, CLEXT.nom, phpProno_matchs.buts_dom, phpProno_matchs.buts_ext,
            phpProno_matchs.id, phpProno_matchs.date_reelle
  ...
  {   
   ...
   $row[1]= stripslashes($row[1]);
    ...
    if ($row[0]=='exempte') {echo "<tr bgcolor=\"$bgcolor1\" class=\"trphpProno\"><td colspan=\"7\">".ADMIN_RESULTS_1." : $row[1]</td></tr>";}
    if ($row[1]=='exempte') {echo "<tr bgcolor=\"$bgcolor1\" class=\"trphpProno\"><td colspan=\"7\" >".ADMIN_RESULTS_1." : $row[0]</td></tr>";}
  }
...
{
  $nb_requete=0;
  if($cache=="1"){ mysql_query("DELETE FROM phpProno_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysql_error());}
  else{ mysql_query("DELETE FROM phpProno_clmnt") or die (mysql_error());}
          $nb_requete++;
...
$query="SELECT pts_victoire, pts_nul, pts_defaite FROM phpProno_parametres WHERE id_champ='$champ'";
$nb_requete++;
...
// victoires domicile
$query="SELECT dom.id, count(dom.id), phpProno_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phpProno_equipes as dom, phpProno_clubs, phpProno_matchs, phpProno_journees, phpProno_championnats
...
$query="SELECT phpProno_clubs.nom FROM phpProno_clubs, phpProno_equipes, phpProno_championnats
...
// victoires exterieur
$query="SELECT ext.id, count(ext.id), phpProno_clubs.nom, sum(buts_ext), sum(buts_dom) FROM phpProno_equipes as ext, phpProno_clubs, phpProno_matchs, phpProno_journees, phpProno_championnats
...
$query="SELECT phpProno_clubs.nom, phpProno_equipes.penalite, phpProno_equipes.id
...
//mysql_query("LOCK TABLE phpProno_clmnt WRITE, phpProno_equipes WRITE, phpProno_clubs WRITE") or die (mysql_error());
    ...
    {
    $query="SELECT phpProno_clubs.nom, phpProno_equipes.id 
            FROM phpProno_clubs, phpProno_equipes, phpProno_championnats
            WHERE phpProno_equipes.id_champ=phpProno_championnats.id
            AND phpProno_championnats.id='$champ'
            AND phpProno_equipes.id_club=phpProno_clubs.id";
...
if($cache=="1"){$requete="DELETE FROM phpProno_clmnt_cache WHERE nom='exempte'" or die (mysql_error());}
else{$requete="DELETE FROM phpProno_clmnt WHERE nom='exempte'" or die (mysql_error());}
...
{
echo "<table class=\"tablephpProno2\" align=\"center\" cellspacing=\"0\" width=\"95%\"><tr class=\"trphpProno3\"><th colspan=\"11\">".$legende."</th></tr>\n";
echo "<tr class=\"trphpProno3\">
...
function clmntmini_barre($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini,  $champmini, $requetemini, $nb_dessusmini, $nb_dessousmini, $lienmini, $phpPronoEAGUE_RACINE, $id_equipe_fetiche)
{
echo "<table class=tablephpProno2 align=\"center\" cellspacing=\"0\" width=\"200\"><tr class=trphpProno3><th colspan=10>".$legendemini;
echo "<tr class=trphpProno3>";
echo "<th align=center>".CLMNT_POSITION."";
...
function clmntred($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini, $champmini, $requetemini, $lienmini, $phpPronoEAGUE_RACINE, $id_equipe_fetiche)
{
echo "<table class=tablephpProno2 align=\"center\" cellspacing=\"0\"  width=\"200\"><tr class=trphpProno3><th colspan=10>".$legendemini;
echo "<tr class=trphpProno3>
               ...
               echo "<td>";
                    ...
                    if ($x==0)
                    {
                     ...
                     if ($lienmini=='non'){echo "$row[$x]";}
                      ...
                      {
                        echo "<a href=\"".$phpPronoEAGUE_RACINE."consult/detaileq.php?champ=$champmini&amp;id_equipe=".$row['ID_EQUIPE']."\" target=\"_parent\">$row[$x]</a>";
                      }
                     if ($row['ID_EQUIPE']==$id_equipe_fetiche){echo "</b>";} 
                     }
...
function clmnt_barrered($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini,  $champmini, $requetemini, $lienmini, $phpPronoEAGUE_RACINE, $id_equipe_fetiche)
{
