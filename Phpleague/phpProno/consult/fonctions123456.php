...
/************* mise en commentaire suite passage php7 ******************
  $requete=$idconnect->query("SELECT phpProno_clubs.nom, CLEXT.nom, phpProno_matchs.buts_dom, phpProno_matchs.buts_ext,
...
{
  $nb_requete=0;
  if($cache=="1"){ mysqli_query("DELETE FROM phpProno_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysqli_error());}
  else{ mysqli_query("DELETE FROM phpProno_clmnt") or die (mysqli_error());}
          $nb_requete++;
...
$result=$idconnect->query("SELECT pts_victoire, pts_nul, pts_defaite FROM phpProno_parametres WHERE id_champ='$champ'");
$nb_requete++;
...
$query=$idconnect->query("SELECT phpProno_clubs.nom FROM phpProno_clubs, phpProno_equipes, phpProno_championnats
