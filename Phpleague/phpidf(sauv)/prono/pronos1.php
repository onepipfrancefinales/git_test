...
if ($action == "reset") {
  ...
  while ($row = mysqli_fetch_array($resultat)) {
    $resultat1 = $idconnect->query("SELECT phpidf_matchs.date_reelle 
                                    FROM phpidf_matchs 
                                    WHERE phpidf_matchs.id='$row[0]'");
    ...
    if ($date_actuelle < ($date_match_timestamp + $temps_avant_prono * 60)) {
      mysqli_query($idconnect, ("UPDATE  phpidf_pronostics 
                                 SET pronostic='0' 
...
if ($action == "valid_pronos") {
  ...
  for ($i = 1; $i <= $_REQUEST['nb_fiche']; $i++) {
    ...
    $resultat = $idconnect->query("SELECT phpidf_matchs.date_reelle 
                                   FROM phpidf_matchs 
                                   WHERE phpidf_matchs.id='$id_match[$i]'");
    ...
    if ($f_prono[$i] !== "undefined") {
      ...
      mysqli_query($idconnect, ("DELETE FROM phpidf_pronostics 
                                 WHERE pronostic=' '"));

      $resultat = $idconnect->query("SELECT * 
                                     FROM phpidf_matchs, phpidf_pronostics, phpidf_membres 
                                     WHERE phpidf_membres.id_prono='$user_id'
      ...
      if ($nb_prono == "0") {
        if ($date_actuelle < ($date_match_timestamp + $temps_avant_prono * 60)) {
          mysqli_query($idconnect, ("INSERT INTO phpidf_pronostics (id_membre, pronostic, id_match, id_champ) 
...
//echo "<br>";
  ...
  $resultat = $idconnect->query("
			 SELECT phpidf_clubs.nom, CLEXT.nom, phpidf_matchs.id, phpidf_matchs.date_reelle, phpidf_journees.numero, phpidf_matchs.id_equipe_dom, phpidf_matchs.id_equipe_ext
			 FROM phpidf_clubs, phpidf_clubs as CLEXT, phpidf_matchs, phpidf_journees, phpidf_equipes, phpidf_equipes as EXT, phpidf_gr_championnats
			 WHERE phpidf_clubs.id=phpidf_equipes.id_club
			 AND CLEXT.id=EXT.id_club
			 AND phpidf_equipes.id=phpidf_matchs.id_equipe_dom
       ...
       AND phpidf_clubs.nom!='Exempt'
       AND CLEXT.nom!='Exempt'
			 AND CLEXT.nom!='exempte'

       ORDER by phpidf_matchs.date_reelle, phpidf_matchs.id
			 LIMIT $debut, $fin ");
...
/*
       ...
       AND CLEXT.nom!='Exempt'
			 AND CLEXT.nom!='exempte'
       AND phpidf_matchs.date_reelle < '$dateAffichageMaxi'
       ORDER by phpidf_matchs.date_reelle, phpidf_matchs.id
			 LIMIT $debut, $fin ");
*/
  $i = 0;
