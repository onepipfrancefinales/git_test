...
if (empty ($numero))
  {
    $resultats=$idconnect->query("SELECT max(phpidf_journees.numero) 
...
{ 
  ...
  // recherche de id_journee de la journee miroir si existe
    if ($miroir<>"none")
     {     
       $query = "SELECT id FROM phpidf_journees WHERE  id_champ='$champ' AND numero='$miroir'";
       $result=mysql_query ($query);
          while ($row=mysql_fetch_array($result))
          {
           $id_journee_miroir=$row[0] ;
          }
       $query="DELETE FROM phpidf_matchs WHERE id_journee='$id_journee_miroir'";
       mysql_query($query) or die(mysql_error());

     }
  // effacer les anciennes données
  $x=$numero-1;
  $query = "SELECT id FROM phpidf_journees WHERE id_champ='$champ' AND numero='$x'";
  $result=mysql_query ($query);
       while ($row=mysql_fetch_array($result))
       {
         $id_journee=$row[0] ;
       }
  $query="DELETE FROM phpidf_matchs WHERE id_journee='$id_journee'";
  mysql_query($query) or die(mysql_error());
  ...
  for ( $counter=0; $counter<((nb_equipes($champ, $idconnect))/2); $counter++ )
   {
     ...
     mysql_query("INSERT INTO phpidf_matchs (id_journee, id_equipe_dom, id_equipe_ext) VALUES ('$id_journee','$id_domicile[$counter]','$id_exterieur[$counter]') ") or die(mysql_error());

    //insertion journée miroir, si existe
	if ($miroir<>"none")
	{
          //echo "<br />MIROIR=".$id_journee_miroir."N°:".$miroir;
	  $query="INSERT INTO phpidf_matchs (id_journee, id_equipe_dom, id_equipe_ext) VALUES ('$id_journee_miroir','$id_exterieur[$counter]','$id_domicile[$counter]') ";
          mysql_query ($query) or die(mysql_error());
        }        
...
<table class=phpidf width="80%">
            <tr>
              <td class=phpidf2 align="center" colspan="3" width="100%"><?php echo ADMIN_MATCHS_TITRE." "; affich_champ ($champ, $idconnect); ?></td><td class=phpidf2 align="right"><a href="#" onclick="window.open('Assistant_fr/matchs.htm','Assistant','toolbar=0,location=0,directories=0,status=0,scrollbars=1,resizable=0,copyhistory=0,menuBar=0,width=512,height=512');return false;"><img border="0" alt="Assistant" src="aide.gif"></a></td>
            </tr>
            <tr>
...
for ($counter=$nb_rencontres; $counter>0 ; $counter=$counter-1 ) // Nb de rencontres dans la journée
        ...
        $result =$idconnect( "SELECT DISTINCT phpidf_clubs.nom, phpidf_equipes.id FROM phpidf_clubs, phpidf_equipes
        ...
        $result =$idconnect->query( "SELECT DISTINCT phpidf_clubs.nom, phpidf_equipes.id 
        FROM phpidf_clubs, phpidf_equipes WHERE phpidf_equipes.id_champ='$champ' AND phpidf_clubs.id=phpidf_equipes.id_club ORDER BY phpidf_clubs.nom");
...
//JOURNEE MIROIR ?
		...
		$miroir="none";
                ...
                $result =$idconnect->query( "SELECT numero, id FROM phpidf_journees WHERE id_champ='$champ'  ORDER BY phpidf_journees.numero");
               // $result=mysql_query($query);
                while ($row = mysqli_fetch_array($result)) {
...
<table class=phpidf width="80%">
  <tr> 
    <td class=phpidf2 align=center><?php echo ADMIN_COHERENCE_TITRE; ?>
    </td>
  </tr>
  <tr>
    <td>
...
// CHECK SUM DU CHAMPIONNAT
  $result=$idconnect->query(" SELECT sum(id) FROM phpidf_equipes WHERE id_champ='$champ' ");
...
while($x<=$nb_journees)
    {
$result=$idconnect->query(" SELECT sum(phpidf_matchs.id_equipe_dom), sum(phpidf_matchs.id_equipe_ext) FROM phpidf_matchs, phpidf_journees WHERE phpidf_matchs.id_journee=phpidf_journees.id AND phpidf_journees.id_champ='$champ'   AND phpidf_journees.numero='$x' ");
//$result=mysql_query($query);
$sum=mysqli_fetch_array($result);
$sum_day=$sum[0]+$sum[1];
echo "<small>MATCH_SUM = ".$sum_day."  </small>";
    $query=$idconnect->query("SELECT count(DISTINCT phpidf_matchs.id_equipe_dom), count(DISTINCT phpidf_matchs.id_equipe_ext) FROM phpidf_matchs, phpidf_journees   WHERE phpidf_matchs.id_journee=phpidf_journees.id    AND phpidf_journees.id_champ='$champ'    AND phpidf_journees.numero='$x' ");
   // $result=mysql_query($query);
...
$result=$idconnect->query(" SELECT sum(phpidf_matchs.id_equipe_dom), sum(phpidf_matchs.id_equipe_ext) FROM phpidf_matchs, phpidf_journees WHERE phpidf_matchs.id_journee=phpidf_journees.id AND phpidf_journees.id_champ='$champ'  ");
//$result=mysql_query($query);
$sum=mysqli_fetch_array($result);
$sumsum=$sum[0];
 $result=$idconnect->query("SELECT sum(phpidf_matchs.id_equipe_dom), sum(phpidf_matchs.id_equipe_ext) FROM phpidf_matchs, phpidf_journees WHERE  phpidf_journees.id_champ='$champ' AND phpidf_matchs.id_journee=phpidf_journees.id  ");
//$result=mysql_query($query);
while($row=mysqli_fetch_array($result))
    {
 ...
 $result=$idconnect->query("SELECT   phpidf_matchs.id_equipe_dom as DOM,  count(*) as ct FROM phpidf_matchs, phpidf_journees WHERE  phpidf_journees.id_champ='$champ' AND phpidf_matchs.id_journee=phpidf_journees.id GROUP BY DOM");
//$result=mysql_query($query); 
while($row=mysqli_fetch_array($result))
		{
 ...
 $result=$idconnect->query("SELECT phpidf_matchs.id_equipe_ext as DOM,  count(*) as ct FROM phpidf_matchs, phpidf_journees WHERE  phpidf_journees.id_champ='$champ' AND phpidf_matchs.id_journee=phpidf_journees.id GROUP BY DOM");
//$result=mysql_query($query); 
while($row=mysql_fetch_array($result))
		{
		if ($row[1]<>$nb_journees/2)
