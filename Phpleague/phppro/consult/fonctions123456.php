...
function aff_journee($champ, $numero, $legende, $proba, $fiches_clubs, $id_equipe_fetiche, $idconnect)
  ...
  $color=0;
        $result=$idconnect->query("SELECT cldom.nom as cldom, clext.nom as clext, phppro_matchs.buts_dom, phppro_matchs.buts_ext,
...
//mise en forme tableau oppositions
        echo "<table align=\"center\" class=\"tablephppro2\"  width=\"100%\" cellspacing=\"1\" cellpadding=\"1\"><tr><td><table cellspacing=\"0\" align=\"center\" width=\"100%\" cellpadding=\"1\">\n";
        $x=1;
        $minute = 0;
        $heure = 0;
        $jour = 0;
        ...
        $annee = 0;
             ...
             $extproba= $row[3];
                ...
                {
                $result2=$idconnect->query("SELECT DOMBUTSPOUR, DOMG, DOMN, DOMP, DOMBUTSCONTRE FROM phppro_clmnt_cache WHERE ID_EQUIPE='$row[eqdom]'");
                ...
                $result2=$idconnect->query("SELECT EXTG, EXTN, EXTP, EXTBUTSCONTRE, EXTBUTSPOUR FROM phppro_clmnt_cache WHERE ID_EQUIPE='$row[eqext]'");
               // $result2=mysql_query($query2);
...
/************* mise en commentaire suite passage php7 ******************
  $requete=$idconnect->query("SELECT phppro_clubs.nom, CLEXT.nom, phppro_matchs.buts_dom, phppro_matchs.buts_ext,
            phppro_matchs.id, phppro_matchs.date_reelle
  ...
  //$resultats=mysqli_query($requete) or die (mysqli_error());
  ...
  {   
   ...
   $row[1]= stripslashes($row[1]);
    ...
    if ($row[0]=='exempte') {echo "<tr bgcolor=\"$bgcolor1\" class=\"trphppro\"><td colspan=\"7\">".ADMIN_RESULTS_1." : $row[1]</td></tr>";}
    if ($row[1]=='exempte') {echo "<tr bgcolor=\"$bgcolor1\" class=\"trphppro\"><td colspan=\"7\" >".ADMIN_RESULTS_1." : $row[0]</td></tr>";}
  }
 */
...
{
  $nb_requete=0;
  if($cache=="1"){ mysqli_query("DELETE FROM phppro_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysqli_error());}
  else{ mysqli_query("DELETE FROM phppro_clmnt") or die (mysqli_error());}
          $nb_requete++;
...
$result=$idconnect->query("SELECT pts_victoire, pts_nul, pts_defaite FROM phppro_parametres WHERE id_champ='$champ'");
$nb_requete++;
...
$query=$idconnect->query("SELECT phppro_clubs.nom FROM phppro_clubs, phppro_equipes, phppro_championnats
...
//mysql_query("LOCK TABLE phppro_clmnt WRITE, phppro_equipes WRITE, phppro_clubs WRITE") or die (mysql_error());
    ...
    {
    $result=$idconnect->query("SELECT phppro_clubs.nom, phppro_equipes.id 
            FROM phppro_clubs, phppro_equipes, phppro_championnats
            WHERE phppro_equipes.id_champ=phppro_championnats.id
            AND phppro_championnats.id='$champ'
            AND phppro_equipes.id_club=phppro_clubs.id");
...
if($cache=="1"){$requete="DELETE FROM phppro_clmnt_cache WHERE nom='exempte'" or die (mysql_error());}
else{$requete="DELETE FROM phppro_clmnt WHERE nom='exempte'" or die (mysql_error());}
...
function clmntmini_barre($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini,  $champmini, $requetemini, $nb_dessusmini, $nb_dessousmini, $lienmini, $phpproEAGUE_RACINE, $id_equipe_fetiche)
{
echo "<table class=tablephppro2 align=\"center\" cellspacing=\"0\" width=\"200\"><tr class=trphppro3><th colspan=10>".$legendemini;
echo "<tr class=trphppro3>";
echo "<th align=center>".CLMNT_POSITION."";
...
function clmntred($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini, $champmini, $requetemini, $lienmini, $phpproEAGUE_RACINE, $id_equipe_fetiche)
{
echo "<table class=tablephppro2 align=\"center\" cellspacing=\"0\"  width=\"200\"><tr class=trphppro3><th colspan=10>".$legendemini;
echo "<tr class=trphppro3>
...
function clmnt_barrered($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini,  $champmini, $requetemini, $lienmini, $phpproEAGUE_RACINE, $id_equipe_fetiche)
{
...
function demande_champ ($idconnect)
{
