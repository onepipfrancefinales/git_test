...
function aff_journee($champ, $numero, $legende, $proba, $fiches_clubs, $id_equipe_fetiche, $idconnect)
  ...
  $color=0;
        $result=$idconnect->query("SELECT cldom.nom as cldom, clext.nom as clext, phpfed3GS_matchs.buts_dom, phpfed3GS_matchs.buts_ext,
...
//mise en forme tableau oppositions
        ...
        $annee = 0;
             ...
             $extproba= $row[3];
                ...
                $result2=$idconnect->query("SELECT EXTG, EXTN, EXTP, EXTBUTSCONTRE, EXTBUTSPOUR FROM phpfed3GS_clmnt_cache WHERE ID_EQUIPE='$row[eqext]'");
               // $result2=mysql_query($query2);
...
/************* mise en commentaire suite passage php7 ******************
  $requete=$idconnect->query("SELECT phpfed3GS_clubs.nom, CLEXT.nom, phpfed3GS_matchs.buts_dom, phpfed3GS_matchs.buts_ext,
...
{
  $nb_requete=0;
  if($cache=="1"){ mysqli_query("DELETE FROM phpfed3GS_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysqli_error());}
  else{ mysqli_query("DELETE FROM phpfed3GS_clmnt") or die (mysqli_error());}
          $nb_requete++;
...
$result=$idconnect->query("SELECT pts_victoire, pts_nul, pts_defaite FROM phpfed3GS_parametres WHERE id_champ='$champ'");
$nb_requete++;
...
$query=$idconnect->query("SELECT phpfed3GS_clubs.nom FROM phpfed3GS_clubs, phpfed3GS_equipes, phpfed3GS_championnats
