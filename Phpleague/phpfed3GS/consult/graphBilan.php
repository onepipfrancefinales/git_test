...
$result=$idconnect->query("SELECT phpfed3GS_clubs.nom, phpfed3GS_equipes.id_club, phpfed3GS_equipes.id_champ FROM phpfed3GS_equipes, phpfed3GS_clubs where phpfed3GS_clubs.id=phpfed3GS_equipes.id_club AND phpfed3GS_equipes.id='$equipe'");
...
$result=$idconnect->query("SELECT phpfed3GS_equipes.id FROM phpfed3GS_equipes, phpfed3GS_clubs WHERE id_champ='$champ' AND phpfed3GS_equipes.id_club=phpfed3GS_clubs.id AND phpfed3GS_clubs.nom!='exempte'");
//$result=mysql_query($query);
$nb_equipes=mysqli_num_rows($result)+1;

// on détermine la dernière journee jouée
$result=$idconnect->query("SELECT max(phpfed3GS_journees.numero) FROM phpfed3GS_journees, phpfed3GS_matchs where phpfed3GS_journees.id=phpfed3GS_matchs.id_journee and buts_dom is not NULL AND phpfed3GS_journees.id_champ='$champ'");
//$result=mysql_query($query);
while ($row=mysqli_fetch_array($result)){$fin=$row[0];}

// On détermine le nombre de journée total
$result=$idconnect->query("SELECT id FROM phpfed3GS_journees WHERE id_champ='$champ'");
//$result=mysql_query($query);
$nb_journees=mysqli_num_rows($result);

// On déterrmine l'année et le nom de la division
$result=$idconnect->query("SELECT phpfed3GS_saisons.annee, phpfed3GS_divisions.nom FROM phpfed3GS_saisons, phpfed3GS_divisions, phpfed3GS_championnats where phpfed3GS_championnats.id=$champ AND phpfed3GS_divisions.id=phpfed3GS_championnats.id_division and phpfed3GS_championnats.id_saison=phpfed3GS_saisons.id");
//$result=mysql_query($query);
...
$requete_sql = "SELECT classement FROM phpfed3GS_clmnt_graph WHERE id_equipe='$equipe' ORDER BY fin";
