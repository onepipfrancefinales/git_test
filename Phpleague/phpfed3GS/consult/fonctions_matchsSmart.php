...
function nb_journeesGS($id_champ, $idconnect)
         {
        global $nb_journees;
$result=$idconnect->query("SELECT id FROM phpfed3GS_equipes WHERE id_champ='$id_champ'");
...
function nbre_journeesGS($id_champ, $bdd)
{
...
function nb_equipesGS($id_champ, $idconnect)
         {	 
 $res=$idconnect->query("SELECT id 
        ...
        // $query="SELECT id FROM phpfed3GS_equipes WHERE id_champ='$id_champ'";
        // $result=mysql_query($query);
...
function aff_journeeGS($champ, $bdd)
{
 ...
 $res3=$bdd->query("SELECT phpfed3GS_matchs.id_journee
					...
					ORDER BY phpfed3GS_matchs.id_journee asc");
...
//  echo "journee : ".$journee;echo "</br>";
 ...
 //echo $numero;
 // echo "</br>";
 $result=$bdd->query("SELECT cldom.nom as cldom, clext.nom as clext, phpfed3GS_matchs.buts_dom, phpfed3GS_matchs.buts_ext,
  ...
  echo "<br />";
  echo "<table align=\"center\" class=\"tablephpfed3GS2\"  width=\"90%\" cellspacing=\"1\" cellpadding=\"1\"><tr><td><table cellspacing=\"0\" align=\"center\" width=\"100%\" cellpadding=\"1\">\n";
  $x=1;
...
function aff_journeeSuppGS($champ, $bdd)
{
 ...
 $result=$bdd->query("SELECT cldom.nom as cldom, clext.nom as clext, phpfed3GS_matchs.buts_dom, phpfed3GS_matchs.buts_ext,
                ...
                WHERE phpfed3GS_matchs.id_equipe_dom=dom.id
                        AND phpfed3GS_matchs.id_equipe_ext=ext.id
                        AND phpfed3GS_journees.id_champ='$champ'
                        AND phpfed3GS_journees.numero='$journeeSupp'
                        AND dom.id_club=cldom.id
                        AND ext.id_club=clext.id
                        AND phpfed3GS_matchs.id_journee=phpfed3GS_journees.id
                        AND cldom.nom!='exempte'
...
// ***********************
function classementDetailleGS($champ, $bdd)
{
  $comite ="phpfed3GS";
  $lien="oui";
  $type= "GENERAL";
 ...
 while ($row = $res->fetch())
  ...
  echo "<table class=\"tablephpfed3GS2\" align=\"center\" cellspacing=\"0\" width=\"80%\"><tr class=\"trphpfed3GS3\"><th colspan=\"11\">".$legende."</th></tr>\n";
  echo "<tr class=\"trphpfed3GS3\">
...
function oppositionsDetailleesGS($champ, $id_equipe, $smart, $bdd)
{
 $resultat=$bdd->query("
...
echo "<table class=\"tablephpfed3GS2\" align=\"center\" cellspacing=\"0\"  width=\"90%\">";
...
echo "<br />";echo "<br />";
	if ($smart == false)
	echo "<tr class=\"trphpfed3GS3\"><td align=\"center\"><font color=\"#FFFF00\"><b>".JOURNEE."</b></font></td><td width=\"5% align=\"center\"></td><td width=\"30%\" ><font color=\"#FFFF00\"><b>".DATE."</b></font></td><td colspan=\"5\ align=\"center\"><font color=\"#FFFF00\"><b>Oppositions</b></font></td></tr>";
    else
	echo"<tr class=\"trphpfed3GS3\"><td align=\"center\"><font color=\"#FFFF00\"><b>".JOURNEE."</b></font></td><td width=\"5% align=\"center\"></td><td colspan=\"5\ align=\"center\"><font color=\"#FFFF00\"><b>Oppositions</b></font></td></tr>";
...
function calendrierGS($champ, $smart, $bdd)
{	
 //nbre d'equipes
 $result=$bdd->query("SELECT COUNT(*) 
					  FROM phpfed3GS_equipes 
 ...
 $resultats2=$bdd->query("SELECT phpfed3GS_clubs.nom, CLEXT.nom, phpfed3GS_matchs.buts_dom, phpfed3GS_matchs.buts_ext, phpfed3GS_matchs.id, phpfed3GS_matchs.date_reelle
 ...
 $result=$bdd->query("SELECT phpfed3GS_journees.numero, phpfed3GS_journees.date_prevue, cldom.nom, clext.nom, phpfed3GS_matchs.buts_dom, phpfed3GS_matchs.buts_ext, dom.id, ext.id
...
if ($smart == false)
 ...
 while ($row=$result->fetch())
 {
  ...
  { 
   ...
   ?>
   <br>
   <table class="tablephpfed3GS2" cellspacing="0" width="290" align="center">
   ...
   {   
    ...
    ?>    
	<table class="tablephpfed3GS2" cellspacing="0" width="290" align="center">
	  <tr class="trphpfed3GS3"><td colspan="3" align="center"><b><font color="#FFFF00">
...
function db_clmntGS($champ, $debut, $fin, $cache, $idconnect)
{
	 
  $nb_requete=0;

  if($cache=="1"){ 
  mysqli_query("DELETE FROM phpfed3GS_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysqli_error());
  }
  else{ 
  mysqli_query("DELETE FROM phpfed3GS_clmnt") or die (mysqli_error());}
          $nb_requete++;
...
$resultats=$idconnect->query=("SELECT pts_victoire, pts_nul, pts_defaite FROM phpfed3GS_parametres WHERE id_champ='$champ'");
$nb_requete++;
...
// victoires domicile
$resultats=$idconnect->query("SELECT dom.id, count(dom.id), phpfed3GS_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phpfed3GS_equipes as dom, phpfed3GS_clubs, phpfed3GS_matchs, phpfed3GS_journees, phpfed3GS_championnats
...
// Defaites domicile
$dom=$idconnect->query("SELECT dom.id, count(dom.id), phpfed3GS_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phpfed3GS_equipes as dom, phpfed3GS_clubs, phpfed3GS_matchs, phpfed3GS_journees, phpfed3GS_championnats
WHERE dom.id_champ='$champ'
      AND dom.id_club=phpfed3GS_clubs.id
      AND dom.id=phpfed3GS_matchs.id_equipe_dom
      AND buts_dom < buts_ext
...
// Resultats � domicile
$result=$idconnect->query("SELECT phpfed3GS_clubs.nom FROM phpfed3GS_clubs, phpfed3GS_equipes, phpfed3GS_championnats
...
//mysql_query("LOCK TABLE phpfed3GS_clmnt WRITE, phpfed3GS_equipes WRITE, phpfed3GS_clubs WRITE") or die (mysql_error());
    ...
    {
    $result="SELECT phpfed3GS_clubs.nom, phpfed3GS_equipes.id 
            FROM phpfed3GS_clubs, phpfed3GS_equipes, phpfed3GS_championnats
            WHERE phpfed3GS_equipes.id_champ=phpfed3GS_championnats.id
            AND phpfed3GS_championnats.id='$champ'
            AND phpfed3GS_equipes.id_club=phpfed3GS_clubs.id";
...
if($cache=="1")
	{
	mysqli_query($idconnect, ("DELETE FROM phpfed3GS_clmnt_cache WHERE nom='exempte'"));
	}
else
	{
	mysqli_query($idconnect, ("DELETE FROM phpfed3GS_clmnt WHERE nom='exempte'"));
	}
...
function clmntGS($champ,$smart, $bdd)
{
  $comite ="phpfed3GS";
  $lien="oui";
  $type= "GENERAL";
...
function ButeurGS($legende, $requete, $type, $EquipeFetiche, $champ, $debut, $fin, $equipe, $complet)
{
...
//               statistiques, am�lioration de la gestion des buteurs pour phpfed3GSeague.          //
// Auteur      : Alexis MANGIN                                                                 //
// Email       : Alexis@univert.org                                                            //
// Url         : http://www.univert.org                                                        //
// D�mo        : http://phpfed3GSeague.univert.org/demo.php                                         //
...
function clmntminiGS($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini,  $champmini, $requetemini, $nb_dessusmini, $nb_dessousmini, $lienmini, $phpfed3GSEAGUE_RACINE, $id_equipe_fetiche)
{
...
function clmnt_barreGS($legende, $type, $accession, $barrage, $relegation,  $champ, $requete, $lien, $id_equipe_fetiche)
{
...
function clmntmini_barreGS($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini,  $champmini, $requetemini, $nb_dessusmini, $nb_dessousmini, $lienmini, $PHPLEAGUE_RACINE, $id_equipe_fetiche)
{
echo "<table class=tablephpfed3GS2 align=\"center\" cellspacing=\"0\" width=\"200\"><tr class=trphpfed3GS3><th colspan=10>".$legendemini;
echo "<tr class=trphpfed3GS3>";
echo "<th align=center>".CLMNT_POSITION."";
...
function clmntredGS($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini, $champmini, $requetemini, $lienmini, $PHPLEAGUE_RACINE, $id_equipe_fetiche)
{
echo "<table class=tablephpfed3GS2 align=\"center\" cellspacing=\"0\"  width=\"200\"><tr class=trphpfed3GS3><th colspan=10>".$legendemini;
echo "<tr class=trphpfed3GS3>
...
function clmnt_barreredGS($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini,  $champmini, $requetemini, $lienmini, $PHPLEAGUE_RACINE, $id_equipe_fetiche)
{
...
function demande_equipeGS($champ)
{
