...
elseif ($action == "supp" and $confirm=="oui")
 {
   ...
   $result=$idconnect->query("SELECT id FROM phpfed3GS_membres WHERE pseudo='$user_pseudo'");
  // $result=mysql_query($requete) or die ("probleme " .mysql_error());
   $row=mysqli_fetch_array($result);
   
   mysqli_query($idconnect, ("DELETE FROM phpfed3GS_pronostics WHERE id_membre='$row[0]'"));
   mysqli_query($idconnect, ("DELETE FROM phpfed3GS_membres WHERE id='$row[0]'"));
   mysqli_query($idconnect, ("DELETE FROM phpfed3GS_clmnt_pronos WHERE id_membre='$row[0]'"));
  ?>
  ...
  if ($action == "1")
      ...
      $result = $idconnect->query("
          ...
          if (empty ($ancien_mdp) and empty ($nouveau_mdp) and empty ($nouveau_mdp2))
           {
             mysqli_query ($idconnect, ("UPDATE phpfed3GS_membres 
