...
echo $autoidentification;
	...
	else
	{
        ...
        $pass = $_REQUEST['pass'];
        $result =$idconnect->query( "SELECT mot_de_passe FROM phpfed3GS_membres WHERE pseudo='$user'");
	...
	$time = time();
	$result =$idconnect->query( "SELECT * FROM phpfed3GS_membres WHERE pseudo='$user'");
	...
	mysqli_query( $idconnect,("UPDATE phpfed3GS_membres SET ip='$ip', last_connect='$time' WHERE pseudo='$user'"));
