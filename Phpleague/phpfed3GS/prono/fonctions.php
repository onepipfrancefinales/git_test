...
function affiche_points ($user_id, $gr_champ, $idconnect)
{
 // $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $result=$idconnect->query("SELECT points 
						    FROM phpfed3GS_clmnt_pronos, phpfed3GS_membres 
						    WHERE phpfed3GS_membres.id=phpfed3GS_clmnt_pronos.id_membre 
						    and phpfed3GS_membres.id_prono='$user_id' and type='general' 
...
$result=$idconnect->query("SELECT phpfed3GS_clubs.nom, CLEXT.nom, phpfed3GS_matchs.buts_dom, phpfed3GS_matchs.buts_ext, phpfed3GS_matchs.id, phpfed3GS_matchs.date_reelle, phpfed3GS_journees.numero
...
AND phpfed3GS_matchs.buts_dom is null
AND phpfed3GS_matchs.buts_ext is null
AND phpfed3GS_clubs.nom!='exempte'
AND CLEXT.nom!='exempte'
ORDER by phpfed3GS_matchs.date_reelle, phpfed3GS_clubs.nom
LIMIT 0, 10");
...
$result2=$idconnect->query( "SELECT pronostic 
							FROM phpfed3GS_pronostics, phpfed3GS_gr_championnats 
...
if ($nb_pronos == "0") {$prono="0";}
{
  ...
  $resultats2=$idconnect->query("SELECT tps_avant_prono FROM phpfed3GS_gr_championnats WHERE id='$gr_champ'");
...
{
   mysqli_query($idconnect, ("DELETE FROM phpfed3GS_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_en_cours'"));

   $result=$idconnect->query("SELECT id_membre, pseudo, sum(points) as total, sum(participation) as participations
   FROM phpfed3GS_membres, phpfed3GS_pronostics, phpfed3GS_matchs, phpfed3GS_gr_championnats
   WHERE phpfed3GS_pronostics.id_champ=phpfed3GS_gr_championnats.id
   AND phpfed3GS_gr_championnats.id='$gr_champ'
   AND id_membre=phpfed3GS_membres.id
   AND phpfed3GS_matchs.id=id_match
 ...
 //  $result=mysql_query ($query) or die ("probleme " .mysql_error());
   while ($row=mysqli_fetch_array($result))
         ...
         mysqli_query($idconnect, ("INSERT INTO phpfed3GS_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_en_cours')"));
         }
}          

if ($type=="mensuel_30_jours")
{
   mysqli_query($idconnect, ("DELETE FROM phpfed3GS_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_30_jours'"));
   ...
   //$result=mysql_query ($query) or die ("probleme " .mysql_error());
       ...
       {
       $row[1]=addslashes($row[1]);
       mysqli_query($idconnect, ("INSERT INTO phpfed3GS_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_30_jours')"));
       }
}

if ($type=="hebdo")
{
   mysqli_query($idconnect, ("DELETE FROM phpfed3GS_clmnt_pronos WHERE id_champ='$gr_champ' AND type='hebdo'"));
  ...
  // $result=mysql_query ($query) or die ("probleme " .mysql_error());
       ...
       {
       $row[1]=addslashes($row[1]);
       mysqli_query($idconnect, ("INSERT INTO phpfed3GS_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) 
								  VALUES ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'hebdo')"));
       }
}
if (isset($_REQUEST['complet'])) {$complet=$_REQUEST['complet'];} else {$complet='';}

$result=$idconnect->query("SELECT pseudo, points, participation FROM phpfed3GS_clmnt_pronos
...
function pseudo_admin ($gr_champ, $idconnect)
{
 $resultat=$idconnect->query("SELECT pseudo 
							  FROM phpfed3GS_membres, phpfed3GS_gr_championnats 
							  WHERE phpfed3GS_gr_championnats.id_master=phpfed3GS_membres.id 
							  AND phpfed3GS_gr_championnats.id='$gr_champ'");
...
function champ_prono ($gr_champ, $idconnect)
{
 ...
 $resultat=$idconnect->query("SELECT DISTINCT id, nom FROM phpfed3GS_gr_championnats WHERE  phpfed3GS_gr_championnats.activ_prono='1' ORDER by id");
