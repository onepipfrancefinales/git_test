...
$resultat=$idconnect->query("SELECT phpfed3GS_matchs.id
  ...
  //$resultat=mysql_query($requete) or die ("probleme " .mysql_error());
      while ($row=mysqli_fetch_array($resultat))
      {
        $resultat1=$idconnect->query("SELECT phpfed3GS_matchs.date_reelle FROM phpfed3GS_matchs WHERE phpfed3GS_matchs.id='$row[0]'");
       // $resultat1=mysql_query($requete1);
        ...
        {
           mysqli_query($idconnect, ("UPDATE  phpfed3GS_pronostics SET pronostic='0' WHERE id_match='$row[0]' AND id_membre='$id'"));}
           mysqli_query($idconnect, ("DELETE FROM phpfed3GS_pronostics WHERE pronostic='0'"));
        }
      }
  ...
  if ($action == "valid_pronos")
  {
    ...
    {   
     ...
     $resultat=$idconnect->query("SELECT phpfed3GS_matchs.date_reelle FROM phpfed3GS_matchs WHERE phpfed3GS_matchs.id='$id_match[$i]'");
    // $resultat=mysql_query($requete);
     ...
     $resultat=$idconnect->query("SELECT tps_avant_prono FROM phpfed3GS_gr_championnats WHERE id='$gr_champ'");
     //$resultat=mysql_query($requete);
     ...
     {
       mysqli_query($idconnect, ("DELETE FROM phpfed3GS_pronostics WHERE pronostic=' '"));
       $resultat =$idconnect->query( "SELECT * FROM phpfed3GS_matchs, phpfed3GS_pronostics, phpfed3GS_membres WHERE phpfed3GS_membres.id_prono='$user_id'
                   AND phpfed3GS_membres.id=phpfed3GS_pronostics.id_membre
                   AND phpfed3GS_pronostics.id_match=phpfed3GS_matchs.id
                   AND phpfed3GS_pronostics.id_match='$id_match[$i]'");
      // $resultat=mysql_query($requete);
       $nb_prono=mysqli_num_rows($resultat);

       $resultat = $idconnect->query("SELECT id FROM phpfed3GS_membres WHERE id_prono='$user_id'");
       //$resultat = mysql_query($requete);
       ...
       if ($nb_prono == "1")
       {
         if ($date_actuelle<($date_match_timestamp+$temps_avant_prono*60))
         {
           mysqli_query($idconnect, ("UPDATE phpfed3GS_pronostics SET pronostic='$f_prono[$i]'
                        WHERE phpfed3GS_pronostics.id_membre='$id'
                        AND phpfed3GS_pronostics.id_match='$id_match[$i]'"));
         }
       }
       if ($nb_prono == "0")
       {
         if ($date_actuelle<($date_match_timestamp+$temps_avant_prono*60))
         {
           mysqli_query($idconnect, ("INSERT INTO phpfed3GS_pronostics (id_membre, pronostic, id_match, id_champ) VALUES ('$id','$f_prono[$i]','$id_match[$i]', '$gr_champ')"));
         }
       }
  ...
  {    if ($debut=="0") 
    ...
    $resultat=$idconnect->query("
			 SELECT phpfed3GS_clubs.nom, CLEXT.nom, phpfed3GS_matchs.id, phpfed3GS_matchs.date_reelle, phpfed3GS_journees.numero
			 FROM phpfed3GS_clubs, phpfed3GS_clubs as CLEXT, phpfed3GS_matchs, phpfed3GS_journees, phpfed3GS_equipes, phpfed3GS_equipes as EXT, phpfed3GS_gr_championnats
			 WHERE phpfed3GS_clubs.id=phpfed3GS_equipes.id_club
			 AND CLEXT.id=EXT.id_club
			 AND phpfed3GS_equipes.id=phpfed3GS_matchs.id_equipe_dom
    ...
    if (mysqli_num_rows($resultat)=="0") 
      ...
      while ($row=mysqli_fetch_array($resultat) and $i<$nb_matchs)
      {
       ...
       $resultat2=$idconnect->query( " SELECT pronostic 
									   ...
									   WHERE phpfed3GS_pronostics.id_match='$row[2]' 
									   AND phpfed3GS_membres.id=phpfed3GS_pronostics.id_membre 
       ...
       $resultat2=$idconnect->query("SELECT tps_avant_prono FROM phpfed3GS_gr_championnats WHERE id='$gr_champ'");
      //$resultat2=mysql_query($requete2) or die ("probleme " .mysql_error());
