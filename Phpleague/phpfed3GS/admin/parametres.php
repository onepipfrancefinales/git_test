...
if ($action2=="1")
 ...
 // $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $resultats=$idconnect->query("SELECT id_champ FROM phpfed3GS_parametres WHERE id_champ='$champ'");
 ...
 $nb_resultats=mysqli_num_rows($resultats);
   ...
   if ($nb_resultats>0)
     {
       //Màj des paramètres
       mysqli_query ($idconnect,("UPDATE phpfed3GS_parametres SET pts_victoire='$pts_victoire', pts_nul='$pts_nul', pts_defaite='$pts_defaite', accession='$accession', barrage='$barrage', relegation='$relegation', id_equipe_fetiche='$id_equipe_fetiche', fiches_clubs='$fiches_clubs', estimation='$estimation' WHERE id_champ='$champ'")) or die (mysqli_error($idconnect));
     }

   else
     {
       // Insertion des paramètres dans la bdd
       $requete="INSERT INTO phpfed3GS_parametres (id_champ, pts_victoire, pts_nul, pts_defaite, accession, barrage, relegation,id_equipe_fetiche, fiches_clubs, estimation) VALUES ('$champ', '$pts_victoire', '$pts_nul', '$pts_defaite', '$accession', '$barrage', '$relegation', '$id_equipe_fetiche', '$fiches_clubs', '$estimation')";
...
if ($malus)
 {
  ...
  while($x<$y)
   {
	  //   $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');
    mysqli_query($idconnect, ("UPDATE phpfed3GS_equipes SET penalite = '$malus[$x]'  WHERE id = '$id_equipe[$x]'")) or die(mysql_error($idconnect));
    $x++;
   }
 }
?>
...
<table class=phpfed3GS width="80%">
            <tr>
              <td class=phpfed3GS2 align="center" colspan="3"><?php echo ADMIN_PARAM_TITRE." "; affich_champ ($champ, $idconnect); ?></td>
            </tr>
            <tr>
              <td>
...
// $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $resultats=$idconnect->query("SELECT * FROM phpfed3GS_parametres WHERE id_champ='$champ'");
//$resultats = mysql_query ($requete);
$existant=mysqli_fetch_array($resultats);

echo "<tr>";
echo "<td class=phpfed3GS3><form method=\"post\" action=\"\">";
...
// $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $resultats=$idconnect->query("SELECT phpfed3GS_clubs.nom, phpfed3GS_equipes.id FROM phpfed3GS_equipes, phpfed3GS_clubs WHERE phpfed3GS_equipes.id_champ='$champ' AND phpfed3GS_clubs.id=phpfed3GS_equipes.id_club ORDER by nom");
...
<table class=phpfed3GS width="80%">
            <tr>
              <td class=phpfed3GS2 align="center" colspan="3"><?php echo ADMIN_TAPVERT_TITRE." "; affich_champ ($champ, $idconnect); ?></td>
            </tr>
            <tr>
 ...
 // $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $result=$idconnect->query("SELECT id FROM phpfed3GS_equipes WHERE phpfed3GS_equipes.id_champ='$champ'");
...
// $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $result=$idconnect->query("SELECT DISTINCT phpfed3GS_clubs.nom, phpfed3GS_equipes.id, penalite FROM phpfed3GS_clubs, phpfed3GS_equipes WHERE phpfed3GS_equipes.id_champ='$champ' AND phpfed3GS_clubs.id=phpfed3GS_equipes.id_club ORDER BY phpfed3GS_clubs.nom");
