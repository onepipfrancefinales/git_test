...
if(!isset($_POST['id']))
{
       $result=mysql_query("SELECT id, nom FROM phpfed3GS_clubs ORDER BY nom");
...
{
       // On met a jour la bd
       //$query= "SELECT * FROM phpfed3GS_logo WHERE id_club='$id'";
       ...
       //if ($nb=='0')
       //{
         //mysql_query ("INSERT INTO phpfed3GS_logo (id_club) VALUES ('$id')") or die ("probleme " .mysql_error());
         //mysql_query ("UPDATE phpfed3GS_clubs SET url_logo = '$pts'  WHERE id = '$id_equipe'") or die ("probleme " .mysql_error());

         //}

       $query2= "SELECT * FROM phpfed3GS_donnee WHERE id_clubs='$id'";
       ...
       if ($nb2!=$nb_rens)
       {
         mysqli_query($idconnect, ("DELETE FROM phpfed3GS_donnee WHERE id_clubs=$id"));
				
			$result=$idconnect->query("SELECT id FROM phpfed3GS_rens");
			while($row=mysqli_fetch_array($result))
			{
			 mysqli_query($idconnect, ("INSERT INTO phpfed3GS_donnee (id_clubs, id_rens) VALUES ('$id', '$row[0]')")) or die ("probleme " .mysql_error());
			}
        }
       
       echo "<h3>";
       $query=$idconnect->query("SELECT id, nom FROM phpfed3GS_clubs WHERE id='$id'");
       ...
       echo "</h3><br /><br />";
       echo "<table class=tablephpfed3GS2 border=\"0\" cellpadding=\"2\" cellspacing=\"0\" valign=\"bottom\" align=\"center\" width=\"90%\"><form method=\"post\" action=\"\">";
       echo "<input type=\"hidden\" name=\"id\" value=\"$id\">";
       echo "<input type=\"hidden\" name=\"page\" value=\"fiches_clubs\">";
       echo "<input type=\"hidden\" name=\"action\" value=\"gest\">";
       $query="SELECT phpfed3GS_clubs.id, url_logo FROM phpfed3GS_clubs WHERE phpfed3GS_clubs.id='$id'";
       $result = mysql_query ($query) or die(mysql_error());;

               While($row = mysql_fetch_array($result))
               {
                          echo "<tr class=phpfed3GS3>
                          ...
                          echo "<tr class=phpfed3GS2><td></td><td align=\"left\">".ADMIN_EQUIPE_7."</td>";
                          echo "<td align=\"left\"><input type=\"text\" name=\"urllogo\" value=\"$row[1]\" size=25 maxlength=200><td></td><td></td></td></tr>";
               }

        $query2="SELECT phpfed3GS_classe.nom, phpfed3GS_classe.id FROM phpfed3GS_classe order by rang";
        $result2 = mysql_query ($query2);
                          $i=0;
                 while($row2 = mysql_fetch_array($result2))
                 {
                             $query="SELECT phpfed3GS_rens.nom, phpfed3GS_donnee.nom, phpfed3GS_donnee.id, phpfed3GS_donnee.etat, phpfed3GS_donnee.url, phpfed3GS_classe.nom
                             FROM phpfed3GS_rens, phpfed3GS_classe, phpfed3GS_clubs, phpfed3GS_donnee
                             WHERE phpfed3GS_clubs.id='$id'
                                   AND id_clubs='$id'
                                   AND id_classe='$row2[1]'
                                   AND id_classe=phpfed3GS_classe.id
                                   AND phpfed3GS_rens.id=id_rens
                             ORDER by phpfed3GS_rens.rang";
                             $result=mysql_query ($query) or die (mysql_error());
         ...
         echo "<br /><br />";
         $query="SELECT url_logo FROM phpfed3GS_clubs WHERE id='$id'";
         $result = mysql_query($query);
         ...
         $result=$idconnect->query("SELECT phpfed3GS_classe.nom, phpfed3GS_classe.id FROM phpfed3GS_classe order by rang");
        // $result = mysql_query ($query);

                 while($row = mysql_fetch_array($result))
                 {
                            echo "<table class=tablephpfed3GS2 cellspacing=\"0\" align=center width=\"90%\">";
                            echo "<tr class=phpfed3GS3><td><b><font color=\"#FFFFFF\">$row[0]</font></b></td></tr>";
                            $id_classe=$row[1];
         ...
         echo "<table class=tablephpfed3GS2 cellspacing=\"0\" align=center width=\"90%\"><tr class=phpfed3GS3><td><b><font color=\"#FFFFFF\">".CONSULT_CLUB_3."</font></b></td></tr>";
         ...
         WHERE phpfed3GS_equipes.id_champ=phpfed3GS_championnats.id
               ...
               AND phpfed3GS_equipes.id_club='$id'
               AND phpfed3GS_saisons.id=phpfed3GS_championnats.id_saison order by annee desc");
...
{       $nb_rens=nb_rens2($idconnect);
    ...
    reset ($idd);
	 while ( list ($cle, $val)= each ($url) and list ($cle, $val1)= each ($nom) and list ($cle, $val2)= each ($etat) and list ($cle, $val3)= each ($idd))
         ...
         mysql_query ("UPDATE phpfed3GS_clubs SET url_logo='$urllogo' WHERE id='$id'") or die ("probleme " .mysql_error());
         mysql_query ("UPDATE phpfed3GS_donnee SET nom='$val_1', etat='$val2', url='$val' WHERE id='$val3'") or die ("probleme " .mysql_error());
         }
