...
echo "<font class=phpfed3GS>".ADMIN_RENS_TITRE."</font><br /><br /><br />";
...
switch($go) // Ajout/ suppression
{
     ...
     mysql_query (" DELETE FROM phpfed3GS_rens WHERE id='$rens' ") or die ("probleme " .mysql_error());
     mysql_query (" DELETE FROM phpfed3GS_donnee WHERE id_rens='$rens' ") or die ("probleme " .mysql_error());

	 ?>
    <table class=phpfed3GS width="80%">
            <tr>
              <td class=phpfed3GS2 align="center" colspan="2"><?php echo ADMIN_CLASSE_5; ?></td>
            </tr>
            <tr>
	 ...
	 mysql_query ("INSERT INTO phpfed3GS_rens (nom) VALUES ('$rens')") or die ("probleme " .mysql_error());
         $rens2=rens2($rens, $idconnect);

         $query="SELECT id FROM phpfed3GS_clubs";
         $result=mysql_query($query);
         while($row=mysql_fetch_array($result))
         {
         mysql_query ("INSERT INTO phpfed3GS_donnee (id_clubs, id_rens) VALUES ('$row[0]', '$rens2')") or die ("probleme " .mysql_error());
         }
	 ...
	 while ( list ($cle, $val)= each ($url) and list ($cle, $val2)= each ($nom) and list ($cle, $val3)= each ($id))
         {
          ...
          mysql_query ("UPDATE phpfed3GS_rens SET url='$val', nom='$rens_nom' WHERE id='$val3'") or die ("probleme " .mysql_error());
         }
               ?>
    <table class=phpfed3GS width="80%">
            <tr>
...
switch($ga)
{
	 ...
	 mysql_query (" UPDATE phpfed3GS_rens SET id_classe='0' WHERE phpfed3GS_rens.id='$val'") or die ("probleme " .mysql_error());
	 ?>
    <table class=phpfed3GS width="80%">
            <tr>
              <td class=phpfed3GS2 align="center" colspan="2"><?php echo ADMIN_CLASSE_5; ?></td>
	...
	while ( list($val, $value)= each($classe))
	 mysql_query ("update phpfed3GS_rens SET id_classe='$value' WHERE id='$rens'") or die ("probleme " .mysql_error());
...
if ($actionc=="1")
   {
    ...
    reset ($id);
	 while ( list ($cle, $val)= each ($rang) and list ($cle, $val2)= each ($id))
         {
                 mysql_query ("UPDATE phpfed3GS_rens SET rang='$val' WHERE id='$val2'") or die ("probleme " .mysql_error());
...
<table class=phpfed3GS width="80%">
            <tr>
              <td class=phpfed3GS2 align="center" colspan="2"><?php echo ADMIN_RENS_TITRE; ?></td>
            </tr>
            <tr>
...
$result = mysql_query("SELECT id, nom FROM phpfed3GS_rens ORDER BY nom");
...
echo ADMIN_RENS_10;
echo "<form method=\"post\" action=\"\">";
echo "<table class=phpfed3GS cellspacing=\"0\" align=center border=\"0\" width=\"80%\"><center>";
echo "<tr class=phpfed3GS3><td>".ADMIN_RENS_12."</td><td>".ADMIN_RENS_13."</td></tr>";
$query="SELECT id, nom, url FROM phpfed3GS_rens";
$result = mysql_query($query);
...
echo "<select name=\"rens\">";
echo "<option value=\"0\"> </option>";
$result = mysql_query("SELECT id, nom, id_classe FROM phpfed3GS_rens ORDER BY nom");
        while($row = mysql_fetch_array($result))
        {
           $a=$row[1]+1;
           $rens_nom = stripslashes($row[1]);
...
$result = mysql_query("select id, nom FROM phpfed3GS_classe ");
        while($row = mysql_fetch_array($result))
        {
...
echo ADMIN_RENS_5;
echo "<br />";
$query="SELECT phpfed3GS_rens.id, phpfed3GS_rens.id_classe, phpfed3GS_rens.nom, phpfed3GS_classe.id, phpfed3GS_classe.nom
       FROM phpfed3GS_rens, phpfed3GS_classe
       WHERE phpfed3GS_rens.id_classe=phpfed3GS_classe.id";
...
echo ADMIN_RENS_14;
$query="SELECT phpfed3GS_rens.nom FROM phpfed3GS_rens where id_classe='0'";
$result = mysql_query($query);
...
echo "<br />";
echo ADMIN_RENS_9;
$query = "SELECT id, nom, rang from phpfed3GS_classe ORDER by rang";
echo "<form method=\"post\" action=\"\">";
$result = mysql_query($query);
     
     while($row = mysql_fetch_array($result))
     {
        echo "<table class=phpfed3GS border=\"0\" cellpadding=\"2\" cellspacing=\"0\" valign=\"bottom\" align=\"center\" width=\"90%\"><tr class=phpfed3GS3><td><b> $row[1] </b></td></tr>";
        $query2="SELECT id, nom, rang, id_classe FROM phpfed3GS_rens WHERE id_classe='$row[0]' ORDER by rang";
