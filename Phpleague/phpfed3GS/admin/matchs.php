...
if (empty ($numero))
  {
    $result=$idconnect->query("
			SELECT max(phpfed3GS_journees.numero) 
...
for ($counter=$nb_rencontres; $counter>0 ; $counter=$counter-1 ) // Nb de rencontres dans la journée
        ...
        $query = "SELECT DISTINCT phpfed3GS_clubs.nom, phpfed3GS_equipes.id FROM phpfed3GS_clubs, phpfed3GS_equipes
	   ...
	   $result= $idconnect->query("
				SELECT DISTINCT phpfed3GS_clubs.nom, phpfed3GS_equipes.id 
				FROM phpfed3GS_clubs, phpfed3GS_equipes 
				WHERE phpfed3GS_equipes.id_champ='$champ' 
				AND phpfed3GS_clubs.id=phpfed3GS_equipes.id_club 
				ORDER BY phpfed3GS_clubs.nom");
...
echo "<small>MATCH_SUM = ".$sum_day."  </small>";
    $result=$idconnect->query("
			SELECT count(DISTINCT phpfed3GS_matchs.id_equipe_dom), 
			count(DISTINCT phpfed3GS_matchs.id_equipe_ext) 
			FROM phpfed3GS_matchs, phpfed3GS_journees   
			WHERE phpfed3GS_matchs.id_journee=phpfed3GS_journees.id    
			AND phpfed3GS_journees.id_champ='$champ'    
			AND phpfed3GS_journees.numero='$x' ");
			//$result=mysql_query($query);
...
$result=$idconnect->query(" 
		SELECT sum(phpfed3GS_matchs.id_equipe_dom), 
			   sum(phpfed3GS_matchs.id_equipe_ext) 
		FROM phpfed3GS_matchs, phpfed3GS_journees 
		WHERE phpfed3GS_matchs.id_journee=phpfed3GS_journees.id 
		AND phpfed3GS_journees.id_champ='$champ'  ");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT   phpfed3GS_matchs.id_equipe_dom as DOM,  count(*) as ct 
		 FROM phpfed3GS_matchs, phpfed3GS_journees 
		 WHERE  phpfed3GS_journees.id_champ='$champ' 
		 AND phpfed3GS_matchs.id_journee=phpfed3GS_journees.id 
		 GROUP BY DOM");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT phpfed3GS_matchs.id_equipe_ext as DOM,  
		 count(*) as ct 
		 FROM phpfed3GS_matchs, phpfed3GS_journees 
		 WHERE  phpfed3GS_journees.id_champ='$champ' 
		 AND phpfed3GS_matchs.id_journee=phpfed3GS_journees.id 
