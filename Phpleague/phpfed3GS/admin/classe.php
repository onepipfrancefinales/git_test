...
echo "<font class=phpfed3GS>".ADMIN_CLASSE_TITRE."</font><br /><br /><br />";
...
switch($go)
{
	 ...
	 $nb_classe=nb_classe($data, $idconnect);
	   if($nb_classe==0)
	  {
    mysqli_query($idconnect, ("DELETE FROM phpfed3GS_classe WHERE id='$data' ")) ;
    ?>
    <table class=phpfed3GS width="80%">
	 ...
	 mysqli_query($idconnect, ("INSERT INTO phpfed3GS_classe (nom) values ('$data1')"));
	 ...
	 while ( list ($cle, $val)= each ($rang) and list ($cle, $val2)= each ($id))
         {
                ...
                //while($x <= $nb_classe2)
               // {
               mysqli_query ($idconnect, ("UPDATE phpfed3GS_classe SET rang='$val' WHERE id='$val2'"));
               //$x++;
                }
                ?>
    <table class=phpfed3GS width="80%">
...
<table class=phpfed3GS width="80%">
            <tr>
              <td class=phpfed3GS2 align="center" colspan="2"><?php echo ADMIN_CLASSE_TITRE; ?></td>
            </tr>
            <tr>
              <td align="center"  colspan="2"><b><?php echo ADMIN_RENS_8; ?></b></td></tr>
<?php 
echo "<tr><td class=phpfed3GS6><form method=\"post\" action=\"\">";
echo ADMIN_CLASSE_SUPP1." ";
echo "<select name=\"data\">";
echo "<option value=\"0\"> </option>";

$result = $idconnect->query("SELECT id, nom FROM phpfed3GS_classe ORDER BY nom");
...
echo "<table class=phpfed3GS border=\"0\" cellpadding=\"2\" cellspacing=\"0\" valign=\"bottom\" align=\"center\" width=\"80%\"><tr class=phpfed3GS3>";
echo "<td class=\"phpfed3GS2\" align=\"center\"><input type=\"text\" name=\"rang[]\" value=\"$row[2]\" size=1 maxlength=1><b> $classe_nom</b></td>";
