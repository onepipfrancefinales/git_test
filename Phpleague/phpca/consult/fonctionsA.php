...
function aff_journee($champ, $numero, $legende, $proba, $fiches_clubs, $id_equipe_fetiche)
    ...
    // cellule d'affichage des derniers résultats
      // cellule d'affichage des derniers résultats
  $color=0;
        $query1="SELECT cldom.nom as cldom, clext.nom as clext, phpca_matchs.buts_dom, phpca_matchs.buts_ext,
...
//mise en forme tableau oppositions
        ...
        $annee = 0;
             ...
             $extproba= $row[3];
                ...
                {
                $query2="SELECT DOMBUTSPOUR, DOMG, DOMN, DOMP, DOMBUTSCONTRE FROM phpca_clmnt_cache WHERE ID_EQUIPE='$row[eqdom]'";
                ...
                $query2="SELECT EXTG, EXTN, EXTP, EXTBUTSCONTRE, EXTBUTSPOUR FROM phpca_clmnt_cache WHERE ID_EQUIPE='$row[eqext]'";
                $result2=mysql_query($query2);
                      while ($row2=mysql_fetch_array($result2))
                      {
...
//suppression des liens sur les scores
  ...
  $requete="SELECT phpca_clubs.nom, CLEXT.nom, phpca_matchs.buts_dom, phpca_matchs.buts_ext,
...
{
  $nb_requete=0;
  if($cache=="1"){ mysql_query("DELETE FROM phpca_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysql_error());}
  else{ mysql_query("DELETE FROM phpca_clmnt") or die (mysql_error());}
          $nb_requete++;
...
$query="SELECT pts_victoire, pts_nul, pts_defaite FROM phpca_parametres WHERE id_champ='$champ'";
$nb_requete++;
...
// victoires domicile
$query="SELECT dom.id, count(dom.id), phpca_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phpca_equipes as dom, phpca_clubs, phpca_matchs, phpca_journees, phpca_championnats
...
$query="SELECT phpca_clubs.nom FROM phpca_clubs, phpca_equipes, phpca_championnats
...
// victoires exterieur
$query="SELECT ext.id, count(ext.id), phpca_clubs.nom, sum(buts_ext), sum(buts_dom) FROM phpca_equipes as ext, phpca_clubs, phpca_matchs, phpca_journees, phpca_championnats
...
$query="SELECT phpca_clubs.nom, phpca_equipes.penalite, phpca_equipes.id
...
//mysql_query("LOCK TABLE phpca_clmnt WRITE, phpca_equipes WRITE, phpca_clubs WRITE") or die (mysql_error());
    ...
    {
    $query="SELECT phpca_clubs.nom, phpca_equipes.id 
            FROM phpca_clubs, phpca_equipes, phpca_championnats
            WHERE phpca_equipes.id_champ=phpca_championnats.id
            AND phpca_championnats.id='$champ'
            AND phpca_equipes.id_club=phpca_clubs.id";
...
{
echo "<table class=\"tablephpca2\" align=\"center\" cellspacing=\"0\" width=\"95%\ height=\"95%\"><tr class=\"trphpca3\"><th colspan=\"11\">".$legende."</th></tr>\n";
echo "<tr class=\"trphpca3\">
...
// Démo        : http://phpcaeague.univert.org/demo.php                                         //
