...
function affiche_points ($user_id, $gr_champ, $idconnect)
{
 // $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $result=$idconnect->query("SELECT points 
						    FROM phpca_clmnt_pronos, phpca_membres 
						    WHERE phpca_membres.id=phpca_clmnt_pronos.id_membre 
						    and phpca_membres.id_prono='$user_id' and type='general' 
...
function affiche_clmnt_general ($user_id, $gr_champ, $idconnect)
{
  ...
  $result=$idconnect->query(" SELECT id_prono 
							  ...
							  AND type='general' 
							  AND id_champ='$gr_champ' 
							  ORDER by points desc, participation desc, phpca_membres.pseudo");
  //$result=mysql_query($query) or die (mysql_error());
  $i = "1";
  while ($row=mysqli_fetch_array($result))
  {
...
$result=$idconnect->query("SELECT phpca_clubs.nom, CLEXT.nom, phpca_matchs.buts_dom, phpca_matchs.buts_ext, phpca_matchs.id, phpca_matchs.date_reelle, phpca_journees.numero
...
$result2=$idconnect->query( "SELECT pronostic 
							FROM phpca_pronostics, phpca_gr_championnats 
...
if ($nb_pronos == "0") {$prono="0";}
{
  ...
  $resultats2=$idconnect->query("SELECT tps_avant_prono FROM phpca_gr_championnats WHERE id='$gr_champ'");
...
{
   mysqli_query($idconnect, ("DELETE FROM phpca_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_en_cours'"));

   $result=$idconnect->query("SELECT id_membre, pseudo, sum(points) as total, sum(participation) as participations
   FROM phpca_membres, phpca_pronostics, phpca_matchs, phpca_gr_championnats
   WHERE phpca_pronostics.id_champ=phpca_gr_championnats.id
   AND phpca_gr_championnats.id='$gr_champ'
 ...
 //  $result=mysql_query ($query) or die ("probleme " .mysql_error());
   while ($row=mysqli_fetch_array($result))
         ...
         mysqli_query($idconnect, ("INSERT INTO phpca_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_en_cours')"));
         }
}          

if ($type=="mensuel_30_jours")
{
   mysqli_query($idconnect, ("DELETE FROM phpca_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_30_jours'"));
   ...
   //$result=mysql_query ($query) or die ("probleme " .mysql_error());
       ...
       {
       $row[1]=addslashes($row[1]);
       mysqli_query($idconnect, ("INSERT INTO phpca_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_30_jours')"));
       }
}

if ($type=="hebdo")
{
   mysqli_query($idconnect, ("DELETE FROM phpca_clmnt_pronos WHERE id_champ='$gr_champ' AND type='hebdo'"));
...
$result=$idconnect->query("SELECT pseudo, points, participation FROM phpca_clmnt_pronos
...
function pseudo_admin ($gr_champ, $idconnect)
{
 $resultat=$idconnect->query("SELECT pseudo 
							  FROM phpca_membres, phpca_gr_championnats 
							  WHERE phpca_gr_championnats.id_master=phpca_membres.id 
							  AND phpca_gr_championnats.id='$gr_champ'");
...
function champ_prono ($gr_champ, $idconnect)
{
 ...
 $resultat=$idconnect->query("SELECT DISTINCT id, nom FROM phpca_gr_championnats WHERE  phpca_gr_championnats.activ_prono='1' ORDER by id");
