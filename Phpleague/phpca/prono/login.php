...
echo "pass : ".$pass;echo "<br>";
	...
	if(!isset($_REQUEST['user']) or !isset($_REQUEST['pass']))
	{
		...
		echo "test5";echo "<br>";
   	 header("Location: /Test/phpca/prono/index.php?champLigue=$champLigue&mode=$mode&page=erreur_login&t=0");
	}
	...
	else
	{
      echo "test7";echo "<br>";
		...
		{
        	header("Location: /Test/phpca/prono/index.php?champLigue=$champLigue&mode=$mode&page=erreur_login&t=1");
		}
		...
		$result =$idconnect->query( "SELECT * FROM phpca_membres WHERE pseudo='$user'");
		...
		mysqli_query( $idconnect,("UPDATE phpca_membres SET ip='$ip', last_connect='$time' WHERE pseudo='$user'"));
	...
	echo "login.php : mot_de_passe -".$_SESSION['mot_de_passe'];echo "<br>";
		...
		//header("Location: /Test/phpca/prono/index.php?champLigue=$champLigue");
		header("Location: /Test/phpca/prono/index.php?page=pronos&champLigue=$champLigue");
