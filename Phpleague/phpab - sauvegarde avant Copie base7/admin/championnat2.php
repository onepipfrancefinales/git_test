...
{
  mysqli_query($idconnect, ("INSERT INTO phpab_saisons (annee) values ('$saison')")) or die ("probleme " .mysqli_error());
}

if ($action2=="creer" and $division and $action=="creer")
{
  mysqli_query($idconnect, ("INSERT INTO phpab_divisions (nom) values ('$division')")) or die ("probleme " .mysqli_error());
}

if ($action3=="creer" and $action=="creer")
{
  mysqli_query($idconnect, ("INSERT INTO phpab_championnats (id_division, id_saison) values ('$division','$saison')")) or die ("probleme " .mysqli_error());
}


if ($action2=="supp" and $saison and $action=="creer")
{
  mysqli_query($idconnect, ("DELETE FROM phpab_saisons WHERE id ='$saison'")) or die ("probleme " .mysqli_error());
}

if ($action2=="supp" and $division and $action=="creer")
{
  mysqli_query($idconnect, ("DELETE FROM phpab_divisions WHERE id ='$division'")) or die ("probleme " .mysqli_error());
}

if ($confirm=="ok" and $champ and $action=="supp")
{
   $resultats=$idconnect->query("SELECT phpab_equipes.id FROM phpab_equipes, phpab_championnats WHERE id_champ='$champ'");
 // $resultats=mysql_query($requete);
     while($row = mysqli_fetch_array($resultats))
      {
        mysqli_query($idconnect, ("DELETE FROM phpab_matchs where id_equipe_dom='$row[0]' ")) or die ("probleme" .mysqli_error());
        mysqli_query($idconnect, ("DELETE FROM phpab_matchs where id_equipe_ext='$row[0]' ")) or die ("probleme" .mysqli_error());
      }
    mysqli_query($idconnect, ("DELETE FROM phpab_championnats where id='$champ' ")) or die ("probleme " .mysqli_error());
    mysqli_query($idconnect, ("DELETE FROM phpab_journees WHERE  id_champ = '$champ'")) or die ("probleme " .mysqli_error());
    mysqli_query($idconnect, ("DELETE FROM phpab_equipes WHERE id_champ ='$champ' ")) or die ("probleme " .mysqli_error());
    mysqli_query($idconnect, ("DELETE FROM phpab_parametres WHERE id_champ ='$champ' ")) or die ("probleme " .mysqli_error());
