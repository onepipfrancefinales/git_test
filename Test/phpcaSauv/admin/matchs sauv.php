...
{ 
  ...
  // recherche de id_journee de la journee miroir si existe
    if ($miroir<>"none")
     {     
       $query = "SELECT id FROM phpca_journees WHERE  id_champ='$champ' AND numero='$miroir'";
  ...
  $x=$numero-1;
  $query = "SELECT id FROM phpca_journees WHERE id_champ='$champ' AND numero='$x'";
  $result=mysql_query ($query);
       while ($row=mysql_fetch_array($result))
       {
         $id_journee=$row[0] ;
...
for ($counter=$nb_rencontres; $counter>0 ; $counter=$counter-1 ) // Nb de rencontres dans la journée
        ...
        $result =$idconnect( "SELECT DISTINCT phpca_clubs.nom, phpca_equipes.id FROM phpca_clubs, phpca_equipes
        ...
        $result =$idconnect->query( "SELECT DISTINCT phpca_clubs.nom, phpca_equipes.id 
        FROM phpca_clubs, phpca_equipes WHERE phpca_equipes.id_champ='$champ' AND phpca_clubs.id=phpca_equipes.id_club ORDER BY phpca_clubs.nom");
...
//JOURNEE MIROIR ?
		...
		$miroir="none";
                ...
                $result =$idconnect->query( "SELECT numero, id FROM phpca_journees WHERE id_champ='$champ'  ORDER BY phpca_journees.numero");
               // $result=mysql_query($query);
                while ($row = mysqli_fetch_array($result)) {
...
// CHECK SUM DU CHAMPIONNAT
  $result=$idconnect->query(" SELECT sum(id) FROM phpca_equipes WHERE id_champ='$champ' ");
...
echo "<small>MATCH_SUM = ".$sum_day."  </small>";
    $query=$idconnect->query("SELECT count(DISTINCT phpca_matchs.id_equipe_dom), count(DISTINCT phpca_matchs.id_equipe_ext) FROM phpca_matchs, phpca_journees   WHERE phpca_matchs.id_journee=phpca_journees.id    AND phpca_journees.id_champ='$champ'    AND phpca_journees.numero='$x' ");
   // $result=mysql_query($query);
...
$result=$idconnect->query(" SELECT sum(phpca_matchs.id_equipe_dom), sum(phpca_matchs.id_equipe_ext) FROM phpca_matchs, phpca_journees WHERE phpca_matchs.id_journee=phpca_journees.id AND phpca_journees.id_champ='$champ'  ");
//$result=mysql_query($query);
$sum=mysqli_fetch_array($result);
$sumsum=$sum[0];
 $result=$idconnect->query("SELECT sum(phpca_matchs.id_equipe_dom), sum(phpca_matchs.id_equipe_ext) FROM phpca_matchs, phpca_journees WHERE  phpca_journees.id_champ='$champ' AND phpca_matchs.id_journee=phpca_journees.id  ");
//$result=mysql_query($query);
while($row=mysqli_fetch_array($result))
    {
 ...
 $result=$idconnect->query("SELECT   phpca_matchs.id_equipe_dom as DOM,  count(*) as ct FROM phpca_matchs, phpca_journees WHERE  phpca_journees.id_champ='$champ' AND phpca_matchs.id_journee=phpca_journees.id GROUP BY DOM");
//$result=mysql_query($query); 
while($row=mysqli_fetch_array($result))
		{
 ...
 $result=$idconnect->query("SELECT phpca_matchs.id_equipe_ext as DOM,  count(*) as ct FROM phpca_matchs, phpca_journees WHERE  phpca_journees.id_champ='$champ' AND phpca_matchs.id_journee=phpca_journees.id GROUP BY DOM");
//$result=mysql_query($query); 
while($row=mysql_fetch_array($result))
		{
		if ($row[1]<>$nb_journees/2)
