...
$result=$idconnect->query("SELECT phpca_clubs.nom, phpca_equipes.id_club, phpca_equipes.id_champ FROM phpca_equipes, phpca_clubs where phpca_clubs.id=phpca_equipes.id_club AND phpca_equipes.id='$equipe'");
...
$result=$idconnect->query("SELECT phpca_equipes.id FROM phpca_equipes, phpca_clubs WHERE id_champ='$champ' AND phpca_equipes.id_club=phpca_clubs.id AND phpca_clubs.nom!='exempte'");
//$result=mysql_query($query);
$nb_equipes=mysqli_num_rows($result)+1;

// on détermine la dernière journee jouée
$result=$idconnect->query("SELECT max(phpca_journees.numero) FROM phpca_journees, phpca_matchs where phpca_journees.id=phpca_matchs.id_journee and buts_dom is not NULL AND phpca_journees.id_champ='$champ'");
//$result=mysql_query($query);
while ($row=mysqli_fetch_array($result)){$fin=$row[0];}

// On détermine le nombre de journée total
$result=$idconnect->query("SELECT id FROM phpca_journees WHERE id_champ='$champ'");
//$result=mysql_query($query);
$nb_journees=mysqli_num_rows($result);

// On déterrmine l'année et le nom de la division
$result=$idconnect->query("SELECT phpca_saisons.annee, phpca_divisions.nom FROM phpca_saisons, phpca_divisions, phpca_championnats where phpca_championnats.id=$champ AND phpca_divisions.id=phpca_championnats.id_division and phpca_championnats.id_saison=phpca_saisons.id");
//$result=mysql_query($query);
...
$requete_sql = "SELECT classement FROM phpca_clmnt_graph WHERE id_equipe='$equipe' ORDER BY fin";
