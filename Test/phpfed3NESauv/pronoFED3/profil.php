...
elseif ($action == "supp" and $confirm=="oui")
 {
   ...
   $result=$idconnect->query("SELECT id FROM phpfed3NE_membres WHERE pseudo='$user_pseudo'");
  // $result=mysql_query($requete) or die ("probleme " .mysql_error());
   $row=mysqli_fetch_array($result);
   
   mysqli_query($idconnect, ("DELETE FROM phpfed3NE_pronostics WHERE id_membre='$row[0]'"));
   mysqli_query($idconnect, ("DELETE FROM phpfed3NE_membres WHERE id='$row[0]'"));
   mysqli_query($idconnect, ("DELETE FROM phpfed3NE_clmnt_pronos WHERE id_membre='$row[0]'"));
  ?>
