...
{
  mysqli_query($idconnect, ("INSERT INTO phpfed3NE_saisons (annee) values ('$saison')"));
}

if ($action2=="creer" and $division and $action=="creer")
{
  mysqli_query($idconnect, ("INSERT INTO phpfed3NE_divisions (nom) values ('$division')")) ;
}

if ($action3=="creer" and $action=="creer")
{
  mysqli_query($idconnect, ("INSERT INTO phpfed3NE_championnats (id_division, id_saison) values ('$division','$saison')")) ;
}


if ($action2=="supp" and $saison and $action=="creer")
{
  mysqli_query($idconnect, ("DELETE FROM phpfed3NE_saisons WHERE id ='$saison'")) ;
}

if ($action2=="supp" and $division and $action=="creer")
{
  mysqli_query($idconnect, ("DELETE FROM phpfed3NE_divisions WHERE id ='$division'")) ;
}
 ...
 // $idconnect=@mysqli_connect('127.0.0.0','root','','onepip-france-db3');	
  ...
  //$resultats=mysql_query($requete);
     while($row = mysqli_fetch_array($resultats))
      {
        mysqli_query($idconnect, ("DELETE FROM phpfed3NE_matchs where id_equipe_dom='$row[0]' ")) ;
        mysqli_query($idconnect, ("DELETE FROM phpfed3NE_matchs where id_equipe_ext='$row[0]' ")) ;
      }
    mysqli_query($idconnect, ("DELETE FROM phpfed3NE_championnats where id='$champ' ")) ;
    mysqli_query($idconnect, ("DELETE FROM phpfed3NE_journees WHERE  id_champ = '$champ'"));
    mysqli_query($idconnect, ("DELETE FROM phpfed3NE_equipes WHERE id_champ ='$champ' "));
   mysqli_query($idconnect, ("DELETE FROM phpfed3NE_parametres WHERE id_champ ='$champ' "));
