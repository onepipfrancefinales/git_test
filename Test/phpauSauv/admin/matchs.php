...
if (empty ($numero))
  {
    $result=$idconnect->query("
			SELECT max(phpau_journees.numero) 
...
for ($counter=$nb_rencontres; $counter>0 ; $counter=$counter-1 ) // Nb de rencontres dans la journ�e
        ...
        $query = "SELECT DISTINCT phpau_clubs.nom, phpau_equipes.id FROM phpau_clubs, phpau_equipes
     ...
     //   $result=mysql_query($query) or die(mysql_error());
	   ...
	   $result= $idconnect->query("
				SELECT DISTINCT phpau_clubs.nom, phpau_equipes.id 
...
echo "<small>MATCH_SUM = ".$sum_day."  </small>";
    $result=$idconnect->query("
			SELECT count(DISTINCT phpau_matchs.id_equipe_dom), 
			count(DISTINCT phpau_matchs.id_equipe_ext) 
			FROM phpau_matchs, phpau_journees   
			WHERE phpau_matchs.id_journee=phpau_journees.id    
			AND phpau_journees.id_champ='$champ'    
			AND phpau_journees.numero='$x' ");
			//$result=mysql_query($query);
/*
    while( $row = (mysql_fetch_array($result)) )
        {
...
$result=$idconnect->query(" 
		SELECT sum(phpau_matchs.id_equipe_dom), 
			   sum(phpau_matchs.id_equipe_ext) 
		FROM phpau_matchs, phpau_journees 
		WHERE phpau_matchs.id_journee=phpau_journees.id 
		AND phpau_journees.id_champ='$champ'  ");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT   phpau_matchs.id_equipe_dom as DOM,  count(*) as ct 
		 FROM phpau_matchs, phpau_journees 
		 WHERE  phpau_journees.id_champ='$champ' 
		 AND phpau_matchs.id_journee=phpau_journees.id 
		 GROUP BY DOM");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT phpau_matchs.id_equipe_ext as DOM,  
		 count(*) as ct 
		 FROM phpau_matchs, phpau_journees 
		 WHERE  phpau_journees.id_champ='$champ' 
		 AND phpau_matchs.id_journee=phpau_journees.id 
