...
echo "<font class=phpau>".ADMIN_CLASSE_TITRE."</font><br /><br /><br />";
...
switch($go)
{
	 ...
	 $nb_classe=nb_classe($data, $idconnect);
	   if($nb_classe==0)
	  {
    mysqli_query($idconnect, ("DELETE FROM phpau_classe WHERE id='$data' ")) ;
    ?>
    <table class=phpau width="80%">
	 ...
	 mysqli_query($idconnect, ("INSERT INTO phpau_classe (nom) values ('$data1')"));
	 ...
	 while ( list ($cle, $val)= each ($rang) and list ($cle, $val2)= each ($id))
         {
                ...
                //while($x <= $nb_classe2)
               // {
               mysqli_query ($idconnect, ("UPDATE phpau_classe SET rang='$val' WHERE id='$val2'"));
               //$x++;
                }
                ?>
    <table class=phpau width="80%">
...
<table class=phpau width="80%">
            <tr>
              <td class=phpau2 align="center" colspan="2"><?php echo ADMIN_CLASSE_TITRE; ?></td>
            </tr>
            <tr>
              <td align="center"  colspan="2"><b><?php echo ADMIN_RENS_8; ?></b></td></tr>
<?php 
echo "<tr><td class=phpau6><form method=\"post\" action=\"\">";
echo ADMIN_CLASSE_SUPP1." ";
echo "<select name=\"data\">";
echo "<option value=\"0\"> </option>";

$result = $idconnect->query("SELECT id, nom FROM phpau_classe ORDER BY nom");
...
echo "<table class=phpau border=\"0\" cellpadding=\"2\" cellspacing=\"0\" valign=\"bottom\" align=\"center\" width=\"80%\"><tr class=phpau3>";
echo "<td class=\"phpau2\" align=\"center\"><input type=\"text\" name=\"rang[]\" value=\"$row[2]\" size=1 maxlength=1><b> $classe_nom</b></td>";
