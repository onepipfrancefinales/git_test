...
elseif ($action == "supp" and $confirm=="oui")
 {
   ...
   $requete="SELECT id FROM phpal_membres WHERE pseudo='$user_pseudo'";
   $result=mysql_query($requete) or die ("probleme " .mysql_error());
   $row=mysql_fetch_array($result);
   
   mysql_query("DELETE FROM phpal_pronostics WHERE id_membre='$row[0]'")or die ("probleme " .mysql_error());
   mysql_query("DELETE FROM phpal_membres WHERE id='$row[0]'")or die ("probleme " .mysql_error());
   mysql_query("DELETE FROM phpal_clmnt_pronos WHERE id_membre='$row[0]'")or die ("probleme " .mysql_error());
  ?>
...
<?php
 ...
 $user_pseudo = addslashes($user_pseudo);
  ...
  if ($action == "1")
 	 {
          $query = "SELECT mot_de_passe FROM phpal_membres WHERE id_prono='$user_id' and pseudo='$user_pseudo'";
          ...
          if (empty ($ancien_mdp) and empty ($nouveau_mdp) and empty ($nouveau_mdp2))
           {
             mysql_query ("UPDATE phpal_membres SET nom_site='$site', mail='$mail', nom='$nom', prenom='$prenom', adresse='$adresse', code_postal='$code_postal', ville='$ville', pays='$pays', date_naissance='$date_naissance', profession='$profession', mobile='$mobile' WHERE id_prono='$user_id' and pseudo='$user_pseudo'") or die ("probleme " .mysql_error());
 	...
 	$query = "SELECT pseudo, mot_de_passe, mail, nom_site, nom, prenom, adresse, code_postal, ville, pays, date_naissance, profession, mobile  FROM phpal_membres WHERE pseudo='$user_pseudo' AND id_prono = '$user_id' ";
 	$result=mysql_query($query) or die (mysql_error());
       ...
       { 
         ...
         echo "<tr><td align=\"right\"><div class=\"blanc\">".PRONO_PROFIL_MAIL." :</div></td><td ><input type=\"text\" name=\"mail\" value=\"$row[mail]\"></td></tr>\n";
