...
echo "<font class=phpal>".ADMIN_CLASSE_TITRE."</font><br /><br /><br />";
...
switch($go)
{
	 ...
	 $nb_classe=nb_classe($data, $idconnect);
	   if($nb_classe==0)
	  {
    mysql_query ("DELETE FROM phpal_classe WHERE id='$data' ") or die ("probleme " .mysql_error());
    ?>
    <table class=phpal width="80%">
	 ...
	 mysql_query ("INSERT INTO phpal_classe (nom) values ('$data1')") or die ("probleme " .mysql_error());
	 ...
	 while ( list ($cle, $val)= each ($rang) and list ($cle, $val2)= each ($id))
         {
                ...
                //while($x <= $nb_classe2)
               // {
               mysql_query ("UPDATE phpal_classe SET rang='$val' WHERE id='$val2'") or die ("probleme " .mysql_error());
               //$x++;
                }
                ?>
    <table class=phpal width="80%">
...
<table class=phpal width="80%">
            <tr>
              <td class=phpal2 align="center" colspan="2"><?php echo ADMIN_CLASSE_TITRE; ?></td>
            </tr>
            <tr>
              <td align="center"  colspan="2"><b><?php echo ADMIN_RENS_8; ?></b></td></tr>
<?php 
echo "<tr><td class=phpal6><form method=\"post\" action=\"\">";
echo ADMIN_CLASSE_SUPP1." ";
echo "<select name=\"data\">";
echo "<option value=\"0\"> </option>";
$result = mysql_query("SELECT id, nom FROM phpal_classe ORDER BY nom");
...
echo "<table class=phpal border=\"0\" cellpadding=\"2\" cellspacing=\"0\" valign=\"bottom\" align=\"center\" width=\"80%\"><tr class=phpal3>";
echo "<td class=\"phpal2\" align=\"center\"><input type=\"text\" name=\"rang[]\" value=\"$row[2]\" size=1 maxlength=1><b> $classe_nom</b></td>";
