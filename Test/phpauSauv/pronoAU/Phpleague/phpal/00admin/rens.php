...
echo "<font class=phpal>".ADMIN_RENS_TITRE."</font><br /><br /><br />";
...
switch($go) // Ajout/ suppression
{
     ...
     mysql_query (" DELETE FROM phpal_rens WHERE id='$rens' ") or die ("probleme " .mysql_error());
     mysql_query (" DELETE FROM phpal_donnee WHERE id_rens='$rens' ") or die ("probleme " .mysql_error());

	 ?>
    <table class=phpal width="80%">
            <tr>
              <td class=phpal2 align="center" colspan="2"><?php echo ADMIN_CLASSE_5; ?></td>
            </tr>
            <tr>
	 ...
	 mysql_query ("INSERT INTO phpal_rens (nom) VALUES ('$rens')") or die ("probleme " .mysql_error());
         $rens2=rens2($rens, $idconnect);

         $query="SELECT id FROM phpal_clubs";
         $result=mysql_query($query);
         while($row=mysql_fetch_array($result))
         {
         mysql_query ("INSERT INTO phpal_donnee (id_clubs, id_rens) VALUES ('$row[0]', '$rens2')") or die ("probleme " .mysql_error());
         }
	 ...
	 while ( list ($cle, $val)= each ($url) and list ($cle, $val2)= each ($nom) and list ($cle, $val3)= each ($id))
         {
          ...
          mysql_query ("UPDATE phpal_rens SET url='$val', nom='$rens_nom' WHERE id='$val3'") or die ("probleme " .mysql_error());
         }
               ?>
    <table class=phpal width="80%">
            <tr>
...
switch($ga)
{
	 ...
	 mysql_query (" UPDATE phpal_rens SET id_classe='0' WHERE phpal_rens.id='$val'") or die ("probleme " .mysql_error());
	 ?>
    <table class=phpal width="80%">
            <tr>
              <td class=phpal2 align="center" colspan="2"><?php echo ADMIN_CLASSE_5; ?></td>
	...
	while ( list($val, $value)= each($classe))
	 mysql_query ("update phpal_rens SET id_classe='$value' WHERE id='$rens'") or die ("probleme " .mysql_error());
...
if ($actionc=="1")
   {
    ...
    reset ($id);
	 while ( list ($cle, $val)= each ($rang) and list ($cle, $val2)= each ($id))
         {
                 mysql_query ("UPDATE phpal_rens SET rang='$val' WHERE id='$val2'") or die ("probleme " .mysql_error());
...
<table class=phpal width="80%">
            <tr>
              <td class=phpal2 align="center" colspan="2"><?php echo ADMIN_RENS_TITRE; ?></td>
            </tr>
            <tr>
...
$result = mysql_query("SELECT id, nom FROM phpal_rens ORDER BY nom");
...
echo ADMIN_RENS_10;
echo "<form method=\"post\" action=\"\">";
echo "<table class=phpal cellspacing=\"0\" align=center border=\"0\" width=\"80%\"><center>";
echo "<tr class=phpal3><td>".ADMIN_RENS_12."</td><td>".ADMIN_RENS_13."</td></tr>";
$query="SELECT id, nom, url FROM phpal_rens";
$result = mysql_query($query);
...
echo "<select name=\"rens\">";
echo "<option value=\"0\"> </option>";
$result = mysql_query("SELECT id, nom, id_classe FROM phpal_rens ORDER BY nom");
        while($row = mysql_fetch_array($result))
        {
           $a=$row[1]+1;
           $rens_nom = stripslashes($row[1]);
...
$result = mysql_query("select id, nom FROM phpal_classe ");
        while($row = mysql_fetch_array($result))
        {
...
echo ADMIN_RENS_5;
echo "<br />";
$query="SELECT phpal_rens.id, phpal_rens.id_classe, phpal_rens.nom, phpal_classe.id, phpal_classe.nom
       FROM phpal_rens, phpal_classe
       WHERE phpal_rens.id_classe=phpal_classe.id";
...
echo ADMIN_RENS_14;
$query="SELECT phpal_rens.nom FROM phpal_rens where id_classe='0'";
$result = mysql_query($query);
...
echo "<br />";
echo ADMIN_RENS_9;
$query = "SELECT id, nom, rang from phpal_classe ORDER by rang";
echo "<form method=\"post\" action=\"\">";
$result = mysql_query($query);
     
     while($row = mysql_fetch_array($result))
     {
        echo "<table class=phpal border=\"0\" cellpadding=\"2\" cellspacing=\"0\" valign=\"bottom\" align=\"center\" width=\"90%\"><tr class=phpal3><td><b> $row[1] </b></td></tr>";
        $query2="SELECT id, nom, rang, id_classe FROM phpal_rens WHERE id_classe='$row[0]' ORDER by rang";
