  ...
  if ($action == "1")
 	 {
         ...
         // $result=mysql_query($query) or die (mysql_error());
          while ($row=mysqli_fetch_array($result)){$mot_de_passe_correct=$row["0"];}
          ...
          if (empty ($ancien_mdp) and empty ($nouveau_mdp) and empty ($nouveau_mdp2))
           {
             mysqli_query ($idconnect,("UPDATE phpab_membres SET nom_site='$site', mail='$mail', nom='$nom', prenom='$prenom', adresse='$adresse', code_postal='$code_postal', ville='$ville', pays='$pays', date_naissance='$date_naissance', profession='$profession', mobile='$mobile' WHERE id_prono='$user_id' and pseudo='$user_pseudo'"));
 	...
 	$result=$idconnect->query ("SELECT pseudo, mot_de_passe, mail, nom_site, nom, prenom, adresse, code_postal, ville, pays, date_naissance, profession, mobile  FROM phpab_membres WHERE pseudo='$user_pseudo' AND id_prono = '$user_id' ");
 	//$result=mysql_query($query) or die (mysql_error());
