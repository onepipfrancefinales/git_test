...
if ($action2=="1")
 ...
 if ($nb_resultats>0)
   {
   //M�j des param�tres
    mysqli_query ($idconnect,(" UPDATE phpidf_parametres 
...
if ($malus)
 {
  ...
  while($x<$y)
   {
	mysqli_query($idconnect, (" UPDATE phpidf_equipes 
...
<table class=phpidf width="80%">
            <tr>
              <td class=phpidf2 align="center" colspan="3"><?php echo ADMIN_PARAM_TITRE." "; affich_champ ($champ, $idconnect); ?></td>
            </tr>
            <tr>
              <td>
...
// $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $resultats=$idconnect->query("SELECT * FROM phpidf_parametres WHERE id_champ='$champ'");
//$resultats = mysql_query ($requete);
$existant=mysqli_fetch_array($resultats);

echo "<tr>";
echo "<td class=phpidf3><form method=\"post\" action=\"\">";
...
<table class=phpidf width="80%">
            <tr>
              <td class=phpidf2 align="center" colspan="3"><?php echo ADMIN_TAPVERT_TITRE." "; affich_champ ($champ, $idconnect); ?></td>
            </tr>
            <tr>
 ...
 // $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $result=$idconnect->query("SELECT id FROM phpidf_equipes WHERE phpidf_equipes.id_champ='$champ'");
...
// $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $result=$idconnect->query("SELECT DISTINCT phpidf_clubs.nom, phpidf_equipes.id, penalite FROM phpidf_clubs, phpidf_equipes WHERE phpidf_equipes.id_champ='$champ' AND phpidf_clubs.id=phpidf_equipes.id_club ORDER BY phpidf_clubs.nom");
...
<table class=phpidf width="80%">
  <tr>
    <td class=phpidf2 align="center" colspan="4"><?php echo "Points administratifs "; affich_champ ($champ, $idconnect); ?></td>
  </tr>
  <tr>
    <td><?php 
	...
	$result=$idconnect->query("	SELECT DISTINCT phpidf_clubs.nom, phpidf_equipes.id, phpidf_equipes.pts_admin, phpidf_equipes.jour_pere  
