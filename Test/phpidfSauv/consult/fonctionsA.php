...
function aff_journee($champ, $numero, $legende, $proba, $fiches_clubs, $id_equipe_fetiche)
    ...
    // cellule d'affichage des derniers résultats
      // cellule d'affichage des derniers résultats
  $color=0;
        $query1="SELECT cldom.nom as cldom, clext.nom as clext, phpidf_matchs.buts_dom, phpidf_matchs.buts_ext,
...
//mise en forme tableau oppositions
        ...
        $annee = 0;
             ...
             $extproba= $row[3];
                ...
                {
                $query2="SELECT DOMBUTSPOUR, DOMG, DOMN, DOMP, DOMBUTSCONTRE FROM phpidf_clmnt_cache WHERE ID_EQUIPE='$row[eqdom]'";
                ...
                $query2="SELECT EXTG, EXTN, EXTP, EXTBUTSCONTRE, EXTBUTSPOUR FROM phpidf_clmnt_cache WHERE ID_EQUIPE='$row[eqext]'";
                $result2=mysql_query($query2);
                      while ($row2=mysql_fetch_array($result2))
                      {
...
//suppression des liens sur les scores
  ...
  $requete="SELECT phpidf_clubs.nom, CLEXT.nom, phpidf_matchs.buts_dom, phpidf_matchs.buts_ext,
...
{
echo "<table class=\"tablephpidf2\" align=\"center\" cellspacing=\"0\" width=\"95%\ height=\"95%\"><tr class=\"trphpidf3\"><th colspan=\"11\">".$legende."</th></tr>\n";
echo "<tr class=\"trphpidf3\">
