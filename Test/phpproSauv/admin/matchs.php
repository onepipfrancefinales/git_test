...
if (empty ($numero))
  {
    $result=$idconnect->query("
			SELECT max(phppro_journees.numero) 
...
for ($counter=$nb_rencontres; $counter>0 ; $counter=$counter-1 ) // Nb de rencontres dans la journ�e
        ...
        $query = "SELECT DISTINCT phppro_clubs.nom, phppro_equipes.id FROM phppro_clubs, phppro_equipes
     ...
     //   $result=mysql_query($query) or die(mysql_error());
	   ...
	   $result= $idconnect->query("
				SELECT DISTINCT phppro_clubs.nom, phppro_equipes.id 
...
echo "<small>MATCH_SUM = ".$sum_day."  </small>";
    $result=$idconnect->query("
			SELECT count(DISTINCT phppro_matchs.id_equipe_dom), 
			count(DISTINCT phppro_matchs.id_equipe_ext) 
			FROM phppro_matchs, phppro_journees   
			WHERE phppro_matchs.id_journee=phppro_journees.id    
			AND phppro_journees.id_champ='$champ'    
			AND phppro_journees.numero='$x' ");
			//$result=mysql_query($query);
/*
    while( $row = (mysql_fetch_array($result)) )
        {
...
$result=$idconnect->query(" 
		SELECT sum(phppro_matchs.id_equipe_dom), 
			   sum(phppro_matchs.id_equipe_ext) 
		FROM phppro_matchs, phppro_journees 
		WHERE phppro_matchs.id_journee=phppro_journees.id 
		AND phppro_journees.id_champ='$champ'  ");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT   phppro_matchs.id_equipe_dom as DOM,  count(*) as ct 
		 FROM phppro_matchs, phppro_journees 
		 WHERE  phppro_journees.id_champ='$champ' 
		 AND phppro_matchs.id_journee=phppro_journees.id 
		 GROUP BY DOM");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT phppro_matchs.id_equipe_ext as DOM,  
		 count(*) as ct 
		 FROM phppro_matchs, phppro_journees 
		 WHERE  phppro_journees.id_champ='$champ' 
		 AND phppro_matchs.id_journee=phppro_journees.id 
