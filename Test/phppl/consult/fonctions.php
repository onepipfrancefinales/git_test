...
function nb_journees($id_champ, $idconnect)
         ...
         $reponse=$idconnect->query("SELECT id FROM phppl_equipes WHERE id_champ='$id_champ'");
...
// cellule d'affichage des derniers r�sultats
  $color=0;
        $reponse=$idconnect->query("SELECT cldom.nom as cldom, clext.nom as clext, phppl_matchs.buts_dom, phppl_matchs.buts_ext,
...
//mise en forme tableau oppositions
        ...
        $annee = 0;
             ...
             $extproba= $row[3];
                ...
                {
                $result2=$idconnect->query("SELECT DOMBUTSPOUR, DOMG, DOMN, DOMP, DOMBUTSCONTRE FROM phppl_clmnt_cache WHERE ID_EQUIPE='$row[eqdom]'");
...
//suppression des liens sur les scores
  ...
  $resultats=$idconnect->query("SELECT phppl_clubs.nom, CLEXT.nom, phppl_matchs.buts_dom, phppl_matchs.buts_ext,
            phppl_matchs.id, phppl_matchs.date_reelle
  ...
  {   
   ...
   $row[1]= stripslashes($row[1]);
    ...
    if ($row[0]=='exempte') {echo "<tr bgcolor=\"$bgcolor1\" class=\"trphppl\"><td colspan=\"7\">".ADMIN_RESULTS_1." : $row[1]</td></tr>";}
    if ($row[1]=='exempte') {echo "<tr bgcolor=\"$bgcolor1\" class=\"trphppl\"><td colspan=\"7\" >".ADMIN_RESULTS_1." : $row[0]</td></tr>";}
  }
...
{
  $nb_requete=0;
  if($cache=="1"){ mysql_query("DELETE FROM phppl_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysql_error());}
  else{ mysql_query("DELETE FROM phppl_clmnt") or die (mysql_error());}
          $nb_requete++;
...
$query="SELECT pts_victoire, pts_nul, pts_defaite FROM phppl_parametres WHERE id_champ='$champ'";
$nb_requete++;
...
// victoires domicile
$query="SELECT dom.id, count(dom.id), phppl_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phppl_equipes as dom, phppl_clubs, phppl_matchs, phppl_journees, phppl_championnats
...
// Resultats � domicile
$query="SELECT phppl_clubs.nom FROM phppl_clubs, phppl_equipes, phppl_championnats
...
// victoires exterieur
$query="SELECT ext.id, count(ext.id), phppl_clubs.nom, sum(buts_ext), sum(buts_dom) FROM phppl_equipes as ext, phppl_clubs, phppl_matchs, phppl_journees, phppl_championnats
...
$query="SELECT phppl_clubs.nom, phppl_equipes.penalite, phppl_equipes.id
...
//mysql_query("LOCK TABLE phppl_clmnt WRITE, phppl_equipes WRITE, phppl_clubs WRITE") or die (mysql_error());
    ...
    {
    $query="SELECT phppl_clubs.nom, phppl_equipes.id 
            FROM phppl_clubs, phppl_equipes, phppl_championnats
            WHERE phppl_equipes.id_champ=phppl_championnats.id
            AND phppl_championnats.id='$champ'
            AND phppl_equipes.id_club=phppl_clubs.id";
...
if($cache=="1"){$requete="DELETE FROM phppl_clmnt_cache WHERE nom='exempte'" or die (mysql_error());}
else{$requete="DELETE FROM phppl_clmnt WHERE nom='exempte'" or die (mysql_error());}
...
{
echo "<table class=\"tablephppl2\" align=\"center\" cellspacing=\"0\" width=\"95%\"><tr class=\"trphppl3\"><th colspan=\"11\">".$legende."</th></tr>\n";
echo "<tr class=\"trphppl3\">
...
function clmntmini_barre($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini,  $champmini, $requetemini, $nb_dessusmini, $nb_dessousmini, $lienmini, $phpplEAGUE_RACINE, $id_equipe_fetiche)
{
echo "<table class=tablephppl2 align=\"center\" cellspacing=\"0\" width=\"200\"><tr class=trphppl3><th colspan=10>".$legendemini;
echo "<tr class=trphppl3>";
echo "<th align=center>".CLMNT_POSITION."";
...
function clmntred($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini, $champmini, $requetemini, $lienmini, $phpplEAGUE_RACINE, $id_equipe_fetiche)
{
echo "<table class=tablephppl2 align=\"center\" cellspacing=\"0\"  width=\"200\"><tr class=trphppl3><th colspan=10>".$legendemini;
echo "<tr class=trphppl3>
...
function clmnt_barrered($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini,  $champmini, $requetemini, $lienmini, $phpplEAGUE_RACINE, $id_equipe_fetiche)
{
