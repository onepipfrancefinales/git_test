...
<table border="0" width="72%">
    <tr class="trphpfed23" valign="top"> 
      <td height="18"> 
...
function aff_journee($champ, $numero, $legende, $proba, $fiches_clubs, $id_equipe_fetiche)
        ...
        $color=0;
        $query1="SELECT cldom.nom as cldom, clext.nom as clext, phpfed2_matchs.buts_dom, phpfed2_matchs.buts_ext,
        ...
        echo "<table align=\"center\" class=\"tablephpfed22\" width=\"80%\" cellspacing=\"0\" cellpadding=\"0\"><tr><td><table cellspacing=\"0\" align=\"center\" width=\"100%\" cellpadding=\"1\">\n";
        $x=1;
        $minute = 0;
        $heure = 0;
        $jour = 0;
        ...
        $annee = 0;
             ...
             $extproba= $row[3];
                ...
                {
                $query2="SELECT DOMBUTSPOUR, DOMG, DOMN, DOMP, DOMBUTSCONTRE FROM phpfed2_clmnt_cache WHERE ID_EQUIPE='$row[eqdom]'";
                ...
                $query2="SELECT EXTG, EXTN, EXTP, EXTBUTSCONTRE, EXTBUTSPOUR FROM phpfed2_clmnt_cache WHERE ID_EQUIPE='$row[eqext]'";
                $result2=mysql_query($query2);
                      while ($row2=mysql_fetch_array($result2))
                      {
  ...
  $requete="SELECT phpfed2_clubs.nom, CLEXT.nom, phpfed2_matchs.buts_dom, phpfed2_matchs.buts_ext,
            phpfed2_matchs.id, phpfed2_matchs.date_reelle
  ...
  {   
   ...
   $row[1]= stripslashes($row[1]);
    ...
    if ($row[0]=='exempte') {echo "<tr bgcolor=\"$bgcolor1\" class=\"trphpfed2\"><td colspan=\"7\">".ADMIN_RESULTS_1." : $row[1]</td></tr>";}
    if ($row[1]=='exempte') {echo "<tr bgcolor=\"$bgcolor1\" class=\"trphpfed2\"><td colspan=\"7\" >".ADMIN_RESULTS_1." : $row[0]</td></tr>";}
  }
...
{
  $nb_requete=0;
  if($cache=="1"){ mysql_query("DELETE FROM phpfed2_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysql_error());}
  else{ mysql_query("DELETE FROM phpfed2_clmnt") or die (mysql_error());}
          $nb_requete++;
...
$query="SELECT pts_victoire, pts_nul, pts_defaite FROM phpfed2_parametres WHERE id_champ='$champ'";
$nb_requete++;
...
// victoires domicile
$query="SELECT dom.id, count(dom.id), phpfed2_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phpfed2_equipes as dom, phpfed2_clubs, phpfed2_matchs, phpfed2_journees, phpfed2_championnats
...
$query="SELECT phpfed2_clubs.nom FROM phpfed2_clubs, phpfed2_equipes, phpfed2_championnats
...
// victoires exterieur
$query="SELECT ext.id, count(ext.id), phpfed2_clubs.nom, sum(buts_ext), sum(buts_dom) FROM phpfed2_equipes as ext, phpfed2_clubs, phpfed2_matchs, phpfed2_journees, phpfed2_championnats
...
$query="SELECT phpfed2_clubs.nom, phpfed2_equipes.penalite, phpfed2_equipes.id
...
//mysql_query("LOCK TABLE phpfed2_clmnt WRITE, phpfed2_equipes WRITE, phpfed2_clubs WRITE") or die (mysql_error());
    ...
    {
    $query="SELECT phpfed2_clubs.nom, phpfed2_equipes.id 
            FROM phpfed2_clubs, phpfed2_equipes, phpfed2_championnats
            WHERE phpfed2_equipes.id_champ=phpfed2_championnats.id
            AND phpfed2_championnats.id='$champ'
            AND phpfed2_equipes.id_club=phpfed2_clubs.id";
...
if($cache=="1"){$requete="DELETE FROM phpfed2_clmnt_cache WHERE nom='exempte'" or die (mysql_error());}
else{$requete="DELETE FROM phpfed2_clmnt WHERE nom='exempte'" or die (mysql_error());}
...
{
echo "<table class=\"tablephpfed22\" align=\"center\" cellspacing=\"0\" width=\"100%\"><tr class=\"trphpfed23\"><th colspan=\"11\">".$legende."</th></tr>\n";
echo "<tr class=\"trphpfed23\">
...
{
echo "<table class=\"tablephpfed22\" align=\"center\" cellspacing=\"0\" width=\"60%\" bgcolor=\"#FFFFFF\"><tr class=\"trphpfed23\"><th colspan=\"11\">".$legende."<br /></th></tr>\n";
...
{
echo "<table class=\"tablephpfed22\" align=\"center\" cellspacing=\"0\" width=\"60%\"><tr class=\"trphpfed23\"><th colspan=\"11\">".$legende;
echo "</th></tr>\n<tr class=\"trphpfed23\">
<th align=\"center\">".CLMNT_POSITION."</th>
<th align=\"left\">".CLMNT_EQUIPE."</th>
<th align=\"left\">".CLMNT_POINTS."</th>\n";
...
function clmntmini_barre($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini,  $champmini, $requetemini, $nb_dessusmini, $nb_dessousmini, $lienmini, $phpfed2EAGUE_RACINE, $id_equipe_fetiche)
{
echo "<table class=tablephpfed22 align=\"center\" cellspacing=\"0\" width=\"200\"><tr class=trphpfed23><th colspan=10>".$legendemini;
echo "<tr class=trphpfed23>";
echo "<th align=center>".CLMNT_POSITION."";
...
function clmntred($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini, $champmini, $requetemini, $lienmini, $phpfed2EAGUE_RACINE, $id_equipe_fetiche)
{
echo "<table class=tablephpfed22 align=\"center\" cellspacing=\"0\"  width=\"200\"><tr class=trphpfed23><th colspan=10>".$legendemini;
echo "<tr class=trphpfed23>
               ...
               echo "<td>";
                    ...
                    if ($x==0)
                    {
                     ...
                     if ($lienmini=='non'){echo "$row[$x]";}
                      ...
                      {
                        echo "<a href=\"".$phpfed2EAGUE_RACINE."consult/detaileq.php?champ=$champmini&amp;id_equipe=".$row['ID_EQUIPE']."\" target=\"_parent\">$row[$x]</a>";
                      }
                     if ($row['ID_EQUIPE']==$id_equipe_fetiche){echo "</b>";} 
                     }
...
function clmnt_barrered($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini,  $champmini, $requetemini, $lienmini, $phpfed2EAGUE_RACINE, $id_equipe_fetiche)
{
