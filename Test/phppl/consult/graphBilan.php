...
$result=$idconnect->query("SELECT phppl_clubs.nom, phppl_equipes.id_club, phppl_equipes.id_champ FROM phppl_equipes, phppl_clubs where phppl_clubs.id=phppl_equipes.id_club AND phppl_equipes.id='$equipe'");
...
$result=$idconnect->query("SELECT phppl_equipes.id FROM phppl_equipes, phppl_clubs WHERE id_champ='$champ' AND phppl_equipes.id_club=phppl_clubs.id AND phppl_clubs.nom!='exempte'");
//$result=mysql_query($query);
$nb_equipes=mysqli_num_rows($result)+1;

// on détermine la dernière journee jouée
$result=$idconnect->query("SELECT max(phppl_journees.numero) FROM phppl_journees, phppl_matchs where phppl_journees.id=phppl_matchs.id_journee and buts_dom is not NULL AND phppl_journees.id_champ='$champ'");
//$result=mysql_query($query);
while ($row=mysqli_fetch_array($result)){$fin=$row[0];}

// On détermine le nombre de journée total
$result=$idconnect->query("SELECT id FROM phppl_journees WHERE id_champ='$champ'");
//$result=mysql_query($query);
$nb_journees=mysqli_num_rows($result);

// On déterrmine l'année et le nom de la division
$result=$idconnect->query("SELECT phppl_saisons.annee, phppl_divisions.nom FROM phppl_saisons, phppl_divisions, phppl_championnats where phppl_championnats.id=$champ AND phppl_divisions.id=phppl_championnats.id_division and phppl_championnats.id_saison=phppl_saisons.id");
//$result=mysql_query($query);
...
$requete_sql = "SELECT classement FROM phppl_clmnt_graph WHERE id_equipe='$equipe' ORDER BY fin";
