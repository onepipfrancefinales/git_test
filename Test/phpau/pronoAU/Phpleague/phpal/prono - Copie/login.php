...
echo $autoidentification;
	...
	else
	{
        ...
        $pass = $_REQUEST['pass'];
        $result =$idconnect->query( "SELECT mot_de_passe FROM phpal_membres WHERE pseudo='$user'");
	...
	$time = time();
	$result =$idconnect->query( "SELECT * FROM phpal_membres WHERE pseudo='$user'");
	...
	mysqli_query( $idconnect,("UPDATE phpal_membres SET ip='$ip', last_connect='$time' WHERE pseudo='$user'"));
