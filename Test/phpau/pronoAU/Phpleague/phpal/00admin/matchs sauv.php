...
if (empty ($numero))
  {
    $resultats=$idconnect->query("SELECT max(phpal_journees.numero) 
...
for ($counter=$nb_rencontres; $counter>0 ; $counter=$counter-1 ) // Nb de rencontres dans la journée
        ...
        $result =$idconnect( "SELECT DISTINCT phpal_clubs.nom, phpal_equipes.id FROM phpal_clubs, phpal_equipes
        ...
        $result =$idconnect->query( "SELECT DISTINCT phpal_clubs.nom, phpal_equipes.id 
...
//JOURNEE MIROIR ?
		...
		$miroir="none";
                ...
                $result =$idconnect->query( "SELECT numero, id FROM phpal_journees WHERE id_champ='$champ'  ORDER BY phpal_journees.numero");
               // $result=mysql_query($query);
                while ($row = mysqli_fetch_array($result)) {
...
// CHECK SUM DU CHAMPIONNAT
  $result=$idconnect->query(" SELECT sum(id) FROM phpal_equipes WHERE id_champ='$champ' ");
...
while($x<=$nb_journees)
    {
$result=$idconnect->query(" SELECT sum(phpal_matchs.id_equipe_dom), sum(phpal_matchs.id_equipe_ext) FROM phpal_matchs, phpal_journees WHERE phpal_matchs.id_journee=phpal_journees.id AND phpal_journees.id_champ='$champ'   AND phpal_journees.numero='$x' ");
//$result=mysql_query($query);
$sum=mysqli_fetch_array($result);
$sum_day=$sum[0]+$sum[1];
echo "<small>MATCH_SUM = ".$sum_day."  </small>";
    $query=$idconnect->query("SELECT count(DISTINCT phpal_matchs.id_equipe_dom), count(DISTINCT phpal_matchs.id_equipe_ext) FROM phpal_matchs, phpal_journees   WHERE phpal_matchs.id_journee=phpal_journees.id    AND phpal_journees.id_champ='$champ'    AND phpal_journees.numero='$x' ");
   // $result=mysql_query($query);
...
$result=$idconnect->query(" SELECT sum(phpal_matchs.id_equipe_dom), sum(phpal_matchs.id_equipe_ext) FROM phpal_matchs, phpal_journees WHERE phpal_matchs.id_journee=phpal_journees.id AND phpal_journees.id_champ='$champ'  ");
//$result=mysql_query($query);
$sum=mysqli_fetch_array($result);
$sumsum=$sum[0];
 $result=$idconnect->query("SELECT sum(phpal_matchs.id_equipe_dom), sum(phpal_matchs.id_equipe_ext) FROM phpal_matchs, phpal_journees WHERE  phpal_journees.id_champ='$champ' AND phpal_matchs.id_journee=phpal_journees.id  ");
//$result=mysql_query($query);
while($row=mysqli_fetch_array($result))
    {
 ...
 $result=$idconnect->query("SELECT   phpal_matchs.id_equipe_dom as DOM,  count(*) as ct FROM phpal_matchs, phpal_journees WHERE  phpal_journees.id_champ='$champ' AND phpal_matchs.id_journee=phpal_journees.id GROUP BY DOM");
//$result=mysql_query($query); 
while($row=mysqli_fetch_array($result))
		{
 ...
 $result=$idconnect->query("SELECT phpal_matchs.id_equipe_ext as DOM,  count(*) as ct FROM phpal_matchs, phpal_journees WHERE  phpal_journees.id_champ='$champ' AND phpal_matchs.id_journee=phpal_journees.id GROUP BY DOM");
//$result=mysql_query($query); 
while($row=mysql_fetch_array($result))
		{
		if ($row[1]<>$nb_journees/2)
