    ...
    while ($row = mysqli_fetch_array($resultats))
      {
        ...
        if ($champ=="$row[0]") {$class="phpab7";}
        elseif (($i%2)==0) {$class="phpab3";}
        else {$class="phpab4";}
    ...
    while ($row = mysqli_fetch_array($resultats))
      {
        ...
        if ($gr_champ=="$row[0]") {$class="phpab7";}
        elseif (($i%2)==0) {$class="phpab3";}
        else {$class="phpab4";}
...
{
  $result=$idconnect->query(" SELECT phpab_championnats.id, phpab_divisions.nom, phpab_saisons.annee, (phpab_saisons.annee)+1 
							  FROM phpab_championnats, phpab_divisions, phpab_saisons 
...
function champ_gr_menu($gr_champ, $idconnect)
{
  ...
  $result=$idconnect->query(" SELECT id_champ 
    ...
    while ($row=mysqli_fetch_array($result))
    {
         $result2 = "SELECT phpab_championnats.id, phpab_divisions.nom, phpab_saisons.annee, (phpab_saisons.annee)+1 
					 FROM phpab_championnats, phpab_divisions, phpab_saisons 
					 WHERE phpab_divisions.id=phpab_championnats.id_division 
					 AND phpab_saisons.id=phpab_championnats.id_saison 
...
if($cache=="1")
{
$req = $bdd->prepare('INSERT INTO phpab_clmnt_cache(NOM, POINTS, JOUES, G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN, DOMPOINTS, DOMJOUES, DOMG, DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, EXTPOINTS, EXTJOUES, EXTG, EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, ID_EQUIPE, ID_CHAMP) 
...
{
$req = $bdd->prepare('INSERT INTO phpab_clmnt(NOM, POINTS, JOUES, G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN, DOMPOINTS, DOMJOUES, DOMG, DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, EXTPOINTS, EXTJOUES, EXTG, EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, ID_EQUIPE, ID_CHAMP) 
									  VALUES(:NOM,:POINTS,:JOUES,:G,:N,:P,:BUTSPOUR,:BUTSCONTRE,:DIFF,:PEN,:DOMPOINTS,:DOMJOUES,:DOMG,:DOMN,:DOMP,:DOMBUTSPOUR,:DOMBUTSCONTRE,:DOMDIFF,:EXTPOINTS,:EXTJOUES,:EXTG,:EXTN,:EXTP,:EXTBUTSPOUR,:EXTBUTSCONTRE,:EXTDIFF,:ID_EQUIPE,:ID_CHAMP)');
$req->execute(array(
	'NOM' => $NOM, 
	'POINTS' => $POINTS, 
...
if($cache=="1")
{$requete="DELETE FROM phpab_clmnt_cache WHERE nom='exempte'" or die (mysql_error($idconnect));}
else
{$requete="DELETE FROM phpab_clmnt WHERE nom='exempte'" or die (mysql_error($idconnect));}
