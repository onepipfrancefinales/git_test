...
$result=$idconnect->query("SELECT phpau_clubs.nom, phpau_equipes.id_club, phpau_equipes.id_champ FROM phpau_equipes, phpau_clubs where phpau_clubs.id=phpau_equipes.id_club AND phpau_equipes.id='$equipe'");
...
$result=$idconnect->query("SELECT phpau_equipes.id FROM phpau_equipes, phpau_clubs WHERE id_champ='$champ' AND phpau_equipes.id_club=phpau_clubs.id AND phpau_clubs.nom!='exempte'");
//$result=mysql_query($query);
$nb_equipes=mysqli_num_rows($result)+1;

// on détermine la dernière journee jouée
$result=$idconnect->query("SELECT max(phpau_journees.numero) FROM phpau_journees, phpau_matchs where phpau_journees.id=phpau_matchs.id_journee and buts_dom is not NULL AND phpau_journees.id_champ='$champ'");
//$result=mysql_query($query);
while ($row=mysqli_fetch_array($result)){$fin=$row[0];}

// On détermine le nombre de journée total
$result=$idconnect->query("SELECT id FROM phpau_journees WHERE id_champ='$champ'");
//$result=mysql_query($query);
$nb_journees=mysqli_num_rows($result);

// On déterrmine l'année et le nom de la division
$result=$idconnect->query("SELECT phpau_saisons.annee, phpau_divisions.nom FROM phpau_saisons, phpau_divisions, phpau_championnats where phpau_championnats.id=$champ AND phpau_divisions.id=phpau_championnats.id_division and phpau_championnats.id_saison=phpau_saisons.id");
//$result=mysql_query($query);
