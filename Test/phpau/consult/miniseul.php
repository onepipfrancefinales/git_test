...
//$result=mysql_query("SELECT * FROM phpau_parametres WHERE id_champ='$champmini'");
...
// }
//   $requete = "SELECT * FROM phpau_equipes, phpau_clubs WHERE phpau_clubs.id=phpau_equipes.id_club AND id_champ='$champmini' AND phpau_clubs.nom='exempte'";
...
case DOMICILE;
        {
        $requetemini=$idconnect->query("SELECT NOM, DOMPOINTS, DOMJOUES, DOMG,  DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, ID_EQUIPE FROM phpau_clmnt_cache WHERE ID_CHAMP='$champmini' 
...
case ATTAQUE;
        {
        $requetemini=$idconnect->query("SELECT * FROM phpau_clmnt_cache WHERE ID_CHAMP='$champmini' 
...
case DEFENSE;
        ...
        $requetemini=$idconnect->query("SELECT * FROM phpau_clmnt_cache WHERE ID_CHAMP='$champmini' ORDER BY BUTSCONTRE ASC, DIFF DESC");
        if ($presentationmini=="1")
        {
          if ($classmini=='1')
        {
...
case EXTERIEUR;
        {
         $requetemini=$idconnect->query("SELECT NOM, EXTPOINTS, EXTJOUES, EXTG,  EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, ID_EQUIPE FROM phpau_clmnt_cache WHERE ID_CHAMP='$champmini' 
