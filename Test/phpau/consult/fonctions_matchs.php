...
{
  global $nb_journees;
  $result = $idconnect->query("SELECT id FROM phpau_equipes WHERE id_champ='$id_champ'");
...
function aff_journee($champ, $bdd)
{
  ...
  //echo $numero;
  // echo "</br>";
  $result = $bdd->query("SELECT cldom.nom as cldom, clext.nom as clext, phpau_matchs.buts_dom, phpau_matchs.buts_ext,
  ...
  echo "<br>";
  echo "<table class=\"tablephpau2 marginAuto width90PC \" ><tr><td>
...
function aff_journeeSupp($champ, $bdd)
{
 ...
 $result=$bdd->query("SELECT cldom.nom as cldom, clext.nom as clext, phpau_matchs.buts_dom, phpau_matchs.buts_ext,
                ...
                WHERE phpau_matchs.id_equipe_dom=dom.id
                        AND phpau_matchs.id_equipe_ext=ext.id
                        AND phpau_journees.id_champ='$champ'
                        AND phpau_journees.numero='$journeeSupp'
                        AND dom.id_club=cldom.id
                        AND ext.id_club=clext.id
                        AND phpau_matchs.id_journee=phpau_journees.id
                        AND cldom.nom!='exempte'
...
//mise en forme tableau oppositions
echo "<br>";
echo "<table class=\"marginAuto tablephpau2 width90PC \" ><tr>
...
function classementDetaille($champ, $bdd)
{
  ...
  echo "<table class=\"tablephpau2\" cellspacing=\"0\" width=\"80%\"><tr class=\"trphpau3\"><th colspan=\"14\">" . $legende . "</th></tr>\n";
  echo "<tr class=\"trphpau3\">
  ...
  $res = $bdd->query("SELECT NOM, POINTS, JOUES, G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN, PTS_ADMIN, JOURPERE
					  FROM phpau_clmnt_cache 
					  WHERE ID_CHAMP='$champ' 
					  ORDER BY POINTS DESC, PEN DESC, DIFF DESC, BUTSPOUR DESC , BUTSCONTRE 
					  ASC, NOM");

  //echo "<table class=\"tablephpau2\" align=\"center\" cellspacing=\"0\" width=\"80%\"><tr class=\"trphpau3\"><th colspan=\"11\">".$legende."</th></tr>\n";
  //echo "<tr class=\"trphpau3\>";
...
{
  $resultat = $bdd->query("
		  SELECT phpau_clubs.nom FROM phpau_clubs, phpau_equipes 
		  WHERE phpau_equipes.id='$id_equipe' 
  ...
  echo "<table class=\"tablephpau2\" align=\"center\" cellspacing=\"0\"  width=\"90%\">";

  $resultat = $bdd->query("
		SELECT phpau_journees.numero, cldom.nom, clext.nom, phpau_matchs.buts_dom, phpau_matchs.buts_ext, phpau_matchs.date_reelle, phpau_matchs.id
        FROM phpau_equipes as dom, phpau_equipes as ext, phpau_matchs, phpau_journees, phpau_clubs as cldom  , phpau_clubs as clext
        WHERE phpau_matchs.id_equipe_dom=dom.id
        AND phpau_matchs.id_equipe_ext=ext.id
        AND (phpau_matchs.id_equipe_ext='$id_equipe'
        OR phpau_matchs.id_equipe_dom='$id_equipe')
        AND phpau_journees.id_champ='$champ'
        AND dom.id_club=cldom.id
        AND ext.id_club=clext.id
        AND phpau_matchs.id_journee=phpau_journees.id
  ...
  echo "<br>";
  if ($smart == false)
    echo "<tr class=\"trphpau3\"><td align=\"center\"><font color=\"#FFFF00\"><b>" . JOURNEE . "</b></font></td><td width=\"5% align=\"center\"></td><td width=\"30%\" ><font color=\"#FFFF00\"><b>" . DATE . "</b></font></td><td colspan=\"5\ align=\"center\"><font color=\"#FFFF00\"><b>Oppositions</b></font></td></tr>";
  else
    echo "<tr class=\"trphpau3\"><td align=\"center\"><font color=\"#FFFF00\"><b>" . JOURNEE . "</b></font></td><td width=\"5% align=\"center\"></td><td colspan=\"5\ align=\"center\"><font color=\"#FFFF00\"><b>Oppositions</b></font></td></tr>";
...
function calendrier($champ, $smart, $bdd)
{
  ...
  $resultats2 = $bdd->query("SELECT phpau_clubs.nom, CLEXT.nom, phpau_matchs.buts_dom, phpau_matchs.buts_ext, phpau_matchs.id, phpau_matchs.date_reelle
  ...
  $result = $bdd->query("SELECT phpau_journees.numero, phpau_journees.date_prevue, cldom.nom, clext.nom, phpau_matchs.buts_dom, phpau_matchs.buts_ext, dom.id, ext.id
  ...
  while ($row = $result->fetch()) {
    ...
    ?>
      <br>
      <table class="tablephpau2" cellspacing="0" width="290" align="center">
        <tr class="trphpau3">
          <td colspan="3" align="center"><b>
              ...
              function db_clmnt($champ, $debut, $fin, $cache, $idconnect)
              {
                ...
                if ($cache == "1") {
                  mysqli_query("DELETE FROM phpau_clmnt_cache WHERE ID_CHAMP='$champ'") or die(mysqli_error());
                } else {
                  mysqli_query("DELETE FROM phpau_clmnt") or die(mysqli_error());
                }
                $nb_requete++;
                ...
                $resultats = $idconnect->query = ("SELECT pts_victoire, pts_nul, pts_defaite FROM phpau_parametres WHERE id_champ='$champ'");
                $nb_requete++;
                ...
                // victoires domicile
                $resultats = $idconnect->query("SELECT dom.id, count(dom.id), phpau_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phpau_equipes as dom, phpau_clubs, phpau_matchs, phpau_journees, phpau_championnats
WHERE dom.id_champ='$champ'
      ...
      GROUP by phpau_clubs.nom ");
                ...
                // Defaites domicile
                $dom = $idconnect->query("SELECT dom.id, count(dom.id), phpau_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phpau_equipes as dom, phpau_clubs, phpau_matchs, phpau_journees, phpau_championnats
WHERE dom.id_champ='$champ'
      AND dom.id_club=phpau_clubs.id
      AND dom.id=phpau_matchs.id_equipe_dom
      AND buts_dom < buts_ext
...
WHERE dom.id_champ='$champ'
      ...
      GROUP by phpau_clubs.nom ");
                ...
                $result = $idconnect->query("SELECT phpau_clubs.nom FROM phpau_clubs, phpau_equipes, phpau_championnats
...
WHERE ext.id_champ='$champ'
      ...
      GROUP by phpau_clubs.nom ");
        ...
        AND phpau_equipes.id_club=phpau_clubs.id");
                ...
                if (mysqli_num_rows($result) == 0) {
                  $result = "SELECT phpau_clubs.nom, phpau_equipes.id 
            FROM phpau_clubs, phpau_equipes, phpau_championnats
            WHERE phpau_equipes.id_champ=phpau_championnats.id
            AND phpau_championnats.id='$champ'
            AND phpau_equipes.id_club=phpau_clubs.id";
                ...
                while ($row = mysqli_fetch_array($result)) {
                  ...
                  if ($cache == "1") {
                    $question .= "phpau_clmnt_cache ";
                  } else {
                    $question .= "phpau_clmnt ";
                  }
          ...
          PEN='$PEN'";
                ...
                if ($cache == "1") {
                  mysqli_query($idconnect, ("DELETE FROM phpau_clmnt_cache WHERE nom='exempte'"));
                } else {
                  mysqli_query($idconnect, ("DELETE FROM phpau_clmnt WHERE nom='exempte'"));
                }
              ...
              function clmnt($champ, $smart, $bdd)
              {
                ...
                echo "<table class=\"tablephpau2 marginAuto width80PC borderSpacing\" ><tr class=\"trphpau3\"><th colspan=\"4\">" . $legende . "</th></tr>\n";
                echo "<tr class=\"trphpau3\">
  ...
  <th class=\"alignLeft\">" . CLMNT_JOUES . "</th>
              ...
              //               statistiques, am�lioration de la gestion des buteurs pour phpaueague.          //
              // Auteur      : Alexis MANGIN                                                                 //
              // Email       : Alexis@univert.org                                                            //
              // Url         : http://www.univert.org                                                        //
              // D�mo        : http://phpaueague.univert.org/demo.php                                         //
                ...
                <form method="get" action="">
                  <div align="center">
                    ...
                    <select name="champ">
                      ...
                      <?php
                      $query = "SELECT phpau_divisions.nom, phpau_saisons.annee, phpau_championnats.id
                 ...
                 ORDER BY phpau_saisons.annee DESC, phpau_championnats.id";
