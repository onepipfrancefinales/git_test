...
if ($action3=="2")
 {
 $requete="SELECT id FROM phpab_gr_championnats WHERE id='$gr_champ'";
 ...
 $nb_resultats=mysql_num_rows($resultats);
   ...
   if ($nb_resultats>0)
     {
       //Màj des paramètres
       mysql_query ("UPDATE phpab_gr_championnats SET pts_prono_exact='$pts_prono_exact', pts_prono_participation='$pts_prono_participation', id_master='$id_master', tps_avant_prono='$tps_avant_prono', activ_prono='$activ_prono' WHERE id='$gr_champ'") or die (mysql_error());
     }

   else
     {
       // Insertion des paramètres dans la bdd
       $requete="INSERT INTO phpab_gr_championnats (id_champ, pts_prono_exact, pts_prono_participation, id_master, tps_avant_prono) VALUES ('$champ', '$pts_prono_exact', '$pts_prono_participation', '$id_master', '$tps_avant_prono')";
...
{
  $requete = "SELECT * FROM phpab_gr_championnats WHERE id='$gr_champ'";
  ...
  if (isset($row["tps_avant_prono"])){$tps_avant_prono = $row["tps_avant_prono"];} else {$tps_avant_prono=0;}
        ...
        mysql_query ("DELETE FROM phpab_gr_championnats WHERE id = '$gr_champ'") or die ("probleme " .mysql_error());

         reset ($champ);
	 while ( list($cle, $val)= each($champ))
         {
mysql_query ("INSERT INTO phpab_gr_championnats (id, nom, id_champ, activ_prono, pts_prono_exact, pts_prono_participation, id_master,tps_avant_prono)
...
<table class=phpab width="80%">
            <tr>
              <td class=phpab2 align="center" colspan="3"><?php echo ADMIN_PARAM_MSG13." "; affich_gr_champ ($gr_champ, $idconnect); ?></td>
            </tr>
            <tr>
...
$query2="SELECT pseudo, id FROM phpab_membres order by pseudo";
