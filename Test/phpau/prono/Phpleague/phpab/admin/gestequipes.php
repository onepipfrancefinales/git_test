...
elseif ($go<>"1")
{
       ...
       if ($nb2!=$nb_rens)
       {
         mysql_query ("DELETE FROM phpab_donnee WHERE id_clubs=$id");
         $query="SELECT id FROM phpab_rens";
         $result=mysql_query($query);
         while($row=mysql_fetch_array($result))
         {
         mysql_query ("INSERT INTO phpab_donnee (id_clubs, id_rens) VALUES ('$id', '$row[0]')") or die ("probleme " .mysql_error());
         }
         }
       
       echo "<h3>";
       $query="SELECT id, nom FROM phpab_clubs WHERE id='$id'";
       $result = mysql_query ($query);
