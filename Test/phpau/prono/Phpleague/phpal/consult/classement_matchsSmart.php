...
if (isset($type))
{
   ...
   $res=$idconnect->query("SELECT accession, barrage, estimation, relegation, id_equipe_fetiche, fiches_clubs
    ...
    $res2=$idconnect->query("SELECT phpal_equipes.id FROM phpal_equipes, phpal_clubs
...
if ($debut=="1" and $fin==$nb_journees)
{
   ...
   $res3=$idconnect->query("SELECT max(phpal_journees.numero) FROM phpal_journees, phpal_matchs WHERE phpal_journees.id=phpal_matchs.id_journee and buts_dom is not NULL and     phpal_journees.id_champ='$champ'");
