...
/************* mise en commentaire suite passage php7 ******************
  $requete=$idconnect->query("SELECT phpal_clubs.nom, CLEXT.nom, phpal_matchs.buts_dom, phpal_matchs.buts_ext,
...
{
  $nb_requete=0;
  if($cache=="1"){ mysqli_query("DELETE FROM phpal_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysqli_error());}
  else{ mysqli_query("DELETE FROM phpal_clmnt") or die (mysqli_error());}
          $nb_requete++;
...
$result=$idconnect->query("SELECT pts_victoire, pts_nul, pts_defaite FROM phpal_parametres WHERE id_champ='$champ'");
$nb_requete++;
...
$query=$idconnect->query("SELECT phpal_clubs.nom FROM phpal_clubs, phpal_equipes, phpal_championnats
