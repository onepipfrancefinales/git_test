...
function aff_journee($champ, $numero, $legende, $proba, $fiches_clubs, $id_equipe_fetiche)
        ...
        $color=0;
        $query1="SELECT cldom.nom as cldom, clext.nom as clext, phpal_matchs.buts_dom, phpal_matchs.buts_ext,
        ...
        echo "<table align=\"center\" class=\"tablephpal2\" width=\"80%\" cellspacing=\"0\" cellpadding=\"0\"><tr><td><table cellspacing=\"0\" align=\"center\" width=\"100%\" cellpadding=\"1\">\n";
        $x=1;
        $minute = 0;
        $heure = 0;
        $jour = 0;
        ...
        $annee = 0;
             ...
             $extproba= $row[3];
                ...
                {
                $query2="SELECT DOMBUTSPOUR, DOMG, DOMN, DOMP, DOMBUTSCONTRE FROM phpal_clmnt_cache WHERE ID_EQUIPE='$row[eqdom]'";
                ...
                $query2="SELECT EXTG, EXTN, EXTP, EXTBUTSCONTRE, EXTBUTSPOUR FROM phpal_clmnt_cache WHERE ID_EQUIPE='$row[eqext]'";
                $result2=mysql_query($query2);
                      while ($row2=mysql_fetch_array($result2))
                      {
  ...
  $requete="SELECT phpal_clubs.nom, CLEXT.nom, phpal_matchs.buts_dom, phpal_matchs.buts_ext,
...
{
  $nb_requete=0;
  if($cache=="1"){ mysql_query("DELETE FROM phpal_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysql_error());}
  else{ mysql_query("DELETE FROM phpal_clmnt") or die (mysql_error());}
          $nb_requete++;
...
$query="SELECT pts_victoire, pts_nul, pts_defaite FROM phpal_parametres WHERE id_champ='$champ'";
$nb_requete++;
...
// victoires domicile
$query="SELECT dom.id, count(dom.id), phpal_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phpal_equipes as dom, phpal_clubs, phpal_matchs, phpal_journees, phpal_championnats
...
$query="SELECT phpal_clubs.nom FROM phpal_clubs, phpal_equipes, phpal_championnats
...
// victoires exterieur
$query="SELECT ext.id, count(ext.id), phpal_clubs.nom, sum(buts_ext), sum(buts_dom) FROM phpal_equipes as ext, phpal_clubs, phpal_matchs, phpal_journees, phpal_championnats
...
$query="SELECT phpal_clubs.nom, phpal_equipes.penalite, phpal_equipes.id
...
//mysql_query("LOCK TABLE phpal_clmnt WRITE, phpal_equipes WRITE, phpal_clubs WRITE") or die (mysql_error());
    ...
    {
    $query="SELECT phpal_clubs.nom, phpal_equipes.id 
            FROM phpal_clubs, phpal_equipes, phpal_championnats
            WHERE phpal_equipes.id_champ=phpal_championnats.id
            AND phpal_championnats.id='$champ'
            AND phpal_equipes.id_club=phpal_clubs.id";
...
if($cache=="1"){$requete="DELETE FROM phpal_clmnt_cache WHERE nom='exempte'" or die (mysql_error());}
else{$requete="DELETE FROM phpal_clmnt WHERE nom='exempte'" or die (mysql_error());}
...
{
echo "<table class=\"tablephpal2\" align=\"center\" cellspacing=\"0\" width=\"100%\"><tr class=\"trphpal3\"><th colspan=\"11\">".$legende."</th></tr>\n";
echo "<tr class=\"trphpal3\">
...
{
echo "<table class=\"tablephpal2\" align=\"center\" cellspacing=\"0\" width=\"60%\" bgcolor=\"#FFFFFF\"><tr class=\"trphpal3\"><th colspan=\"11\">".$legende."<br /></th></tr>\n";
...
//               statistiques, amélioration de la gestion des buteurs pour phpaleague.          //
// Auteur      : Alexis MANGIN                                                                 //
// Email       : Alexis@univert.org                                                            //
// Url         : http://www.univert.org                                                        //
// Démo        : http://phpaleague.univert.org/demo.php                                         //
...
{
echo "<table class=\"tablephpal2\" align=\"center\" cellspacing=\"0\" width=\"60%\"><tr class=\"trphpal3\"><th colspan=\"11\">".$legende;
echo "</th></tr>\n<tr class=\"trphpal3\">
<th align=\"center\">".CLMNT_POSITION."</th>
<th align=\"left\">".CLMNT_EQUIPE."</th>
<th align=\"left\">".CLMNT_POINTS."</th>\n";
