...
$result=$idconnect->query("SELECT phpal_clubs.nom, phpal_equipes.id_club, phpal_equipes.id_champ FROM phpal_equipes, phpal_clubs where phpal_clubs.id=phpal_equipes.id_club AND phpal_equipes.id='$equipe'");
...
$result=$idconnect->query("SELECT phpal_equipes.id FROM phpal_equipes, phpal_clubs WHERE id_champ='$champ' AND phpal_equipes.id_club=phpal_clubs.id AND phpal_clubs.nom!='exempte'");
//$result=mysql_query($query);
$nb_equipes=mysqli_num_rows($result)+1;

// on détermine la dernière journee jouée
$result=$idconnect->query("SELECT max(phpal_journees.numero) FROM phpal_journees, phpal_matchs where phpal_journees.id=phpal_matchs.id_journee and buts_dom is not NULL AND phpal_journees.id_champ='$champ'");
//$result=mysql_query($query);
while ($row=mysqli_fetch_array($result)){$fin=$row[0];}

// On détermine le nombre de journée total
$result=$idconnect->query("SELECT id FROM phpal_journees WHERE id_champ='$champ'");
//$result=mysql_query($query);
$nb_journees=mysqli_num_rows($result);

// On déterrmine l'année et le nom de la division
$result=$idconnect->query("SELECT phpal_saisons.annee, phpal_divisions.nom FROM phpal_saisons, phpal_divisions, phpal_championnats where phpal_championnats.id=$champ AND phpal_divisions.id=phpal_championnats.id_division and phpal_championnats.id_saison=phpal_saisons.id");
//$result=mysql_query($query);
...
$requete_sql = "SELECT classement FROM phpal_clmnt_graph WHERE id_equipe='$equipe' ORDER BY fin";
