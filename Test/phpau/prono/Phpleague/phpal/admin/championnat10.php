...
{
  mysql_query ("INSERT INTO phpal_saisons (annee) values ('$saison')") or die ("probleme " .mysql_error());
}

if ($action2=="creer" and $division and $action=="creer")
{
  mysql_query ("INSERT INTO phpal_divisions (nom) values ('$division')") or die ("probleme " .mysql_error());
}

if ($action3=="creer" and $action=="creer")
{
  mysql_query ("INSERT INTO phpal_championnats (id_division, id_saison) values ('$division','$saison')") or die ("probleme " .mysql_error());
}


if ($action2=="supp" and $saison and $action=="creer")
{
  mysql_query ("DELETE FROM phpal_saisons WHERE id ='$saison'") or die ("probleme " .mysql_error());
}

if ($action2=="supp" and $division and $action=="creer")
{
  mysql_query ("DELETE FROM phpal_divisions WHERE id ='$division'") or die ("probleme " .mysql_error());
}

if ($confirm=="ok" and $champ and $action=="supp")
{
  $requete="SELECT phpal_equipes.id 
  			FROM phpal_equipes, phpal_championnats 
  			WHERE id_champ='$champ'";
  $resultats=mysql_query($requete);
     while($row = mysql_fetch_array($resultats))
      {
        mysql_query ("DELETE FROM phpal_matchs where id_equipe_dom='$row[0]' ") or die ("probleme" .mysql_error());
        mysql_query ("DELETE FROM phpal_matchs where id_equipe_ext='$row[0]' ") or die ("probleme" .mysql_error());
      }
    mysql_query("DELETE FROM phpal_championnats where id='$champ' ") or die ("probleme " .mysql_error());
    mysql_query("DELETE FROM phpal_journees WHERE  id_champ = '$champ'")or die ("probleme " .mysql_error());
    mysql_query("DELETE FROM phpal_equipes WHERE id_champ ='$champ' ")or die ("probleme " .mysql_error());
    mysql_query("DELETE FROM phpal_parametres WHERE id_champ ='$champ' ")or die ("probleme " .mysql_error());
...
<table class=phpal width="90%">
    ...
    <td class=phpal2 width="46%"></td>
            ...
            <?php affich_championnats10 ($champ, $action, $idconnect); ?>
