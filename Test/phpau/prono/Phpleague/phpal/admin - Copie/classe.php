...
switch($go)
{
	 ...
	 $nb_classe=nb_classe($data, $idconnect);
	   if($nb_classe==0)
	  {
    mysqli_query($idconnect, ("DELETE FROM phpal_classe WHERE id='$data' ")) ;
    ?>
    <table class=phpal width="80%">
	 ...
	 mysqli_query($idconnect, ("INSERT INTO phpal_classe (nom) values ('$data1')"));
	 ...
	 while ( list ($cle, $val)= each ($rang) and list ($cle, $val2)= each ($id))
         {
                ...
                //while($x <= $nb_classe2)
               // {
               mysqli_query ($idconnect, ("UPDATE phpal_classe SET rang='$val' WHERE id='$val2'"));
               //$x++;
                }
                ?>
    <table class=phpal width="80%">
...
$result = $idconnect->query("SELECT id, nom FROM phpal_classe ORDER BY nom");
