...
/*
   $result=$idconnect->query("SELECT id_membre, pseudo, SUM(points) , SUM(participation) as participations
                              ...
                              AND id_membre=phppro_membres.id
                              AND phppro_matchs.id=id_match
                              AND DATE_ADD( phppro_matchs.date_reelle, INTERVAL 7 DAY) >= NOW()
                              GROUP by pseudo
...
*/
 ...
 $result=$idconnect->query("SELECT id_membre, pseudo, SUM(points) , SUM(participation) as participations
                              ...
                              WHERE phppro_pronostics.id_membre=phppro_membres.id
                               AND phppro_pronostics.id_champ ='$gr_champ'
                              AND phppro_matchs.id=id_match
                              AND DATE_ADD( phppro_matchs.date_reelle, INTERVAL 7 DAY) >= NOW()
                              GROUP by pseudo
                              ORDER by  participations");
...
// echo $maxChampLigue; echo "<br>";
 $resultat=$idconnect->query("SELECT DISTINCT id, nom
                              FROM phppro_gr_championnats 
                              WHERE  phppro_gr_championnats.activ_prono='1' 
