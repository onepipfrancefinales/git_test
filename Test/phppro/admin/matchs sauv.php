...
for ($counter=$nb_rencontres; $counter>0 ; $counter=$counter-1 ) // Nb de rencontres dans la journée
        ...
        $result =$idconnect( "SELECT DISTINCT phpab_clubs.nom, phpab_equipes.id FROM phpab_clubs, phpab_equipes
        ...
        $result =$idconnect->query( "SELECT DISTINCT phpab_clubs.nom, phpab_equipes.id 
...
//JOURNEE MIROIR ?
		...
		$miroir="none";
                ...
                $result =$idconnect->query( "SELECT numero, id FROM phpab_journees WHERE id_champ='$champ'  ORDER BY phpab_journees.numero");
               // $result=mysql_query($query);
                while ($row = mysqli_fetch_array($result)) {
...
// CHECK SUM DU CHAMPIONNAT
  $result=$idconnect->query(" SELECT sum(id) FROM phpab_equipes WHERE id_champ='$champ' ");
...
echo "<small>MATCH_SUM = ".$sum_day."  </small>";
    $query=$idconnect->query("SELECT count(DISTINCT phpab_matchs.id_equipe_dom), count(DISTINCT phpab_matchs.id_equipe_ext) FROM phpab_matchs, phpab_journees   WHERE phpab_matchs.id_journee=phpab_journees.id    AND phpab_journees.id_champ='$champ'    AND phpab_journees.numero='$x' ");
   // $result=mysql_query($query);
...
$result=$idconnect->query(" SELECT sum(phpab_matchs.id_equipe_dom), sum(phpab_matchs.id_equipe_ext) FROM phpab_matchs, phpab_journees WHERE phpab_matchs.id_journee=phpab_journees.id AND phpab_journees.id_champ='$champ'  ");
//$result=mysql_query($query);
$sum=mysqli_fetch_array($result);
$sumsum=$sum[0];
 $result=$idconnect->query("SELECT sum(phpab_matchs.id_equipe_dom), sum(phpab_matchs.id_equipe_ext) FROM phpab_matchs, phpab_journees WHERE  phpab_journees.id_champ='$champ' AND phpab_matchs.id_journee=phpab_journees.id  ");
//$result=mysql_query($query);
while($row=mysqli_fetch_array($result))
    {
 ...
 $result=$idconnect->query("SELECT   phpab_matchs.id_equipe_dom as DOM,  count(*) as ct FROM phpab_matchs, phpab_journees WHERE  phpab_journees.id_champ='$champ' AND phpab_matchs.id_journee=phpab_journees.id GROUP BY DOM");
//$result=mysql_query($query); 
while($row=mysqli_fetch_array($result))
		{
 ...
 $result=$idconnect->query("SELECT phpab_matchs.id_equipe_ext as DOM,  count(*) as ct FROM phpab_matchs, phpab_journees WHERE  phpab_journees.id_champ='$champ' AND phpab_matchs.id_journee=phpab_journees.id GROUP BY DOM");
//$result=mysql_query($query); 
while($row=mysql_fetch_array($result))
		{
		if ($row[1]<>$nb_journees/2)
