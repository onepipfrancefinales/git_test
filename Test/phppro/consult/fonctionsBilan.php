...
function nb_journees($id_champ, $idconnect)
         ...
         $reponse=$idconnect->query("SELECT id FROM phppro_equipes WHERE id_champ='$id_champ'");
...
//suppression des liens sur les scores
  ...
  $requete="SELECT phppro_clubs.nom, CLEXT.nom, phppro_matchs.buts_dom, phppro_matchs.buts_ext,
...
{
  $nb_requete=0;
  if($cache=="1"){ mysql_query("DELETE FROM phppro_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysql_error());}
  else{ mysql_query("DELETE FROM phppro_clmnt") or die (mysql_error());}
          $nb_requete++;
...
$query="SELECT pts_victoire, pts_nul, pts_defaite FROM phppro_parametres WHERE id_champ='$champ'";
$nb_requete++;
...
// victoires domicile
$query="SELECT dom.id, count(dom.id), phppro_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phppro_equipes as dom, phppro_clubs, phppro_matchs, phppro_journees, phppro_championnats
...
$query="SELECT phppro_clubs.nom FROM phppro_clubs, phppro_equipes, phppro_championnats
...
// victoires exterieur
$query="SELECT ext.id, count(ext.id), phppro_clubs.nom, sum(buts_ext), sum(buts_dom) FROM phppro_equipes as ext, phppro_clubs, phppro_matchs, phppro_journees, phppro_championnats
...
$query="SELECT phppro_clubs.nom, phppro_equipes.penalite, phppro_equipes.id
...
//mysql_query("LOCK TABLE phppro_clmnt WRITE, phppro_equipes WRITE, phppro_clubs WRITE") or die (mysql_error());
    ...
    {
    $query="SELECT phppro_clubs.nom, phppro_equipes.id 
            FROM phppro_clubs, phppro_equipes, phppro_championnats
            WHERE phppro_equipes.id_champ=phppro_championnats.id
            AND phppro_championnats.id='$champ'
            AND phppro_equipes.id_club=phppro_clubs.id";
...
{
echo "<table class=\"tablephppro2\" align=\"center\" cellspacing=\"0\" width=\"95%\"><tr class=\"trphppro3\"><th colspan=\"11\">".$legende."</th></tr>\n";
echo "<tr class=\"trphppro3\">
