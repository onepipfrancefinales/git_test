...
$query = "SELECT phppro_divisions.nom, phppro_saisons.annee, (phppro_saisons.annee)+1 
...
if (isset($type))
{
    ...
    $requete = "SELECT phppro_equipes.id FROM phppro_equipes, phppro_clubs
...
{
    $requete="SELECT DISTINCT * FROM phppro_clmnt_cache WHERE ID_CHAMP='$champ' ORDER BY POINTS DESC, PEN DESC, DIFF DESC, BUTSPOUR DESC , BUTSCONTRE ASC, NOM";
   clmnt($legende, $type, $accession, $barrage, $relegation, $champ, $requete, $lien, $id_equipe_fetiche);
  
   $query="SELECT max(phppro_journees.numero) FROM phppro_journees, phppro_matchs WHERE phppro_journees.id=phppro_matchs.id_journee and buts_dom is not NULL and     phppro_journees.id_champ='$champ'";
    $result=mysql_query($query) or die (mysql_error());
        while ($row=mysql_fetch_array($result))
        {
        $numero=$row[0];
...
<br />
  ...
  aff_journee($champ, $numero+1, "<i>".CONSULT_CLMNT_MSG8."</i>", 1, $fiches_clubs, $id_equipe_fetiche);
   ...
   {
   $requete="SELECT DISTINCT * FROM phppro_clmnt WHERE ID_CHAMP='$champ' ORDER BY POINTS DESC, DIFF DESC, BUTSPOUR DESC , BUTSCONTRE ASC, NOM";
...
case DOMICILE;
 {
  ...
  if ($debut=="1" and $fin==$nb_journees)
{
  $requete="SELECT NOM, DOMPOINTS, DOMJOUES, DOMG,  DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, PEN, ID_EQUIPE  FROM phppro_clmnt_cache WHERE ID_CHAMP='$champ' ORDER BY DOMPOINTS DESC, DOMDIFF DESC";
  clmnt($legende, $type, $accession, $barrage, $relegation, $champ, $requete, $lien, $id_equipe_fetiche);
 }

  else
 { 
  $requete="SELECT NOM, DOMPOINTS, DOMJOUES, DOMG,  DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, PEN, ID_EQUIPE  FROM phppro_clmnt WHERE ID_CHAMP='$champ' ORDER BY DOMPOINTS DESC, //DOMDIFF DESC";
  @db_clmnt($champ, $debut, $fin, 0);
  clmnt($legende, $type, $accession, $barrage, $relegation,  $champ, $requete,$lien, $id_equipe_fetiche);  
 }
} 
...
case ATTAQUE;
 {
   ...
   if ($debut=="1" and $fin==$nb_journees)
   {
    $requete="SELECT * FROM phppro_clmnt_cache WHERE ID_CHAMP='$champ' ORDER BY BUTSPOUR DESC, DIFF DESC";
    clmnt($legende, $type, $accession, $barrage, $relegation, $champ, $requete, $lien, $id_equipe_fetiche);
   }

   else
   {
   $requete="SELECT * FROM phppro_clmnt WHERE ID_CHAMP='$champ' ORDER BY BUTSPOUR DESC, DIFF DESC";
   @db_clmnt($champ, $debut, $fin, 0);
   clmnt($legende, $type, $accession, $barrage, $relegation,  $champ, $requete, $lien, $id_equipe_fetiche);
   }
 }
...
case DEFENSE;
 {
  ...
  if ($debut=="1" and $fin==$nb_journees)
   ...
   {
    $requete="SELECT * FROM phppro_clmnt WHERE ID_CHAMP='$champ' ORDER BY BUTSCONTRE ASC, DIFF DESC";
    @db_clmnt($champ, $debut, $fin, 0);
    clmnt($legende, $type, $accession, $barrage, $relegation, $champ, $requete, $lien, $id_equipe_fetiche);
   }
 }
...
case BONUS;
 {
  ...
  if ($debut=="1" and $fin==$nb_journees)
 {
  $requete="SELECT NOM, DOMPOINTS, DOMJOUES, DOMG,  DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, PEN  FROM phppro_clmnt_cache WHERE ID_CHAMP='$champ' ORDER BY PEN DESC, POINTS DESC";
  clmnt($legende, $type, $accession, $barrage, $relegation, $champ, $requete, $lien, $id_equipe_fetiche);
 }

  else
 { 
  $requete="SELECT NOM, DOMPOINTS, DOMJOUES, DOMG,  DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, PEN  FROM phppro_clmnt WHERE ID_CHAMP='$champ' ORDER BY PEN DESC, POINTS DESC";
  @db_clmnt($champ, $debut, $fin, 0);
  clmnt($legende, $type, $accession, $barrage, $relegation,  $champ, $requete,$lien, $id_equipe_fetiche);  
 }
} 
...
case GOALDIFF;
 {
  ...
  if ($debut=="1" and $fin==$nb_journees)
  {
    $requete="SELECT * FROM phppro_clmnt_cache WHERE id_champ='$champ' ORDER BY DIFF DESC, BUTSPOUR DESC, BUTSCONTRE ASC ";
    clmnt($legende, $type, $accession, $barrage, $relegation, $champ, $requete, $lien, $id_equipe_fetiche);
   }
  else
  {
   $requete="SELECT * FROM phppro_clmnt WHERE id_champ='$champ' ORDER BY DIFF DESC, BUTSPOUR DESC, BUTSCONTRE ASC ";
   @db_clmnt($champ, $debut, $fin, 0);
   clmnt($legende, $type, $accession, $barrage, $relegation,  $champ, $requete,$lien, $id_equipe_fetiche);
  }
 }
...
case EXTERIEUR;
{
 $legende=CONSULT_CLMNT_MSG14.$debut.CONSULT_CLMNT_MSG5.$fin;
   ...
   if ($debut=="1" and $fin==$nb_journees)
   {
    $requete="SELECT NOM, EXTPOINTS, EXTJOUES, EXTG,  EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, PEN, ID_EQUIPE  FROM phppro_clmnt_cache WHERE ID_CHAMP='$champ' ORDER BY EXTPOINTS DESC, EXTDIFF DESC ";
    clmnt($legende, $type, $accession, $barrage, $relegation, $champ, $requete, $lien, $id_equipe_fetiche);
   }
  else
  {
 $requete="SELECT NOM, EXTPOINTS, EXTJOUES, EXTG,  EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, PEN, ID_EQUIPE  FROM phppro_clmnt WHERE ID_CHAMP='$champ' ORDER BY EXTPOINTS DESC, EXTDIFF DESC ";
 @db_clmnt($champ, $debut, $fin, 0);
 clmnt($legende, $type, $accession, $barrage, $relegation, $champ, $requete, $lien, $id_equipe_fetiche);
  }
}
