...
{ 
  ...
  // recherche de id_journee de la journee miroir si existe
    if ($miroir<>"none")
     {     
       $query = "SELECT id FROM phpfed3NE_journees WHERE  id_champ='$champ' AND numero='$miroir'";
  ...
  $x=$numero-1;
  $query = "SELECT id FROM phpfed3NE_journees WHERE id_champ='$champ' AND numero='$x'";
  $result=mysql_query ($query);
       while ($row=mysql_fetch_array($result))
       {
         $id_journee=$row[0] ;
...
for ($counter=$nb_rencontres; $counter>0 ; $counter=$counter-1 ) // Nb de rencontres dans la journée
        ...
        $result =$idconnect( "SELECT DISTINCT phpfed3NE_clubs.nom, phpfed3NE_equipes.id FROM phpfed3NE_clubs, phpfed3NE_equipes
        ...
        $result =$idconnect->query( "SELECT DISTINCT phpfed3NE_clubs.nom, phpfed3NE_equipes.id 
        FROM phpfed3NE_clubs, phpfed3NE_equipes WHERE phpfed3NE_equipes.id_champ='$champ' AND phpfed3NE_clubs.id=phpfed3NE_equipes.id_club ORDER BY phpfed3NE_clubs.nom");
...
//JOURNEE MIROIR ?
		...
		$miroir="none";
                ...
                $result =$idconnect->query( "SELECT numero, id FROM phpfed3NE_journees WHERE id_champ='$champ'  ORDER BY phpfed3NE_journees.numero");
               // $result=mysql_query($query);
                while ($row = mysqli_fetch_array($result)) {
...
// CHECK SUM DU CHAMPIONNAT
  $result=$idconnect->query(" SELECT sum(id) FROM phpfed3NE_equipes WHERE id_champ='$champ' ");
...
echo "<small>MATCH_SUM = ".$sum_day."  </small>";
    $query=$idconnect->query("SELECT count(DISTINCT phpfed3NE_matchs.id_equipe_dom), count(DISTINCT phpfed3NE_matchs.id_equipe_ext) FROM phpfed3NE_matchs, phpfed3NE_journees   WHERE phpfed3NE_matchs.id_journee=phpfed3NE_journees.id    AND phpfed3NE_journees.id_champ='$champ'    AND phpfed3NE_journees.numero='$x' ");
   // $result=mysql_query($query);
...
$result=$idconnect->query(" SELECT sum(phpfed3NE_matchs.id_equipe_dom), sum(phpfed3NE_matchs.id_equipe_ext) FROM phpfed3NE_matchs, phpfed3NE_journees WHERE phpfed3NE_matchs.id_journee=phpfed3NE_journees.id AND phpfed3NE_journees.id_champ='$champ'  ");
//$result=mysql_query($query);
$sum=mysqli_fetch_array($result);
$sumsum=$sum[0];
 $result=$idconnect->query("SELECT sum(phpfed3NE_matchs.id_equipe_dom), sum(phpfed3NE_matchs.id_equipe_ext) FROM phpfed3NE_matchs, phpfed3NE_journees WHERE  phpfed3NE_journees.id_champ='$champ' AND phpfed3NE_matchs.id_journee=phpfed3NE_journees.id  ");
//$result=mysql_query($query);
while($row=mysqli_fetch_array($result))
    {
 ...
 $result=$idconnect->query("SELECT   phpfed3NE_matchs.id_equipe_dom as DOM,  count(*) as ct FROM phpfed3NE_matchs, phpfed3NE_journees WHERE  phpfed3NE_journees.id_champ='$champ' AND phpfed3NE_matchs.id_journee=phpfed3NE_journees.id GROUP BY DOM");
//$result=mysql_query($query); 
while($row=mysqli_fetch_array($result))
		{
 ...
 $result=$idconnect->query("SELECT phpfed3NE_matchs.id_equipe_ext as DOM,  count(*) as ct FROM phpfed3NE_matchs, phpfed3NE_journees WHERE  phpfed3NE_journees.id_champ='$champ' AND phpfed3NE_matchs.id_journee=phpfed3NE_journees.id GROUP BY DOM");
//$result=mysql_query($query); 
while($row=mysql_fetch_array($result))
		{
		if ($row[1]<>$nb_journees/2)
