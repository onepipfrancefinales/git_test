...
if ($action2=="1")
 ...
 if ($nb_resultats>0)
   {
   //M�j des param�tres
    mysqli_query ($idconnect,(" UPDATE phpfed3NE_parametres 
   ...
   else
   {
    ...
    mysqli_query ($idconnect,(" UPDATE phpfed3NE_parametres 
   (id_champ, pts_victoire, pts_nul, pts_defaite, accession, barrage, relegation,id_equipe_fetiche, fiches_clubs, estimation) 
    ...
    //$requete="INSERT INTO phpfed3NE_parametres (id_champ, pts_victoire, pts_nul, pts_defaite, accession, barrage, relegation,id_equipe_fetiche, fiches_clubs, estimation) 
	//VALUES ('$champ', '$pts_victoire', '$pts_nul', '$pts_defaite', '$accession', '$barrage', '$relegation', '$id_equipe_fetiche', '$fiches_clubs', '$estimation')";
...
if ($malus)
 {
  ...
  while($x<$y)
   {
	mysqli_query($idconnect, (" UPDATE phpfed3NE_equipes 
...
// $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
 /*
  $resultats=$idconnect->query("SELECT phpfed3NE_clubs.nom, phpfed3NE_equipes.id 
  FROM phpfed3NE_equipes, phpfed3NE_clubs 
  WHERE phpfed3NE_equipes.id_champ='$champ' AND phpfed3NE_clubs.id=phpfed3NE_equipes.id_club ORDER by nom");
//$resultats = mysqli_query ($requete);
echo "<tr><td class=phpfed3NE3>";
...
<table class=phpfed3NE width="80%">
  <tr>
    <td class=phpfed3NE2 align="center" colspan="4"><?php echo "Points administratifs "; affich_champ ($champ, $idconnect); ?></td>
  </tr>
  <tr>
    <td><?php 
	...
	$result=$idconnect->query("	SELECT DISTINCT phpfed3NE_clubs.nom, phpfed3NE_equipes.id, phpfed3NE_equipes.pts_admin, phpfed3NE_equipes.jour_pere  
