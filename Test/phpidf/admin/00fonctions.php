    ...
    while ($row = mysqli_fetch_array($resultats))
      {
        ...
        if ($champ=="$row[0]") {$class="phpidf7";}
        elseif (($i%2)==0) {$class="phpidf3";}
        else {$class="phpidf4";}
    ...
    while ($row = mysqli_fetch_array($resultats))
      {
        ...
        if ($gr_champ=="$row[0]") {$class="phpidf7";}
        elseif (($i%2)==0) {$class="phpidf3";}
        else {$class="phpidf4";}
  ...
  $result=$idconnect->query("SELECT phpidf_clubs.nom, phpidf_equipes.penalite, phpidf_equipes.id
	...
	*/
    while($row = mysqli_fetch_array($result))
    {
		...
		if($cache=="1")
		{
		 mysqli_query($idconnect, ("INSERT INTO phpidf_clmnt_cache(NOM, POINTS, JOUES, G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN, DOMPOINTS, DOMJOUES, DOMG, DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, EXTPOINTS, EXTJOUES, EXTG, EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, ID_EQUIPE, ID_CHAMP) 
		...
		{
		 mysqli_query($idconnect, ("INSERT INTO phpidf_clmnt(NOM, POINTS, JOUES, G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN, DOMPOINTS, DOMJOUES, DOMG, DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, EXTPOINTS, EXTJOUES, EXTG, EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, ID_EQUIPE, ID_CHAMP) 
									  VALUES('$NOM','$POINTS','$JOUES','$G','$N','$P','$BUTSPOUR','$BUTSCONTRE','$DIFF','$PEN','$DOMPOINTS','$DOMJOUES','$DOMG','$DOMN','$DOMP','$DOMBUTSPOUR','$DOMBUTSCONTRE','$DOMDIFF','$EXTPOINTS','$EXTJOUES','$EXTG','$EXTN','$EXTP','$EXTBUTSPOUR','$EXTBUTSCONTRE','$EXTDIFF','$ID_EQUIPE','$ID_CHAMP')"));
	...
	if($cache=="1")
	{$requete="DELETE FROM phpidf_clmnt_cache WHERE nom='exempte'" or die (mysql_error($idconnect));}
	else
	{$requete="DELETE FROM phpidf_clmnt WHERE nom='exempte'" or die (mysql_error($idconnect));}
...
// Nombres de renseignement dans cette classe à partir de l'id_classe  (utilisé dans phpidf_classe.php)
...
// Nombres de classes enregistrées (ulilisé dans phpidf_classe.php)
