...
if (empty ($numero))
  {
    $result=$idconnect->query("
			SELECT max(phpidf_journees.numero) 
...
for ($counter=$nb_rencontres; $counter>0 ; $counter=$counter-1 ) // Nb de rencontres dans la journ�e
     ...
     //   $result=mysql_query($query) or die(mysql_error());
	   ...
	   $result= $idconnect->query("
				SELECT DISTINCT phpidf_clubs.nom, phpidf_equipes.id 
				FROM phpidf_clubs, phpidf_equipes 
				WHERE phpidf_equipes.id_champ='$champ' 
				AND phpidf_clubs.id=phpidf_equipes.id_club 
				ORDER BY phpidf_clubs.nom");
...
echo "<small>MATCH_SUM = ".$sum_day."  </small>";
    $result=$idconnect->query("
			SELECT count(DISTINCT phpidf_matchs.id_equipe_dom), 
			count(DISTINCT phpidf_matchs.id_equipe_ext) 
			FROM phpidf_matchs, phpidf_journees   
			WHERE phpidf_matchs.id_journee=phpidf_journees.id    
			AND phpidf_journees.id_champ='$champ'    
			AND phpidf_journees.numero='$x' ");
			//$result=mysql_query($query);
/*
    while( $row = (mysql_fetch_array($result)) )
        {
...
$result=$idconnect->query(" 
		SELECT sum(phpidf_matchs.id_equipe_dom), 
			   sum(phpidf_matchs.id_equipe_ext) 
		FROM phpidf_matchs, phpidf_journees 
		WHERE phpidf_matchs.id_journee=phpidf_journees.id 
		AND phpidf_journees.id_champ='$champ'  ");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT   phpidf_matchs.id_equipe_dom as DOM,  count(*) as ct 
		 FROM phpidf_matchs, phpidf_journees 
		 WHERE  phpidf_journees.id_champ='$champ' 
		 AND phpidf_matchs.id_journee=phpidf_journees.id 
		 GROUP BY DOM");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT phpidf_matchs.id_equipe_ext as DOM,  
		 count(*) as ct 
		 FROM phpidf_matchs, phpidf_journees 
		 WHERE  phpidf_journees.id_champ='$champ' 
		 AND phpidf_matchs.id_journee=phpidf_journees.id 
