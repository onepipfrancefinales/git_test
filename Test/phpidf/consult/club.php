...
$result= $idconnect->query("SELECT phpidf_clubs.id, phpidf_clubs.nom, id_champ, id_club
							FROM phpidf_clubs, phpidf_equipes
							WHERE phpidf_equipes.id_champ='$champ' and phpidf_equipes.id_club=phpidf_clubs.id
							ORDER BY nom");
...
$result= $idconnect->query("SELECT phpidf_classe.nom, phpidf_classe.id 
							FROM phpidf_classe 
							order by rang");
//$result=mysql_query ($query);
        ...
        echo "<tr class=\"trphpidf3\"><td align=\"center\"><b>$row[0]</b></td></tr>";
        $id_classe=$row[1];
        echo "<tr><td><table cellspacing=\"0\"><tr class=\"trphpidf\"><td>";
...
echo "<table class=\"tablephpidf2\" cellspacing=\"0\" align=\"center\" width=\"90%\"><tr class=\"trphpidf3\"><td align=\"center\"><b>Effectif</b></td></tr><tr><td>";

$result= $idconnect->query("SELECT phpidf_equipes.id 
							FROM phpidf_equipes, phpidf_clubs 
							WHERE phpidf_clubs.id='$id_clubs' 
							AND id_champ='$champ' 
...
{ $row[0]=addslashes($row[0]);
 $resultats1 = $idconnect->query("SELECT nom, prenom, phpidf_joueurs.id
									FROM phpidf_joueurs, phpidf_effectif
									WHERE phpidf_joueurs.id=phpidf_effectif.id_joueur
									AND phpidf_effectif.id_equipe='$equipe'
...
echo "<table class=\"tablephpidf2\" cellspacing=\"0\" align=\"center\" width=\"90%\"><tr class=\"trphpidf3\"><td align=\"center\"><b>".CONSULT_CLUB_4."</b></td></tr>";
        echo "<tr><td><center><img src=\"graph.php?equipe=$equipe\" alt=\"\"></center></td></tr>";


echo"</table><br /><br /><br />";

echo "<table class=\"tablephpidf2\" cellspacing=\"0\" align=\"center\" width=\"90%\"><tr class=\"trphpidf3\" align=\"center\"><td><b>".CONSULT_CLUB_3."</b></td></tr>";
...
$result= $idconnect->query("SELECT annee, phpidf_divisions.nom, phpidf_championnats.id, phpidf_equipes.id
							...
							AND phpidf_equipes.id_club='$id_clubs'
