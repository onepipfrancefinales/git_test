<?php $annee=$_GET['variable_1'];?>
   
<?php

$clubA3201= ' Flandres 1 ';
$clubA3202= ' Normandie 3';

$clubA3203= ' Centre 1';
$clubA3204= ' Ile-de-France 5  ';

$clubA3205= ' Flandres 2';
$clubA3206= ' Ile-de-France 4';

$clubA3207= ' Pays de la Loire 1';
$clubA3208= ' Normandie 2  ';

$clubA3209= ' Normandie 1 ';
$clubA3210= ' Bretagne 3';

$clubA3211= ' Pays de la Loire 2';
$clubA3212= ' Ile de France 3';

$clubA3213= ' Bretagne 1';
$clubA3214= ' Centre 2';

$clubA3215= ' Poitou-Charentes 1';
$clubA3216= ' Limousin 3 ';

$clubA3217= ' Bretagne 2';
$clubA3218= ' Poitou-Charente 1';

$clubA3219= ' Limousin 1 ';
$clubA3220= ' Barrage CE 3 - CA 3';

$clubA3221= ' Bourgogne 1';
$clubA3222= ' Barrage AU 3 - LM 4';

$clubA3223= ' Alsace Lorraine 1';
$clubA3224= ' Barrage LY 3 - IDF 6 ';

$clubA3225= ' Ile de France 2';
$clubA3226= ' Flandres 3';

$clubA3227= ' Alsace Lorraine 2';
$clubA3228= ' Bourgogne 2 ';

$clubA3229= ' Ile de France 1';
$clubA3230= ' Bourgogne 3';

$clubA3231= ' Auvergne 1';
$clubA3232= ' Lyonnais 2 ';

$clubA3233= ' Lyonnais 1';
$clubA3234= ' Alpes 2';

$clubA3235= ' Auvergne 2';
$clubA3236= ' Barrage DA 2 - PR 3 ';

$clubA3237= ' C�te d Azur 1';
$clubA3238= ' Provence 2';

$clubA3239= ' Languedoc 1';
$clubA3240= '  Barrage PC 2 - MPY 7';

$clubA3241= ' Dr�me Ard�che 1';
$clubA3242= ' Alpes 1 ';

$clubA3243= ' Armagnac Bigorre 2 ';
$clubA3244= ' Midi-Py�n�es 2 ';

$clubA3245= ' Pays Catalan 1';
$clubA3246= ' Languedoc 2';

$clubA3247= ' Bearn 1 ';
$clubA3248= ' Midi-Py�n�es 6 ';

$clubA3249= ' Provence 1';
$clubA3250= ' Languedoc 3';

$clubA3251= ' C�te Basque Landes 1 ';
$clubA3252= ' Midi-Py�n�es 5 ';

$clubA3253= ' P�rigord Agenais 1 ';
$clubA3254= 'Barrage  AB 3 - BE 2 ';

$clubA3255= ' Limousin 2 ';
$clubA3256= ' Midi-Py�n�es 3 ';

$clubA3257= ' C�te d Argent 1';
$clubA3258= ' C�te Basque Landes 2'; 

$clubA3259= ' Midi-Py�n�es 1 ';
$clubA3260= ' P�rigord Agenais 3 ';

$clubA3261= ' Armagnac Bigorre 1 ';
$clubA3262= ' P�rigord Agenais 2';

$clubA3263= ' C�te d Argent 2 ';
$clubA3264= ' Midi-Py�n�es 4 ';


?>

<body bgcolor="#FFFFFF">
<div id="Layer2" style="position:absolute; width:1800px; height:20px; z-index:4; left: 1px; top: 900px; background-color: #FFCC66; layer-background-color: #FFCC66; border: 1px none #000000"> 
  <p align="center"><a href="http://francefinalesrugby.franceserv.com/divisions/page1serie.php"></a> 
  </p>
</div>
<div id="Layer5" style="position:absolute; width:350px; height:20px; z-index:7; left: 20px; top: 240px; background-image: url(../unique32.jpg); layer-background-image: url(../unique32.jpg); border: 1px none #000000"> 
  <div align="center"> 
    <div id="Layer6" style="position:absolute; width:350px; height:20px; z-index:8; left: 0px; top: 20px; background-image: url(../unique32.jpg); layer-background-image: url(../unique32.jpg); border: 1px none #000000"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      </b></font> <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      <?php echo $clubA3203;
  ?>
      - 
      <?php echo $clubA3204;
  ?>
      </b></font> 
      <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdouble.jpg); layer-background-image: url(../resultdouble.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
        <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
          - </b></font></div>
      </div>
      
    </div>
    <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
    <?php echo $clubA3201;
  ?>
    - 
    <?php echo $clubA3202;
  ?>
    </b></font> 
    <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdouble.jpg); layer-background-image: url(../resultdouble.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
      <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
        - </b></font></div>
    </div>
    
  </div>
</div>
<div id="Layer5" style="position:absolute; width:350px; height:20px; z-index:7; left: 20px; top: 280px; background-image: url(../unique32.jpg); layer-background-image: url(../unique32.jpg); border: 1px none #000000"> 
  <div align="center"> 
    <div id="Layer6" style="position:absolute; width:350px; height:20px; z-index:8; left: 0px; top: 20px; background-image: url(../unique32.jpg); layer-background-image: url(../unique32.jpg); border: 1px none #000000"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      </b></font> <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      <?php echo $clubA3207;
  ?>
      - 
      <?php echo $clubA3208;
  ?>
      </b></font> 
      <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdouble.jpg); layer-background-image: url(../resultdouble.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
        <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
          - </b></font></div>
      </div>
      
    </div>
    <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
    <?php echo $clubA3205;
  ?>
    - 
    <?php echo $clubA3206;
  ?>
    </b></font> 
    <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdouble.jpg); layer-background-image: url(../resultdouble.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
      <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
        - </b></font></div>
    </div>
    
  </div>
</div>
<div id="Layer5" style="position:absolute; width:350px; height:20px; z-index:7; left: 20px; top: 320px; background-image: url(../unique32B.jpg); layer-background-image: url(../unique32B.jpg); border: 1px none #000000"> 
  <div align="center"> 
    <div id="Layer6" style="position:absolute; width:350px; height:20px; z-index:8; left: 0px; top: 20px; background-image: url(../unique32B.jpg); layer-background-image: url(../unique32B.jpg); border: 1px none #000000"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      </b></font> <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      <?php echo $clubA3211;
  ?>
      - 
      <?php echo $clubA3212;
  ?>
      </b></font> 
      <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdoubleB.jpg); layer-background-image: url(../resultdoubleB.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
        <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
          - </b></font></div>
      </div>
      
    </div>
    <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
    <?php echo $clubA3209;
  ?>
    - 
    <?php echo $clubA3210;
  ?>
    </b></font> 
    <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdoubleB.jpg); layer-background-image: url(../resultdoubleB.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
      <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
        - </b></font></div>
    </div>
    
  </div>
</div>
<div id="Layer5" style="position:absolute; width:350px; height:20px; z-index:7; left: 20px; top: 360px; background-image: url(../unique32B.jpg); layer-background-image: url(../unique32B.jpg); border: 1px none #000000"> 
  <div align="center"> 
    <div id="Layer6" style="position:absolute; width:350px; height:20px; z-index:8; left: 0px; top: 20px; background-image: url(../unique32B.jpg); layer-background-image: url(../unique32B.jpg); border: 1px none #000000"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      </b></font> <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      <?php echo $clubA3215;
  ?>
      - 
      <?php echo $clubA3216;
  ?>
      </b></font> 
      <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdoubleB.jpg); layer-background-image: url(../resultdoubleB.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
        <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
          - </b></font></div>
      </div>
      
    </div>
    <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
    <?php echo $clubA3213;
  ?>
    - 
    <?php echo $clubA3214;
  ?>
    </b></font> 
    <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdoubleB.jpg); layer-background-image: url(../resultdoubleB.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
      <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
        - </b></font></div>
    </div>
    
  </div>
</div>
<div id="Layer5" style="position:absolute; width:350px; height:20px; z-index:7; left: 20px; top: 400px; background-image: url(../unique32.jpg); layer-background-image: url(../unique32.jpg); border: 1px none #000000"> 
  <div align="center"> 
    <div id="Layer6" style="position:absolute; width:350px; height:20px; z-index:8; left: 0px; top: 20px; background-image: url(../unique32.jpg); layer-background-image: url(../unique32.jpg); border: 1px none #000000"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      </b></font> <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      <font color="#FF0000"> <font color="#FFFFFF">
      <?php echo $clubA3219;
  ?>
      </font> <font size="1" color="#FFFFFF">-</font><font size="1"> 
      <?php echo $clubA3220;
  ?>
      </font></font></b></font> 
      <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdouble.jpg); layer-background-image: url(../resultdouble.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
        <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
          - </b></font></div>
      </div>
      
    </div>
    <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
    <?php echo $clubA3217;
  ?>
    - 
    <?php echo $clubA3218;
  ?>
    </b></font> 
    <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdouble.jpg); layer-background-image: url(../resultdouble.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
      <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
        - </b></font></div>
    </div>
    
  </div>
</div>
<div id="Layer5" style="position:absolute; width:350px; height:20px; z-index:7; left: 20px; top: 440px; background-image: url(../unique32.jpg); layer-background-image: url(../unique32.jpg); border: 1px none #000000"> 
  <div align="center"> 
    <div id="Layer6" style="position:absolute; width:350px; height:20px; z-index:8; left: 0px; top: 20px; background-image: url(../unique32.jpg); layer-background-image: url(../unique32.jpg); border: 1px none #000000"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      </b></font> <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      <?php echo $clubA3223;
  ?>
      -<font size="1" color="#FF0000"> 
      <?php echo $clubA3224;
  ?>
      </font></b></font> 
      <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdouble.jpg); layer-background-image: url(../resultdouble.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
        <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
          - </b></font></div>
      </div>
      
    </div>
    <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
    <?php echo $clubA3221;
  ?>
    - <font color="#FF0000" size="1">
    <?php echo $clubA3222;
  ?>
    </font></b></font> 
    <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdouble.jpg); layer-background-image: url(../resultdouble.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
      <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
        - </b></font></div>
    </div>
    
  </div>
</div>
<div id="Layer5" style="position:absolute; width:350px; height:20px; z-index:7; left: 20px; top: 480px; background-image: url(../unique32B.jpg); layer-background-image: url(../unique32B.jpg); border: 1px none #000000"> 
  <div align="center"> 
    <div id="Layer6" style="position:absolute; width:350px; height:20px; z-index:8; left: 0px; top: 20px; background-image: url(../unique32B.jpg); layer-background-image: url(../unique32B.jpg); border: 1px none #000000"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      </b></font> <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      <?php echo $clubA3227;
  ?>
      - 
      <?php echo $clubA3228;
  ?>
      </b></font> 
      <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdoubleB.jpg); layer-background-image: url(../resultdoubleB.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
        <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
          - </b></font></div>
      </div>
      
    </div>
    <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
    <?php echo $clubA3225;
  ?>
    - 
    <?php echo $clubA3226;
  ?>
    </b></font> 
    <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdoubleB.jpg); layer-background-image: url(../resultdoubleB.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
      <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
        - </b></font></div>
    </div>
    
  </div>
</div>
<div id="Layer5" style="position:absolute; width:350px; height:20px; z-index:7; left: 20px; top: 520px; background-image: url(../unique32B.jpg); layer-background-image: url(../unique32B.jpg); border: 1px none #000000"> 
  <div align="center"> 
    <div id="Layer6" style="position:absolute; width:350px; height:20px; z-index:8; left: 0px; top: 20px; background-image: url(../unique32B.jpg); layer-background-image: url(../unique32B.jpg); border: 1px none #000000"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      </b></font> <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      <?php echo $clubA3231;
  ?>
      - 
      <?php echo $clubA3232;
  ?>
      </b></font> 
      <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdoubleB.jpg); layer-background-image: url(../resultdoubleB.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
        <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
          - </b></font></div>
      </div>
      
    </div>
    <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
    <?php echo $clubA3229;
  ?>
    - 
    <?php echo $clubA3230;
  ?>
    </b></font> 
    <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdoubleB.jpg); layer-background-image: url(../resultdoubleB.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
      <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
        - </b></font></div>
    </div>
    
  </div>
</div>
<div id="Layer5" style="position:absolute; width:350px; height:20px; z-index:7; left: 20px; top: 560px; background-image: url(../unique32.jpg); layer-background-image: url(../unique32.jpg); border: 1px none #000000"> 
  <div align="center"> 
    <div id="Layer6" style="position:absolute; width:350px; height:20px; z-index:8; left: 0px; top: 20px; background-image: url(../unique32.jpg); layer-background-image: url(../unique32.jpg); border: 1px none #000000"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      </b></font> <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      <?php echo $clubA3235;
  ?>
      -<font size="1" color="#FF0000">
      <?php echo $clubA3236;
  ?>
      </font></b></font> 
      <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdouble.jpg); layer-background-image: url(../resultdouble.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
        <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
          - </b></font></div>
      </div>
      
    </div>
    <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
    <?php echo $clubA3233;
  ?>
    - 
    <?php echo $clubA3234;
  ?>
    </b></font> 
    <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdouble.jpg); layer-background-image: url(../resultdouble.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
      <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
        - </b></font></div>
    </div>
    
  </div>
</div>
<div id="Layer5" style="position:absolute; width:350px; height:20px; z-index:7; left: 20px; top: 600px; background-image: url(../unique32.jpg); layer-background-image: url(../unique32.jpg); border: 1px none #000000"> 
  <div align="center"> 
    <div id="Layer6" style="position:absolute; width:350px; height:20px; z-index:8; left: 0px; top: 20px; background-image: url(../unique32.jpg); layer-background-image: url(../unique32.jpg); border: 1px none #000000"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      </b></font> <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      <?php echo $clubA3239;
  ?>
      -<font size="1" color="#FF0000"> 
      <?php echo $clubA3240;
  ?>
      </font> </b></font> 
      <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdouble.jpg); layer-background-image: url(../resultdouble.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
        <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
          - </b></font></div>
      </div>
      
    </div>
    <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
    <?php echo $clubA3237;
  ?>
    - 
    <?php echo $clubA3238;
  ?>
    </b></font> 
    <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdouble.jpg); layer-background-image: url(../resultdouble.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
      <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
        - </b></font></div>
    </div>
    
  </div>
</div>
<div id="Layer5" style="position:absolute; width:350px; height:20px; z-index:7; left: 20px; top: 640px; background-image: url(../unique32B.jpg); layer-background-image: url(../unique32B.jpg); border: 1px none #000000"> 
  <div align="center"> 
    <div id="Layer6" style="position:absolute; width:350px; height:20px; z-index:8; left: 0px; top: 20px; background-image: url(../unique32B.jpg); layer-background-image: url(../unique32B.jpg); border: 1px none #000000"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      </b></font> <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      <?php echo $clubA3243;
  ?>
      - 
      <?php echo $clubA3244;
  ?>
      </b></font> 
      <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdoubleB.jpg); layer-background-image: url(../resultdoubleB.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
        <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
          - </b></font></div>
      </div>
      
    </div>
    <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
    <?php echo $clubA3241;
  ?>
    - 
    <?php echo $clubA3242;
  ?>
    </b></font> 
    <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdoubleB.jpg); layer-background-image: url(../resultdoubleB.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
      <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
        - </b></font></div>
    </div>
    
  </div>
</div>
<div id="Layer5" style="position:absolute; width:350px; height:20px; z-index:7; left: 20px; top: 680px; background-image: url(../unique32B.jpg); layer-background-image: url(../unique32B.jpg); border: 1px none #000000"> 
  <div align="center"> 
    <div id="Layer6" style="position:absolute; width:350px; height:20px; z-index:8; left: 0px; top: 20px; background-image: url(../unique32B.jpg); layer-background-image: url(../unique32B.jpg); border: 1px none #000000"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      </b></font> <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      <?php echo $clubA3247;
  ?>
      - 
      <?php echo $clubA3248;
  ?>
      </b></font> 
      <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdoubleB.jpg); layer-background-image: url(../resultdoubleB.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
        <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
          - </b></font></div>
      </div>
      
    </div>
    <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
    <?php echo $clubA3245;
  ?>
    - 
    <?php echo $clubA3246;
  ?>
    </b></font> 
    <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdoubleB.jpg); layer-background-image: url(../resultdoubleB.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
      <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
        - </b></font></div>
    </div>
    
  </div>
</div>
<div id="Layer5" style="position:absolute; width:350px; height:20px; z-index:7; left: 20px; top: 720px; background-image: url(../unique32.jpg); layer-background-image: url(../unique32.jpg); border: 1px none #000000"> 
  <div align="center"> 
    <div id="Layer6" style="position:absolute; width:350px; height:20px; z-index:8; left: 0px; top: 20px; background-image: url(../unique32.jpg); layer-background-image: url(../unique32.jpg); border: 1px none #000000"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      </b></font> <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      <?php echo $clubA3251;
  ?>
      - 
      <?php echo $clubA3252;
  ?>
      </b></font> 
      <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdouble.jpg); layer-background-image: url(../resultdouble.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
        <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
          - </b></font></div>
      </div>
      
    </div>
    <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
    <?php echo $clubA3249;
  ?>
    - 
    <?php echo $clubA3250;
  ?>
    </b></font> 
    <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdouble.jpg); layer-background-image: url(../resultdouble.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
      <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
        - </b></font></div>
    </div>
    
  </div>
</div>
<div id="Layer5" style="position:absolute; width:350px; height:20px; z-index:7; left: 20px; top: 760px; background-image: url(../unique32.jpg); layer-background-image: url(../unique32.jpg); border: 1px none #000000"> 
  <div align="center"> 
    <div id="Layer6" style="position:absolute; width:350px; height:20px; z-index:8; left: 0px; top: 20px; background-image: url(../unique32.jpg); layer-background-image: url(../unique32.jpg); border: 1px none #000000"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      </b></font> <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      <?php echo $clubA3255;
  ?>
      - 
      <?php echo $clubA3256;
  ?>
      </b></font> 
      <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdouble.jpg); layer-background-image: url(../resultdouble.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
        <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
          - </b></font></div>
      </div>
      
    </div>
    <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
    <?php echo $clubA3253;
  ?>
    -<font size="1" color="#FF0000">
    <?php echo $clubA3254;
  ?>
    </font> </b></font> 
    <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdouble.jpg); layer-background-image: url(../resultdouble.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
    </div>
    
  </div>
</div>
<div id="Layer5" style="position:absolute; width:350px; height:20px; z-index:7; left: 20px; top: 800px; background-image: url(../unique32B.jpg); layer-background-image: url(../unique32B.jpg); border: 1px none #000000"> 
  <div align="center"> 
    <div id="Layer6" style="position:absolute; width:350px; height:20px; z-index:8; left: 0px; top: 20px; background-image: url(../unique32B.jpg); layer-background-image: url(../unique32B.jpg); border: 1px none #000000"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      </b></font> <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      <?php echo $clubA3259;
  ?>
      - 
      <?php echo $clubA3260;
  ?>
      </b></font> 
      <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdoubleB.jpg); layer-background-image: url(../resultdoubleB.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
        <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
          - </b></font></div>
      </div>
      
    </div>
    <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
    <?php echo $clubA3257;
  ?>
    - 
    <?php echo $clubA3258;
  ?>
    </b></font> 
    <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdoubleB.jpg); layer-background-image: url(../resultdoubleB.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
      <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
        - </b></font></div>
    </div>
    
  </div>
</div>
<div id="Layer5" style="position:absolute; width:350px; height:20px; z-index:7; left: 19px; top: 842px; background-image: url(../unique32B.jpg); layer-background-image: url(../unique32B.jpg); border: 1px none #000000"> 
  <div align="center"> 
    <div id="Layer6" style="position:absolute; width:350px; height:20px; z-index:8; left: 0px; top: 20px; background-image: url(../unique32B.jpg); layer-background-image: url(../unique32B.jpg); border: 1px none #000000"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      </b></font> <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
      <?php echo $clubA3263;
  ?>
      - 
      <?php echo $clubA3264;
  ?>
      </b></font> 
      <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdoubleB.jpg); layer-background-image: url(../resultdoubleB.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
        <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
          - </b></font></div>
      </div>
      
    </div>
    <font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="2"><b> 
    <?php echo $clubA3261;
  ?>
    - 
    <?php echo $clubA3262;
  ?>
    </b></font> 
    <div id="Layer7" style="position:absolute; width:55px; height:20px; z-index:8; background-image: url(../resultdoubleB.jpg); layer-background-image: url(../resultdoubleB.jpg); border: 1px none #000000; left: 350px; top: 0px"> 
      <div align="center"><font color="#FFFFFF" face="Geneva, Arial, Helvetica, san-serif"size="3"><b> 
        - </b></font></div>
    </div>
    
  </div>
</div>
<div id="Layer3" style="position:absolute; width:160px; height:20px; z-index:5; left: 953px; top: 202px; background-image:  url(../titre.jpg); layer-background-image:  url(../titre.jpg); border: 1px none #000000"> 
  <div align="center"><font color="#FFFFFF"><b>Quarts de finale</b></font></div>
</div>
<div id="Layer3" style="position:absolute; width:160px; height:20px; z-index:5; left: 699px; top: 201px; background-image:  url(../titre.jpg); layer-background-image:  url(../titre.jpg); border: 1px none #000000"> 
  <div align="center"><font color="#FFFFFF"><b>8me de finale</b></font></div>
</div>
<div id="Layer3" style="position:absolute; width:160px; height:20px; z-index:5; left: 1446px; top: 201px; background-image:  url(../titre.jpg); layer-background-image:  url(../titre.jpg); border: 1px none #000000"> 
  <div align="center"><font color="#FFFFFF"><b>Finale</b></font></div>
</div>
<div id="Layer3" style="position:absolute; width:160px; height:20px; z-index:5; left: 160px; top: 200px; background-image:  url(../titre.jpg); layer-background-image:  url(../titre.jpg); border: 1px none #000000"> 
  <div align="center"><font color="#FFFFFF"><b>32me de finale</b></font></div>
</div>
<div id="Layer3" style="position:absolute; width:160px; height:20px; z-index:5; left: 1199px; top: 202px; background-image:  url(../titre.jpg); layer-background-image:  url(../titre.jpg); border: 1px none #000000"> 
  <div align="center"><font color="#FFFFFF"><b>Demi finale</b></font></div>
</div>
<div id="Layer3" style="position:absolute; width:160px; height:20px; z-index:5; left: 452px; top: 201px; background-image:  url(../titre.jpg); layer-background-image:  url(../titre.jpg); border: 1px none #000000"> 
  <div align="center"><font color="#FFFFFF"><b>16me de finale</b></font></div>
</div>
<div id="Layer2" style="position:absolute; width:1600px; height:20px; z-index:4; left: -2px; top: 175px; background-color: #FFCC66; layer-background-color: #FFCC66; border: 1px none #000000">
  <div align="center"><i><b>2me Série 2me Série 2me Série 
    2me Série 2me Série 2me Série 2me Série 2me Série 
    2me Série 2me Série 2me Série 2me Série 2me Série 
    2me Série 2me Série 2me Série 2me Série 2me Série 
    2me Série 2me Série 2me Série 2me Série 2me Série 
    </b></i></div>
</div>
<div id="Layer3" style="position:absolute; width:99px; height:20px; z-index:5; left: 26px; top: 200px; background-image:  url(../barrage.jpg); layer-background-image:  url(../barrage.jpg); border: 1px none #000000"> 
  <div align="center"><b><a href="../barrages_terr.php" target="_blank"><font color="#FFFFFF">Barrages</font></a></b></div>
</div>
<div id="Layer4" style="position:absolute; width:991px; height:538px; z-index:6; left: 481px; top: 267px; background-color: #FFFFFF; layer-background-color: #FFFFFF; border: 1px none #000000"> 
  <table width="109%" border="1">
    <tr bgcolor="#FFFFFF"> 
      <td height="55" colspan="9" bgcolor="#FFFFFF"> 
        <div align="center"><font size="5" color="#000000"><b>Nombre de clubs 
          qualifiés par comité (2014-2015)</b></font></div>
      </td>
    </tr>
    <tr> 
      <td height="83" width="18%"> 
        <div align="center"><b>COMITES</b></div>
      </td>
      <td height="83" width="11%"> 
        <div align="center"><b>Nombre de clubs qualifiés</b><b> directement 
          en 32me</b></div>
      </td>
      <td height="83" width="9%"> 
        <div align="center"><b>Nbre de clubs Barragistes</b></div>
      </td>
      <td height="83" width="10%"> 
        <div align="center"><b>Nbre de clubs dans la division</b></div>
      </td>
      <td height="36" rowspan="17" bgcolor="#003399">&nbsp;</td>
      <td height="83" width="18%"> 
        <div align="center"><b>COMITES</b></div>
      </td>
      <td height="83" width="12%"> 
        <div align="center"> 
          <p><b>Nombre de clubs qualifiés</b><b> directement en 32me</b></p>
        </div>
      </td>
      <td height="83" width="9%"> 
        <div align="center"><b>Nbre de clubs Barragistes</b></div>
      </td>
      <td height="83" width="10%" bgcolor="#FFFFFF"> 
        <div align="center"> 
          <p><b><br>
            Nbre de clubs dans la<br>
            </b><b>division</b></p>
        </div>
      </td>
    </tr>
    <tr> 
      <td height="24" width="18%">Alpes</td>
      <td height="24" width="11%"> 
        <div align="center">2</div>
      </td>
      <td height="24" width="9%"> 
        <div align="center"></div>
      </td>
      <td height="24" width="10%"> 
        <div align="center"><b><font size="4"><font size="4"><font color="#000000"></font></font></font></b></div>
      </td>
      <td width="18%" height="24" bgcolor="#FFFFFF">Franche Comté</td>
      <td width="12%" height="24"> 
        <div align="center">avec Bourgogne</div>
      </td>
      <td width="9%" height="24">&nbsp; </td>
      <td width="10%" height="24" bgcolor="#FFFFFF"> 
        <div align="center"><b><font color="#000000"><font size="4"></font></font></b></div>
      </td>
    </tr>
    <tr> 
      <td width="18%" bgcolor="#999999" height="21">Alsace</td>
      <td width="11%" bgcolor="#999999" height="21"> 
        <div align="center">2</div>
      </td>
      <td width="9%" bgcolor="#999999" height="21"> 
        <div align="center"></div>
      </td>
      <td width="10%" bgcolor="#999999" height="21"> 
        <div align="center"><font size="4" color="#000000"><b>6</b></font></div>
      </td>
      <td width="18%" bgcolor="#999999" height="21">Flandres</td>
      <td width="12%" height="21" bgcolor="#999999"> 
        <div align="center">3</div>
      </td>
      <td width="9%" height="21" bgcolor="#999999"> 
        <div align="center"></div>
      </td>
      <td width="10%" bgcolor="#999999" height="21"> 
        <div align="center"><font color="#000000" size="4"><b>5 </b></font></div>
      </td>
    </tr>
    <tr> 
      <td width="18%">Armagnac Bigorre</td>
      <td width="11%"> 
        <div align="center">2</div>
      </td>
      <td width="9%"> 
        <div align="center">1</div>
      </td>
      <td width="10%"> 
        <div align="center"><b><font size="4"><font size="4"><font color="#000000">6</font></font></font></b></div>
      </td>
      <td width="18%" bgcolor="#FFFFFF">Ile de France</td>
      <td width="12%" bgcolor="#FFFFFF"> 
        <div align="center">5</div>
      </td>
      <td width="9%" bgcolor="#FFFFFF"> 
        <div align="center">1</div>
      </td>
      <td width="10%" bgcolor="#FFFFFF"> 
        <div align="center"><font color="#000000" size="4"><b>18 </b></font></div>
      </td>
    </tr>
    <tr bgcolor="#999999"> 
      <td width="18%">Auvergne</td>
      <td width="11%"> 
        <div align="center">2</div>
      </td>
      <td width="9%"> 
        <div align="center">1</div>
      </td>
      <td width="10%"> 
        <div align="center"><font size="4" color="#000000"><b>10</b></font></div>
      </td>
      <td width="18%" bgcolor="#999999">Languedoc</td>
      <td width="12%"> 
        <div align="center">3</div>
      </td>
      <td width="9%"> 
        <div align="center"></div>
      </td>
      <td width="10%"> 
        <div align="center"><font color="#000000" size="4"><b>10 </b></font></div>
      </td>
    </tr>
    <tr> 
      <td width="18%">Bearn</td>
      <td width="11%"> 
        <div align="center">1</div>
      </td>
      <td width="9%"> 
        <div align="center">1</div>
      </td>
      <td width="10%"> 
        <div align="center"><b><font size="4"><font size="4"><font color="#000000"></font></font></font></b></div>
      </td>
      <td width="18%" bgcolor="#FFFFFF">Limousin</td>
      <td width="12%" bgcolor="#FFFFFF"> 
        <div align="center">3</div>
      </td>
      <td width="9%" bgcolor="#FFFFFF"> 
        <div align="center">1</div>
      </td>
      <td width="10%" bgcolor="#FFFFFF"> 
        <div align="center"><font color="#000000" size="4"><b>10 </b></font></div>
      </td>
    </tr>
    <tr bgcolor="#999999"> 
      <td width="18%">Bourgogne</td>
      <td width="11%"> 
        <div align="center">3</div>
      </td>
      <td width="9%"> 
        <div align="center"></div>
      </td>
      <td width="10%"> 
        <div align="center"><font size="4" color="#000000"><b>11</b></font></div>
      </td>
      <td width="18%" bgcolor="#999999">Lyonnais</td>
      <td width="12%"> 
        <div align="center">2</div>
      </td>
      <td width="9%"> 
        <div align="center">1</div>
      </td>
      <td width="10%"> 
        <div align="center"><b><font color="#000000"><font size="4"></font></font></b></div>
      </td>
    </tr>
    <tr> 
      <td width="18%">Bretagne</td>
      <td width="11%"> 
        <div align="center">3</div>
      </td>
      <td width="9%"> 
        <div align="center"></div>
      </td>
      <td width="10%"> 
        <div align="center"><b><font size="4"><font size="4"><font color="#000000"></font></font></font></b></div>
      </td>
      <td width="18%" bgcolor="#FFFFFF">Midi Pyrénées</td>
      <td width="12%" bgcolor="#FFFFFF"> 
        <div align="center">6</div>
      </td>
      <td width="9%" bgcolor="#FFFFFF"> 
        <div align="center">1</div>
      </td>
      <td width="10%" bgcolor="#FFFFFF"> 
        <div align="center"><font color="#000000" size="4"><b>18 </b></font></div>
      </td>
    </tr>
    <tr bgcolor="#999999"> 
      <td width="18%">Centre</td>
      <td width="11%"> 
        <div align="center">2</div>
      </td>
      <td width="9%"> 
        <div align="center">1</div>
      </td>
      <td width="10%"> 
        <div align="center"><font size="4" color="#000000"><b>5</b></font></div>
      </td>
      <td width="18%" bgcolor="#999999">Normandie</td>
      <td width="12%" bgcolor="#999999"> 
        <div align="center">3</div>
      </td>
      <td width="9%"> 
        <div align="center"></div>
      </td>
      <td width="10%"> 
        <div align="center"><font color="#000000" size="4"><b>5 </b></font></div>
      </td>
    </tr>
    <tr> 
      <td width="18%">Cote D&#146;Argent</td>
      <td width="11%"> 
        <div align="center">2</div>
      </td>
      <td width="9%"> 
        <div align="center">1</div>
      </td>
      <td width="10%"> 
        <div align="center"><font size="4" color="#000000"><b>6</b></font></div>
      </td>
      <td width="18%" bgcolor="#FFFFFF">Pays Catalan</td>
      <td width="12%" bgcolor="#FFFFFF"> 
        <div align="center">1</div>
      </td>
      <td width="9%" bgcolor="#FFFFFF"> 
        <div align="center">1</div>
      </td>
      <td width="10%" bgcolor="#FFFFFF"> 
        <div align="center"><b><font color="#000000"><font size="4"></font></font></b></div>
      </td>
    </tr>
    <tr bgcolor="#999999"> 
      <td width="18%">Cote D&#146;Azur </td>
      <td width="11%"> 
        <div align="center">1</div>
      </td>
      <td width="9%"> 
        <div align="center"></div>
      </td>
      <td width="10%"> 
        <div align="center"><b><font size="4"><font size="4"><font color="#000000"></font></font></font></b></div>
      </td>
      <td width="18%">Pays de Loire</td>
      <td width="12%"> 
        <div align="center">2</div>
      </td>
      <td width="9%"> 
        <div align="center"></div>
      </td>
      <td width="10%"> 
        <div align="center"><font color="#000000" size="4"><b>5 </b></font></div>
      </td>
    </tr>
    <tr> 
      <td width="18%">Corse </td>
      <td width="11%"> 
        <div align="center">avec Provence</div>
      </td>
      <td width="9%"> 
        <div align="center"></div>
      </td>
      <td width="10%"> 
        <div align="center"><b><font size="4"><font size="4"><font color="#000000"></font></font></font></b></div>
      </td>
      <td width="18%" bgcolor="#FFFFFF">Poitou Charente</td>
      <td width="12%" bgcolor="#FFFFFF"> 
        <div align="center">2</div>
      </td>
      <td width="9%" bgcolor="#FFFFFF"> 
        <div align="center"></div>
      </td>
      <td width="10%" bgcolor="#FFFFFF"> 
        <div align="center"><font color="#000000" size="4"><b>6 </b></font></div>
      </td>
    </tr>
    <tr bgcolor="#999999"> 
      <td width="18%">Cote Basque Landes</td>
      <td width="11%"> 
        <div align="center">2</div>
      </td>
      <td width="9%"> 
        <div align="center"></div>
      </td>
      <td width="10%"> 
        <div align="center"><font size="4" color="#000000"><b>6</b></font></div>
      </td>
      <td width="18%">Perigord Agenais</td>
      <td width="12%"> 
        <div align="center">3</div>
      </td>
      <td width="9%"> 
        <div align="center"></div>
      </td>
      <td width="10%"> 
        <div align="center"><font color="#000000" size="4"><b>10</b></font></div>
      </td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
      <td width="18%" height="24">Drome Ardeche</td>
      <td width="12%" height="24"> 
        <div align="center">1</div>
      </td>
      <td width="9%"> 
        <div align="center">1</div>
      </td>
      <td width="10%"> 
        <div align="center"><b><font size="4"><font size="4"><font color="#000000"></font></font></font></b></div>
      </td>
      <td width="18%"><font color="#000000">Provence</font></td>
      <td width="12%"> 
        <div align="center"><font color="#000000">2</font></div>
      </td>
      <td width="9%"> 
        <div align="center">1</div>
      </td>
      <td width="10%"> 
        <div align="center"><font color="#FFFFFF"><b><font color="#000000"><font size="4"></font></font></b></font> 
        </div>
      </td>
    </tr>
    <tr bgcolor="#999999"> 
      <td width="18%">&nbsp;</td>
      <td width="11%">&nbsp;</td>
      <td width="9%"> 
        <div align="center"></div>
      </td>
      <td width="10%"> 
        <div align="center"><b><font size="4"><font size="4"><font color="#000000"></font></font></font></b></div>
      </td>
      <td width="18%"><font color="#FFFFFF"></font></td>
      <td width="12%"><font color="#FFFFFF"></font></td>
      <td width="9%"><font color="#FFFFFF"></font></td>
      <td width="10%"><font color="#FFFFFF"></font> </td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
      <td width="18%"> 
        <div align="center"><b>Totaux I</b></div>
      </td>
      <td width="11%"> 
        <div align="center"><b>23</b></div>
      </td>
      <td width="9%"> 
        <div align="center"><b>6</b></div>
      </td>
      <td width="10%"><b> </b></td>
      <td width="18%"> 
        <div align="center"><b><font color="#000000">Total II</font></b></div>
      </td>
      <td width="12%"> 
        <div align="center"><b><font color="#000000">35</font></b></div>
      </td>
      <td width="9%"> 
        <div align="center"><b><font color="#000000">6</font></b></div>
      </td>
      <td width="10%"><b><font color="#000000"></font> </b></td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
      <td width="18%">&nbsp;</td>
      <td width="11%">&nbsp;</td>
      <td width="9%">&nbsp;</td>
      <td width="10%">&nbsp;</td>
      <td width="18%"><font color="#FFFFFF"></font></td>
      <td width="12%"><font color="#FFFFFF"></font></td>
      <td width="9%"><font color="#FFFFFF"></font></td>
      <td width="10%"><font color="#FFFFFF"></font> </td>
    </tr>
  </table>
</div>
