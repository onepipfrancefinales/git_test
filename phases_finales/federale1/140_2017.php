<?php
//$champ = $_GET['champ']; 
$id = 140; 
$annee =2017; 
$annee2 ="bdpf".''.$annee.''."_2"; 
?><head>
<link rel="stylesheet" type="text/css" href="../2.css">
<meta name="description"
content="tous les champions de france de rugby, ffr,FFR, presentation de tous les clubs de rugby francais, Champions des differents comit�s territoriaux, Tous les championnats de rugby, f�d�rale 2, f�d�rale 3, f�d�rale 1, top 14.">
<meta name="keywords"
content="Brennus,Comite Midi Pyrenees de Rugby,ffr,FFR,Comite territoriaux de rugby,Stade toulousain,Bourgogne,FanceFinalesRugby,Rugby,championnat de france de rugby,Fran�aise,honneur,promotion honneur,Sport,Ballon,Ovale,">
<meta name="classification" content="Sport,Rugby">
<meta name="resource-type" content="document">
<meta name="copyright" content="Comite Midi Pyrenees de Rugby">
<meta name="author" content="Equipe Onepip">
<meta name="robots" content="All">
<title>1re S�rie</title>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">

</head>
<?php


//require ("../fonctions_phases_finales.php");
require ("../fonctions_phases_finalesAR.php");

?>


<?php

//$clubA8001=$clubA3201;
//$clubA8002=$clubA3202;
//$clubA8003=$clubA3203;
//$clubA8004=$clubA3204;

//$clubA8005=$clubA3205;
//$clubA8006=$clubA3206;
//$clubA8007=$clubA3207;
//$clubA8008=$clubA3208;

//$clubA8009=$clubA3209;
//$clubA8010=$clubA3210;
//$clubA8011=$clubA3211;
//$clubA8012=$clubA3212;

//$clubA8013=$clubA3213;
//$clubA8014=$clubA3214;
//$clubA8015=$clubA3215;
//$clubA8016=$clubA3216;

 ?>




<table width="1500" border="0" align="left" height="500" cellspacing="0">
  <tr> 
    <td height="9" colspan="15" bgcolor="#FFCC66"> 
      <div align="center"><b><i>Fédérale 1 Jean PRAT - Fédérale 
        1 Jean PRAT - Fédérale 1 Jean PRAT - Fédérale 
        1 Jean PRAT - Fédérale 1 Jean PRAT - Fédérale 
        1 Jean PRAT </i></b></div>
    </td>
    <td height="9" bgcolor="#FFCC66" width="28">&nbsp;</td>
    <td height="9" colspan="5" bgcolor="#FF9900"> 
      <div align="center"><i><font size="4"><b>Fédérale 1</b></font></i></div>
    </td>
  </tr>
  <tr> 
    <td width="229" height="13">&nbsp;</td>
    <td width="28" height="13">&nbsp;</td>
    <td width="32" height="13">&nbsp;</td>
    <td width="28" height="13">&nbsp;</td>
    <td width="204" height="13">&nbsp;</td>
    <td width="29" height="13">&nbsp;</td>
    <td width="24" height="13">&nbsp;</td>
    <td width="26" height="13">&nbsp;</td>
    <td width="180" align="left" height="13">&nbsp;</td>
    <td width="27" height="13">&nbsp;</td>
    <td width="27" height="13">&nbsp;</td>
    <td width="21" height="13">&nbsp;</td>
    <td width="176" height="13">&nbsp;</td>
    <td width="30" height="13">&nbsp;</td>
    <td width="34" height="13">&nbsp;</td>
    <td width="28" height="13">&nbsp;</td>
    <td height="13" colspan="5">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229" height="13"> 
      <div id="coin5" >8me de finale</div>
    </td>
    <td width="28" height="13">&nbsp;</td>
    <td width="32" height="13">&nbsp;</td>
    <td width="28" height="13">&nbsp;</td>
    <td width="204" height="13"> 
      <div id="coin5" >Quarts de finale</div>
    </td>
    <td width="29" height="13">&nbsp;</td>
    <td width="24" height="13">&nbsp;</td>
    <td width="26" height="13">&nbsp;</td>
    <td width="180" align="left" height="13"> 
      <div id="coin5" >Demi finale</div>
    </td>
    <td width="27" height="13">&nbsp;</td>
    <td width="27" height="13">&nbsp;</td>
    <td width="21" height="13">&nbsp;</td>
    <td width="176" height="13"> 
      <div id="coin5" >Finale</div>
    </td>
    <td width="30" height="13">&nbsp;</td>
    <td width="34" height="13">&nbsp;</td>
    <td width="28" height="13">&nbsp;</td>
    <td height="13" colspan="5"> 
      <div id="coin5" >Accession &agrave; la Pro D2</div>
    </td>
  </tr>
  <tr> 
    <td width="229" height="22">&nbsp;</td>
    <td width="28" height="22">&nbsp;</td>
    <td width="32" height="22">&nbsp;</td>
    <td width="28" height="22">&nbsp;</td>
    <td width="204" height="22">&nbsp; </td>
    <td width="29" height="22">&nbsp;</td>
    <td width="24" height="22">&nbsp;</td>
    <td width="26" height="22">&nbsp;</td>
    <td width="180" align="left" height="22">&nbsp;</td>
    <td width="27" height="22">&nbsp;</td>
    <td width="27" height="22">&nbsp;</td>
    <td width="21" height="22">&nbsp;</td>
    <td width="176" height="22">&nbsp;</td>
    <td width="30" height="22">&nbsp;</td>
    <td width="34" height="22">&nbsp;</td>
    <td width="28" height="22">&nbsp;</td>
    <td height="22" colspan="5"> 
      <div align="center"><b></b></div>
    </td>
  </tr>
  <tr> 
    <td width="229" height="13">&nbsp;</td>
    <td width="28" height="13"> 
      <div align="center"><b><font face="Arial, Helvetica, sans-serif">Pts</font></b></div>
    </td>
    <td width="32" height="13"> 
      <div align="center"><b><font face="Arial, Helvetica, sans-serif">GA</font></b></div>
    </td>
    <td width="28" height="13">&nbsp;</td>
    <td width="204" height="13">&nbsp; </td>
    <td width="29" height="13">&nbsp;</td>
    <td width="24" height="13">&nbsp;</td>
    <td width="26" height="13">&nbsp;</td>
    <td width="180" align="left" height="13">&nbsp; </td>
    <td width="27" height="13">&nbsp;</td>
    <td width="27" height="13">&nbsp;</td>
    <td width="21" height="13">&nbsp;</td>
    <td width="176" height="13">&nbsp; </td>
    <td width="30" height="13">&nbsp;</td>
    <td width="34" height="13">&nbsp;</td>
    <td width="28" height="13" bgcolor="#FFCC66">&nbsp;</td>
    <td height="13" colspan="5"> 
      <div align="center"><b><font face="Arial, Helvetica, sans-serif" size="5">1er 
        de poule</font></b></div>
    </td>
  </tr>
  <tr> 
    <td width="229"> 
      <div id="coin4" > 
        <?php
	
	echo $clubA8001;
	 ?>
      </div>
    </td>
    <td width="28"> 
      <div id="coin4pts" > 
        <?php	echo $A1601; ?>
      </div>
    </td>
    <td width="32"> 
      <div id="coin4" > 
        <?php	echo $A8001; ?>
      </div>
    </td>
    <td rowspan="4" width="28"><img src="../../images/accolade.png" width="7" height="75"></td>
    <td width="204">&nbsp;</td>
    <td width="29" height="13"> 
      <div align="center"><b><font face="Arial, Helvetica, sans-serif">Pts</font></b></div>
    </td>
    <td width="24" height="13"> 
      <div align="center"><b><font face="Arial, Helvetica, sans-serif">GA</font></b></div>
    </td>
    <td width="26">&nbsp;</td>
    <td width="180" align="left">&nbsp; </td>
    <td width="27">&nbsp;</td>
    <td width="27">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="176">&nbsp;</td>
    <td width="30">&nbsp;</td>
    <td width="34">&nbsp;</td>
    <td width="28" bgcolor="#FFCC66">&nbsp;</td>
    <td valign="top" width="36"> </td>
    <td valign="top" colspan="3"> 
      <div align="center"><b><font face="Arial, Helvetica, sans-serif">Acc&egrave;s 
        direct en pro D2</font></b></div>
    </td>
    <td valign="top" width="24">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229" height="5"> 
      <div id="coin4" > 
        <?php
	
	echo $clubA8002;
	 ?>
      </div>
    </td>
    <td width="28" height="5"> 
      <div id="coin4pts" > 
        <?php	echo $A1602; ?>
      </div>
    </td>
    <td width="32" height="5"> 
      <div id="coin4" ><font size="2"> 
        <?php	echo $A8002; ?>
        </font></div>
    </td>
    <td width="204" height="5"> 
      <div id="coin4" > 
        <?php
	
	echo $clubA4001;
	 ?>
      </div>
    </td>
    <td width="29" height="5"> 
      <div id="coin4pts" > 
        <?php	echo $A1617; ?>
      </div>
    </td>
    <td width="24" height="5"> 
      <div id="coin4" > 
        <?php	echo $A4001; ?>
      </div>
    </td>
    <td rowspan="7" height="25" width="26"><img src="../../images/accolade.png" width="8" height="140"></td>
    <td width="180" height="5" align="left">&nbsp;</td>
    <td width="27" height="5">&nbsp;</td>
    <td width="27" height="5">&nbsp;</td>
    <td width="21" height="5">&nbsp;</td>
    <td width="176" height="5">&nbsp;</td>
    <td width="30" height="5">&nbsp;</td>
    <td width="34" height="5">&nbsp;</td>
    <td width="28" height="5" bgcolor="#FFCC66">&nbsp;</td>
    <td valign="top" width="36">&nbsp;</td>
    <td valign="top" width="217"> </td>
    <td valign="top" width="27"> </td>
    <td valign="top" width="31">&nbsp;</td>
    <td valign="top" width="24">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229" height="22"> 
      <div id="coin4" > 
        <?php
	
	echo $clubA8003;
	 ?>
      </div>
    </td>
    <td width="28" height="22"> 
      <div id="coin4pts" > 
        <?php	echo $A1603; ?>
      </div>
    </td>
    <td width="32" height="22"> 
      <div id="coin4" ><font size="2"> 
        <?php	echo $A8003; ?>
        </font></div>
    </td>
    <td width="204" height="22"> 
      <div id="coin4" > 
        <?php	echo $clubA4002; ?>
      </div>
    </td>
    <td width="29" height="22"> 
      <div id="coin4pts" > 
        <?php	echo $A1618; ?>
      </div>
    </td>
    <td width="24" height="22"> 
      <div id="coin4" > 
        <?php	echo $A4002; ?>
      </div>
    </td>
    <td width="180" align="left" height="22">&nbsp;</td>
    <td width="27" height="22">&nbsp;</td>
    <td width="27" height="22">&nbsp;</td>
    <td width="21" height="22">&nbsp;</td>
    <td width="176" height="22">&nbsp; </td>
    <td width="30" height="22">&nbsp;</td>
    <td width="34" height="22">&nbsp;</td>
    <td width="28" bgcolor="#FFCC66" height="22">&nbsp;</td>
    <td valign="top" width="36" height="22">&nbsp;</td>
    <td valign="top" colspan="3" height="22" bgcolor="#FFFF00"> 
      <div align="center"><b><font size="5" face="Arial, Helvetica, sans-serif" color="#FF0000">
        <?php
//mysql_connect("sql.franceserv.fr", "onepip-france", "lavelan09"); 
//mysql_select_db("onepip-france-db1"); 

require '../../connect1/connection1.php';
?>
        <?php //E01

$result =$bdd->query( "	SELECT bdclubs.id, bdclubs.nom_1, bdclubs.siglecomite, bdpf$annee.E00 
			FROM bdclubs, bdpf$annee 
			WHERE substr(bdclubs.id,-5)=bdpf$annee.E00 and bdpf$annee.id=130 "); 
					//$result = mysql_query($query) or die(mysql_error());
					while ($row = $result->fetch() )
	{ 
	$nom = $row[1];
	$sigle = $row[2];
	}
$nom20 = substr($nom,0,20);	
$clubB2000 = ($nom20.' '.$sigle);
echo $clubB2000;
?>
        </font></b></div>
    </td>
    <td valign="top" width="24" height="22">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229"> 
      <div id="coin4" > 
        <?php
	
	echo $clubA8004;
	 ?>
      </div>
    </td>
    <td width="28"> 
      <div id="coin4pts" > 
        <?php	echo $A1604; ?>
      </div>
    </td>
    <td width="32"> 
      <div id="coin4" ><font size="2"> 
        <?php	echo $A8004; ?>
        </font></div>
    </td>
    <td width="204">&nbsp;</td>
    <td width="29">&nbsp;</td>
    <td width="24">&nbsp;</td>
    <td width="180" align="left">&nbsp;</td>
    <td width="27" height="13"> 
      <div align="center"><b><font face="Arial, Helvetica, sans-serif">Pts</font></b></div>
    </td>
    <td width="27" height="13"> 
      <div align="center"><b><font face="Arial, Helvetica, sans-serif">GA</font></b></div>
    </td>
    <td width="21">&nbsp;</td>
    <td width="176">&nbsp;</td>
    <td width="30">&nbsp;</td>
    <td width="34">&nbsp;</td>
    <td width="28" bgcolor="#FFCC66">&nbsp;</td>
    <td valign="top" width="36">&nbsp;</td>
    <td valign="top" width="217"> </td>
    <td valign="top" width="27"> </td>
    <td valign="top" width="31">&nbsp;</td>
    <td valign="top" width="24">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229"> </td>
    <td width="28"></td>
    <td width="32"> </td>
    <td width="28">&nbsp;</td>
    <td width="204">&nbsp;</td>
    <td width="29">&nbsp;</td>
    <td width="24">&nbsp;</td>
    <td width="180"> 
      <div id="coin4" > 
        <?php	echo $clubA2001; ?>
      </div>
    </td>
    <td width="27"> 
      <div id="coin4pts" > 
        <?php	echo $A1625; ?>
      </div>
    </td>
    <td width="27"> 
      <div id="coin4" > 
        <?php	echo $A2001; ?>
      </div>
    </td>
    <td rowspan="11" width="21"><img src="../../images/accolade.png" width="10" height="220"></td>
    <td width="176">&nbsp;</td>
    <td width="30">&nbsp;</td>
    <td width="34">&nbsp;</td>
    <td width="28" bgcolor="#FFCC66">&nbsp;</td>
    <td valign="top" width="36">&nbsp;</td>
    <td valign="top" width="217"> </td>
    <td valign="top" width="27"> </td>
    <td valign="top" width="31">&nbsp;</td>
    <td valign="top" width="24">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229" height="11"> 
      <div id="coin5" > 
        <?php	echo $clubA8005; ?>
      </div>
    </td>
    <td width="28" height="11"> 
      <div id="coin5pts" > 
        <?php	echo $A1605; ?>
      </div>
    </td>
    <td width="32" height="11"> 
      <div id="coin5" ><font size="2"> 
        <?php	echo $A8005; ?>
        </font></div>
    </td>
    <td rowspan="4" width="28"><img src="../../images/accolade.png" width="7" height="75"></td>
    <td width="204" height="11">&nbsp;</td>
    <td width="29" height="11">&nbsp;</td>
    <td width="24" height="11">&nbsp;</td>
    <td width="180"> 
      <div id="coin4" > 
        <?php	echo $clubA2002; ?>
      </div>
    </td>
    <td width="27"> 
      <div id="coin4pts" > 
        <?php	echo $A1626; ?>
      </div>
    </td>
    <td width="27"> 
      <div id="coin4" > 
        <?php	echo $A2002; ?>
      </div>
    </td>
    <td width="176" height="11">&nbsp;</td>
    <td width="30" height="11">&nbsp;</td>
    <td width="34" height="11">&nbsp;</td>
    <td width="28" height="11" bgcolor="#FFCC66">&nbsp;</td>
    <td valign="top" width="36">&nbsp;</td>
    <td valign="top" colspan="3"> 
      <div align="center"><b><font face="Arial, Helvetica, sans-serif" size="5">Demi 
        finale </font></b> </div>
    </td>
    <td valign="top" width="24">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229"> 
      <div id="coin5" > 
        <?php	echo $clubA8006; ?>
      </div>
    </td>
    <td width="28"> 
      <div id="coin5pts" > 
        <?php	echo $A1606; ?>
      </div>
    </td>
    <td width="32"> 
      <div id="coin5" ><font size="2"> 
        <?php	echo $A8006; ?>
        </font></div>
    </td>
    <td width="204"> 
      <div id="coin4" > 
        <?php	echo $clubA4003; ?>
      </div>
    </td>
    <td width="29"> 
      <div id="coin4pts" > 
        <?php	echo $A1619; ?>
      </div>
    </td>
    <td width="24"> 
      <div id="coin4" > 
        <?php	echo $A4003; ?>
      </div>
    </td>
    <td width="180" align="left">&nbsp;</td>
    <td width="27">&nbsp;</td>
    <td width="27">&nbsp;</td>
    <td width="176">&nbsp;</td>
    <td width="30">&nbsp;</td>
    <td width="34">&nbsp;</td>
    <td width="28" bgcolor="#FFCC66">&nbsp;</td>
    <td valign="top" width="36">&nbsp;</td>
    <td valign="top" width="217"> </td>
    <td valign="top" width="27"> </td>
    <td valign="top" width="31"> </td>
    <td valign="top" width="24">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229" height="15"> 
      <div id="coin5" > 
        <?php	echo $clubA8007; ?>
      </div>
    </td>
    <td width="28" height="15"> 
      <div id="coin5pts" > 
        <?php	echo $A1607; ?>
      </div>
    </td>
    <td width="32" height="15"> 
      <div id="coin5" ><font size="2"> 
        <?php	echo $A8007; ?>
        </font></div>
    </td>
    <td width="204" height="15"> 
      <div id="coin4" > 
        <?php	echo $clubA4004; ?>
      </div>
    </td>
    <td width="29" height="15"> 
      <div id="coin4pts" > 
        <?php	echo $A1620; ?>
      </div>
    </td>
    <td width="24" height="15"> 
      <div id="coin4" > 
        <?php	echo $A4004; ?>
      </div>
    </td>
    <td width="180" height="15">&nbsp;</td>
    <td width="27" height="15">&nbsp;</td>
    <td width="27" height="15">&nbsp;</td>
    <td width="176" height="15">&nbsp;</td>
    <td width="30" height="15">&nbsp;</td>
    <td width="34" height="15">&nbsp;</td>
    <td width="28" height="15" bgcolor="#FFCC66">&nbsp;</td>
    <td valign="top" height="15"> 
      <div align="center"><b></b></div>
    </td>
    <td valign="top" height="15">&nbsp;</td>
    <td valign="top" height="15"><b><font face="Arial, Helvetica, sans-serif">Pts</font></b></td>
    <td valign="top" height="15"><b><font face="Arial, Helvetica, sans-serif">GA</font></b></td>
    <td valign="top" height="15">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229" height="22"> 
      <div id="coin5" > 
        <?php	echo $clubA8008; ?>
      </div>
    </td>
    <td width="28" height="22"> 
      <div id="coin5pts" > 
        <?php	echo $A1608; ?>
      </div>
    </td>
    <td width="32" height="22"> 
      <div id="coin5" ><font size="2"> 
        <?php	echo $A8008; ?>
        </font></div>
    </td>
    <td width="204" height="22">&nbsp;</td>
    <td width="29" height="22">&nbsp;</td>
    <td width="24" height="22">&nbsp;</td>
    <td width="26" height="22">&nbsp;</td>
    <td width="180" align="left" height="22">&nbsp;</td>
    <td width="27" height="22">&nbsp;</td>
    <td width="27" height="22">&nbsp;</td>
    <td width="176" height="22"> 
      <div id="coin4" > 
        <?php	echo $clubA1001; ?>
      </div>
    </td>
    <td width="30" height="22"> 
      <div id="coin4" > 
        <?php	echo $A1001; ?>
      </div>
    </td>
    <td width="34" height="22">&nbsp;</td>
    <td width="28" bgcolor="#FFCC66" height="22">&nbsp;</td>
    <td valign="top" width="36" height="22"> 
      <div align="center"></div>
    </td>
    <td valign="top" width="217" height="22"> 
      <div id="coin4" > 
        
        <?php //E01

$result =$bdd->query("SELECT bdclubs.id, bdclubs.nom_1, bdclubs.siglecomite, bdpf$annee.E01 
			FROM bdclubs, bdpf$annee 
			WHERE substr(bdclubs.id,-5)=bdpf$annee.E01 and bdpf$annee.id=130 "); 
				//	$result = mysql_query($query) or die(mysql_error());
					while ($row = $result->fetch() )
	{ 
	$nom = $row[1];
	$sigle = $row[2];
	}
$nom20 = substr($nom,0,20);	
$clubB2001 = ($nom20.' '.$sigle);
echo $clubB2001;
?>
      </div>
    </td>
    <td valign="bottom" width="27" height="22"> 
      <div id="coin4pts" >-</div>
    </td>
    <td valign="bottom" width="31" height="22"> 
      <div id="coin4" >-</div>
    </td>
    <td valign="top" width="24" height="22">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229" height="25">&nbsp;</td>
    <td width="28" height="25">&nbsp;</td>
    <td width="32" height="25">&nbsp;</td>
    <td width="28">&nbsp;</td>
    <td width="204" height="25">&nbsp;</td>
    <td width="29" height="25">&nbsp;</td>
    <td width="24" height="25">&nbsp;</td>
    <td width="26">&nbsp;</td>
    <td width="180" height="25" align="left">&nbsp;</td>
    <td width="27" height="25">&nbsp;</td>
    <td width="27" height="25">&nbsp;</td>
    <td width="176"> 
      <div id="coin4" > 
        <?php	echo $clubA1002; ?>
      </div>
    </td>
    <td width="30"> 
      <div id="coin4" > 
        <?php	echo $A1002; ?>
      </div>
    </td>
    <td width="34" height="25">&nbsp;</td>
    <td width="28" height="25" bgcolor="#FFCC66">&nbsp;</td>
    <td valign="top" width="36">&nbsp;</td>
    <td valign="top" width="217"> 
      <div id="coin4" >
        <?php
//mysql_connect("sql.franceserv.fr", "onepip-france", "lavelan09"); 
//mysql_select_db("onepip-france-db1"); 
?>
        <?php //E01

$result =$bdd->query("	SELECT bdclubs.id, bdclubs.nom_1, bdclubs.siglecomite, bdpf$annee.E02
			FROM bdclubs, bdpf$annee 
			WHERE substr(bdclubs.id,-5)=bdpf$annee.E02 and bdpf$annee.id=130 "); 
				//	$result = mysql_query($query) or die(mysql_error());
					while ($row = $result->fetch() )
	{ 
	$nom = $row[1];
	$sigle = $row[2];
	}
$nom20 = substr($nom,0,20);	
$clubB2002 = ($nom20.' '.$sigle);
echo $clubB2002;
?>
      </div>
    </td>
    <td valign="top" width="27"> 
      <div id="coin4pts" >-</div>
    </td>
    <td valign="top" width="31"> 
      <div id="coin4" >-</div>
    </td>
    <td valign="top" width="24">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229"> 
      <div id="coin4" > 
        <?php	echo $clubA8009; ?>
      </div>
    </td>
    <td width="28"> 
      <div id="coin4pts" > 
        <?php	echo $A1609; ?>
      </div>
    </td>
    <td width="32"> 
      <div id="coin4" ><font size="2"> 
        <?php	echo $A8009; ?>
        </font></div>
    </td>
    <td rowspan="4" width="28"><img src="../../images/accolade.png" width="7" height="75"></td>
    <td width="204">&nbsp;</td>
    <td width="29">&nbsp;</td>
    <td width="24">&nbsp;</td>
    <td width="26">&nbsp;</td>
    <td width="180" align="left">&nbsp;</td>
    <td width="27">&nbsp;</td>
    <td width="27">&nbsp;</td>
    <td width="176">&nbsp;</td>
    <td width="30">&nbsp;</td>
    <td width="34">&nbsp;</td>
    <td width="28" bgcolor="#FFCC66">&nbsp;</td>
    <td valign="top" width="36">&nbsp;</td>
    <td valign="top" width="217">&nbsp;</td>
    <td valign="top" width="27"><b><font face="Arial, Helvetica, sans-serif">Pts</font></b></td>
    <td valign="top" width="31"><b><font face="Arial, Helvetica, sans-serif">GA</font></b></td>
    <td valign="top" width="24">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229"> 
      <div id="coin4" > 
        <?php	echo $clubA8010; ?>
      </div>
    </td>
    <td width="28"> 
      <div id="coin4pts" > 
        <?php	echo $A1610; ?>
      </div>
    </td>
    <td width="32"> 
      <div id="coin4" ><font size="2"> 
        <?php	echo $A8010; ?>
        </font></div>
    </td>
    <td width="204"> 
      <div id="coin4" > 
        <?php	echo $clubA4005; ?>
      </div>
    </td>
    <td width="29"> 
      <div id="coin4pts" > 
        <?php	echo $A1621; ?>
      </div>
    </td>
    <td width="24"> 
      <div id="coin4" > 
        <?php	echo $A4005; ?>
      </div>
    </td>
    <td rowspan="7" width="26">&nbsp;</td>
    <td width="180" align="left">&nbsp;</td>
    <td width="27">&nbsp;</td>
    <td width="27">&nbsp;</td>
    <td width="176">&nbsp;</td>
    <td width="30">&nbsp;</td>
    <td width="34">&nbsp;</td>
    <td width="28" bgcolor="#FFCC66">&nbsp;</td>
    <td valign="top" width="36">&nbsp;</td>
    <td valign="top" width="217"> 
      <div id="coin4" > 
        <?php
//mysql_connect("sql.franceserv.fr", "onepip-france", "lavelan09"); 
//mysql_select_db("onepip-france-db1"); 
?>
        <?php //E01

$result =$bdd->query("SELECT bdclubs.id, bdclubs.nom_1, bdclubs.siglecomite, bdpf$annee.E03
			FROM bdclubs, bdpf$annee 
			WHERE substr(bdclubs.id,-5)=bdpf$annee.E03 and bdpf$annee.id=130 "); 
				//	$result = mysql_query($query) or die(mysql_error());
					while ($row = $result->fetch() )
	{ 
	$nom = $row[1];
	$sigle = $row[2];
	}
$nom20 = substr($nom,0,20);	
$clubB2003 = ($nom20.' '.$sigle);
echo $clubB2003;
?>
      </div>
    </td>
    <td valign="bottom" width="27"> 
      <div id="coin4pts" >-</div>
    </td>
    <td valign="bottom" width="31"> 
      <div id="coin4" >-</div>
    </td>
    <td valign="top" width="24">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229" height="20"> 
      <div id="coin4" > 
        <?php	echo $clubA8011; ?>
      </div>
    </td>
    <td width="28" height="20"> 
      <div id="coin4pts" > 
        <?php	echo $A1611; ?>
      </div>
    </td>
    <td width="32" height="20"> 
      <div id="coin4" ><font size="2"> 
        <?php	echo $A8011; ?>
        </font></div>
    </td>
    <td width="204" height="20"> 
      <div id="coin4" > 
        <?php	echo $clubA4006; ?>
      </div>
    </td>
    <td width="29" height="20"> 
      <div id="coin4pts" > 
        <?php	echo $A1622; ?>
      </div>
    </td>
    <td width="24" height="20"> 
      <div id="coin4" > 
        <?php	echo $A4006; ?>
      </div>
    </td>
    <td width="180" align="left" height="20">&nbsp;</td>
    <td width="27" height="20">&nbsp;</td>
    <td width="27" height="20">&nbsp;</td>
    <td width="176" height="20">&nbsp;</td>
    <td width="30" height="20">&nbsp;</td>
    <td width="34" height="20">&nbsp;</td>
    <td width="28" height="20" bgcolor="#FFCC66">&nbsp;</td>
    <td valign="top" width="36" height="20">&nbsp;</td>
    <td valign="top" width="217" height="20"> 
      <div id="coin4" > 
        <?php
//mysql_connect("sql.franceserv.fr", "onepip-france", "lavelan09"); 
//mysql_select_db("onepip-france-db1"); 
?>
        <?php //E01

$result =$bdd->query("	SELECT bdclubs.id, bdclubs.nom_1, bdclubs.siglecomite, bdpf$annee.E04
			FROM bdclubs, bdpf$annee 
			WHERE substr(bdclubs.id,-5)=bdpf$annee.E04 and bdpf$annee.id=130 "); 
				//	$result = mysql_query($query) or die(mysql_error());
					while ($row = $result->fetch() )
	{ 
	$nom = $row[1];
	$sigle = $row[2];
	}
$nom20 = substr($nom,0,20);	
$clubB2004 = ($nom20.' '.$sigle);
echo $clubB2004;
?>
      </div>
    </td>
    <td valign="top" width="27" height="20"> 
      <div id="coin4pts" >-</div>
    </td>
    <td valign="top" width="31" height="20"> 
      <div id="coin4" >- </div>
    </td>
    <td valign="top" width="24" height="20">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229" height="11"> 
      <div id="coin4" > 
        <?php	echo $clubA8012; ?>
      </div>
    </td>
    <td width="28" height="11"> 
      <div id="coin4pts" > 
        <?php	echo $A1612; ?>
      </div>
    </td>
    <td width="32" height="11"> 
      <div id="coin4" ><font size="2"> 
        <?php	echo $A8012; ?>
        </font></div>
    </td>
    <td width="204" height="11">&nbsp;</td>
    <td width="29" height="11">&nbsp;</td>
    <td width="24" height="11">&nbsp;</td>
    <td width="180" valign="bottom"> 
      <div id="coin4" > 
        <?php	echo $clubA2003; ?>
      </div>
    </td>
    <td width="27" valign="bottom"> 
      <div id="coin4pts" > 
        <?php	echo $A1627; ?>
      </div>
    </td>
    <td width="27" valign="bottom"> 
      <div id="coin4" > 
        <?php	echo $A2003; ?>
      </div>
    </td>
    <td width="176" height="11">&nbsp;</td>
    <td width="30" height="11">&nbsp;</td>
    <td width="34" height="11">&nbsp;</td>
    <td width="28" height="11" bgcolor="#FFCC66">&nbsp;</td>
    <td valign="top" width="36">&nbsp;</td>
    <td valign="top" width="217">&nbsp;</td>
    <td valign="top" width="27">&nbsp;</td>
    <td valign="top" width="31">&nbsp;</td>
    <td valign="top" width="24">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229" height="16">&nbsp;</td>
    <td width="28" height="16">&nbsp;</td>
    <td width="32" height="16">&nbsp;</td>
    <td width="28" height="16">&nbsp;</td>
    <td width="204" height="16">&nbsp;</td>
    <td width="29" height="16">&nbsp;</td>
    <td width="24" height="16">&nbsp;</td>
    <td width="180" height="16" valign="middle"> 
      <div id="coin4" > 
        <?php	echo $clubA2004; ?>
      </div>
    </td>
    <td width="27" height="16" valign="middle"> 
      <div id="coin4pts" > 
        <?php	echo $A1628; ?>
      </div>
    </td>
    <td width="27" height="16" valign="middle"> 
      <div id="coin4" > 
        <?php	echo $A2004; ?>
      </div>
    </td>
    <td width="176" height="16">&nbsp;</td>
    <td width="30" height="16">&nbsp;</td>
    <td width="34" height="16">&nbsp;</td>
    <td width="28" bgcolor="#FFCC66" height="16">&nbsp;</td>
    <td valign="top" colspan="5" height="16"> 
      <div align="center"><b></b></div>
      <div align="center"><b><font face="Arial, Helvetica, sans-serif" size="5">Finale</font></b></div>
    </td>
  </tr>
  <tr> 
    <td width="229" height="20"> 
      <div id="coin5" > 
        <?php	echo $clubA8013; ?>
      </div>
    </td>
    <td width="28" height="20"> 
      <div id="coin5pts" > 
        <?php	echo $A1613; ?>
      </div>
    </td>
    <td width="32" height="20"> 
      <div id="coin5" ><font size="2"> 
        <?php	echo $A8013; ?>
        </font></div>
    </td>
    <td rowspan="4" width="28"><img src="../../images/accolade.png" width="7" height="75"></td>
    <td width="204" height="20">&nbsp;</td>
    <td width="29" height="20">&nbsp;</td>
    <td width="24" height="20">&nbsp;</td>
    <td width="180" align="left" height="20">&nbsp;</td>
    <td width="27" height="20">&nbsp;</td>
    <td width="27" height="20">&nbsp;</td>
    <td height="20" width="21">&nbsp;</td>
    <td width="176" height="20">&nbsp;</td>
    <td width="30" height="20">&nbsp;</td>
    <td width="34" height="20">&nbsp;</td>
    <td width="28" height="20" bgcolor="#FFCC66">&nbsp;</td>
    <td valign="top" width="36" height="20">&nbsp;</td>
    <td valign="bottom" width="217" height="20"> 
      
    </td>
    <td valign="bottom" width="27" height="20"><b><font face="Arial, Helvetica, sans-serif">Pts</font></b> 
    </td>
    <td valign="bottom" width="31" height="20"><b><font face="Arial, Helvetica, sans-serif">GA</font></b> 
    </td>
    <td valign="top" width="24" height="20">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229" height="5"> 
      <div id="coin5" > 
        <?php	echo $clubA8014; ?>
      </div>
    </td>
    <td width="28" height="5"> 
      <div id="coin5pts" > 
        <?php	echo $A1614; ?>
      </div>
    </td>
    <td width="32" height="5"> 
      <div id="coin5" ><font size="2"> 
        <?php	echo $A8014; ?>
        </font></div>
    </td>
    <td width="204" height="5"> 
      <div id="coin4" > 
        <?php	echo $clubA4007; ?>
      </div>
    </td>
    <td width="29" height="5"> 
      <div id="coin4pts" > 
        <?php	echo $A1623; ?>
      </div>
    </td>
    <td width="24" height="5"> 
      <div id="coin4" > 
        <?php	echo $A4007; ?>
      </div>
    </td>
    <td width="180" align="left" height="5">&nbsp;</td>
    <td width="27" height="5">&nbsp;</td>
    <td width="27" height="5">&nbsp;</td>
    <td height="5" width="21">&nbsp;</td>
    <td width="176" height="5">&nbsp;</td>
    <td width="30" height="5">&nbsp;</td>
    <td width="34" height="5">&nbsp;</td>
    <td width="28" height="5" bgcolor="#FFCC66">&nbsp;</td>
    <td valign="top" width="36" height="20">&nbsp;</td>
    <td valign="top" width="217" height="20"> 
      <div id="coin4" >-</div>
    </td>
    <td valign="top" width="27" height="20"> 
      <div id="coin4pts" >- </div>
    </td>
    <td valign="top" width="31" height="20">  
      <div id="coin4" >- </div>
    </td>
    <td valign="top" width="24" height="20">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229" height="2"> 
      <div id="coin5" > 
        <?php	echo $clubA8015; ?>
      </div>
    </td>
    <td width="28" height="2"> 
      <div id="coin5pts" > 
        <?php	echo $A1615; ?>
      </div>
    </td>
    <td width="32" height="2"> 
      <div id="coin5" ><font size="2"> 
        <?php	echo $A8015; ?>
        </font></div>
    </td>
    <td width="204" height="2"> 
      <div id="coin4" > 
        <?php	echo $clubA4008; ?>
      </div>
    </td>
    <td width="29" height="2"> 
      <div id="coin4pts" > 
        <?php	echo $A1624; ?>
      </div>
    </td>
    <td width="24" height="2"> 
      <div id="coin4" > 
        <?php	echo $A4008; ?>
      </div>
    </td>
    <td width="180" height="2" align="left">&nbsp;</td>
    <td width="27" height="2">&nbsp;</td>
    <td width="27" height="2">&nbsp;</td>
    <td height="2" width="21">&nbsp;</td>
    <td width="176" height="2">&nbsp;</td>
    <td width="30" height="2">&nbsp;</td>
    <td width="34" height="2">&nbsp;</td>
    <td width="28" height="2" bgcolor="#FFCC66">&nbsp;</td>
    <td valign="top" height="2">&nbsp;</td>
    <td valign="top" height="2"> 
      <div id="coin4" >- </div>
    </td>
    <td valign="top" height="2"> 
      <div id="coin4pts" >- </div>
    </td>
    <td valign="top" height="2">  
      <div id="coin4" >- </div>
    </td>
    <td valign="top" height="2">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229" height="3"> 
      <div id="coin5" > 
        <?php	echo $clubA8016; ?>
      </div>
    </td>
    <td width="28" height="3"> 
      <div id="coin5pts" > 
        <?php	echo $A1616; ?>
      </div>
    </td>
    <td width="32" height="3"> 
      <div id="coin5" ><font size="2"> 
        <?php	echo $A8016; ?>
        </font></div>
    </td>
    <td width="204" height="3">&nbsp;</td>
    <td width="29" height="3">&nbsp;</td>
    <td width="24" height="3">&nbsp;</td>
    <td height="3" width="26">&nbsp;</td>
    <td width="180" align="left" height="3">&nbsp;</td>
    <td width="27" height="3">&nbsp;</td>
    <td width="27" height="3">&nbsp;</td>
    <td height="3" width="21">&nbsp;</td>
    <td width="176" height="3">&nbsp;</td>
    <td width="30" height="3">&nbsp;</td>
    <td width="34" height="3">&nbsp;</td>
    <td width="28" height="3" bgcolor="#FFCC66">&nbsp;</td>
    <td valign="top" height="3" colspan="5">&nbsp;</td>
  </tr>
  <tr> 
    <td width="229" height="17">&nbsp;</td>
    <td width="28" height="17">&nbsp;</td>
    <td width="32" height="17">&nbsp;</td>
    <td width="28">&nbsp;</td>
    <td width="204" height="17">&nbsp;</td>
    <td width="29" height="17">&nbsp;</td>
    <td width="24" height="17">&nbsp;</td>
    <td width="26">&nbsp;</td>
    <td width="180" align="left" height="17">&nbsp;</td>
    <td width="27" height="17">&nbsp;</td>
    <td width="27" height="17">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="176" height="17">&nbsp;</td>
    <td width="30" height="17">&nbsp;</td>
    <td width="34" height="17">&nbsp;</td>
    <td width="28" height="17">&nbsp;</td>
    <td height="17" colspan="5">&nbsp;</td>
  </tr>
</table>
