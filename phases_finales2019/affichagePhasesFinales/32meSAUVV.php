
<table  border="0" align="left"  cellspacing="0">
  <tr> 
    <td colspan="18" height="13" bgcolor="#FFCC66"> 
      <div align="center"><i><b> </b></i></div>
    </td>
  </tr>
  <tr> 
    <td width="399" height="13"> 
      <div id="cal"><a href="../affichagePhasesFinales/pagebarrages.php?annee=<?php echo $annee;?>&division=990">Barrages 
        s&eacute;ries Territoriales</a></div>
    </td>
    <td width="65" height="13">&nbsp;</td>
    <td width="14" height="13">&nbsp;</td>
    <td width="190" height="13">&nbsp;</td>
    <td width="36" height="13">&nbsp;</td>
    <td width="14" height="13">&nbsp;</td>
    <td width="198" height="13">&nbsp;</td>
    <td width="41" height="13">&nbsp;</td>
    <td width="22" height="13">&nbsp;</td>
    <td width="201" height="13">&nbsp;</td>
    <td width="31" height="13">&nbsp;</td>
    <td width="18" height="13">&nbsp;</td>
    <td width="201" align="left" height="13">&nbsp;</td>
    <td width="23" height="13">&nbsp;</td>
    <td width="11" height="13">&nbsp;</td>
    <td width="198" height="13">&nbsp;</td>
    <td width="21" height="13">&nbsp;</td>
    <td width="21" height="13">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399" height="13"> 
      <div class="coin5" >32me de finale</div>
    </td>
    <td width="65" height="13">&nbsp;</td>
    <td width="14" height="13">&nbsp;</td>
    <td width="190" height="13"> 
      <div class="coin5" >16me de finale</div>
    </td>
    <td width="36" height="13">&nbsp;</td>
    <td width="14" height="13">&nbsp;</td>
    <td width="198" height="13"> 
      <div class="coin5" >8me de finale</div>
    </td>
    <td width="41" height="13">&nbsp;</td>
    <td width="22" height="13">&nbsp;</td>
    <td width="201" height="13"> 
      <div class="coin5" >Quarts de finale</div>
    </td>
    <td width="31" height="13">&nbsp;</td>
    <td width="18" height="13">&nbsp;</td>
    <td width="201" align="left" height="13"> 
      <div class="coin5" >Demi finale</div>
    </td>
    <td width="23" height="13">&nbsp;</td>
    <td width="11" height="13">&nbsp;</td>
    <td width="198" height="13"> 
      <div class="coin5" >Finale</div>
    </td>
    <td width="21" height="13">&nbsp;</td>
    <td width="21" height="13">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399" height="13">&nbsp;</td>
    <td width="65" height="13">&nbsp;</td>
    <td width="14" height="13">&nbsp;</td>
    <td width="190" height="13">&nbsp;</td>
    <td width="36" height="13">&nbsp;</td>
    <td width="14" height="13">&nbsp;</td>
    <td width="198" height="13">&nbsp;</td>
    <td width="41" height="13">&nbsp;</td>
    <td width="22" height="13">&nbsp;</td>
    <td width="201" height="13">&nbsp;</td>
    <td width="31" height="13">&nbsp;</td>
    <td width="18" height="13">&nbsp;</td>
    <td width="201" align="left" height="13">&nbsp;</td>
    <td width="23" height="13">&nbsp;</td>
    <td width="11" height="13">&nbsp;</td>
    <td width="198" height="13">&nbsp;</td>
    <td width="21" height="13">&nbsp;</td>
    <td width="21" height="13">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399" height="13"> 
      <div class="coin4" > 
        <?php	echo $clubA3201; ?>
        - 
        <?php	echo $clubA3202; ?>
      </div>
    </td>
    <td width="65" height="13"> 
      <div class="coin4" > 
        <?php  echo $A3201; echo " - ";echo $A3202;  ?>
      </div>
    </td>
    <td width="14" height="13" valign="middle" align="center">&nbsp;</td>
    <td width="190" height="13"> 
      <div class="coin4" > 
        <?php
	
	echo $clubA1601;
	 ?>
      </div>
    </td>
    <td width="36" height="13"> 
      <div class="coin4" > 
        <?php	echo $A1601; ?>
      </div>
    </td>
    <td rowspan="4" height="38" width="14"><img src="../../images/accolade.png" width="8" height="82"> 
    </td>
    <td width="198" height="13">&nbsp;</td>
    <td width="41" height="13">&nbsp;</td>
    <td width="22" height="13">&nbsp;</td>
    <td width="201" height="13">&nbsp;</td>
    <td width="31" height="13">&nbsp;</td>
    <td width="18" height="13">&nbsp;</td>
    <td width="201" align="left" height="13">&nbsp;</td>
    <td width="23" height="13">&nbsp;</td>
    <td width="11" height="13">&nbsp;</td>
    <td width="198" height="13">&nbsp;</td>
    <td width="21" height="13">&nbsp;</td>
    <td width="21" height="13">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin4" > 
        <?php	echo $clubA3203; ?>
        - 
        <?php	echo $clubA3204; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin4" > 
      
        <?php echo $A3203; echo " - ";echo $A3204; 
        
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin4" > 
        <?php
	
	echo $clubA1602;
	 ?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin4" > 
        <?php	echo $A1602; ?>
      </div>
    </td>
    <td width="198"> 
      <div class="coin4" > 
        <?php
	
	echo $clubA8001;
	 ?>
      </div>
    </td>
    <td width="41"> 
      <div class="coin4" > 
        <?php	echo $A8001; ?>
      </div>
    </td>
    <td rowspan="6" width="22"><img src="../../images/accolade.png" width="8" height="127"></td>
    <td width="201">&nbsp;</td>
    <td width="31">&nbsp;</td>
    <td width="18">&nbsp;</td>
    <td width="201" align="left">&nbsp;</td>
    <td width="23">&nbsp;</td>
    <td width="11">&nbsp;</td>
    <td width="198">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399" height="25"> 
      <div class="coin4" > 
        <?php	echo $clubA3205; ?>
        - 
        <?php	echo $clubA3206; ?>
      </div>
    </td>
    <td width="65" height="25"> 
      <div class="coin4" > 
        
        <?php echo $A3205; echo " - ";echo $A3206; 
       
        ?>
      </div>
    </td>
    <td width="14" height="25">&nbsp;</td>
    <td width="190" height="25"> 
      <div class="coin4" > 
        <?php
	
	echo $clubA1603;
	?>
      </div>
    </td>
    <td width="36" height="25"> 
      <div class="coin4" > 
        <?php	echo $A1603; ?>
      </div>
    </td>
    <td width="198" height="25"> 
      <div class="coin4" > 
        <?php
	
	echo $clubA8002;
	 ?>
      </div>
    </td>
    <td width="41" height="25"> 
      <div class="coin4" > 
        <?php	echo $A8002; ?>
      </div>
    </td>
    <td width="201" height="25">&nbsp;</td>
    <td width="31" height="25">&nbsp;</td>
    <td width="18" height="25">&nbsp;</td>
    <td width="201" height="25" align="left">&nbsp;</td>
    <td width="23" height="25">&nbsp;</td>
    <td width="11" height="25">&nbsp;</td>
    <td width="198" height="25">&nbsp;</td>
    <td width="21" height="25">&nbsp;</td>
    <td width="21" height="25">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399" height="7"> 
      <div class="coin4"> 
        <?php	echo $clubA3207; ?>
        - 
        <?php	echo $clubA3208; ?>
      </div>
    </td>
    <td width="65" height="7"> 
      <div class="coin4" > 
       
        <?php echo $A3207; echo " - ";echo $A3208; 
      
        ?>
      </div>
    </td>
    <td width="14" height="7">&nbsp;</td>
    <td width="190" height="7"> 
      <div class="coin4" > 
        <?php

	echo $clubA1604;
	?>
      </div>
    </td>
    <td width="36" height="7"> 
      <div class="coin4" > 
        <?php	echo $A1604; ?>
      </div>
    </td>
    <td width="198" height="7"> </td>
    <td width="41" height="7"> </td>
    <td width="201" height="7"> 
      <div class="coin4" > 
        <?php	echo $clubA4001; ?>
      </div>
    </td>
    <td width="31" height="7"> 
      <div class="coin4" > 
        <?php	echo $A4001; ?>
      </div>
    </td>
    <td rowspan="10" height="43" width="18"><img src="../../images/accolade.png" width="10" height="207"></td>
    <td width="201" align="left" height="7">&nbsp;</td>
    <td width="23" height="7">&nbsp;</td>
    <td width="11" height="7">&nbsp;</td>
    <td width="198" height="7">&nbsp;</td>
    <td width="21" height="7">&nbsp;</td>
    <td width="21" height="7">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin5" > 
        <?php	echo $clubA3209; ?>
        - 
        <?php	echo $clubA3210; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin5" > 
       
        <?php echo $A3209; echo " - ";echo $A3210; ?>
       
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin5" > 
        <?php
	
	echo $clubA1605;
	?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin5" >
        <?php	echo $A1605; ?>
      </div>
    </td>
    <td rowspan="4" width="14"><img src="../../images/accolade.png" width="8" height="82"></td>
    <td width="198">&nbsp; </td>
    <td width="41">&nbsp;</td>
    <td width="201"> 
      <div class="coin4" > 
        <?php	echo $clubA4002; ?>
      </div>
    </td>
    <td width="31"> 
      <div class="coin4" > 
        <?php	echo $A4002; ?>
      </div>
    </td>
    <td width="201" align="left">&nbsp;</td>
    <td width="23">&nbsp;</td>
    <td width="11">&nbsp;</td>
    <td width="198">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin5" > 
        <?php	echo $clubA3211; ?>
        - 
        <?php	echo $clubA3212; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin5" > 
        
        <?php echo $A3211; echo " - ";echo $A3212; 
       
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin5" > 
        <?php
	
	echo $clubA1606;
	?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin5" > 
        <?php	echo $A1606; ?>
      </div>
    </td>
    <td width="198"> 
      <div class="coin4" > 
        <?php
	
	echo $clubA8003;
	 ?>
      </div>
    </td>
    <td width="41"> 
      <div class="coin4" > 
        <?php	echo $A8003; ?>
      </div>
    </td>
    <td width="201">&nbsp;</td>
    <td width="31">&nbsp;</td>
    <td width="201" align="left">&nbsp;</td>
    <td width="23">&nbsp;</td>
    <td width="11">&nbsp;</td>
    <td width="198">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399" height="11"> 
      <div class="coin5" > 
        <?php	echo $clubA3213; ?>
        - 
        <?php	echo $clubA3214; ?>
      </div>
    </td>
    <td width="65" height="11"> 
      <div class="coin5" > 
       
		
        <?php echo $A3213; echo " - ";echo $A3214; 
        
        ?>
      </div>
    </td>
    <td width="14" height="11">&nbsp;</td>
    <td width="190" height="11"> 
      <div class="coin5" > 
        <?php
	
	echo $clubA1607;
	 ?>
      </div>
    </td>
    <td width="36" height="11"> 
      <div class="coin5" > 
        <?php	echo $A1607; ?>
      </div>
    </td>
    <td width="198" height="11"> 
      <div class="coin4" > 
        <?php
	
	echo $clubA8004;
	 ?>
      </div>
    </td>
    <td width="41" height="11"> 
      <div class="coin4" > 
        <?php	echo $A8004; ?>
      </div>
    </td>
    <td width="201" height="11">&nbsp;</td>
    <td width="31" height="11">&nbsp;</td>
    <td width="201" height="11" align="left">&nbsp;</td>
    <td width="23" height="11">&nbsp;</td>
    <td width="11" height="11">&nbsp;</td>
    <td width="198" height="11">&nbsp;</td>
    <td width="21" height="11">&nbsp;</td>
    <td width="21" height="11">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin5" > 
        <?php	echo $clubA3215; ?>
        - 
        <?php	echo $clubA3216; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin5"> 
        
        <?php echo $A3215; echo " - ";echo $A3216; 
       
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin5" > 
        <?php
	
	echo $clubA1608;
	 ?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin5" > 
        <?php	echo $A1608; ?>
      </div>
    </td>
    <td width="198">&nbsp;</td>
    <td width="41">&nbsp;</td>
    <td width="22">&nbsp;</td>
    <td width="201">&nbsp;</td>
    <td width="31">&nbsp;</td>
    <td width="201" align="left"> 
      <div class="coin4" > 
        <?php	echo $clubA2001; ?>
      </div>
    </td>
    <td width="23"> 
      <div class="coin4" > 
        <?php	echo $A2001; ?>
      </div>
    </td>
    <td rowspan="18" width="11"><img src="../../images/accolade.png" width="10" height="383"></td>
    <td width="198">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin4" > 
        <?php	echo $clubA3217; ?>
        - 
        <?php	echo $clubA3218; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin4" > 
        
        <?php echo $A3217; echo " - ";echo $A3218; 
       
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin4" > 
        <?php	echo $clubA1609; ?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin4" > 
        <?php	echo $A1609; ?>
      </div>
    </td>
    <td rowspan="4" width="14"><img src="../../images/accolade.png" width="8" height="82"> 
    </td>
    <td width="198">&nbsp;</td>
    <td width="41">&nbsp;</td>
    <td width="22">&nbsp;</td>
    <td width="201">&nbsp;</td>
    <td width="31">&nbsp;</td>
    <td width="201"> 
      <div class="coin4" > 
        <?php	echo $clubA2002; ?>
      </div>
    </td>
    <td width="23"> 
      <div class="coin4" > 
        <?php	echo $A2002; ?>
      </div>
    </td>
    <td width="198">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin4" > 
        <?php	echo $clubA3219; ?>
        - 
        <?php	echo $clubA3220; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin4" > 
       
        <?php echo $A3219; echo " - ";echo $A3220; 
       
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin4" > 
        <?php	echo $clubA1610; ?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin4" > 
        <?php	echo $A1610; ?>
      </div>
    </td>
    <td width="198"> 
      <div class="coin4" > 
        <?php	echo $clubA8005; ?>
      </div>
    </td>
    <td width="41"> 
      <div class="coin4" > 
        <?php	echo $A8005; ?>
      </div>
    </td>
    <td rowspan="6" width="22"><img src="../../images/accolade.png" width="8" height="127"></td>
    <td width="201">&nbsp;</td>
    <td width="31">&nbsp;</td>
    <td width="201" align="left">&nbsp;</td>
    <td width="23">&nbsp;</td>
    <td width="198">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399" height="25"> 
      <div class="coin4" > 
        <?php	echo $clubA3221; ?>
        - 
        <?php	echo $clubA3222; ?>
      </div>
    </td>
    <td width="65" height="25"> 
      <div class="coin4" > 
       
        <?php echo $A3221; echo " - ";echo $A3222; 
        
        ?>
      </div>
    </td>
    <td width="14" height="25">&nbsp;</td>
    <td width="190" height="25"> 
      <div class="coin4" > 
        <?php	echo $clubA1611; ?>
      </div>
    </td>
    <td width="36" height="25"> 
      <div class="coin4" > 
        <?php	echo $A1611; ?>
      </div>
    </td>
    <td width="198" height="25"> 
      <div class="coin4" > 
        <?php	echo $clubA8006; ?>
      </div>
    </td>
    <td width="41" height="25"> 
      <div class="coin4" > 
        <?php	echo $A8006; ?>
      </div>
    </td>
    <td width="201" height="25">&nbsp;</td>
    <td width="31" height="25">&nbsp;</td>
    <td width="201" height="25" align="left">&nbsp;</td>
    <td width="23" height="25">&nbsp;</td>
    <td width="198" height="25">&nbsp;</td>
    <td width="21" height="25">&nbsp;</td>
    <td width="21" height="25">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin4"> 
        <?php	echo $clubA3223; ?>
        - 
        <?php	echo $clubA3224; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin4" > 
       
        <?php echo $A3223; echo " - ";echo $A3224; 
       
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin4" > 
        <?php	echo $clubA1612; ?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin4" > 
        <?php	echo $A1612; ?>
      </div>
    </td>
    <td width="198"> </td>
    <td width="41"> </td>
    <td width="201"> 
      <div class="coin4" > 
        <?php	echo $clubA4003; ?>
      </div>
    </td>
    <td width="31"> 
      <div class="coin4" > 
        <?php	echo $A4003; ?>
      </div>
    </td>
    <td width="201" align="left">&nbsp;</td>
    <td width="23">&nbsp;</td>
    <td width="198">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin5" > 
        <?php	echo $clubA3225; ?>
        - 
        <?php	echo $clubA3226; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin5" > 
      
        <?php echo $A3225; echo " - ";echo $A3226; 
       
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin5" > 
        <?php	echo $clubA1613; ?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin5" > 
        <?php	echo $A1613; ?>
      </div>
    </td>
    <td rowspan="4" width="14"><img src="../../images/accolade.png" width="8" height="82"></td>
    <td width="198">&nbsp;</td>
    <td width="41">&nbsp;</td>
    <td width="201"> 
      <div class="coin4" > 
        <?php	echo $clubA4004; ?>
      </div>
    </td>
    <td width="31"> 
      <div class="coin4" > 
        <?php	echo $A4004; ?>
      </div>
    </td>
    <td width="201" align="left">&nbsp;</td>
    <td width="23">&nbsp;</td>
    <td width="198">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin5" > 
        <?php	echo $clubA3227; ?>
        - 
        <?php	echo $clubA3228; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin5" > 
       
        <?php echo $A3227; echo " - ";echo $A3228; 
       
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin5" > 
        <?php	echo $clubA1614; ?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin5" > 
        <?php	echo $A1614; ?>
      </div>
    </td>
    <td width="198"> 
      <div class="coin4" > 
        <?php	echo $clubA8007; ?>
      </div>
    </td>
    <td width="41"> 
      <div class="coin4" > 
        <?php	echo $A8007; ?>
      </div>
    </td>
    <td width="201">&nbsp;</td>
    <td width="31">&nbsp;</td>
    <td width="18">&nbsp;</td>
    <td width="201" align="left">&nbsp;</td>
    <td width="23">&nbsp;</td>
    <td width="198">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399" height="11"> 
      <div class="coin5" > 
        <?php	echo $clubA3229; ?>
        - 
        <?php	echo $clubA3230; ?>
      </div>
    </td>
    <td width="65" height="11"> 
      <div class="coin5" > 
      
        <?php echo $A3229; echo " - ";echo $A3230; 
       
        ?>
      </div>
    </td>
    <td width="14" height="11">&nbsp;</td>
    <td width="190" height="11"> 
      <div class="coin5" > 
        <?php	echo $clubA1615; ?>
      </div>
    </td>
    <td width="36" height="11"> 
      <div class="coin5" > 
        <?php	echo $A1615; ?>
      </div>
    </td>
    <td width="198" height="11"> 
      <div class="coin4" > 
        <?php	echo $clubA8008; ?>
      </div>
    </td>
    <td width="41" height="11"> 
      <div class="coin4" > 
        <?php	echo $A8008; ?>
      </div>
    </td>
    <td width="201" height="11">&nbsp;</td>
    <td width="31" height="11">&nbsp;</td>
    <td width="18" height="11">&nbsp;</td>
    <td width="201" height="11" align="left">&nbsp;</td>
    <td width="23" height="11">&nbsp;</td>
    <td width="198" height="11"> 
      <div class="coin4" > 
        <?php	echo $clubA1001; ?>
      </div>
    </td>
    <td width="21" height="11"> 
      <div class="coin4" > 
        <?php	echo $A1001; ?>
      </div>
    </td>
    <td width="21" height="11">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399" height="14"> 
      <div class="coin5" > 
        <?php	echo $clubA3231; ?>
        - 
        <?php	echo $clubA3232; ?>
      </div>
    </td>
    <td width="65" height="14"> 
      <div class="coin5"> 
        
        <?php echo $A3231; echo " - ";echo $A3232; 
       
        ?>
      </div>
    </td>
    <td width="14" height="14">&nbsp;</td>
    <td width="190" height="14"> 
      <div class="coin5" > 
        <?php	echo $clubA1616; ?>
      </div>
    </td>
    <td width="36" height="14"> 
      <div class="coin5" > 
        <?php	echo $A1616; ?>
      </div>
    </td>
    <td width="198" height="14">&nbsp;</td>
    <td width="41" height="14">&nbsp;</td>
    <td width="22" height="14">&nbsp;</td>
    <td width="201" height="14">&nbsp;</td>
    <td width="31" height="14">&nbsp;</td>
    <td width="18" height="14">&nbsp;</td>
    <td width="201" align="left" height="14">&nbsp;</td>
    <td width="23" height="14">&nbsp;</td>
    <td width="198" height="14"> 
      <div class="coin4" > 
        <?php	echo $clubA1002; ?>
      </div>
    </td>
    <td width="21" height="14"> 
      <div class="coin4" > 
        <?php	echo $A1002; ?>
      </div>
    </td>
    <td width="21" height="14">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin4" > 
        <?php	echo $clubA3233; ?>
        - 
        <?php	echo $clubA3234; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin4" > 
        
        <?php echo $A3233; echo " - ";echo $A3234; 
       
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin4" > 
        <?php	echo $clubA1617; ?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin4" > 
        <?php	echo $A1617; ?>
      </div>
    </td>
    <td rowspan="4" width="14"><img src="../../images/accolade.png" width="8" height="82"> 
    </td>
    <td width="198">&nbsp;</td>
    <td width="41">&nbsp;</td>
    <td width="22">&nbsp;</td>
    <td width="201">&nbsp;</td>
    <td width="31">&nbsp;</td>
    <td width="18">&nbsp;</td>
    <td width="201" align="left">&nbsp;</td>
    <td width="23">&nbsp;</td>
    <td width="198">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin4" > 
        <?php	echo $clubA3235; ?>
        - 
        <?php	echo $clubA3236; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin4" > 
      
        <?php echo $A3235; echo " - ";echo $A3236; 
       
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin4" > 
        <?php	echo $clubA1618; ?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin4" > 
        <?php	echo $A1618; ?>
      </div>
    </td>
    <td width="198"> 
      <div class="coin4" > 
        <?php	echo $clubA8009; ?>
      </div>
    </td>
    <td width="41"> 
      <div class="coin4" > 
        <?php	echo $A8009; ?>
      </div>
    </td>
    <td rowspan="6" width="22"><img src="../../images/accolade.png" width="8" height="127"></td>
    <td width="201">&nbsp;</td>
    <td width="31">&nbsp;</td>
    <td width="18">&nbsp;</td>
    <td width="201" align="left">&nbsp;</td>
    <td width="23">&nbsp;</td>
    <td colspan="3" rowspan="3"> 
      <div align="center"><font size="5"><b> <?php //echo "Champion de France".' '.$annee;?><br>
       
        </b></font></div>
    </td>
  </tr>
  <tr> 
    <td width="399" height="25"> 
      <div class="coin4" > 
        <?php	echo $clubA3237; ?>
        - 
        <?php	echo $clubA3238; ?>
      </div>
    </td>
    <td width="65" height="25"> 
      <div class="coin4" > 
      
        <?php echo $A3237; echo " - ";echo $A3238; 
       
        ?>
      </div>
    </td>
    <td width="14" height="25">&nbsp;</td>
    <td width="190" height="25"> 
      <div class="coin4" > 
        <?php	echo $clubA1619; ?>
      </div>
    </td>
    <td width="36" height="25"> 
      <div class="coin4" > 
        <?php	echo $A1619; ?>
      </div>
    </td>
    <td width="198" height="25"> 
      <div class="coin4" > 
        <?php	echo $clubA8010; ?>
      </div>
    </td>
    <td width="41" height="25"> 
      <div class="coin4" > 
        <?php	echo $A8010; ?>
      </div>
    </td>
    <td width="201" height="25">&nbsp;</td>
    <td width="31" height="25">&nbsp;</td>
    <td width="18" height="25">&nbsp;</td>
    <td width="201" height="25" align="left">&nbsp;</td>
    <td width="23" height="25">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin4"> 
        <?php	echo $clubA3239; ?>
        - 
        <?php	echo $clubA3240; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin4" > 
        
        <?php echo $A3239; echo " - ";echo $A3240; 
        
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin4" > 
        <?php	echo $clubA1620; ?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin4" > 
        <?php	echo $A1620; ?>
      </div>
    </td>
    <td width="198"> </td>
    <td width="41"> </td>
    <td width="201"> 
      <div class="coin4" > 
        <?php	echo $clubA4005; ?>
      </div>
    </td>
    <td width="31"> 
      <div class="coin4" > 
        <?php	echo $A4005; ?>
      </div>
    </td>
    <td rowspan="10" width="18"><img src="../../images/accolade.png" width="10" height="207"></td>
    <td width="201" align="left">&nbsp;</td>
    <td width="23">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399" height="17"> 
      <div class="coin5" > 
        <?php	echo $clubA3241; ?>
        - 
        <?php	echo $clubA3242; ?>
      </div>
    </td>
    <td width="65" height="17"> 
      <div class="coin5" > 
       
        <?php echo $A3241; echo " - ";echo $A3242; 
      
        ?>
      </div>
    </td>
    <td width="14" height="17">&nbsp;</td>
    <td width="190" height="17"> 
      <div class="coin5" > 
        <?php	echo $clubA1621; ?>
      </div>
    </td>
    <td width="36" height="17"> 
      <div class="coin5" > 
        <?php	echo $A1621; ?>
      </div>
    </td>
    <td rowspan="4" height="28" width="14"><img src="../../images/accolade.png" width="8" height="82"></td>
    <td width="198" height="17">&nbsp;</td>
    <td width="41" height="17">&nbsp;</td>
    <td width="201" height="17"> 
      <div class="coin4" > 
        <?php	echo $clubA4006; ?>
      </div>
    </td>
    <td width="31" height="17"> 
      <div class="coin4" > 
        <?php	echo $A4006; ?>
      </div>
    </td>
    <td width="201" align="left" height="17">&nbsp;</td>
    <td width="23" height="17">&nbsp;</td>
    <td colspan="3" height="50" valign="middle" rowspan="4"> 
      <div align="center"><b><font color="#FF0000" size="5" face="Arial, Helvetica, sans-serif">
        <?php 
		
		//echo $clubA1000;
		?>
        </font></b></div>
      <div align="center"></div>
      <div align="center"></div>
      <div align="center"></div>
      <div align="center"></div>
      <div align="center"></div>
    </td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin5" > 
        <?php	echo $clubA3243; ?>
        - 
        <?php	echo $clubA3244; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin5" > 
      
        <?php echo $A3243; echo " - ";echo $A3244; 
      
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin5" > 
        <?php	echo $clubA1622; ?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin5" > 
        <?php	echo $A1622; ?>
      </div>
    </td>
    <td width="198"> 
      <div class="coin4" > 
        <?php	echo $clubA8011; ?>
      </div>
    </td>
    <td width="41"> 
      <div class="coin4" > 
        <?php	echo $A8011; ?>
      </div>
    </td>
    <td width="201">&nbsp;</td>
    <td width="31">&nbsp;</td>
    <td width="201" align="left">&nbsp;</td>
    <td width="23">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399" height="11"> 
      <div class="coin5" > 
        <?php	echo $clubA3245; ?>
        - 
        <?php	echo $clubA3246; ?>
      </div>
    </td>
    <td width="65" height="11"> 
      <div class="coin5" > 
      
		
        <?php echo $A3245; echo " - ";echo $A3246; 
       
        ?>
      </div>
    </td>
    <td width="14" height="11">&nbsp;</td>
    <td width="190" height="11"> 
      <div class="coin5" > 
        <?php	echo $clubA1623; ?>
      </div>
    </td>
    <td width="36" height="11"> 
      <div class="coin5" > 
        <?php	echo $A1623; ?>
      </div>
    </td>
    <td width="198" height="11"> 
      <div class="coin4" > 
        <?php	echo $clubA8012; ?>
      </div>
    </td>
    <td width="41" height="11"> 
      <div class="coin4" > 
        <?php	echo $A8012; ?>
      </div>
    </td>
    <td width="201" height="11">&nbsp;</td>
    <td width="31" height="11">&nbsp;</td>
    <td width="201" height="11" align="left">&nbsp;</td>
    <td width="23" height="11">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin5" > 
        <?php	echo $clubA3247; ?>
        - 
        <?php	echo $clubA3248; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin5"> 
        <?php echo $A3247; echo " - ";echo $A3248; 
       
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin5" > 
        <?php	echo $clubA1624; ?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin5" > 
        <?php	echo $A1624; ?>
      </div>
    </td>
    <td width="198">&nbsp;</td>
    <td width="41">&nbsp;</td>
    <td width="22">&nbsp;</td>
    <td width="201">&nbsp;</td>
    <td width="31">&nbsp;</td>
    <td width="201" align="left"> 
      <div class="coin4" > 
        <?php	echo $clubA2003; ?>
      </div>
    </td>
    <td width="23"> 
      <div class="coin4" > 
        <?php	echo $A2003; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="399" height="18"> 
      <div class="coin4" > 
        <?php	echo $clubA3249; ?>
        - 
        <?php	echo $clubA3250; ?>
      </div>
    </td>
    <td width="65" height="18"> 
      <div class="coin4" > 
        <?php echo $A3249; echo " - ";echo $A3250; 
       
        ?>
      </div>
    </td>
    <td width="14" height="18">&nbsp;</td>
    <td width="190" height="18"> 
      <div class="coin4" > 
        <?php	echo $clubA1625; ?>
      </div>
    </td>
    <td width="36" height="18"> 
      <div class="coin4" > 
        <?php	echo $A1625; ?>
      </div>
    </td>
    <td rowspan="4" width="14"><img src="../../images/accolade.png" width="8" height="82"> 
    </td>
    <td width="198" height="18">&nbsp;</td>
    <td width="41" height="18">&nbsp;</td>
    <td width="22" height="18">&nbsp;</td>
    <td width="201" height="18">&nbsp;</td>
    <td width="31" height="18">&nbsp;</td>
    <td width="201" align="left" height="18"> 
      <div class="coin4" > 
        <?php	echo $clubA2004; ?>
      </div>
    </td>
    <td width="23" height="18"> 
      <div class="coin4" > 
        <?php	echo $A2004; ?>
      </div>
    </td>
    <td width="198" height="18">&nbsp;</td>
    <td width="21" height="18">&nbsp;</td>
    <td width="21" height="18">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin4" > 
        <?php	echo $clubA3251; ?>
        - 
        <?php	echo $clubA3252; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin4" > 
        <?php echo $A3251; echo " - ";echo $A3252; 
        
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin4" > 
        <?php	echo $clubA1626; ?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin4" > 
        <?php	echo $A1626; ?>
      </div>
    </td>
    <td width="198"> 
      <div class="coin4" > 
        <?php	echo $clubA8013; ?>
      </div>
    </td>
    <td width="41"> 
      <div class="coin4" > 
        <?php	echo $A8013; ?>
      </div>
    </td>
    <td rowspan="6" width="22"><img src="../../images/accolade.png" width="8" height="127"></td>
    <td width="201">&nbsp;</td>
    <td width="31">&nbsp;</td>
    <td width="201" align="left">&nbsp;</td>
    <td width="23">&nbsp;</td>
    <td width="11">&nbsp;</td>
    <td width="198">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399" height="5"> 
      <div class="coin4" > 
        <?php	echo $clubA3253; ?>
        - 
        <?php	echo $clubA3254; ?>
      </div>
    </td>
    <td width="65" height="5"> 
      <div class="coin4" > 
        <?php echo $A3253; echo " - ";echo $A3254; 
        
        ?>
      </div>
    </td>
    <td width="14" height="5">&nbsp;</td>
    <td width="190" height="5"> 
      <div class="coin4" > 
        <?php	echo $clubA1627; ?>
      </div>
    </td>
    <td width="36" height="5"> 
      <div class="coin4" > 
        <?php	echo $A1627; ?>
      </div>
    </td>
    <td width="198" height="5"> 
      <div class="coin4" > 
        <?php	echo $clubA8014; ?>
      </div>
    </td>
    <td width="41" height="5"> 
      <div class="coin4" > 
        <?php	echo $A8014; ?>
      </div>
    </td>
    <td width="201" height="5">&nbsp;</td>
    <td width="31" height="5">&nbsp;</td>
    <td width="201" height="5" align="left">&nbsp;</td>
    <td width="23" height="5">&nbsp;</td>
    <td width="11" height="5">&nbsp;</td>
    <td width="198" height="5">&nbsp;</td>
    <td width="21" height="5">&nbsp;</td>
    <td width="21" height="5">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin4"> 
        <?php	echo $clubA3255; ?>
        - 
        <?php	echo $clubA3256; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin4" > 
        <?php echo $A3255; echo " - ";echo $A3256; 
       
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin4" > 
        <?php	echo $clubA1628; ?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin4" > 
        <?php	echo $A1628; ?>
      </div>
    </td>
    <td width="198"> </td>
    <td width="41"> </td>
    <td width="201"> 
      <div class="coin4" > 
        <?php	echo $clubA4007; ?>
      </div>
    </td>
    <td width="31"> 
      <div class="coin4" > 
        <?php	echo $A4007; ?>
      </div>
    </td>
    <td width="201" align="left">&nbsp;</td>
    <td width="23">&nbsp;</td>
    <td width="11">&nbsp;</td>
    <td width="198">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin5" > 
        <?php	echo $clubA3257; ?>
        - 
        <?php	echo $clubA3258; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin5" > 
        <?php echo $A3257; echo " - ";echo $A3258; 
       
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin5" > 
        <?php	echo $clubA1629; ?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin5" > 
        <?php	echo $A1629; ?>
      </div>
    </td>
    <td rowspan="4" width="14"><img src="../../images/accolade.png" width="8" height="82"></td>
    <td width="198">&nbsp;</td>
    <td width="41">&nbsp;</td>
    <td width="201"> 
      <div class="coin4" > 
        <?php	echo $clubA4008; ?>
      </div>
    </td>
    <td width="31"> 
      <div class="coin4" > 
        <?php	echo $A4008; ?>
      </div>
    </td>
    <td width="201" align="left">&nbsp;</td>
    <td width="23">&nbsp;</td>
    <td width="11">&nbsp;</td>
    <td width="198">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin5" > 
        <?php	echo $clubA3259; ?>
        - 
        <?php	echo $clubA3260; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin5" > 
        <?php echo $A3259; echo " - ";echo $A3260; 
        
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin5" > 
        <?php	echo $clubA1630; ?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin5" > 
        <?php	echo $A1630; ?>
      </div>
    </td>
    <td width="198"> 
      <div class="coin4" > 
        <?php	echo $clubA8015; ?>
      </div>
    </td>
    <td width="41"> 
      <div class="coin4" > 
        <?php	echo $A8015; ?>
      </div>
    </td>
    <td width="201">&nbsp;</td>
    <td width="31">&nbsp;</td>
    <td width="18">&nbsp;</td>
    <td width="201" align="left">&nbsp;</td>
    <td width="23">&nbsp;</td>
    <td width="11">&nbsp;</td>
    <td width="198">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399" height="11"> 
      <div class="coin5" > 
        <?php	echo $clubA3261; ?>
        - 
        <?php	echo $clubA3262; ?>
      </div>
    </td>
    <td width="65" height="11"> 
      <div class="coin5" > 
        <?php echo $A3261; echo " - ";echo $A3262; 
       
        ?>
      </div>
    </td>
    <td width="14" height="11">&nbsp;</td>
    <td width="190" height="11"> 
      <div class="coin5" > 
        <?php	echo $clubA1631; ?>
      </div>
    </td>
    <td width="36" height="11"> 
      <div class="coin5" > 
        <?php	echo $A1631; ?>
      </div>
    </td>
    <td width="198" height="11"> 
      <div class="coin4" > 
        <?php	echo $clubA8016; ?>
      </div>
    </td>
    <td width="41" height="11"> 
      <div class="coin4" > 
        <?php	echo $A8016; ?>
      </div>
    </td>
    <td width="201" height="11">&nbsp;</td>
    <td width="31" height="11">&nbsp;</td>
    <td width="18" height="11">&nbsp;</td>
    <td width="201" height="11" align="left">&nbsp;</td>
    <td width="23" height="11">&nbsp;</td>
    <td width="11" height="11">&nbsp;</td>
    <td width="198" height="11">&nbsp;</td>
    <td width="21" height="11">&nbsp;</td>
    <td width="21" height="11">&nbsp;</td>
  </tr>
  <tr> 
    <td width="399"> 
      <div class="coin5" > 
        <?php	echo $clubA3263; ?>
        - 
        <?php	echo $clubA3264; ?>
      </div>
    </td>
    <td width="65"> 
      <div class="coin5"> 
        <?php echo $A3263; echo " - ";echo $A3264; 
      
        ?>
      </div>
    </td>
    <td width="14">&nbsp;</td>
    <td width="190"> 
      <div class="coin5" > 
        <?php	echo $clubA1632; ?>
      </div>
    </td>
    <td width="36"> 
      <div class="coin5" > 
        <?php	echo $A1632; ?>
      </div>
    </td>
    <td width="198">&nbsp;</td>
    <td width="41">&nbsp;</td>
    <td width="22">&nbsp;</td>
    <td width="201">&nbsp;</td>
    <td width="31">&nbsp;</td>
    <td width="18">&nbsp;</td>
    <td width="201" align="left">&nbsp;</td>
    <td width="23">&nbsp;</td>
    <td width="11">&nbsp;</td>
    <td width="198">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
  <tr> 
    <td colspan="16">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
  <tr> 
    <td colspan="16" bgcolor="#FFCC66">&nbsp;</td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
  <tr> 
    <td colspan="16"> 
       
      <div align="center"> </div>
    </td>
    <td width="21">&nbsp;</td>
    <td width="21">&nbsp;</td>
  </tr>
</table>
