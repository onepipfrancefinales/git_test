...
function toggleAccountMenu() {
    ...
    // ( Exercice 6, Ordre de tabulation :) Le sous-menu est caché au moyen de la propriété display et non opacity. La définition du style opacity:0 au chargement est supprimé.
    if (!!accountBtn) {
        accountBtn.addEventListener('click', function() {
            // ( Exercice 6, Ordre de tabulation :) Le sous-menu est caché au moyen de la propriété display et non opacity.
            // ( Exercice 6, Disclosure :) La valeur de la propriété aria-expanded est mise à jour de manière synchronisée avec celle du display CSS.
...
function closeDialog() {
    dialogBtn.focus(); // ( Exercice 6 :) Déplace le focus sur le bouton d'ouverture de la fenêtre modale
...
function openDialog() {
    ...
    dialogWrapper.focus(); // ( Exercice 6 :) Déplace le focus sur la fenêtre modale
...
function verifyRequiredFields(event) {
    ...
    if (canSubmitForm) {
        ...
        // ( Exercice 6 :) Déplace le focus sur le premier champ en erreur
        errorFields[0].querySelector('.form-control').focus();
    }
}
...
function sendForm() {
    ...
    // ( Exercice 6 :) Pour que la reprise de focus sur le message de succès puisse être fait, on ajoute la propriété tabindex="-1"
    successCtn.setAttribute('tabindex', '-1');
