...
global $clubA1002;
	...
	$requete = "SELECT  $bdcomiteClub.nom, $bdcomiteAnnee_e.$scores , $bdcomiteAnnee_e.$equipes
				FROM $bdcomiteClub
				INNER JOIN $bdcomiteAnnee_e ON $bdcomiteAnnee_e.$equipes=$bdcomiteClub.id
				WHERE  $bdcomiteAnnee_e.division=$division and $bdcomiteAnnee_e.annee=$annee"; 
					$result = mysql_query($requete) ;
					while ($row = mysql_fetch_array($result) )
						{ 
						$tabEquipes[] = $row[0];
