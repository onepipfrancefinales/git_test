  ...
  <table width="100%" border="0" height="30">
    ...
    <tr valign="top" align="center"> 
      <td colspan="3" bgcolor="#006699" height="31"><font size="5"><b><font color="#FFCC00"> 
        ...
        <?php echo "Championnat des Pyr&eacute;n&eacute;es $annee : R&eacute;serve";?>
...
<table width="1000" border="0" align="left" height="500" cellspacing="0">
  <tr> 
    <td height="13" bgcolor="#FFCC66" colspan="12"><i><b>R&eacute;serve R&eacute;serve R&eacute;serve 
        R&eacute;serve R&eacute;serve R&eacute;serve R&eacute;serve R&eacute;serve 
        R&eacute;serve R&eacute;serve R&eacute;serve R&eacute;serve R&eacute;serve 
        R&eacute;serve R&eacute;serve R&eacute;serve </b></i></td>
  </tr>
  <tr> 
...
$reponse = mysql_query("SELECT Quart1 
						FROM phpmpy_pf$annee WHERE id='270' "); // Requête SQL
while ($donnees = mysql_fetch_array($reponse) )
{ 
?>
      <strong> 
...
$query = "	SELECT  phpmpy_clubs.nom, phpmpy_clubs.url_logo 
			...
			WHERE phpmpy_pf$annee.clubA8004=phpmpy_clubs.id and phpmpy_pf$annee.id='270'";
$result = mysql_query($query);
         while($row = mysql_fetch_array($result))
         {                      
		$clubA8004=$row[0];
...
$query = "	SELECT phpmpy_clubs.nom, phpmpy_clubs.url_logo 
			FROM phpmpy_pf$annee, phpmpy_clubs 
			WHERE phpmpy_pf$annee.clubA4001=phpmpy_clubs.id and phpmpy_pf$annee.id='270'";
$result = mysql_query($query);
         while($row = mysql_fetch_array($result))
         {                                            
		$clubA4001=$row[0];
...
$query = "	SELECT phpmpy_clubs.nom, phpmpy_clubs.url_logo 
			FROM phpmpy_pf$annee, phpmpy_clubs 
			WHERE phpmpy_pf$annee.clubA4003=phpmpy_clubs.id and phpmpy_pf$annee.id='270'";
$result = mysql_query($query);
         while($row = mysql_fetch_array($result))
         {                            		      
		$clubA4003=$row[0];
