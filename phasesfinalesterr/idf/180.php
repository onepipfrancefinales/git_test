	...
	include("../../pub7.php"); ?>
      ...
      <td valign="middle" bgcolor="#EAECEB" height="29" align="left">
        <div align="center"><font size="5"><b><font size="5"><b><font size="5"><b><font size="5"><b><font color="#FF0000" size="2"> 
          ...
          </font></b></font></b><font color="#000000" size="2"><a href="http://francefinalesrugby.franceserv.com/phasesfinalesterr/idf/pagepromotion.php?variable_1=<?php echo $anneemoins;?>" target="_top"><font size="6" color="#FF0000">&lt;&lt;</font></a></font></font></b></font><font color="#000000" size="2"><font size="5" color="#EAECEB">nnnne</font><font size="5"> 
          </font></font></b><a href="http://francefinalesrugby.franceserv.com/phasesfinalesterr/idf/pagepromotion.php?variable_1=<?php echo $anneeplus;?>"target="_top"><font color="#FF0000" size="6"><b>&gt;&gt;</b></font></a></font></div>
        </td>
...
<table width="1000" border="0" align="left" height="500" cellspacing="0">
  <tr bgcolor="#FFCC66"> 
    <td height="13" colspan="10"><i></i><i><b>e Promotion Promotion Promotion 
      Promotion Promotion Promotion Promotion Promotion Promotion Promotion Promotion 
      Promotion Promotio</b></i></td>
  </tr>
  <tr> 
...
mysql_select_db("onepip-france-db2"); // S�lection de la base onepip	   
   ?>
    ...
    <td width="185" height="26"> 
      <?  
$query = "SELECT  phpidf_clubs.nom, phpidf_clubs.url_logo FROM phpidf_pf$annee, phpidf_clubs WHERE phpidf_pf$annee.clubA4005=phpidf_clubs.id and phpidf_pf$annee.id='180'";
...
?>
  ...
  ?>
      <?  
$query = "SELECT  phpidf_clubs.nom, phpidf_clubs.url_logo FROM phpidf_pf$annee, phpidf_clubs WHERE phpidf_pf$annee.clubA4007=phpidf_clubs.id and phpidf_pf$annee.id='180'";
$result = mysql_query($query);
         while($row = mysql_fetch_array($result))
         {                                		      
		$clubA4007=$row[0];
...
?>
  ...
  ?>
      <?
$reponse = mysql_query("SELECT A4005, A4006, A4007, A4008 FROM phpidf_pf$annee WHERE id='180'");
...
?>
    ...
    <td width="205" align="left" height="26"> 
      <?
$reponse = mysql_query("SELECT A2001, A2002, A2003, A2004 FROM phpidf_pf$annee WHERE id='180'");
while ($donnees = mysql_fetch_array($reponse) )
{ 
 $A2001= $donnees['A2001'];
 $A2002= $donnees['A2002'];
...
$reponse = mysql_query("SELECT * FROM phpidf_pf$annee WHERE id='180' "); // Requ�te SQL
while ($donnees = mysql_fetch_array($reponse) )
{ 
?>
      <strong> 
...
?>
  ...
  <tr> 
    ...
    <td width="185"> 
      <div id="quarts" > 
        <?  
$query = "SELECT phpidf_clubs.nom, phpidf_clubs.url_logo FROM phpidf_pf$annee, phpidf_clubs WHERE phpidf_pf$annee.clubA4001=phpidf_clubs.id and phpidf_pf$annee.id='180'";
$result = mysql_query($query);
         while($row = mysql_fetch_array($result))
         {                                            
		$clubA4001=$row[0];
...
echo "";	
  ...
  ?>
        <?  
$query = "SELECT phpidf_clubs.nom, phpidf_clubs.url_logo FROM phpidf_pf$annee, phpidf_clubs WHERE phpidf_pf$annee.clubA4003=phpidf_clubs.id and phpidf_pf$annee.id='180'";
$result = mysql_query($query);
         while($row = mysql_fetch_array($result))
         {                            		      
		$clubA4003=$row[0];
...
echo "";	
  ...
  ?>
        <?
$reponse = mysql_query("SELECT A4001, A4002, A4003, A4004 FROM phpidf_pf$annee WHERE id='180'");
while ($donnees = mysql_fetch_array($reponse) )
{ 
 $A4001= $donnees['A4001'];
 $A4002= $donnees['A4002'];
...
echo "";	
  ...
  <tr> 
    ...
    <td width="104" height="24">&nbsp;</td>
    <td width="205" align="left" height="24">&nbsp;</td>
    <td width="39" height="24">&nbsp;</td>
    <td width="67" height="24">&nbsp;</td>
    <td width="209" height="24"><font size="2" face="Arial, Helvetica, sans-serif"><b><i> 
...
echo "";	
  ...
  ?>
    ...
    <td width="32"> 
      <div id="quarts" > 
        <?
$reponse = mysql_query("SELECT A1001 FROM phpidf_pf$annee WHERE id='180'");
while ($donnees = mysql_fetch_array($reponse) )
{ 
 $A1001= $donnees['A1001'];
 echo $A1001;     
...
echo "";	
  ...
  ?>
    ...
    <td width="32" height="11"> 
      <div id="quarts" > 
        <?
$reponse = mysql_query("SELECT A1002 FROM phpidf_pf$annee WHERE id='180'");
while ($donnees = mysql_fetch_array($reponse) )
{ 
 $A1002= $donnees['A1002'];
 echo $A1002;     
...
echo "";	
  ...
  <tr> 
    <td colspan="8"> 
      <div align="right">_______________________________________________________________________________________________________________________</div>
    </td>
    <td>&nbsp;</td>
    <td width="2">&nbsp;</td>
  </tr>
