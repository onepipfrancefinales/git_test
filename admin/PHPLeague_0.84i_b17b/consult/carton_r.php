...
if ($ACT_CR == 1)
{
	...
	if (isset($_REQUEST['champ']) or isset($_REQUEST['gr_champ']) or isset($_REQUEST['equipe']))
	{
		...
		<select name="debut">'."\n";
			...
			echo "</select>";
			echo " ".FormatText(CONSULT_CLMNT_MSG3)." <select name=\"fin\">";
			...
			echo " ".utf8_encode(ADMIN_JOUEURS_3)." ";
			echo "<select name=\"equipe\" class=\"texte_noir\">";
			...
			{ 
				...
				echo "<option value=\"$row[1]\"$select>$clubs_nom</option>";
			}
...
//	if (!$gr_champ){
		...
		$requete = "
		SELECT 	Sum(phpl_compoCR.CR) AS Total,
			...
			FROM 	phpl_joueurs, phpl_compoCR, phpl_matchs, phpl_journees, phpl_equipes, phpl_clubs, phpl_effectif
...
if (!isset($complet) or !($complet == 1)) {$requete = $requete . " LIMIT 0,40";}
	...
	CR($legende, $requete, $type, $id_club_fetiche, $champ, $debut, $fin, $equipe, $complet);
	}
	break;
