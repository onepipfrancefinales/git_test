...
if (empty($numero))
{
	$REQ = mysql_query('SELECT MAX(phpl_journees.numero) 
...
if ($action3 == 2)
{
	...
	if ($REQ)
	{
		echo InfoReqSql('InfUpdTps', 1, 'highlight', 'check', ADMIN_TPS_MAJ2);
	}
	else
	{
...
while($row_menu = mysql_fetch_array($result_menu)) 
{
	...
	while ($row3 = mysql_fetch_array($result3))
	{
		...
		if ($row3[5] == 1)
		{
			echo "<input type=\"text\" name=\"tpsdom[$y]\" id=\"tpsdom[$y]\" size=\"1\" value=\"$tps\" style=\"float: right; margin-left: 5px;\" class=\"TxtCenter\" disabled=\"disabled\" />&nbsp;<label for=\"tpsdom[$y]\"><strong>$joueurs_nom $joueurs_prenom [ T ]</strong></label><br /><div style=\"clear: both; margin-bottom: 3px;\"></div>";
		}
		else
		{
			echo "<input type=\"text\" name=\"tpsdom[$y]\" id=\"tpsdom[$y]\" size=\"1\" value=\"$tps\" style=\"float: right; margin-left: 5px;\" class=\"TxtCenter\" disabled=\"disabled\" />&nbsp;<label for=\"tpsdom[$y]\">$joueurs_nom $joueurs_prenom [ R ]</label><br /><div style=\"clear: both; margin-bottom: 3px;\"></div>";
		}		
	...
	while ($row3 = mysql_fetch_array($result3))
	{
		...
		if ($row3[5] == 1)
		{
			echo "<input type=\"text\" name=\"tps[$i]\" id=\"tps[$i]\" size=\"1\" maxlength=\"3\" value=\"$tps\" style=\"float: left;\" class=\"TxtCenter\" />&nbsp;<label for=\"tps[$i]\"><strong>[ T ] $joueurs_nom $joueurs_prenom</strong></label><br /><div style=\"clear: both; margin-bottom: 3px;\"></div>";
		}
		else
		{
			echo "<input type=\"text\" name=\"tps[$i]\" id=\"tps[$i]\" size=\"1\" maxlength=\"3\" value=\"$tps\" style=\"float: left;\" class=\"TxtCenter\" />&nbsp;<label for=\"tps[$i]\">[ R ] $joueurs_nom $joueurs_prenom</label><br /><div style=\"clear: both; margin-bottom: 3px;\"></div>";
		}
