<?php

//require ("../../phases_finales2019/fonctionsChampFrance2019.php");  
  //	nomDivision($division);
	//trenteDeuxieme2019 ($division, $annee);
?>



<table width="90%" border="1" align="center" bordercolor="#000000">
  <tr> 
    <td bgcolor="#FFFFFF" height="32"> 
      <div id="titre"> 32me de finale</div>
    </td>
  </tr>
</table>
<br>
<table width="90%" border="0" align="center">
  <tr> 
    <td width="3%" bgcolor="#E4EFFF"> 
      <div id="coin3"> 1</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3201; ?>
      </div>
    </td>
    <td width="16%"> 
      <div id="coin4" > 
        <?php echo $A3201; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF"> 
      <div id="coin3"> 2</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3202; ?>
      </div>
    </td>
    <td width="16%"> 
      <div id="coin4" > 
        <?php echo $A3202; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" height="25" bordercolor="#9FC6FF"> 
      <div id="coin5"> 3</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3203; ?>
      </div>
    </td>
    <td width="16%" height="25"> 
      <div id="coin5" > 
        <?php  echo $A3203; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" bordercolor="#9FC6FF" height="22"> 
      <div id="coin5"> 4</div>
    </td>
    <td width="81%" height="22"> 
      <div id="coin5"> 
        <?php	echo $clubA3204; ?>
      </div>
    </td>
    <td width="16%" height="22"> 
      <div id="coin5" >
        <?php echo $A3204; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF"> 
      <div id="coin4"> 5</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3205; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin4" > 
        <?php echo $A3205; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF"> 
      <div id="coin4"> 6</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3206; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin4" >
        <?php  echo $A3206; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" height="11"> 
       
      <div id="coin3"> 7</div>
    </td>
    <td width="81%" height="11"> 
      <div id="coin5" > 
        <?php	echo $clubA3207; ?>
        </div>
    </td>
    <td width="16%" height="11"> 
      <div id="coin5" > 
        <?php  echo $A3207; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF"> 
      <div id="coin3"> 8</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3208; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin5">
        <?php echo $A3208; ?>
      </div>
    </td>
  </tr>
</table>
<table width="90%" border="0" align="center">
  <tr> 
    <td colspan="2" height="21"> 
       
        
      <div id="liensGauche"><a href="barrages.php?annee=<?php echo $annee; ?>"><font color="#FF0000">&lt;&lt; 
        Barrages</font></a></div>
     
    </td>
    <td height="21" width="42%"> 
      <div id="liensDroit"><a href="16me.php?division=<?php echo $division ?>&annee=<?php echo $annee; ?>"><font color="#FF0000">16me 
        &gt;&gt;</font></a></div>
    </td>
  </tr>
</table>
<table width="90%" border="0" align="center">
  <tr> 
    <td width="3%" height="9" bgcolor="#E4EFFF" bordercolor="#0066FF"> 
        
      <div id="coin3"> 9</div>
    </td>
    <td width="81%" height="9"> 
      <div id="coin4" > 
        <?php	echo $clubA3209; ?>
        </div>
    </td>
    <td width="16%" height="9"> 
      <div id="coin4" > 
        <?php echo $A3209; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF" bordercolor="#0066FF"> 
        
      <div id="coin3"> 10</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3210; ?>
       </div>
    </td>
    <td width="16%"> 
      <div id="coin4"> 
        <?php echo $A3210; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF" bordercolor="#0066FF"> 
       
      <div id="coin5"> 11</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3211; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php echo $A3211; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF" bordercolor="#0066FF"> 
       
      <div id="coin5"> 12</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3212; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php echo $A3212; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bordercolor="#0066FF"> 
      <div id="coin4"> 13</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3213; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin4" > 
        <?php echo $A3213;  ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bordercolor="#0066FF"> 
      <div id="coin4"> 14</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3214; ?>
       </div>
    </td>
    <td width="16%"> 
      <div id="coin4" align="center">
        <?php echo $A3214; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" bordercolor="#0066FF"> 
        
      <div id="coin3"> 15</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3215; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin5" align="center">
        <?php  echo $A3215; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" bordercolor="#0066FF"> 
        
      <div id="coin3"> 16</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3216; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php echo $A3216; ?>
      </div>
    </td>
  </tr>
</table>
<p align="center"> 
  <?php
  
    
	include("../../pub/pub20.php"); ?>
</p>
<table width="90%" border="0" align="center">
  <tr> 
    <td width="64%"> 
      <div id="liensGauche"><a href="barrages.php?annee=<?php echo $annee; ?>"><font color="#FF0000">&lt;&lt; 
        Barrages</font></a></div>
    </td>
    <td width="36%"> 
      <div id="liensDroit"><a href="16me.php?division=<?php echo $division ?>&annee=<?php echo $annee; ?>"><font color="#FF0000">16me 
        &gt;&gt;</font></a></div>
    </td>
  </tr>
</table>
<table width="90%" border="0" align="center">
  <tr> 
    <td width="3%" bgcolor="#E4EFFF"> 
       
      <div id="coin3"> 17</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3217; ?>
       </div>
    </td>
    <td width="16%" height="9"> 
      <div id="coin4" > 
        <?php echo $A3217;  ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF" height="21"> 
      <div id="coin3"> 18</div>
    </td>
    <td width="81%" height="21"> 
      <div id="coin4" > 
        <?php	echo $clubA3218; ?>
        </div>
    </td>
    <td width="16%" height="21"> 
      <div id="coin4" > 
        <?php  echo $A3218; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF" height="25"> 
        
      <div id="coin5"> 19</div>
    </td>
    <td width="81%" height="25"> 
      <div id="coin5" > 
        <?php	echo $clubA3219; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin5" > 
        <?php echo $A3219; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF"> 
        
      <div id="coin5"> 20</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
	      <?php	echo $clubA3220; ?>  
	  </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
	       <?php echo $A3220; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" height="22"> 
      <div id="coin4"> 21</div>
    </td>
    <td width="81%" height="22"> 
      <div id="coin4" > 
        <?php	echo $clubA3221; ?>
        </div>
    </td>
    <td width="16%" height="22"> 
      <div id="coin4"> 
        <?php  echo $A3221; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF"> 
        
      <div id="coin4"> 22</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3222; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin4" >
        <?php  echo $A3222; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" height="11"> 
       
      <div id="coin3"> 23</div>
    </td>
    <td width="81%" height="11"> 
      <div id="coin5" > 
        <?php	echo $clubA3223; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin5" > 
        <?php  echo $A3223; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF"> 
        
      <div id="coin3"> 24</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3224; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php  echo $A3224; ?>
      </div>
    </td>
  </tr>
</table>
<table width="90%" border="0" align="center">
  <tr> 
    <td colspan="2" height="20"> 
      <div id="liensGauche"><a href="barrages.php?annee=<?php echo $annee; ?>"><font color="#FF0000">&lt;&lt; 
        Barrages</font></a></div>
      
    </td>
    <td height="20" width="16%"> 
      <div id="liensDroit"><a href="16me.php?division=<?php echo $division ?>&annee=<?php echo $annee; ?>"><font color="#FF0000">16me 
        &gt;&gt;</font></a></div>
    </td>
  </tr>
</table>
<table width="90%" border="0" align="center">
  <tr> 
    <td width="3%" height="9" bgcolor="#E4EFFF" bordercolor="#0066FF"> 
        
      <div id="coin3"> 25</div>
    </td>
    <td width="81%" height="9"> 
      <div id="coin4" > 
        <?php	echo $clubA3225; ?>
       </div>
    </td>
    <td width="16%" height="9"> 
      <div id="coin4" >
        <?php echo $A3225; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF" bordercolor="#0066FF" height="23"> 
        
      <div id="coin3"> 26</div>
    </td>
    <td width="81%" height="23"> 
      <div id="coin4" > 
        <?php	echo $clubA3226; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin4" >
        <?php echo $A3226;        ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF" bordercolor="#0066FF"> 
        
      <div id="coin5"> 27</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3227; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php  echo $A3227;  ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF" bordercolor="#0066FF"> 
       
      <div id="coin5"> 28</div>
    </td>
    <td width="81%"> 
      <div id="coin5"> 
        <?php	echo $clubA3228; ?>
       </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php echo $A3228; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" bordercolor="#0066FF"> 
      <div id="coin4"> 29</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3229; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin4" > 
        <?php echo $A3229; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" bordercolor="#0066FF" height="21"> 
      <div id="coin4"> 30</div>
    </td>
    <td width="81%" height="21"> 
      <div id="coin4" > 
        <?php	echo $clubA3230; ?>
        </div>
    </td>
    <td width="16%" height="21"> 
      <div id="coin4" >
        <?php echo $A3230; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" bordercolor="#0066FF"> 
       
      <div id="coin3"> 31</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3231; ?>
       </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php echo $A3231; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" bordercolor="#0066FF"> 
        
      <div id="coin3"> 32</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3232; ?>
       </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php echo $A3232;?>
      </div>
    </td>
  </tr>
</table>
<p align="center"> 
  <?php
  
    
	include("../../pub/pub25.php"); ?>
</p>
<table width="90%" border="0" align="center">
  <tr> 
    <td height="13" width="67%"> 
      <div id="liensGauche"><a href="barrages.php?annee=<?php echo $annee; ?>"><font color="#FF0000">&lt;&lt; 
        Barrages</font></a></div>
     
    </td>
    <td height="13" width="33%"> 
      <div id="liensDroit"><a href="16me.php?division=<?php echo $division ?>&annee=<?php echo $annee; ?>"><font color="#FF0000">16me 
        &gt;&gt;</font></a></div>
    </td>
  </tr>
</table>
<table width="90%" border="0" align="center">
  <tr> 
    <td width="3%" bgcolor="#E4EFFF"> 
      <div id="coin3"> 33</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3233; ?>
       </div>
    </td>
    <td width="16%" height="9"> 
      <div id="coin4" >
        <?php  echo $A3233;?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF"> 
       
      <div id="coin3"> 34</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3234; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin4" >
        <?php echo $A3234; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF" height="27"> 
       
      <div id="coin5"> 35</div>
    </td>
    <td width="81%" height="27"> 
      <div id="coin5" > 
        <?php	echo $clubA3235; ?>
       </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php echo $A3235; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF"> 
        
      <div id="coin5"> 36</div>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3236; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php echo $A3236; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF"> 
      <div id="coin4"> 37</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3237; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin4" >
        <?php echo $A3237; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF"> 
        
      <div id="coin4"> 38</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3238; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin4" >
        <?php echo $A3238; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" height="11"> 
        
      <div id="coin3"> 39</div>
    </td>
    <td width="81%" height="11"> 
      <div id="coin5" > 
        <?php	echo $clubA3239; ?>
       </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php echo $A3239; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF"> 
       
      <div id="coin3"> 40</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3240; ?>
      </div>
    </td>
    <td width="16%"> 
      <div id="coin5">
        <?php echo $A3240; ?>
      </div>
    </td>
  </tr>
</table>
<table width="90%" border="0" align="center">
  <tr> 
    <td colspan="2" height="21"> 
      <div id="liensGauche"><a href="barrages.php?annee=<?php echo $annee; ?>"><font color="#FF0000">&lt;&lt; 
        Barrages</font></a></div>

    </td>
    <td height="21" width="32%"> 
      <div id="liensDroit"><a href="16me.php?division=<?php echo $division ?>&annee=<?php echo $annee; ?>"><font color="#FF0000">16me 
        &gt;&gt;</font></a></div>
    </td>
  </tr>
</table>
<table width="90%" border="0" align="center">
  <tr> 
    <td width="3%" height="9" bgcolor="#E4EFFF" bordercolor="#0066FF"> 
        
      <div id="coin3"> 41</div>
    </td>
    <td width="81%" height="9"> 
      <div id="coin4" > 
        <?php	echo $clubA3241; ?>
        </div>
    </td>
    <td width="16%" height="9"> 
      <div id="coin4" >
        <?php echo $A3241; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF" bordercolor="#0066FF"> 
       
      <div id="coin3"> 42</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3242; ?>
       </div>
    </td>
    <td width="16%"> 
      <div id="coin4" >
        <?php echo $A3242; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF" bordercolor="#0066FF"> 
        
      <div id="coin5"> 43</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3243; ?>
       </div>
    </td>
    <td width="16%"> 
      <div id="coin5" > 
        <?php echo $A3243; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF" bordercolor="#0066FF"> 
        
      <div id="coin5"> 44</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3244; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php echo $A3244; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" bordercolor="#0066FF"> 
        
      <div id="coin4"> 45</div>
    </td>
    <td width="81%"> 
      <div id="coin4" b> 
        <?php	echo $clubA3245; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin4" align="center">
        <?php echo $A3245;  ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" bordercolor="#0066FF"> 
        
      <div id="coin4"> 46</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3246; ?>
       </div>
    </td>
    <td width="16%"> 
      <div id="coin4" >
        <?php echo $A3246; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" bordercolor="#0066FF"> 
       
      <div id="coin3"> 47</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php echo $clubA3247; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php echo $A3247; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" bordercolor="#0066FF"> 
        
      <div id="coin3"> 48</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3248; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php echo $A3248; ?>
      </div>
    </td>
  </tr>
</table>
<p align="center"> 
  <?php	include("../../pub/pub20.php"); ?>
</p>
<table width="90%" border="0" align="center">
  <tr> 
    <td height="25" width="69%"> 
      <div id="liensGauche"> <a href="barrages.php?annee=<?php echo $annee; ?>"><font color="#FF0000">&lt;&lt; 
        Barrages</font></a></div>
  
    </td>
    <td height="25" width="31%"> 
      <div id="liensDroit"><a href="16me.php?division=<?php echo $division ?>&annee=<?php echo $annee; ?>"><font color="#FF0000">16me 
        &gt;&gt;</font></a></div>
    </td>
  </tr>
</table>
<table width="90%" border="0" align="center">
  <tr> 
    <td width="3%" bgcolor="#E4EFFF"> 
        
      <div id="coin3"> 49</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3249; ?>
      </div>
    </td>
    <td width="16%" height="9"> 
      <div id="coin4">
        <?php echo $A3249; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF"> 
       
      <div id="coin3"> 50</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3250; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin4" >
        <?php echo $A3250; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF" height="25"> 
       
      <div id="coin5"> 51</div>
    </td>
    <td width="81%" height="25"> 
      <div id="coin5" > 
        <?php	echo $clubA3251; ?>
      </div>
    </td>
    <td width="16%"> 
      <div id="coin5" > 
        <?php echo $A3251; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF"> 
        
      <div id="coin5"> 52</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3252; ?>
       </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php echo $A3252; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF"> 
        
      <div id="coin4"> 53</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3253; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin4" >
        <?php echo $A3253; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF"> 
        
      <div id="coin4"> 54</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3254; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin4" >
        <?php echo $A3254; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" height="11"> 
       
      <div id="coin3"> 55</div>
    </td>
    <td width="81%" height="11"> 
      <div id="coin5" > 
        <?php	echo $clubA3255; ?>
       </div>
    </td>
    <td width="16%"> 
      <div id="coin5"> 
        <?php echo $A3255; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF"> 
      <div id="coin3"> 56</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3256; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php echo $A3256; ?>
      </div>
    </td>
  </tr>
</table>
<table width="90%" border="0" align="center">
  <tr> 
    <td colspan="2" height="24"> 
     
       
      <div id="liensGauche"><a href="barrages.php?annee=<?php echo $annee; ?>"><font color="#FF0000">&lt;&lt; 
        Barrages</font></a></div>
     
    </td>
    <td height="24" width="29%"> 
      <div id="liensDroit"><a href="16me.php?division=<?php echo $division ?>&annee=<?php echo $annee; ?>"><font color="#FF0000">16me 
        &gt;&gt;</font></a></div>
    </td>
  </tr>
</table>
<table width="90%" border="0" align="center">
  <tr> 
    <td width="3%" height="9" bgcolor="#E4EFFF" bordercolor="#0066FF"> 
       
      <div id="coin3"> 57</div>
    </td>
    <td width="81%" height="9"> 
      <div id="coin4" > 
        <?php	echo $clubA3257; ?>
       </div>
    </td>
    <td width="16%" height="9"> 
      <div id="coin4" >
        <?php echo $A3257; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF" bordercolor="#0066FF"> 
        
      <div id="coin3"> 58</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3258; ?>
       </div>
    </td>
    <td width="16%"> 
      <div id="coin4"  >
        <?php echo $A3258; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF" bordercolor="#0066FF"> 
       
      <div id="coin5"> 59</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3259; ?>
       </div>
    </td>
    <td width="16%"> 
      <div id="coin5" > 
        <?php echo $A3259; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#E4EFFF" bordercolor="#0066FF"> 
       <div id="coin5"> 60</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3260; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php echo $A3260; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" bordercolor="#0066FF"> 
      <div id="coin4"> 61</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3261; ?>
       </div>
    </td>
    <td width="16%"> 
      <div id="coin4" > 
        <?php echo $A3261; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" bordercolor="#0066FF"> 
        
      <div id="coin4"> 62</div>
    </td>
    <td width="81%"> 
      <div id="coin4" > 
        <?php	echo $clubA3262; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin4" a>
        <?php echo $A3262; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" bordercolor="#0066FF"> 
       <div id="coin3"> 63</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3263; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin5" > 
        <?php echo $A3263; ?>
      </div>
    </td>
  </tr>
  <tr> 
    <td width="3%" bgcolor="#9FC6FF" bordercolor="#0066FF"> 
      <div id="coin3"> 64</div>
    </td>
    <td width="81%"> 
      <div id="coin5" > 
        <?php	echo $clubA3264; ?>
        </div>
    </td>
    <td width="16%"> 
      <div id="coin5" >
        <?php echo $A3264; ?>
      </div>
    </td>
  </tr>
</table>

